...
- **Мир в конверте по политике типа** (C-01 v1.10, T-460): `Policy.World` — `WorldOptional`
  (нулевое значение, прежнее поведение) или `WorldRequired`. При `WorldRequired` конверт без
  `World` или с пустым `World.Entity.ID` отвергается с `ErrPolicyViolation`: `Publish` — всегда,
  чтение — при включённой валидации (событие уходит в `dead_letters` с `attempts = 0`, обработчик
  не вызывается). Строки типов ставит владелец типа в своём файле реестра; первые —
  `entity.create.proposed` и `entity.update.proposed` (C-02 v1.7, T-056).
...
- **Исходы `Delivery` в логе** (T-441): письмо записано — `Warn` «event parked in dead letters»
  («undecodable message parked…» у нераспознанного тела); не записано — «event not parked in dead
  letters; it stays uncommitted and will be delivered again» с `handled=false`: `Warn` и
  `bus_closed=true` на закрытой шине (штатная остановка, цикл возвращает `nil`), `Error` и
  `bus_closed=false` на живой шине или без приёмника (при живом контексте подписки она падает и
  топик стоит; если контекст уже отменён, цикл всё равно возвращает `nil`); паника
  обработчика — дополнительно `Error` со стеком; окончательная ошибка — `Error` «event handler
  failed permanently…» с `handled=true`, только после записи письма (см. ниже).
...
- **Граница `Close` — транспорт, а не обработчик** (C-01 v1.7, ADR-023 п. 4). `Close` вправе
  ...
  отмена по `Close`) — только на `FetchMessage` и `CommitMessages`. Подписка считается
  остановленной, если отменён этот контекст **или** ошибка говорит о закрытой шине (`ErrClosed`,
  `io.ErrClosedPipe`, `io.EOF` закрытого читателя): `Close` ставит флаг закрытия раньше, чем отмена
  доходит до цикла, и письмо в `dead_letters`, отвергнутое в этом промежутке, — тоже штатная остановка,
  `nil` (T-443). Эта проверка (`stopped`) шире той, по которой лог ставит `bus_closed`, и включает её,
  поэтому `Warn` с `bus_closed=true` всегда ведёт к `nil` из цикла; обратное неверно — отменённый
  контекст даёт `nil` и без такой строки. Обработчику уходит контекст вызывающего.
  Останавливает обработчик тот, кто вправе прервать работу: процесс отменяет контекст подписки
  в `Stop` её контекста до `Close` шины,
...
- **Окончательная ошибка** (C-01 v1.10, T-460). Обработчик помечает ошибку
  `eventbus.Permanent(err)`: `errors.Is(w, eventbus.ErrPermanent)` и `errors.Is(w, err)` истинны,
  `errors.As` доходит до причины, текст — текст причины, `Permanent(nil)` — `nil`.
  - **Что окончательно.** Только дефект самого события, который повтор не исправит **ни в каком
    процессе**: конверт или payload нарушает контракт, и обработчик это распознал (например, при
    выключенной валидации при чтении). Недоступная зависимость, таймаут, отменённый контекст и
    любое **состояние получателя** — не окончательные. Мир State, остановленный после паники или с
    `publish_failed`, отвечает обычной ошибкой (`ErrWorldStopped`), а не `Permanent` (КД State §9).
  - **При живом контексте** `Deliver` не повторяет: событие сразу, без пауз `Backoff`, уходит в
    `dead_letters` — `attempts` = номер вызова, `error` = текст причины; затем лог `Error` с полями
    события и `handled=true`, без `panic` и `stack` (в `service_panics` не входит). Если письмо не
    записалось — как у любой неудачной записи: ошибка из `Deliver`, офсет не коммитится,
    `handled=false`. Одинаково в `Subscribe`, `ReadRange` и `Tail` обеих реализаций.
  - **При отменённом контексте** не паркуется: `Deliver` возвращает `ctx.Err()`, событие не
    фиксируется и после рестарта приходит снова. Проверка отмены стоит раньше парковки: процесс,
    который останавливается, не отличит «событие плохое» от «я не довёл его до конца», а на
    досылке после рестарта построен `Stop` State (T-055). Паника по-прежнему паркуется сразу, в том
    числе под отменой (`ErrHandlerPanic`).
  - Якоря — кейсы `APermanentErrorIsParkedAtOnceWithoutAPause` (шина с паузами на ручных таймерах,
    которые никто не двигает) и `APermanentErrorUnderCancellationIsDeliveredAgain` contract-набора.
