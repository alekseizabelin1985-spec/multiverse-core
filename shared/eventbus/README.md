...
`dead_letters` и до обработчика не доходит. При `MV_BUS_VALIDATE_ON_READ=false` он доходит, паника
выходит из `Deliver`, `recover` нигде нет — и падает процесс со всеми контекстами. Обработчик, который
может работать без валидации при чтении, проверяет `ev.ID != ""` до `Derive(…, WithCauseID(…))`.
