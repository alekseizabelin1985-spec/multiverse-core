...
- **`Drain(ctx, quiet)`** — всё, что есть у харнесса: сначала «входящие», затем
  всё, что отдаёт long-poll, пока опрос длиной `quiet` (настенное время) не
  вернётся пустым. Каждая доставка подтверждается, как в `AwaitDelivery`.
  `quiet ≤ 0` — опрос с `wait_ms=0`: возврат при первом опросе, где готовых
  доставок нет. После успешного вызова «входящие» пусты. При ошибке вызов
  ничего не возвращает, но и не теряет: доставки прежних опросов вызова и
  подтверждённые шлюзом доставки упавшего опроса остаются во «входящих» до
  следующего `Drain` или `AwaitDelivery`; доставки опроса, чей ack не прошёл,
  остаются в лизинге шлюза. Так шаг сценария собирает всё, что вызвал, не
  перечисляя виды заранее. Пустой опрос — лишь признак того, что шлюзу пока
  нечего отдать: тест, которому нужна уверенность, сначала ждёт на шине факт,
  стоящий за доставкой (e2e T-313, `test/e2e/gateway_world_test.go`).
