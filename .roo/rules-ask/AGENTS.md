...
- Package.json scripts must be run from specific directories, not root - this is required for proper execution
- Locales in root are for extension, webview-ui/src/i18n for UI (two separate systems)
- World generator uses Ascension Oracle (Qwen3) for generating world details and schemas - this is the only correct approach
