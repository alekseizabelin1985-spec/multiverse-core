...
- Services communicate through event bus, not direct API calls - this is the core architectural principle
- Entity manager uses MinIO for storage with bucket naming pattern: `entities-{world_id}` - this is the only storage approach
- All services are stateful and support recovery via snapshots and event replay - this is a core architectural requirement
- World entities are stored in MinIO buckets named `entities-{world_id}` - this is the only correct storage approach
- Global entities are stored in `entities-global` bucket - this is the only correct global storage approach
- Schema versions are stored in MinIO with path pattern: `schemas/{type}/{name}/v{version}.json` - this is the only correct schema storage approach