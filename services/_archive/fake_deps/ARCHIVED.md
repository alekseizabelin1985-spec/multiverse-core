...
| Поле | Значение |
|---|---|
| Исходный путь | `fake_deps` |
| Причина | заглушки внешних библиотек (onnxruntime, tokenizers) для `shared/tinyml`; вне стека MVP-1 (`audit-facts.md`, `foundation.md` §11) |
...
| Эпик возврата | нет |
| Что использовало | `shared/tinyml` (через `replace` в go.mod сервисов) |
