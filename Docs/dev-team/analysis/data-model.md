...
| `encounter_chance` | number | да | Шанс встречи при тике региона (`0.25`; из блупринта, фикстура КД State §4.10) | `0 ≤ x ≤ 1`; скаляр (C-02 v1.8 п. 2) *(изм. T-470, итерация 2; атрибут был в фикстурах и коде, но не в таблице)* |
...
У персонажа нет `died_at` и `killed_by`. Смерть — `status: dead` (и `participants[].state = dead` во встрече); время и убийца — в журнале: `timestamp` факта и `combat.decided` (C-03 v1.4). Записи смерти в сущности — у NPC (§3.4) *(изм. T-470; просмотр T-056, ответ 3)*.
...
| `loot_claimed_by` | ref Character | нет | Кто забрал трофей павшего NPC | ставится один раз (инвариант 3); путь трупа (КД State §4.5 п. 5, §4.6); скаляр (C-02 v1.8 п. 2) *(изм. T-470, итерация 2; атрибут был в КД и коде, но не в таблице)* |
...
| `called_at` | timestamp | нет (шлюз пишет в каждой записи с C-07 v1.6) | момент попытки по `Clock` шлюза, сразу перед вызовом провайдера; окно бюджета считает от него, без поля — от `timestamp` записи *(изм. T-456, C-07 v1.6)* | в `timestamp` события, ключ записи и сверку replay не входит |
