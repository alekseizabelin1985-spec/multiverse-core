...
Версия 0.2 · 2026-09-09 · system-architect#1 · статус: к утверждению на G2.
Основание: `analysis/api-contracts.md` v0.1 (поля событий и HTTP API — здесь не дублируются, даются ссылки), `analysis/data-model.md`, ADR-001…ADR-010 (с дополнениями 2026-09-09), ADR-021, `plan/epics.md`, `plan/ownership.md`; запросы на изменение из `components/*.md` §14, `threat-model.md` §8, `infrastructure.md` §11 — сведены в `architecture/consolidation.md`.
...
### История изменений
v1 — 2026-09-09: по `api-contracts.md` §2.3.4 (ADR-003). v1.1 — 2026-09-09: семантика `rest`/`atomic=false`/`applied_at`/`append`, `duplicate_entity`, источник `OwnershipRules`.
...
### История изменений
v1 — 2026-09-09. Уточнения 2026-09-09: `say` и раунд, лидерство при смерти (без изменения схем).
...
### Заглушка
`testkit/swarm.FakeNarrator` — публикует `narrative.output generated_by=template` на каждый `combat.decided`/`round.closed`/`player.looked` в `membus` (для e2e gateway до готовности роя).

### История изменений
v1 — 2026-09-09. v1.1 — 2026-09-09: `encounter.started.round{}`, `narrative_event_id`.
...
### История изменений
v1 — 2026-09-09. Уточнение 2026-09-09: адрес `MV_CORE_ADDR=127.0.0.1:8090` (замечание infra п. 7; порт 8090 сохранён по трём дизайнам, devops правит `infrastructure.md` §1.4), `lod_allowed` обязателен.
...
### Интерфейс
`llm.output` — поля `data-model.md` §7.2 (`provider, model, params, prompt_hash, response_raw?, response_hash, validation_status, laws_version, phase, attempt, latency_ms, tokens, cost_usd, lod?, tools_used[]?, error?`; **`parse{strategy, recovered}`** опц. (ADR-016 п. 1, v1.1); `correlation_id`, `actor_kind`, `agent`, `replay`, `gm_path` — в `meta`); `validation_status` дополнен значением `error` с `error.code=yielded` (уступка фона, ADR-014 п. 2); `llm.output.rejected` — §2.3.10, `reason` enum по ADR-017 п. 3; `content.incident.recorded` — §2.3.15; `agent.spawned` + `content_hash` (опц.). Ключ записи для replay: `(meta.correlation_id, meta.agent.id, phase, attempt)`.
...
### История изменений
v1 — 2026-09-09. v1.1 — 2026-09-09: `parse{}`, `yielded`, `content_hash`, ограничение записей `actor_kind=ci`.
...
### История изменений
v1 — 2026-09-09 (ADR-006). v1.1 — 2026-09-09: коды, `pending`, `creating`, `link_id`, лимиты (gateway §14 п. 1–3, threat-model п. 2/6/12).
...
### История изменений
v1 — 2026-09-09. v1.1 — 2026-09-09: `latest.json` указатель, `component` enum, префикс `gateway/`, совладение схемы `replay.completed`, независимость от versioning.

## 15. C-15 · EPIC-003 → EPIC-013 (E-H облако), EPIC-005 (эмбеддинги): интерфейс провайдера LLM

Тип: Go API · Версия: 1 · Владелец: EPIC-003. `llm.Provider{Generate, Embed, Health, Models}` (ADR-005 п. 1; `Models(ctx) []string` — список доступных моделей для валидатора C-11), `llm.Request/Response`; реализации регистрируются в `providers.Registry` по имени; конфигурация `MV_LLM_PROVIDER`, ключи — env (`MV_OPENAI_API_KEY` …). Middleware (бюджет, запись, парсер, язык, фильтр, страж) — вне провайдера и общие для всех. Включение облака — только `MV_LLM_CLOUD_ENABLED=true` + `config.cloud_enabled` в `system_events`; блупринт провайдера не выбирает.
...
| EPIC-003 | C-02 факты State | `testkit/state.FakeState` | `shared/testkit/state` |
...
| EPIC-004 | C-05 нарратив/механика | `testkit/swarm.FakeNarrator` | `shared/testkit/swarm` |
