...
4. `Resolve` — чистая функция таблицы исходов §5.4; `NPCTarget` — `last_damager → min_hp → id asc` с исключением `dead|idle|out_of_combat`; `ChangesFor` строит ops для `entity.update.proposed` (HP `set` clamped, `status`, `died_at/killed_by`, `inventory append` с `source{entity, event_id}`, `encounter.npcs[].last_damager`).
