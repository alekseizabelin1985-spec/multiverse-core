...
- **п. 1** — для нарратива `Input.Mentions map[string]string` (ярлык → id сущности) и `AbsenceEventIDs map[string]string` (ярлык → id события; прежде множество id) заполняет роль из таблицы сборщика. Пустая таблица — пустая карта, а не `nil`. Непустой `mentions` при `Mentions == nil` или непустой `background_refs` при `AbsenceEventIDs == nil` — ошибка программы: весь ответ `invalid`, `reason=other` без `element`, лог `error`. Условие проверяется по каждому массиву отдельно (N-6 ревью #2 T-439). См. ADR-029 п. 5.
- **п. 2, правило 3, нарратив** — по элементу:
  - ярлык есть в таблице и сущность видима — элемент остаётся;
  - ярлык есть в таблице, но сущность не видима — элемент отброшен, `unknown_entity` с `entity` из `View`;
  - ярлыка в таблице нет — элемент отброшен, текст остаётся, `partially_rejected`, `unknown_entity` с `ref` (C-07 v1.5).

  Выдуманный id тика — так же `unknown_entity` с `ref`. Статусы и порядок эффектов не меняются. См. ADR-029 п. 5–6.
- **доп. 1 п. 4 (US-018)** — «отсутствующая ссылка» в событии — поле `ref` (C-07 v1.5). Для нарратива «ссылка на `npc-999`» читается как «ярлык вне таблицы вызова, например `e7`». В v1.3 выдуманную фоновую ссылку закрывал `background_ref`, ярлыки этот пробел вновь открыли, v1.5 его закрывает (Mi-9 ревью #2 T-439). Временная форма ADR-029 п. 6 (`other` до новой версии C-07) отменена, поэтому расхождения с критерием 2 US-018 в коде не будет. Запись для business-analyst — отчёт T-457.
