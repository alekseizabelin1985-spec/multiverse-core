...
1. **`/forget` — checkpoint и vacuum сразу, не по расписанию (T-8, SEC-04/05; уточнение п. 5)**: `links.Store.Forget` в одном вызове выполняет `DELETE FROM links …` (транзакция, каскад `character_requests`), затем `PRAGMA wal_checkpoint(TRUNCATE)` и `PRAGMA incremental_vacuum` — до ответа клиенту; для этого `links.db` открывается с `auto_vacuum=INCREMENTAL` (п. 1). Sweeper раз в час (`Store.Compact`) остаётся страховкой (например, после аварийного завершения между DELETE и checkpoint). Интеграционный тест: `strings`-скан файла **и** `-wal` сразу после ответа `/forget` = 0.
