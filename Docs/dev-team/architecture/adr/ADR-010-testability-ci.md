...
4. **In-memory шина** (`shared/testkit/membus`, с T-418 — `shared/eventbus/membus`): топики как упорядоченные очереди, ключ игнорируется (одна партиция), режим `--chaos=duplicate` переиздаёт 5 % событий (NFR-013), `--chaos=reorder-topics` тасует между топиками (проверка ожидания по `correlation_id`).
