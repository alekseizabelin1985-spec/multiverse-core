...
| Модели | **решение пользователя (OQ-A-18, уточнено U-8; правило и порядок — ADR-005 доп. 3, T-435)**: конфигурация принимается, если **проходит пороги** (NFR-002, NFR-090, B3 в трёх прогонах) **и даёт нужную длину нарратива**; **базовая на переход — (E)** одна `Qwen3.8-27B UD-Q3_K_XL` на llama-server на нарратив и тики; **целевая конфигурация нарратива — `Qwen3.6-35B-A3B`**; порядок запасных — **E → Qwen3.6-35B-A3B → C → A** (C — `qwen3:30b-a3b`, A — `qwen3:8b` + `qwen3:14b`); итоги замера и потолки длины — `ops/metrics/baseline.md` §5; блупринты задают модель базовой конфигурации | E: 13,1 ГБ весов + KV 256 КиБ/токен (64 слоя × 4 KV-головы × 256) ≈ 2–4 ГБ при 8–16k → 16–19 ГБ, помещается целиком; для E на переход (обоснование 2026-09-09): dense 27B — качество русского ожидаемо выше MoE-3B-активных; качество целевой MoE `Qwen3.6-35B-A3B` проверяют T-438 и S1 *(редакционно, T-440)*, скорость — замер; Phase 1 в MVP-1 без LLM | (B) 8b + 30b-a3b с выгрузкой в RAM, (D) `qwen3:32b` dense, (E+) 27B + `qwen3-8b` в router-режиме — только материал замера |
...
| **Размещение моделей** NFR-076 | матрица замера §18.1 (E/Q36-A3B/C/A/B/D; редакционно, T-440); конфигурация модели на фазу в блупринте; llama-server — одна модель в single-model режиме (E) или router-режим `--models-dir` для двух (E+/A); Ollama — `OLLAMA_MAX_LOADED_MODELS=2`, `num_gpu` при частичной выгрузке; результат — `ops/metrics/baseline.md` |
...
| Q36-A3B | `Qwen3.6-35B-A3B` (MoE, ~3 млрд активных) на нативном llama-server, провайдер `openai_compat` — **целевая конфигурация нарратива** (ADR-005 доп. 3; добавлена после первого замера) | размещение и скорость — `ops/metrics/baseline.md` §5 | три прогона по методике `infrastructure.md` §6.4 и разбор хвоста «прочего» по `prompt_ms` — T-438 |
...
Решение пользователя (OQ-A-18, 2026-09-09; уточнено U-8; правило и порядок изменены ADR-005 доп. 3, T-435, 2026-09-12): конфигурация принимается, если проходит пороги **и** даёт нужную длину нарратива. Базовая на переход — **E**, целевая конфигурация нарратива — **`Qwen3.6-35B-A3B`**, порядок запасных — **E → Qwen3.6-35B-A3B → C → A**. Итоги замера, потолки длины и условия перехода на целевую — `ops/metrics/baseline.md` §5 и ADR-005 доп. 3; расчёты здесь не дублируются. *(Редакционная правка T-440, 2026-09-13: прежнее правило первого прохода и порядок без Qwen3.6 заменены.)* Блупринты уже задают модель на фазу (`model` = имя из `GET /v1/models`), поэтому смена конфигурации — правка YAML и скрипта запуска, не кода. Скрипт замера ходит в `/v1/chat/completions` (`response_format json_schema`, `chat_template_kwargs.enable_thinking=false`), метрики — из `usage`/`timings` и `nvidia-smi`.
