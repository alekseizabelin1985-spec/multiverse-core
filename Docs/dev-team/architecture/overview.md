...
C4Container
    ...
    Container_Ext(ollama, "Ollama", ":11434, GPU", "qwen3:* ; structured output; keep_alive")
    ...
    Rel(core, ollama, "HTTP /api/chat, format=JSON-schema")
...
| Ollama | LLM | Ollama ≥ 0.12, теги `qwen3:8b`, `qwen3:14b`, `qwen3:30b-a3b`, `qwen3:32b`; `OLLAMA_KEEP_ALIVE=-1`, `OLLAMA_MAX_LOADED_MODELS=2` | — | NFR-004, NFR-071, NFR-076 |
...
**Compose-профили** (`infrastructure.md` §1.3): без профиля — `redpanda`, `redpanda-init`, `minio`, `minio-init`, `gateway`, `core` (минимальный стек «соло без памяти и LLM»); `memory` — `qdrant`, `neo4j`, `memory`; `gpu` — `ollama` в контейнере (при нативном Ollama не нужен); **`bot`** — `telegram-bot` (только при заданном `MV_TELEGRAM_BOT_TOKEN`; в CI/e2e выключен; `env_file` с токеном только у него); `dev` — `redpanda-console`; `legacy` — as-is `narrative-orchestrator` + `semantic-memory` + `chromadb` до S5. **Все порты публикуются только на `127.0.0.1`** (ADR-009 дополнение п. 3; `compose-lint`): gateway 8088, core 8090, memory 8082, Redpanda 19092/9644, MinIO 9000/9001, Qdrant 6333/6334, Neo4j 7474/7687, Ollama 11434, Console 8092. Переменные окружения платформы — с префиксом `MV_` (`shared/env.Declare`); сторонние (`OLLAMA_*`, `MINIO_ROOT_*`, `NEO4J_AUTH`) — без.
...
| LLM | LLM-шлюз с интерфейсом `Provider`; реализации: Ollama (native `/api/chat`, `format`=JSON-схема, `think=false` для Phase 1/тиков, `keep_alive=-1`), OpenAI-совместимый HTTP (OpenAI, DeepSeek), Anthropic (целевое); модель — на фазу/уровень из блупринта | FR-070; structured output Ollama даёт валидный JSON по схеме (NFR-022); native API нужен ради `think` и `keep_alive`, которых нет в OpenAI-совместимом эндпоинте Ollama | `/v1/chat/completions` Ollama как основной путь (нет `think`/`keep_alive`); LangChain-подобные обёртки |
| Модели | **решение пользователя (OQ-A-18)**: базовая конфигурация — **(C)** одна `qwen3:30b-a3b` на нарратив и тики, если проходит замер NFR-002/NFR-090 по матрице §18.1 (F-8); **иначе (A)** `qwen3:8b` + `qwen3:14b`; блупринты до `baseline.md` задают C | Q4_K_M: 30b-a3b ≈ 18,6 ГБ + 8b ≈ 5 ГБ + KV-кэш > 24 ГБ — пара из решения р5 полностью в VRAM **не помещается**; Phase 1 в MVP-1 без LLM | (B) 8b + 30b-a3b с выгрузкой в RAM и (D) `qwen3:32b` dense — только материал замера |
...
|---|---|
| **Латентность** NFR-001…NFR-005 | Phase 1 без LLM (правила, in-process); шина на localhost с `MinBytes=1`/`MaxWait≤100ms`; State держит мир в памяти, PUT в MinIO асинхронно не блокирует публикацию `entity.updated`? — **нет**: факт публикуется после успешного PUT (истина — снапшот), PUT ≤ 20 мс; Phase 2 в отдельной очереди `interactive`, один вызов на раунд; ack `202` до публикации в шину (NFR-003); холодный старт исключён `keep_alive=-1` + прогрев при `make up` (NFR-004) |
| **Размещение моделей** NFR-076 | матрица замера §18.1; конфигурация модели на фазу в блупринте; `OLLAMA_MAX_LOADED_MODELS=2`; `num_gpu` в Modelfile при частичной выгрузке; результат — `ops/metrics/baseline.md` |
...
| # | Конфигурация Ollama | Ожидание по размеру (Q4_K_M, VRAM) | Что измеряем |
|---|---|---|---|
| A | `qwen3:8b` (тики) + `qwen3:14b` (нарратив), обе резидентны | ≈ 5,2 + 9,0 + KV ≈ 17–19 ГБ — помещается | NFR-002 p95, NFR-090 качество русского на 14b, NFR-022 |
...
| C | одна `qwen3:30b-a3b` на нарратив **и** тики (Phase 1 в MVP-1 без LLM) | ≈ 18,6 + KV ≈ 21–23 ГБ — помещается впритык | NFR-002; стоимость тика на MoE (3B активных — быстро); упрощение до одной модели |
| D | `qwen3:32b` dense + без второй модели | ≈ 20 ГБ + KV | только если C проваливает NFR-090 |

Решение пользователя (OQ-A-18, 2026-09-09): базовая конфигурация — **C**, если проходит NFR-002/NFR-090; иначе **A**; фиксируется в `ops/metrics/baseline.md` после матрицы F-8. Блупринты уже задают модель на фазу, поэтому смена конфигурации — правка YAML, не кода.
...
| Structured output Ollama на Qwen3 «протекает» преамбулой на некоторых версиях | NFR-022 | парсер с восстановлением JSON + повтор ≤ 2 + шаблон; пин версии Ollama; золотой набор |
