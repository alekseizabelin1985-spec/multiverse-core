...
**Как запускается.** `make scripts-parity` (входит в `make ci`, CI-задание `scripts-parity`), фильтр — `PARITY_ARGS='-run U0 -v'`. Цель сначала выполняет `go vet ./testdata/script-parity`: линтер каталог `testdata` не видит. Самопроверка — `make parity-mutants`: 29 мутантов (M00–M28) портят скрипты по одному дефекту в копии дерева, первым идёт контроль (синтаксическая ошибка), вторым — тождественный мутант (должен остаться зелёным); каждый остальной обязан покраснить стенд. Три из них (M13–M15) проверяют сами известные дефекты: новое расхождение типов JSON рядом с K02, новое расхождение потоков рядом с K01, исчезновение одного пункта K02. M16 — отказ `nvidia-smi` в `llm-bench.sh`. M17 — пункты K01, которых нет в `Reports`, исправлены разом (`Stop-Bench` пишет в stderr): ловит только сводка по прогону. M18–M28 — правило «локальный адрес» и печать значения в его отказах (T-450, §6.3.1). В `make ci` мутанты не входят: это проверка самого стенда, её запускают при правке стенда или сценариев.
...
`T01`/`T02` выполняются при любом `-run` стенда, как пары сообщений: мутант правила краснеет, даже если ни один сценарий не выбран. Мутанты стенда M18–M28: однословное имя снова `cloud`, `0.0.0.0` снова не any-address, IPv4-mapped формы снова `cloud`, адрес LAN без порта снова проходит; точка снова снимается у IPv4 — отдельно в bash (M24) и в PowerShell (M25); `%` в скобках уходит в разбор IPv6 и меняет только текст ошибки (M26, ловит сверка текста в `T02`); обратная косая в хосте снова классифицируется (M27); отказы до проверки `@` в PowerShell снова печатают `user:pass@` (M28, ловят сценарии `H61`–`H66` по отсутствию фиктивного пароля). Сценарии `D07` (`0.0.0.0` в `down`) и `D08` (адрес LAN без порта) переносят изменение правила в `llm-server`. Новый случай добавляется в таблицу, а не в одну из реализаций.
