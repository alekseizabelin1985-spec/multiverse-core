...
**Как запускается.** `make scripts-parity` (входит в `make ci`, CI-задание `scripts-parity`), фильтр — `PARITY_ARGS='-run U0 -v'`. Цель сначала выполняет `go vet ./testdata/script-parity`: линтер каталог `testdata` не видит. Самопроверка — `make parity-mutants`: 18 мутантов портят скрипты по одному дефекту в копии дерева, первым идёт контроль (синтаксическая ошибка), вторым — тождественный мутант (должен остаться зелёным); каждый остальной обязан покраснить стенд. Три из них (M13–M15) проверяют сами известные дефекты: новое расхождение типов JSON рядом с K02, новое расхождение потоков рядом с K01, исчезновение одного пункта K02. M16 — отказ `nvidia-smi` в `llm-bench.sh`. M17 — пункты K01, которых нет в `Reports`, исправлены разом (`Stop-Bench` пишет в stderr): ловит только сводка по прогону. В `make ci` мутанты не входят: это проверка самого стенда, её запускают при правке стенда или сценариев.
...
**Состав (T-405):** 97 сценариев. 60 — `health` (матрица T-404: адрес, схема, IPv6, userinfo, невидимые символы, TLS, ключи, провайдеры, 401/503/500, пин сборки, отсутствие модуля, отказ `nvidia-smi`). 18 — `up`/`down` (T-437: `--alias` в single и его отсутствие в router, `.GGUF`, точки в имени, пустой id, пробелы в `-m`/`--models-dir`/`--slot-save-path`, SEC-15, чужой сервер на адресе, повторный `up`, `--with-ui`, прогрев 404). 6 — `down` (битый и устаревший PID-файл, адрес LAN и облака без пробы). 11 — замер (T-434: матрица из репозитория и вне его, id-путь с `\` и `"`, прогрев 500, без `/props`, округление доли кэша и `tps` на десятичной середине, пропуск фаз, недоступный сервер, неизвестная конфигурация, отказ `nvidia-smi`). 2 — известные дефекты. И 2 пары сообщений (`P01`, `P02`). Сознательно не входят: обратная косая в `MV_LLM_URL` (bash превращает её в `/` по пути к pwsh); облачный адрес с живой пробой (нужен DNS-адрес, резолвящийся в loopback, то есть сеть); PID-файл с живым чужим процессом (под Git Bash `kill -0` не видит номер нативного процесса Windows — расхождение окружения, а не скриптов).
