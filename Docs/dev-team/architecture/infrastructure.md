...
Проверяет модель `docker compose config --format json` (после интерполяции) и сами файлы в том виде, как они написаны: литерал пароля и верная `${VAR:?}` в модели выглядят одинаково. Правил **восемь** — ровно столько, сколько перечисляет шапка `scripts/compose-lint.sh`. Прежняя редакция этого раздела знала пять и держала правило про `llama-server` внутри пятого (сверка T-416). T-413 уточнил правила 3, 7 и 8, их число не изменилось:
...
Фикстуры — `testdata/compose-lint/`. Каждая `bad-*.yml` называет своё правило строкой `# expect-rule: N`, а причину — строками `# expect-text:`. Каждая `good-*.yml` обязана проходить. `scripts/compose-lint.sh --fixtures` (второй шаг `make compose-lint` и задания CI `compose-lint`) требует, чтобы «плохую» фикстуру отверг ровно её правило и по её причине: раньше проверялся только ненулевой код (приёмка T-411, T-413). Прогон отказывает и в этих случаях:
   ...
   Иначе переезд каталога тихо выключил бы самопроверку (ревью #1 T-413).
