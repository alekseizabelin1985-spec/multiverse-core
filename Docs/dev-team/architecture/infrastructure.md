...
3. Порядок решения — `decision_order`: сначала **E** (`num_ctx` 8192 и 16384, KV `f16` и `q8_0` — четыре ячейки), затем **Qwen3.6-35B-A3B**, затем **C**, затем **A**. Конфигурация принимается, если прошла пороги **и** даёт нужную длину нарратива (ADR-005 доп. 3; прежняя формулировка «прошла → базовая» отменена). E+ прогоняется как материал, если E не проходит по стоимости тика, но проходит по нарративу. **Факт замера T-402 (`baseline.md` §5):** зачётные прогоны E шли в окне 4 × 200 192 и на KV `q4_0` — два отклонения от этого правила, принятые для вердикта; ячейку платформы (8192 × 1, `f16`) доказывает контрольный прогон сессии T-438, там же три прогона Qwen3.6.
