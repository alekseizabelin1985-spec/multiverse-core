...
**Правка T-476** (2026-09-14, devops-engineer#1; тексты — решения system-architect по T-059 и T-315): §2.2 — `make health` печатает настоящие статусы, `DEGRADED_STRICT`, строки `make up` и `make test-race` (`RACE_E2E_PKGS` с e2e бота); §2.3 — проба принимает `ok` и `degraded`; §3.1 — задание `race`; §7.2 — `degraded` у пробы compose; §8 п. 1, 3 и §9.1 п. 3 — флаги `make health` и `core degraded` до `mvctl world init`.
...
| `make test` | `go test -short -race -count=1 -cover ./...` (unit, без сети); `-race` подставляется, только если доступны cgo и компилятор C, иначе цель вслух печатает, что прогон без детектора (ОВ-5) |
| `make test-integration` | `go test -tags integration -race -count=1 -timeout 20m ./...` (testcontainers; нужен Docker и собранный образ MinIO — цель зависит от `minio-image`); `-race` — по тому же правилу, что у `make test` (T-401) |
| `make test-e2e` | `go test -tags e2e -count=1 -timeout 10m ./...` (один процесс `--contexts=all --mode=replay --bus=memory`) |
| `make test-race` | то, что вызывает задание CI `race` (T-401): `go test -race -count=$(RACE_COUNT) -timeout 10m $(RACE_PKGS)`, затем `go test -race -tags e2e -count=1 -timeout 10m $(RACE_E2E_PKGS)` с `GOFLAGS=-race`, где `RACE_E2E_PKGS` — `./test/e2e/... ./cmd/telegram-bot/e2e/...`. `RACE_PKGS`, `RACE_E2E_PKGS` и `RACE_COUNT=3` — единственный экземпляр списков пакетов и числа повторов. E2e бота — во второй команде, а не в `RACE_PKGS`: все файлы `cmd/telegram-bot/e2e` несут `//go:build e2e`, без тега шаблон не находит пакетов, и `go test` лишь предупреждает (решение по T-315, T-476). Без cgo — отказ с кодом 1, причина называет сработавшую половину условия: компилятора нет в `PATH` или `CGO_ENABLED=0` при найденном компиляторе. Внутри `make ci` — `SKIPPED` с кодом 0 (`ci: RACE_OPTIONAL := 1`). `RACE_OPTIONAL` задан в файле, поэтому значение из окружения его не меняет; `make ci RACE_OPTIONAL=` делает отказ строгим |
...
| `make up` | `$(COMPOSE) up -d --wait` → `make health LLM_STRICT=0 DEGRADED_STRICT=0`; состав `$(COMPOSE)` — абзац под таблицей. Если `legacy` в активном наборе профилей, сначала выполняется `make legacy-src`. LLM-процесс **не запускает** — только проверяет `GET $MV_LLM_URL/health` и печатает предупреждение, если не `200` (деградация FR-080/NFR-072) |
...
| `make health` | обходит `/health` gateway/core/memory/bot, вызывает `make llm-health`, печатает таблицу настоящих статусов (`ok`, `degraded`, `FAIL`) и возраст последнего бэкапа; код возврата ≠ 0, если процесс `fail` или недоступен, а также при `degraded`, пока `DEGRADED_STRICT=1` (по умолчанию). `make up`, `deploy` и `rollback` зовут его с `DEGRADED_STRICT=0`: `degraded` — работающий процесс (§7.2), например `core` с миром `uninitialized` до `mvctl world init` (КД State §18, §19) |
...
- В образе нет shell и `curl` — healthcheck compose вызывает подкоманду бинарника `/multiverse health` без `--url` (§7.2): код 0 — HTTP 200 и `status ∈ {ok, degraded}`, код 1 — `fail` (HTTP 503), другой код, недоступный адрес, неразборчивое тело или статус вне перечня. Подкоманда — часть каркаса `cmd/multiverse` (F-2).
...
| `integration` | setup-go → buildx → `docker build -f build/minio.Dockerfile … --cache-from type=gha --cache-to type=gha,mode=max --load` (образ `${MINIO_IMAGE}` для testcontainers) → `go test -tags integration -race -count=1 -timeout 20m ./...` (testcontainers-go поднимает Redpanda/MinIO/Qdrant/Neo4j из `testkit.Versions()`; `TESTCONTAINERS_RYUK_DISABLED=false`, `CGO_ENABLED=1`). `-race` — с T-401: контрактный набор на живом брокере (кейс `Close`, кейсы с хвостом) и kafka-адаптер больше нигде под детектор не попадают. Флаг стоит у существующего задания, а не в отдельном: иначе образ MinIO и контейнеры пришлось бы поднимать второй раз ради того же набора. `timeout-minutes: 30` (было 25): холодный кэш buildx (~5 мин) плюс 20 мин теста уже давали 25 | 6–10 мин с детектором, оценка, не замер (первый раз +5 мин сборка MinIO) | да |
| `e2e` | setup-go → `go test -tags e2e -count=1 -timeout 10m ./...` (один процесс, `--bus=memory`, записи `testdata/recordings/*.jsonl`; без Docker) → upload `ops/metrics/sessions/*.json` как артефакт. Без `-race`: это ворота «e2e ≤ 10 мин» ADR-010, детектор для e2e — в задании `race` | 1–2 мин | да |
| `race` (T-401) | setup-go → `make test-race` в строгом режиме: без cgo цель громко отказывает. Список пакетов и число повторов существуют в одном экземпляре — `RACE_PKGS` и `RACE_COUNT=3` в Makefile (ревью #1 T-401, Mi-1); правила «CI не вызывает make» в проекте нет. Цель выполняет `go test -race -count=$(RACE_COUNT) -timeout 10m $(RACE_PKGS)`, затем `go test -race -tags e2e -count=1 -timeout 10m ./test/e2e/... ./cmd/telegram-bot/e2e/...` (`RACE_E2E_PKGS`) с `GOFLAGS=-race` и печатает обе команды в лог. E2e бота (T-315: три горутины бота и шлюз в одном процессе) идёт под детектором только здесь; задание `e2e` и `make test-e2e` гоняют его без детектора (T-476). Без Docker, `CGO_ENABLED=1`. `timeout-minutes: 25` складывается из настройки и компиляции плюс двух вызовов по 10m: `-timeout` действует на тестовый бинарник, и все проходы `-count` идут внутри него. Зачем при `-race` в `unit`: детектор видит только гонки, которые случились в прогоне, а `unit` проходит каждое чередование один раз. Здесь пакеты, где встречаются горутины, идут трижды: `membus` (`deliverNext`, `SetChaos` на работающей шине), контракт на `membus` (`Close`, кейсы с хвостом), `Harness.mu`, двойники роя (проекция `FakeNarrator`, `failMu` у `FakeEncounter`), `runtime`, `clock`. Список — пакеты, а не `-run`: шаблон, переставший совпадать после переименования, даёт зелёный `[no tests to run]`. `GOFLAGS=-race` наследует `go build` дочернего `cmd/multiverse` в e2e: гонка в процессе даёт код выхода 66, и кейс остановки по сигналу, требующий 0, падает с выводом процесса. Повторяет локально `make test-race` | 4–7 мин, оценка, не замер: без детектора тот же набор с `-count=5` идёт ~19 с на машине владельца, детектор — ×2–20 по CPU | да — после того как владелец добавит `race` в required checks |
...
| `scripts-parity` (T-405) | setup-go → версии `bash`, `curl`, `python3`, `pwsh` в лог → `make scripts-parity`: стенд паритета `testdata/script-parity` (§3.1.2) прогоняет одни и те же входы через `scripts/llm-server.{sh,ps1}` и `scripts/llm-bench.{sh,ps1}` и сверяет коды возврата, сообщения, аргументы запуска `llama-server`, запросы, CSV и поля JSON. Без Docker и без сети: вместо `llama-server` — двойник на loopback. Задание зовёт стенд с `-pwsh=require`: CI — единственное место, где `.ps1` проверяется на Linux, поэтому образ раннера без `pwsh` роняет задание, а не переводит его молча на половину для оболочки (ревью #1 T-405, Mi-2). `make ci` у владельца остаётся в режиме `auto` | 1–3 мин с настройкой, оценка, не замер: на машине владельца весь набор идёт ~65 с при 4 параллельных, раннер берёт 2 | да — после того как владелец добавит `scripts-parity` в required checks |
...
**Что читается.** По умолчанию линтер берёт всю топологию §1.3: `docker-compose.yml`, `docker-compose.bot.yml`, `docker-compose.legacy.yml`. Первый `-f` заменяет набор, следующие добавляют к нему; первый файл набора — тот, к которому применяется правило 7. Правила 1–6 читают одну модель по всем файлам и всем профилям после интерполяции: `docker compose config --format json` с `build/versions.env` и `.github/ci.env`. Правила 3, 7 и 8, а также источник образа у правила 1 читают каждый файл отдельно, до интерполяции: `config --no-interpolate --no-consistency --format json --profile '*'` (T-429). Скрипт воспроизводит только интерполяцию, YAML разбирает сам compose. Значения берутся только из файлов: имена, объявленные в `--env-file` линтера, в `build/versions.env` и в `.env.example` правила 7, скрипт снимает со своего окружения до первого `docker compose`. Compose ставит переменную процесса выше `--env-file`, и пустой `CHROMA_IMAGE` (D-3), экспортированный заданием CI через `$GITHUB_ENV`, скрывал заглушку `.github/ci.env`: модель не собиралась в CI и собиралась на машине, где пины не экспортированы (T-455). Итоговая строка: `ok — N services in 3 file(s), 8 rules`. Правил восемь:
...
3. **Нет учёток по умолчанию.** Секрет в `environment` — только подстановка переменной, не обязательно одноимённой (`NEO4J_AUTH: neo4j/${NEO4J_PASSWORD:?…}`), у обязательных — `${VAR:?}`. Литерал и экранированная `$${VAR}` — нарушения у любого секрета. Ключ без значения — только у обязательного; законно пусты облачные `MV_LLM_API_KEY` и `MV_ANTHROPIC_API_KEY` и соль `MV_TELEGRAM_ACTION_KEY_SALT` (без соли ключ HMAC `action_key` выводится из токена бота, ADR-018, T-310): у тех из них, что передаются в композицию, — `${VAR:-}` с пустым умолчанием манифеста (`MV_ANTHROPIC_API_KEY` в композицию пока не передаётся — T-469), а `:?` остановил бы стек, который вправе стартовать (T-464; изм. T-470). `minioadmin` нет вне `Docs/` и `services/_archive/` (SEC-14; T-411, T-413).
...
6. **LLM вне compose.** Сервиса `llama-server` нет. `MV_LLM_URL` и `MV_OLLAMA_URL` сервисов — локальные адреса по единому правилу §6.3.1: линтер вызывает `llm_endpoint_classify` из `scripts/lib/llm-endpoint.sh`, своей копии правила у него нет. Облачный адрес — нарушение; недопустимый (`0.0.0.0`, локальный хост без порта) — тоже (ADR-005 доп. 2 п. 3, п. 7; SEC-15; T-450).
...
**Фикстуры** — `testdata/compose-lint/`, порядок и перечень в её `README.md`. `scripts/compose-lint.sh --fixtures` — второй шаг `make compose-lint` и задания CI `compose-lint`. Каждую `bad-*.yml` должно отвергнуть ровно правило её строки `# expect-rule: N`, и по причине из строк `# expect-text:`; строки `# expect-absent:` — фрагменты, которых в отказе не должно быть ни в каком регистре (T-463). Каждая `good-*.yml` обязана проходить. Пустой или пропавший каталог — отказ, а не зелёный прогон (T-411, T-413, T-429). Все прогоны фикстур идут с этими именами, экспортированными пустыми: линтер, который пропустил к compose окружение вызывающего по именам своих `--env-file`, краснеет на машине владельца, а не только в CI. Снятие имён `.env.example` (и парного `.env` фикстуры) — защитная мера без отдельной проверки: `${VAR:?}` фикстуры сначала разрешается в модели правил 1–6, а модель читает только `--env-file` (T-455).

Выход ≠ 0 при любом нарушении; тот же скрипт вызывают `make compose-lint` и задание CI `compose-lint`.

Ветки (ADR-010, `teams.md` §3, `epics.md` v0.2 п. 4): интеграционная ветка **`integration/mvp-1`** (создана в F-0 от `feature/agent-gm-core`), ветки эпиков `epic/EPIC-00N-<slug>` на весь MVP-1, `main` — то, что задеплоено. CI одинаков на всех; правило защиты `main` и `integration/mvp-1` — required checks `unit, integration, e2e, race, contracts, security, compose-lint, scripts-parity` (`race` добавлен T-401, `scripts-parity` — T-405; в настройки репозитория их вносит владелец — пока задание не отмечено, оно отчитывается, но мерж не блокирует), линейная история, без force-push, CODEOWNERS-ревью. Порядок слияния инкрементов — `epics.md` §5/§6; отдельного «интеграционного» CI не нужно — тот же workflow на `integration/mvp-1`. Сборка интеграционной ветки перед этапом интеграции = зелёный `go.yml` на `integration/mvp-1` после слияния каждого инкремента + `make ci-full` на машине владельца перед I1-α.

#### 3.1.2. Стенд паритета скриптов LLM (T-405)

**Зачем.** `scripts/llm-server.{sh,ps1}`, `scripts/llm-bench.{sh,ps1}` и модули `scripts/lib/llm-endpoint.sh` / `LlmEndpoint.psm1` — одно правило, записанное дважды. Свойство «обе половины на одном входе говорят и делают одно и то же» дважды нарушалось в одной задаче: ревью #1 T-404 нашло 10 расхождений на 24 входах, в том числе противоположные коды возврата. Каждая следующая задача (T-404, T-434, T-437) собирала свой стенд во временном каталоге и выбрасывала его, поэтому между задачами свойство не держалось ничем.

**Где и почему там.** `testdata/script-parity/` — программа на Go из стандартной библиотеки. Шаблоны `./...` инструмента go пропускают каталоги `testdata`, поэтому стенд не попадает в `go build`/`go vet`/`golangci-lint`/`govulncheck`/порог покрытия модуля. Запреты воспроизводимости платформы (`forbidigo`: `shared/clock`, `shared/env`) к нему не относятся: стенду нужно настенное время и окружение дочерних процессов. `scripts/lib/` — место того, что подключают сами production-скрипты; двойник сервера рядом с единственным правилом адреса уехал бы вместе с ним.

**Как запускается.** `make scripts-parity` (входит в `make ci`, CI-задание `scripts-parity`), фильтр — `PARITY_ARGS='-run U0 -v'`. Цель сначала выполняет `go vet ./testdata/script-parity`: линтер каталог `testdata` не видит. Самопроверка — `make parity-mutants`: 33 мутанта (M00–M32) портят скрипты по одному дефекту в копии дерева, первым идёт контроль (синтаксическая ошибка), вторым — тождественный мутант (должен остаться зелёным); каждый остальной обязан покраснить стенд. Три из них (M13–M15) проверяют сами известные дефекты: новое расхождение типов JSON рядом с K02, новое расхождение потоков рядом с K01, исчезновение одного пункта K02. M16 — отказ `nvidia-smi` в `llm-bench.sh`. M17 — пункты K01, которых нет в `Reports`, исправлены разом (`Stop-Bench` пишет в stderr): ловит только сводка по прогону. M18–M32 — правило «локальный адрес» и печать значения в его отказах (T-450, T-463, §6.3.1). В `make ci` мутанты не входят: это проверка самого стенда, её запускают при правке стенда или сценариев.

**Что делает.**
- Каждый сценарий получает свежую копию нужных файлов (`scripts/llm-*`, `scripts/lib/*`, `build/versions.env`, матрица и промпты замера) отдельно для каждой реализации. `up` пишет `ops/llm-server.pid`, замер — отчёты; репозиторий не трогается.
- Окружение строится с нуля: все `MV_*` и переменные прокси вызывающего убираются, сценарий задаёт свои.
- Вместо `llama-server` — двойник. В сценариях `health` и замера это HTTP-сервер внутри стенда на `127.0.0.1:<свободный порт>`: `/health`, `/v1/models`, `/props`, `/v1/chat/completions` с настраиваемыми ответами (404 на `/health`, как у стенда владельца; id-путь; не-200 на прогреве; без `timings.prompt_ms`; 401 без ключа). В сценариях `up`/`down` это тот же бинарник под нейтральным именем `parity-double` (антивирус удалял двойника с именем `llama-server.exe`, ревью T-437), его путь уходит в `MV_LLM_BIN`. Он записывает свой argv и полученные запросы, слушает `--host:--port` и сам завершается по токену прогона или по истечении срока жизни. Стенд не завершает процессы по номеру или имени; в конце он проверяет, что на портах двойника никто не слушает. Порт 8888 стенд не слушает и не называет ни в одном входе: адреса, которые скрипты обязаны отвергнуть до пробы (голый IPv6, незакрытая скобка, адрес без хоста, адрес LAN для `down`), берут заведомо свободный порт прогона (порт выдаётся одному сценарию на весь прогон: закрытый порт ОС возвращает следующему `listen :0`, и «мёртвый» порт одного сценария становился портом двойника другого — это однажды покраснило B07 и B08), чтобы при любой регрессии разбора проба не ушла в сервер владельца (ревью #1 T-405, Mi-1).
- Первым в `PATH` каждого скрипта стоит `nvidia-smi` стенда (тот же бинарник под этим именем): строка VRAM `health` и `vram_used_mb` замера одинаковы на любой машине, с GPU и без, настоящая утилита не вызывается. С `MVPARITY_GPU=fail` он ведёт себя как `nvidia-smi` без драйвера: жалоба в stdout и код 9 (сценарии H60, B11).
- Ctrl+C и отмена задания: стенд перестаёт брать сценарии, дочерние процессы получают отмену, двойники гасятся по токену прогона, каталог удаляется по сохранённому пути; если Windows не отдаёт каталог (exe ещё завершается), стенд печатает точный путь. Паника в сценарии — `FAIL` этого сценария, остальные идут дальше (ревью #1 T-405, Mi-3).
- Сравнивается: код возврата; stdout и stderr построчно; argv запуска `llama-server` — побайтно; запросы к двойнику (метод, путь, заголовок `Authorization`, `model` тела); `/v1/models` после `up`. Для замера — CSV (строки целиком, кроме `first_call_ms` (заполнено/пусто), `p50_ms`, `p95_ms`, `started_at`), таблица (кроме P50/P95), сообщения (отсортированный набор, поток каждой строки и порядок строк внутри потока), набор полей JSON-отчёта, значения (`"8192"` и `8192` равны) и — отдельной проверкой — типы значений каждого поля.
- Нормализуется только то, что не может повториться: порт, pid, миллисекунды, путь временной копии (три формы: Windows, `/`, MSYS `/tmp/...`, плюс длинная форма коротких имён 8.3), метка времени в имени отчёта, имя модуля в сообщении об его отсутствии.
- У каждого сценария, кроме сравнения, есть абсолютные ожидания (код возврата, ключевые строки, точный argv, `/v1/models`, свободный порт, поля CSV/JSON). Иначе две половины, ошибающиеся одинаково, прошли бы.

**Без `pwsh`** (Linux-раннер без PowerShell) стенд прогоняет половину для оболочки против абсолютных ожиданий. Кроме того, он статически сверяет тексты сообщений пар файлов (`P01`, `P02`): литералы с `llm: `/`bench: ` и аргументы `die`/`fail()`/`Stop-Bench`/`LLM_EP_ERROR=`/`.Error =` приводятся к шаблону с `<V>` вместо подстановок. Шаблон, который есть только в одном файле пары, — находка. Исключение из этого правила записывается в `messageExceptions` с причиной. Исключение, которое больше ничего не извиняет, — тоже находка. Сценарии, которые без второй половины проверить нечем, печатаются `SKIP` с причиной.

**Известные дефекты** помечаются не сценарием, а поимённо — каждым расхождением, из которых дефект состоит (`testdata/script-parity/known.go`; ревью #1 T-405, M-1: прежняя пометка сценария глушила весь класс проверок, и под ней уже пряталось незаписанное расхождение). Проверки, которые могут нести известный дефект (`streams`, `json-types`), выдают каждое отличие отдельно. Отличие, совпавшее с пунктом дефекта, откладывается; любое другое отличие того же класса валит сценарий — в **каждом** сценарии замера, не только в отчётном. Отчётный сценарий называет пункты, которые обязан увидеть: все на месте и больше ничего — `KNOWN-FAILING`; пункт пропал — `FIXED-BUT-MARKED` (пункт убрать из списка, при пустом списке закрыть строку бэклога); что-то сверх пунктов или неисполненное ожидание — `FAIL`. Отчётный сценарий видит не все пункты, поэтому в конце прогона стенд сводит пункты по **всем** сценариям (ревью #2 T-405, Mi-1). Пункт, не встреченный ни в одном сценарии, — строка `FIXED-BUT-MARKED` и код выхода 1: так в CI красное задание, а в логе названо, какой пункт убрать. Пункт, для которого сценария пока нет, помечен в `known.go` как `withoutScenario` и печатается строкой `UNCOVERED`, не валя прогон; если такой пункт всё же встретился — `FAIL`, пометку снять. Сводка судит только полный прогон: обе половины и все сценарии замера (`B*`, `K*`); частичный прогон печатает, что судить не может. На T-405 два дефекта:
- `K01` — потоки `llm-bench`: 16 строк диагностики, которые `.sh` пишет в stderr, а `.ps1` — в stdout через `Write-Host` (строка о пути промптов, сборка, пропуск фазы, путь отчёта и CSV, «this is ONE run», сообщения `Stop-Bench` и др.). Строка прогрева и отсутствие модуля в обеих идут в stderr и в список не входят: их перенос — новый дефект. Отчётный сценарий видит пять из них, весь прогон — десять; шесть (`loading`, `no usable answer`, `failed requests`, `not an endpoint`, `still loading`, `endpoint variable`) помечены `withoutScenario` и ждут сценариев (бэклог). Правило потоков выбирается на приёмке T-405;
- `K02` — типы JSON-отчёта, ровно семь путей: `meta.num_ctx`, `meta.n_per_cell`, `meta.repeats`, `meta.runs_required`, `meta.first_call_ms` и `requests[].http` — строка в `.sh`, число в `.ps1`; `cells[].vram_used_mb` — наоборот, число в `.sh`, строка в `.ps1` (стабильно благодаря `nvidia-smi` стенда).

**Состав (T-405, T-450, T-463):** 111 сценариев. 71 — `health` (матрица T-404: адрес, схема, IPv6, userinfo, невидимые символы, TLS, ключи, провайдеры, 401/503/500, пин сборки, отсутствие модуля, отказ `nvidia-smi`; T-450 — userinfo не печатается ни одним отказом, `H61`–`H66`; T-463 — значение с `@` после первого `/` не печатается фразами правила локального адреса, `H67`–`H71`). 19 — `up`/`down` (T-437: `--alias` в single и его отсутствие в router, `.GGUF`, точки в имени, пустой id, пробелы в `-m`/`--models-dir`/`--slot-save-path`, SEC-15, чужой сервер на адресе, повторный `up`, `--with-ui`, прогрев 404; T-463 — отказ «второй сервер» не печатает значение с `@` и его хост, `U19`). 8 — `down` (битый и устаревший PID-файл, адрес LAN и облака без пробы; T-450 — `0.0.0.0` и адрес LAN без порта, `D07`, `D08`). 11 — замер (T-434: матрица из репозитория и вне его, id-путь с `\` и `"`, прогрев 500, без `/props`, округление доли кэша и `tps` на десятичной середине, пропуск фаз, недоступный сервер, неизвестная конфигурация, отказ `nvidia-smi`). 2 — известные дефекты. И 2 пары сообщений (`P01`, `P02`) и 2 проверки таблицы локального адреса (`T01`, `T02`, §6.3.1). Сознательно не входят: обратная косая в `MV_LLM_URL` (bash превращает её в `/` по пути к pwsh); облачный адрес с живой пробой (нужен DNS-адрес, резолвящийся в loopback, то есть сеть); PID-файл с живым чужим процессом (под Git Bash `kill -0` не видит номер нативного процесса Windows — расхождение окружения, а не скриптов).

**Правило округления замера** закреплено стендом (`B01`, `B02`). Все округлённые числа CSV, JSON и таблицы считаются как `round` Python: двоичное значение double, половина — к чётному. В `llm-bench.sh` так было всегда, и все отчёты репозитория сделаны им. `llm-bench.ps1` получил `Get-BenchRound` вместо `[math]::Round`, который масштабирует значение в плавающей точке и попадает на середину, где double её не имеет: 348/800 давал `0.44` против `0.43`, 1333/20 — `tps` `66.6` против `66.7`. Сверено с Python на 190 425 значениях: 0 расхождений (у `[math]::Round` — 636).
...
2. `MV_OLLAMA_URL` и блок `OLLAMA_*` — **не обязательны**: они нужны только при `MV_LLM_PROVIDER=ollama` (или при `MV_MEMORY_EMBED_PROVIDER=ollama`, ADR-005 доп. 2 п. 8). `MV_OLLAMA_URL` объявлена в реестре `shared/env` с пустым дефолтом и **условно обязательна** — адрес не угадать: `shared/env` требует её, только когда выбран провайдер `ollama`, и `mvctl env check` проверяет это же правило. Блок `OLLAMA_*` с T-413 объявлен **с умолчаниями** — это значения настройки §6.4 и loopback-origins SEC-15 (`contracts.md` §16 п. 5): `OLLAMA_KEEP_ALIVE=-1`, `OLLAMA_MAX_LOADED_MODELS=2`, `OLLAMA_NUM_PARALLEL=1`, `OLLAMA_FLASH_ATTENTION=1`, `OLLAMA_KV_CACHE_TYPE=f16`, `OLLAMA_ORIGINS=http://127.0.0.1,http://localhost`. Условной обязательности у блока нет, compose профиля `gpu` подставляет ровно эти значения, а правило 8 `compose-lint` сверяет их с манифестом. В `.env.example` строки закомментированы — это не ошибка, а правило «обязательная переменная пуста в примере» сохраняется.
...
4. `MV_LLM_CLOUD_ENABLED=false` — гейт **по host** в `MV_LLM_URL`: какие адреса локальные, а какие — ошибка конфигурации, задаёт единое правило §6.3.1 (таблица `testdata/llm/local-endpoints.tsv`, C-15 v1.3); адрес класса `cloud` требует `true`, иначе провайдер не стартует (ADR-005 доп. 2 п. 3). Смена `MV_LLM_URL` на облачный без флага — не «тихая деградация», а fatal при старте.
...
|---|---|---|---|---|
| Том `minio_data` (истина: сущности + снапшоты; `prompts-*` **исключён**, SEC-22) | `make backup` → остановить `core` → `docker run --rm -v multiverse_minio_data:/src:ro -v "$BACKUP_DIR":/dst alpine:3.22 tar czf /dst/minio-<date>.tgz --exclude='prompts-*' -C /src .` → запустить `core`; альтернатива без остановки — `mc mirror --overwrite --exclude 'prompts-*/**' local/ ./backups/minio-<date>/` | перед каждым `make deploy`; еженедельно (Task Scheduler → `pwsh scripts/backup.ps1`); очистка — ежедневно (Task Scheduler → `scripts/backup-prune.sh`, T-463) | `%USERPROFILE%\multiverse-backups\` (вне репозитория и Docker-томов; OneDrive `Documents` не синхронизируется — U-6, отдельного требования нет); **≤ 30 дней** по штампу в имени `minio-<%Y%m%d-%H%M%S>.tgz`, строго по возрасту, без «последних N» (решение оркестратора 2026-09-13; `make backup-prune`, runbook §4) | ежемесячно: `make restore FILE=… TARGET=scratch` в отдельный compose-проект (`COMPOSE_PROJECT_NAME=mv-restore`) → `mvctl report --audit` → `state_hash` совпадает со снапшотом |
...
*Пометка (T-463, devops):* в таблице изменена только эксплуатационная часть — срок хранения, ежедневная очистка и имя её скрипта. Текст раздела принадлежит system-architect и devops (`ownership.md`); расхождения по существу переданы system-architect в карточке T-463.
...
else         { $args += @('-m', $Model, '--alias', <имя файла без .gguf>) }   # T-437
...
| — | **`--alias <имя файла без .gguf>`** добавлен, только в single-режиме (T-437) | без него `llama-server` отдаёт в `/v1/models` путь к файлу, и модель блупринта не сверяется ни валидатором (C-11 правило 7а), ни `Health` провайдера, а `make bench` пропускает фазы (ADR-005 «Уточнение исполнения» п. 6). В router-режиме сервер сам называет модели по имени файла |
...
Пин версии: `llama-server --version` печатает `build: <N> (<sha>)`; номер вносится в `build/versions.env` как `LLAMACPP_BUILD` при F-6 и печатается `make llm-health` (с T-437 пин — `b10878`, билд зачётных прогонов T-402 по `build_info` из `GET /props`; было `b10441`) — расхождение пина и факта = предупреждение (не ошибка) с ссылкой на §9.5 и риск §12.
...
| `GET /v1/models` | одно имя — загруженная модель, имя её файла без `.gguf`: так его называет `--alias` (T-437); без алиаса сервер отдаёт путь к файлу | имена (stem) всех `.gguf` в каталоге; загруженные — по `POST /models/load` / `POST /models/unload` и по первому запросу |
...
#### 6.3.1. Локальный адрес модели и гейт облака: одно правило, одна таблица (T-450)

**Зачем.** Вопрос «локален ли адрес модели» задают трое: гейт облака платформы (`IsLocalEndpoint` в `internal/llm`, T-206), обвязка (`scripts/lib/llm-endpoint.sh` / `LlmEndpoint.psm1`) и правило 6 `compose-lint`. До T-450 правило было записано трижды и по-разному. Обвязка доверяла RFC 1918, но не имени сервиса compose. Линтер доверял сервису своего файла и всему, что `ipaddress` Python считает private, включая `0.0.0.0` и адреса документации. Платформа не доверяла ни тому, ни другому. Итог: `MV_OLLAMA_URL=http://ollama:11434` линтер принимал, а платформа отвергала.

**Правило** (решение system-architect#1). У адреса один из трёх ответов:

| Ответ | Что входит |
|---|---|
| `local` | loopback `127.0.0.0/8` и `::1`; `localhost` и `*.localhost`; `host.docker.internal`; однословное имя без точки — имя сервиса compose (`ollama`, `core`); RFC 1918; link-local `169.254.0.0/16` и `fe80::/10`; IPv6 ULA `fc00::/7`; IPv4-mapped форма каждого из них |
| `invalid` | ошибка конфигурации, а не облако: `0.0.0.0` и `::`; локальный хост без явного порта; порт вне 1–65535; всё, что отвергает разбор адреса (нет схемы http/https, userinfo, `?` или `#`, даже пустые) |
| `cloud` | всё остальное: публичные адреса, любое имя с точкой (включая `.local`), CGNAT `100.64.0.0/10`, адреса документации, числовые формы (`2130706433`, `0x7f000001`, `134744072`, `0x08080808`), IPv4 с завершающей точкой (`127.0.0.1.`) |

Имена не резолвятся: **класс** адреса не зависит от DNS в момент проверки. **Куда уйдёт запрос** — зависит. Однословное имя проходит список поиска (`search`) резолвера хоста или контейнера и может разрешиться в корпоративный или внешний адрес. `*.localhost` в loopback отправляет не каждый резолвер: RFC 6761 выполняется не везде. Это принятый риск решения system-architect#1, строка риска — `Docs/ops/runbook.md` §3. Уточнения записаны в шапке таблицы (решения system-architect#1 по ревью #1 T-450). Кратко:
- завершающая точка снимается только у зарезервированных имён: `localhost.`, `api.localhost.`, `host.docker.internal.` — `local`. IPv4 с точкой (`127.0.0.1.`, `192.168.1.10.`, `0.0.0.0.`) для `netip.ParseAddr` не адрес: клиент Go, curl и `Invoke-WebRequest` резолвят его как DNS-имя, поэтому это `cloud`, как `127.1`. Однословное имя с точкой (`ollama.`) — абсолютное DNS-имя, тоже `cloud`;
- однословное имя начинается с буквы и состоит из `[a-z0-9_-]`; числовые записи хоста — `cloud`;
- записанный порт считается явным, в том числе `:80`; ведущие нули порта незначимы; любой `local` без порта — `invalid`;
- `host:` с пустым портом, скобки не вокруг IPv6, `%` в хосте, схема не http/https, userinfo, пустые `?`/`#` — `invalid`;
- символы, которые Go `url.Parse` не пускает в хост (пробел, `\`, `^`, `` ` ``, `{`, `|`, `}`), — `invalid`. Символы, которые `url.Parse` пропускает (`~ ! $ & ' ( ) * + , ; =`), дают имя не однословной формы, то есть `cloud`;
- any-address — только `0.0.0.0` и `::`, включая `::ffff:0.0.0.0`; `::127.0.0.1` и NAT64 `64:ff9b::/96` не mapped-формы.

**Одна таблица случаев** — [`testdata/llm/local-endpoints.tsv`](../../../testdata/llm/local-endpoints.tsv): URL, ответ, причина словами; 121 случай на итерации 2 T-450. Пробел в URL формат таблицы не допускает: два таких случая стенд держит сам (`endpointStandCases` в `testdata/script-parity/endpoints.go`). Реализации держатся за неё, а не друг за друга:

| Реализация | Где | Чем проверяется |
|---|---|---|
| bash | `llm_endpoint_classify URL [VAR]` в `scripts/lib/llm-endpoint.sh` | стенд паритета, проверка `T01`: класс каждой строки |
| PowerShell | `Get-LlmEndpointClass -Url -Var` в `scripts/lib/LlmEndpoint.psm1` | стенд паритета, `T02`: класс каждой строки, тот же `kind` (причина одним словом) и тот же текст ошибки, что у bash |
| `compose-lint`, правило 6 | вызывает функцию bash; своей копии нет | фикстуры `bad-llm-url-*`, `good-llm-url-local` |
| Go, `IsLocalEndpoint` | `internal/llm` | Go-тест по той же таблице — T-451 (EPIC-003) |

`T01`/`T02` выполняются при любом `-run` стенда, как пары сообщений: мутант правила краснеет, даже если ни один сценарий не выбран. Мутанты стенда M18–M32: однословное имя снова `cloud`, `0.0.0.0` снова не any-address, IPv4-mapped формы снова `cloud`, адрес LAN без порта снова проходит; точка снова снимается у IPv4 — отдельно в bash (M24) и в PowerShell (M25); `%` в скобках уходит в разбор IPv6 и меняет только текст ошибки (M26, ловит сверка текста в `T02`); обратная косая в хосте снова классифицируется (M27); отказы до проверки `@` в PowerShell снова печатают `user:pass@` (M28, ловят сценарии `H61`–`H66` по отсутствию фиктивного пароля); фразы правила снова печатают значение с `@` и его хост — отдельно в bash (M29) и в PowerShell (M30), ловят `H67`–`H70` по отсутствию `fakepw` в любом регистре (T-463); отказ `up` «refusing to start a second server» снова печатает пробу значения с `@` — отдельно в bash (M31) и в PowerShell (M32), ловит `U19` (T-463, ревью #1 Ma-1). Сценарии `D07` (`0.0.0.0` в `down`) и `D08` (адрес LAN без порта) переносят изменение правила в `llm-server`. Новый случай добавляется в таблицу, а не в одну из реализаций.

**Как правило используют скрипты.** Два вопроса, у каждого один ответ:
- **доверенный** — вопрос гейта облака (ADR-005 доп. 2 п. 3). Им считается любой адрес `local`. Для `cloud` при `MV_LLM_CLOUD_ENABLED` не `true` `make llm-health` предупреждает и завершается ненулевым кодом.
- **запускаемый отсюда** — у `openai_compat` это loopback, `localhost` и `host.docker.internal`, записанные без завершающей точки. Только такой адрес поднимает `make llm-up`. Сервер LAN и сервис compose доверенные, но не наши.

Изменения поведения обвязки на T-450:

| Было | Стало | Направление |
|---|---|---|
| однословное имя (`http://ollama:11434`) — облако, гейт облака краснит `llm-health` | `local`: гейт молчит | ослабление |
| `0.0.0.0`, `::` — «адрес этой машины», `llm-up` поднимал сервер | `invalid`: ошибка конфигурации | ужесточение |
| без порта отвергались только запускаемые адреса | отвергается любой локальный: адрес LAN, сервис compose, адрес при провайдере `ollama` | ужесточение |
| `http://127.0.0.1:80` — «нет порта» (порт схемы не считался явным) | порт записан и считается явным | ослабление |
| облаком считались `169.254.0.0/16`, `fe80::/10` вне префикса `fe80:`, `fc00::/8`, IPv4-mapped формы, `0:0:0:0:0:0:0:1`, `*.localhost` (строковое сравнение по префиксу) | `local` | ослабление |
| запускаемыми были только записи из списка (`127.*`, `::1`, `localhost`, `host.docker.internal`) | запускаемый — любой `kind=loopback`, в том числе `[::ffff:127.0.0.1]`, `[::ffff:7f00:1]` и несжатый `[0:0:0:0:0:0:0:1]`: `make llm-up` поднимет сервер на `MV_LLM_HOST` и будет пробовать записанный адрес | ослабление |
| по префиксу строки локальными были `127.1`, `127.0.0.1.`, `127.evil.com`, доверенными — `10.evil.com`, `192.168.1.10.` | `cloud`: это не строгая запись четырёх октетов, а имя для DNS | ужесточение |
| `\`, пробел, `^`, `` ` ``, `{`, `\|`, `}` в хосте — облако | `invalid`: такой адрес отвергает `url.Parse` платформы | уточнение |
| отказ на `?`/`#` печатал значение целиком | печатается только часть до `?`/`#`, отказ идёт раньше проверки на не-ASCII: ключ из query не попадает в консоль, лог CI и отчёт `compose-lint` | уточнение |
| отказы на `?`/`#`, не-ASCII, отсутствие схемы и схему не http/https шли раньше проверки `@` и печатали `user:pass@` вместе со значением (в `compose-lint` — впервые: прежнее правило печатало только хост) | любой отказ разбора печатает значение без данных пользователя: всё до последнего `@` после схемы — `…@`; если `@` есть в отрезанном query или фрагменте, печатается только схема. Шире разбора намеренно: пароль с `/`, `?` или `#` тоже не печатается (ревью #2 T-450, Mi-R2-1; фикстура `bad-llm-url-userinfo`) | уточнение |
| фразы правила — any-address и «нет порта» — печатали значение целиком и хост даже при `@` после первого `/`: `http://fakepw/x@127.0.0.1:8888` разбирается как хост `fakepw` без порта, и отказ печатал голову пароля | при `@` в любом месте значения фраза печатает только схему (`MV_LLM_URL=http://…`) и не называет хост — как `aboutTheHost` платформы (C-15 v1.5; T-463; фикстура `bad-llm-url-at-after-path`). Так же и вне фраз правила: облачный отказ правила 6 `compose-lint` при `@` не называет хост (фикстура `bad-llm-url-at-cloud`), отказ `up` «второй сервер» называет переменную вместо пробы (`U19`; T-463, ревью #1 Ma-1) | уточнение |
| `https://host:` (пустой порт), `[localhost]`, `%` в хосте, порт с ведущими нулями или из 20 цифр | первые три — `invalid`; `:008888` — порт 8888; 20 цифр — вне диапазона, а не переполнение | уточнение |

`scripts/llm-bench.*` классом адреса не пользуется. Для него меняются только разбор (пустой порт, скобки, `%`, ведущие нули) и отказ на недопустимом адресе.

**Для платформы (T-451).** `IsLocalEndpoint` и `needsExplicitPort` приводятся к той же таблице, включая строки итерации 2: `canonicalHost` снимает завершающую точку до `netip.ParseAddr`, а по таблице `127.0.0.1.` — `cloud`. Сегодня Go считает однословное имя, link-local, ULA и `*.localhost` облаком, а `0.0.0.0` — облаком, а не ошибкой. Пока T-451 не слита, `http://ollama:11434` проходит линтер и обвязку, но на старте платформы требует `MV_LLM_CLOUD_ENABLED=true`. `.env.example` и `docker-compose.yml` такого значения по умолчанию не содержат. Оператору до T-451 гейт облака ради имени сервиса не включать: указать адрес RFC 1918 из сети compose или дождаться T-451 (`runbook.md` §3).

### 6.4. Матрица замера как скрипт (F-8; OQ-A-18 уточнён U-8)

**Цель замера:** решить, проходит ли **вариант E** — одна `Qwen3.8-27B UD-Q3_K_XL` на llama-server на нарратив **и** тики — пороги **NFR-002** (нарратив Phase 2: p95 ≤ 5 с от `combat.decided` до доставки при одном активном scope; отдельная строка — группа из 3, один нарратив на раунд) и **NFR-090** (русский язык: 0 CJK-иероглифов, латиница ≤ 10 % символов, ≥ 98 % ответов). **Критерий выбора базовой конфигурации** (ADR-005 доп. 2 п. 5, `overview.md` §18.1; **уточнён дополнением 3 ADR-005 от 2026-09-12, решение пользователя по T-435**): конфигурация принимается, если **проходит пороги и даёт нужную длину нарратива**, а порядок — **E → Qwen3.6-35B-A3B → C → A**. По итогу замера (`ops/metrics/baseline.md` §5): **E — базовая на переход**, **Qwen3.6-35B-A3B — целевая конфигурация нарратива** (принимается сессией T-438). E+ (27B + 8B в router-режиме), B и D — только материал. Дополнительно фиксируются B2 (холодный старт), B3 (`valid_json_ratio` с грамматикой llama.cpp — NFR-022), VRAM/RAM-размещение (NFR-076), доля `cached_tokens`.
...
3. Порядок решения — `decision_order`: сначала **E** (`num_ctx` 8192 и 16384, KV `f16` и `q8_0` — четыре ячейки), затем **Qwen3.6-35B-A3B**, затем **C**, затем **A**. Конфигурация принимается, если прошла пороги **и** даёт нужную длину нарратива (ADR-005 доп. 3; прежнее правило первого прохода по `decision_order` отменено; редакционно, T-440). E+ прогоняется как материал, если E не проходит по стоимости тика, но проходит по нарративу. **Факт замера T-402 (`baseline.md` §5):** зачётные прогоны E шли в окне 4 × 200 192 и на KV `q4_0` — два отклонения от этого правила, принятые для вердикта; ячейку платформы (8192 × 1, `f16`) доказывает контрольный прогон сессии T-438, там же три прогона Qwen3.6.
...
- переменные `OLLAMA_*` и `MV_OLLAMA_URL` в `.env.example` закомментированы, из обязательных исключены (§4.2); при `MV_LLM_PROVIDER=ollama` заполняется `MV_OLLAMA_URL`, а значения `OLLAMA_KEEP_ALIVE=-1`, `OLLAMA_MAX_LOADED_MODELS=2`, `OLLAMA_NUM_PARALLEL=1`, `OLLAMA_FLASH_ATTENTION=1`, `OLLAMA_KV_CACHE_TYPE=f16` (вариант замера — `q8_0`), `OLLAMA_ORIGINS=http://127.0.0.1,http://localhost` — умолчания манифеста (`shared/env/infra.go`, T-413); контейнер профиля `gpu` получает их от compose, строку в `.env` пишут, только чтобы отличаться; нативной Ollama те же значения задаются переменными окружения ОС. `OLLAMA_HOST` — не `0.0.0.0` без файрвола, `OLLAMA_ORIGINS` — никогда `*` (SEC-15, `compose-lint` п. 5);
...
`ops/models.txt` состоит из двух секций: `gguf` — файлы для llama-server, скачиваются вручную в `MV_LLM_MODELS_DIR` (секция — комментарии); `ollama` — теги для `ollama pull`. **Текущий состав (T-437, по решению T-435 — `decision_order` матрицы):** `gguf` — E (`Qwen3.8-27B-UD-Q3_K_XL`) и целевая Q36-A3B (`Qwen3.6-35B-A3B-UD-Q3_K_XL`), `ollama` — теги запасных C и A (`qwen3:30b-a3b`, `qwen3:8b`, `qwen3:14b`). Список синхронизируется с блупринтами (`mvctl blueprint validate` через `Provider.Models()` — ADR-005 доп. п. 3, C-11 правило 7а). До T-437 (v0.3, до решения F-8) в файле были ещё `qwen3-8b` для E+ и эмбеддинги `nomic-embed-text`/`bge-m3` в обеих секциях; E+ и эмбеддинги возвращаются в список вместе с решением, которое их выберет (эмбеддинги — architect#1 в 005-memory, ADR-005 доп. 2 п. 8).
...
1. `make deploy` = `make ci` (локально, без Docker-тестов) → `make backup` (§5.6, включая шифрованный `links.db`) → `make image` (`multiverse-core:<git-sha>`, плюс тег `current`) → запись `MV_IMAGE_TAG=<git-sha>` в `.env` (предыдущее значение — в `.env.previous`) → `docker compose up -d --wait --remove-orphans` (профили из `COMPOSE_PROFILES`) → `make health LLM_STRICT=0 DEGRADED_STRICT=0` (§2.2; в т. ч. `make llm-health`; если `llama-server` не поднят — `make llm-up` **до** `make deploy`, деплой его не запускает) → smoke: `mvctl world status` и один ход `ci-harness` в тестовом scope (`actor_kind=ci`, в S7 не считается).
...
3. `make minio-image` (первый раз ≈ 5 мин; клонирует форк `MINIO_REPO`, §2.5) → `make up` (init-контейнеры создают топики/бакеты/пользователя MinIO) → `make health` → `make warm`. На свежем стеке `make health` показывает `core degraded` (мир не создан) и выходит с кодом ≠ 0 до шага 4; затем `ok` (§2.2, §7.2; T-476).
...
| Модель варианта E (dense 27B) не проходит NFR-002 по p95 **или не даёт нужной длины нарратива** (замер T-402: 40–48 ток/с, ≈ 163 токена в бюджет 5 с) | базовая конфигурация меняется | порядок решения **`E → Qwen3.6-35B-A3B → C → A`** заложен в `bench-matrix.json` и в §6.4 (ADR-005 доп. 3); по итогу замера E — базовая на переход, целевая конфигурация нарратива — Qwen3.6-35B-A3B, её принимает сессия T-438. Смена конфигурации — правка блупринтов, схемы нарратива и аргументов запуска, не кода (`Provider` один) |
...
| Четыре скрипта в двух вариантах (`llm-server`/`llm-bench` × `.ps1`/`.sh`) расходятся | разные аргументы запуска и разные CSV | одна схема колонок и один список аргументов — в этом документе (§6.2, §6.4); `.ps1` — основной (llama-server нативен на Windows). С T-405 паритет держит не документ, а стенд `make scripts-parity` (§3.1.2) — в `make ci` и в CI-задании `scripts-parity`; известные расхождения видны в его выводе как `KNOWN-FAILING` со ссылкой на бэклог |
