...
- По умолчанию выключено; включение — `MV_LLM_CLOUD_ENABLED=true` + ключ; при > 1 связки с `alive`-персонажем — ещё `MV_LLM_CLOUD_ALLOW_EXTERNAL_PLAYERS=true`; факт — `config.cloud_enabled {by: operator, provider, model, at}` в `system_events` **без ключей**.
