...
// Повтор идемпотентен и ответа на уже применённое не ждёт: сущность, чей entity.created с тем же proposal_id
// уже есть в system_events [0, End), не предлагается — State гасит повтор применённого предложения без ответа
// (§4.5 п. 2, C-02 v1.6); ответ duplicate_entity (сущность есть, а окно и last_change этот proposal_id уже
// не называют) — тоже пропуск; иной отказ или нет ответа — ErrBootstrap, созданное остаётся.
// Возвращает созданные и пропущенные сущности; opts меняют только ожидание (таймеры теста).
...
- **Коды выхода.** `0` — мир создан, снапшот записан. `1` — находка: книга правил, фикстуры, хранилище, отказ или молчание State, снапшот не записан. `2` — вызов, который команда в этом состоянии не выполняет, и ничего не записано: неизвестный флаг или значение, «мир инициализирован» без `--force`, недоступный путь (`--bus kafka` до его задачи, `--bus memory` с хранилищем, отличным от `memory`). *Почему «мир инициализирован» — `2`, а не `1`:* это не находка о мире, а вызов без явного согласия оператора. Идемпотентный скрипт спрашивает `mvctl world status` (`0` — мир есть, `1` — нет) и stderr не разбирает.
...
- **`--force` — мир создаётся заново.** До запуска State команда удаляет объекты State этого мира: всё в `entities-{world}/` (включая `_intents/`) и в `snapshots-{world}/state/`, указатель `state/latest.json` — последним; `swarm/` и `gateway/` не трогает. Снапшот `bootstrap` получает seq 0, курсор — за последним предложением bootstrap. *Почему очистка, а не `seq > 0` поверх старого мира:* восстановление (T-059) при старте State поднимает старый мир вместе с окном `proposal_id`, и предложения bootstrap гасятся без ответа — `--force` кончился бы таймаутом; объект сущности не из фикстур остался бы вне снапшота и при сверке §4.8 дал бы `state_divergence` или вернулся бы в мир, которого фикстуры не описывают. Удалённое в бакете с versioning живёт прежней версией 30 дней (§4.3). *Почему указатель последним:* оборванная очистка оставляет указатель, и следующий `init` без `--force` отказывает, а не создаёт мир поверх остатков.
- **Путь `--bus kafka`** — отдельная задача после T-059; до неё отказ `2` до любых действий. (а) Снапшот — `POST /v1/admin/state/{world}/snapshot` State процесса `core`. (б) Что уже создано, решают объекты `entities-{world}` (только чтение через `state.NewObjectStore`): объект есть — сущность не предлагается, объекта нет — предлагается. Журнал для этого не годится: `system_events` живёт 30 дней (`build/redpanda-init.sh`), и повтор оборванной инициализации после retention получил бы тишину дедупа вместо ответа, а после очистки хранилища старые `entity.created` пометили бы созданным то, чего нет. Сущность, чей факт не вышел, потребители получают снапшотом `bootstrap`, который пишется следом (C-14 v1.4: проекция — снапшот и факты). (в) `--force` — отказ `2`: мир держит в памяти работающий State, и очистка хранилища снаружи его не сбрасывает. Пересоздать мир под стеком — остановить `core`, очистить бакеты мира, поднять `core` (мир `uninitialized`, §18), `world init`. (г) Прогон против поднятого стека — стендовая часть, с разрешения пользователя.
