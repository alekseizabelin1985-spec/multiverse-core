...
Версия 0.4.1 · 2026-09-14 · architect#1 (TEAM-1, v0.1–v0.3), system-architect#1 (v0.4, T-057), system-architect#2 (v0.4.1, T-472) · статус: **утверждён на G2 (2026-09-09)**; запросы §14 приняты системным архитектором (`consolidation.md` §2, S-1…S-6 — «П»); детализация инкрементов — `epics/EPIC-002-state-mechanics/design.md`.
...
**v0.4 — решения system-architect#1 по T-057 (2026-09-14)** (§18): §4.4 — только в указателе живёт `size_bytes`, остальной блок метаданных общий у указателя и объекта; §4.8 — висящую запись recovery принимает без публикации, факт досылает повтор предложения; §4.9 — снапшот пишется на worker'е синхронно (ADR-011, дополнение 2026-09-14); §4.10 — окно и курсор объекта seq 0; §9 — ошибка PUT, ошибка `Publish` ответа до и после PUT, ошибка снапшота; §10 — `publish_attempts_failed` вместо `unpublished_facts` (текстом §18); правила для T-059: одна публикация факта через рестарт, мир без `latest.json`.
**v0.4.1 — просмотр T-472 (2026-09-14, system-architect#2)**: §3.2 — пути записывает `shared/entity`, а не `jsonpath.Accessor.Set/Delete`; правило пути и видов действует в `ApplyOps` (таблица по типам, модификатор, ключ из цифр любой длины, форма не проверяет перечисления и ссылки — C-02 v1.8b), сигнатуры `CanonicalPath`, `CheckOp`, `CheckAttributes`; §4.5 п. 1 — `entity.CheckOp` с типом набора; абзац создания — обязательные атрибуты и виды по таблице типа, в списке `player` добавлен `inventory`.
...
**Правка T-059 (2026-09-14, system-architect#1)**, без смены номера версии (§19): §4.5 п. 2 — конверт досланного факта, п. 10 — `batch_size` частично записанного неатомарного пакета, п. 11 — фактическое число запросов записи; §4.8 — разрыв журнала: отметка вышедших фактов по остатку журнала; §4.9 — доступ к admin-маршруту по `X-Client-Id`; §9 — битый снапшот останавливает мир, а не процесс, `snapshot_event_failed`, `analytics.consistency.violated` State пока не публикует; §10 — секция мира `/health` по коду T-059; §19 — мир `uninitialized` остаётся `degraded`, проба контейнера принимает `degraded`.
...
**Правка T-058 (2026-09-14, system-architect#1)**, без смены номера версии: §4.10 — идемпотентность `Bootstrap` описана так, как работает State (повтор применённого создания гасится без ответа), сигнатура дополнена `opts ...BootstrapOption`; правила `world init`: коды выхода, `--bus memory` только с `--store memory`, снапшот `shutdown` у State внутри команды не пишется, `--force` очищает объекты State мира (снапшот seq 0), правила пути `--bus kafka`. Основание — карточка `epics/EPIC-002-state-mechanics/tasks/T-058.md`, раздел «Решения system-architect».
**Правка T-475 (2026-09-14, system-architect#1)**, без смены номера версии (§20): §4.9 — тело admin-маршрута снапшота `{"reason": "admin"|"bootstrap"}`, коды `400 invalid_reason`/`invalid_body`, `bootstrap` только у мира без `latest.json` (`409 world_initialized`, код — итерация 2 T-475); §4.10 — комментарий `Bootstrap` об опциях (`WithObjects` меняет источник решения «уже создано», `Refusal`), заголовки и тело запроса снапшота, коды выхода и порядок пути `--bus kafka`, неразборчивый указатель, умолчание `--bus`; строка «до его задачи — отказ `2`» снята. Контракты не меняются.
...
// CanonicalPath — каноническая запись пути (C-02 v1.8 п. 2). CheckOp — глагол и путь операции для типа сущности:
// грамматика, служебный корень, путь ниже скаляра типа; значение не проверяет. ApplyOps зовёт её перед каждой операцией.
// CheckAttributes — attributes создания по таблице типа: обязательные атрибуты и виды (C-02 v1.8b, T-472).
...
Пути читаются через `shared/jsonpath`, а записываются второй половиной той же грамматики в `shared/entity` (`splitPath`, `setIn`, `deleteIn` в `path.go`): `jsonpath.Accessor.Set` ходит только по объектам и записал бы `members[0]` ключом `"0"`, а удаления элемента списка у него нет *(изм. T-472; прежде «Пути — грамматика `shared/jsonpath` (`a.b[0].c`); запись через `jsonpath.Accessor.Set/Delete` (уже реализованы)»)*. **Каноническая запись пути (C-02 v1.8 п. 2; изм. T-470, просмотр T-056, ответ 2).** Операция несёт путь только в канонической записи: ключи через одну точку; индекс только в скобках после ключа — `[0]` или до девяти цифр без ведущего нуля; ключ не пуст, без `.`, `[`, `]` и не записан как десятичное целое — одни цифры с необязательным знаком, любой длины (`inventory.0`, `+1`, `a.99999999999999999999` недопустимы; изм. T-472, C-02 v1.8b); путь ниже атрибута, который `data-model.md` §3 типизирует одним значением (`status.x`, `hp.x`, `name.x`), недопустим. Перечень скаляров — только таблицы сущностей `data-model.md` §3: общие атрибуты (с `name`; без `scope` — объект `{id, type}`), §3.1–§3.4, §3.6, §3.7; поля Item §3.5 (`item_id`, `acquired_at`, `source`) — значения внутри `inventory[]`, а не атрибуты, и в перечень не входят; контейнеры §3 (`canon[]`, `spawned_by`, `inventory[]`, `participants[]`) и атрибуты вне §3 скалярами не считаются, а список в коде сверяется с этими таблицами *(итерация 2 T-470, Ma-1, Mi-2 ревью #1; приёмка T-470, Mi-3 ревью #2: прежде «только таблицы `data-model.md` §3» без исключения §3.5)*. Разбор `jsonpath` терпимее: `status.`, `.status` и `inventory.0` он читает как второе написание того же пути. Правило применяет `ApplyOps` (с T-472): `invalid_op` на путь не в канонической записи, на путь ниже типизированного скаляра и на значение чужого вида в корне типизированного атрибута после операций (`set hp {x: 999}`); виды — таблица «тип сущности → атрибут → вид значения» в `attrs.go` рядом с `Attr*` (текст, целое, число, время, длительность, ссылка, перечисление, модификатор «int / формула»; строки контейнеров и `scope` несут только обязательность; `null` — у необязательных, из обязательных — у `flee` и `leader_id`). Таблица ведётся по типам: тип — `Entity.Type`, атрибут вне таблицы своего типа и тип вне таблиц не типизированы; вид проверяется у корней, затронутых операциями, удаление атрибута формой не отвергается; членство в перечислении, разрешение ссылок и диапазоны — нормы State и законы, а не форма (C-02 v1.8b; изм. T-472). Создание проверяет `attributes` по той же таблице (`CheckAttributes`, §4.5); правило догона (§4.8) считает путь не в канонической записи повреждённым фактом. Шаг 1 State (§4.5 п. 1) зовёт ту же `entity.CheckOp` с типом, который называет набор *(изм. T-472; прежде «До T-472 (EPIC-002, перенос в `shared/entity`) правило держит шаг 1 State»)*. Зарезервированные пути (`id`, `type`, `world_id`, `version`, `created_at`, `updated_at`, `last_event_id`, `history`, `last_change`, `schema_version`, любой путь с ведущим `_`) → `invalid_op`. `changed[]` строится **после** применения всех ops сущности: один элемент на конечный путь (первый `old`, последний `new`), `no-op` исключается: путь был и до, и после (или не был ни до, ни после), и значение в канонической форме (§3.3) не изменилось *(итерация 2 T-449, ревью #1 Mi-2; T-050 — сравнение каноническое и учитывает наличие пути; прежде «равенство `old == new` по `reflect.DeepEqual`»)*; если `changed` пуст — версия **не** увеличивается, факт `entity.updated` публикуется с `changed: []` (UC-011 E2 «HP уже max — ход засчитан»), `version` не меняется. **Форма элемента (C-02 v1.6, T-448):** `old` есть ⇔ путь существовал до изменения, `new` есть ⇔ путь существует после; `set` существующего — оба, `set` отсутствовавшего (в т. ч. в `null`) и `inc` отсутствовавшего — только `new`, `append` — путь элемента `a[n]` и только `new`, `remove` без `value` (ключ) — только `old`, `remove` с `value` и `remove` элемента по индексу — путь списка, оба (список до и после). Если путь, затронутый операцией, в конце отсутствует, а его предок предложением создан (не было или был скаляр — есть контейнер: `inc fresh.deep` + `remove fresh.deep` оставляет `fresh = {}`), в `changed[]` добавляется элемент предка — иначе догон §4.8 не восстановил бы его. Элемент предка идёт **после** элементов путей; порядок — часть формы, потому что догон применяет элементы по порядку (элемент предка переписывает записанное под ним согласованно).
...
```json
{ "schema_version": 1, "component": "state", "snapshot": { …как в latest, без size_bytes… },
...
**Что живёт только в указателе (сведение волны 0, T-016; уточнено T-057, v0.4).** Только в указателе живёт `size_bytes`: объект не может назвать собственную длину, не изменив её, — длину знает тот, кто объект уже записал, писатель указателя. `rules_version`, `entities_count` и `reason` блок метаданных несёт **и в указателе, и в объекте**: блоки совпадают во всём, кроме `size_bytes` (фикстура seq 0 и тест `TestSnapshotObjectHoldsTheSameWorld`, двойник `shared/testkit/state`, `internal/state.SnapshotMeta`). *(изм. T-057; прежде «`size_bytes`, `entities_count`, `rules_version` и `reason` — поля указателя, а не объекта». Текст расходился с фикстурой T-016, у которой было доказательство — тест равенства блоков. Довод о длине касается только `size_bytes`, а объект, который сам называет причину и версию правил, читается и без указателя — например, предыдущий из K при повреждённом последнем.)* В событие эти три поля не входят: `snapshot.created` строится из указателя **снятием** `rules_version`, `entities_count` и `reason`, а схема `snapshot.created.v1.json` остаётся закрытой (`additionalProperties: false`): принцип fail-closed важнее удобства потребителя, схему под эти поля не расширять. Порядок чтения потребителем — указатель → объект по `snapshot.key` → сверка `state_hash`; ключ объекта **выводится** из `taken_at` и `seq` (§4.3), а не берётся литералом: иначе сдвиг `taken_at` при неизменном имени файла оставляет расхождение указателя и объекта незамеченным (мутация ревьюера T-016, Minor-1).

### 4.5. Обработка `entity.update.proposed` → `entity.updated | entity.update.rejected`

Пошагово (`apply.go`), всё внутри worker'а мира:

1. **Разбор** (`proposal.go`): валидность уже проверена библиотекой шины по схеме; извлекаются `proposal_id`, `changes[]`, `atomic`, `cause`, `meta.agent`. Ошибка формы операций — неизвестный `op`, пустой путь, путь не в канонической записи или ниже скаляра (C-02 v1.8 п. 2), служебный корень, число по модулю от 2^53 (`entity.CheckOp` с типом набора `changes[].entity.type` и `entity.JSONCompatible`; изм. T-472) — даёт `invalid_op` на всё предложение до дедупликации (п. 2) и владения. Правило п. 1а проверяется раньше *(изм. T-470; прежде «Ошибка формата ops (неизвестный `op`, пустой `path`)»)*.
...
2. **Дедуп** (`dedup.go`): `proposal_id ∈ applied LRU` **или** у **хотя бы одной** целевой сущности `last_change.proposal_id == proposal_id` → предложение уже применено *(изм. T-470, C-02 v1.8 п. 5; прежде «у всех»)*. Если факты для него ещё не подтверждены (`last_change.fact_event_id == ""`) — повторно опубликовать факты (§4.8) и выйти; иначе выйти молча (лог `debug`, `handled=true`). *Почему не «у всех»:* частично применённый неатомарный пакет оставляет запись только на применённых сущностях, и правило «у всех» применило бы эту часть второй раз, как только окно забудет id (C-02 v1.8 п. 5). Повтор частично применённого пакета ничего не применяет и ответа не получает; отказ всего предложения в окно не входит, и его повтор решается заново. **Конверт досланного факта** *(изм. T-059, §19 п. 2)*: id, `timestamp`, `causation_id` и payload (с `applied_at` и `cause`) — от первого вывода, по записи коммита (`proposal_event_id`, `applied_at`, `cause`); `correlation_id`, `actor_kind`, `locale`, `scope` — от события, которое пришло повтором. Потребитель узнаёт факт по id; конверт в `last_change` не хранится.
...
10. **Фиксация**: для каждой применённой копии `Version++` (если `changed` не пуст), `UpdatedAt = proposal.Event.Timestamp`, `LastChange = {…, FactEventID: ""}`, `History` append (обрезка до 50). **`last_change.batch_size`** *(изм. T-059, §19 п. 3)* — число **применённых** наборов пакета, решённое на шаге 9 до записи, а не число записанных: у частично записанного неатомарного пакета записанные сущности несут полный `batch_size`, и аудит узнаёт частичную запись по разнице между `batch_size` и числом сущностей, чья запись коммита называет этот `proposal_id`. Roll-forward интента пишет `batch_size` = число наборов интента — для атомарного пакета то же число.
11. **Персист** (`Persist`): если применённых сущностей > 1 и `atomic=true` → `PutIntent` (ADR-013); затем `PutEntity` по каждой в порядке возрастания `id`; после всех PUT — `DeleteIntent`. Ошибка PUT → повтор ×3 (100/300/900 мс; всего четыре попытки). Каждая попытка — вызов клиента `shared/objstore`, который сам повторяет временный сбой (три запроса, паузы 100 и 500 мс), поэтому одна запись — до 12 запросов и ≈ 3,7 с пауз (1,3 с State + 4 × 0,6 с клиента, без времени самих запросов) до `persist_failed`; `objstore.ErrNoBucket` и `ErrNotFound` не повторяются; `ListIntents` на пути досылки (п. 2) повторяется так же *(изм. T-059, N-3 ревью #1 T-057)*. Неуспех → worker переводит мир в `state: persist_failed`, `/health fail`, обработка останавливается (§9).
...
`entity.create.proposed` (те же шаги без 4–5): `proposal_id` обязателен (C-02 v1.6, без него — пропуск по п. 1); `attributes` проходят `entity.JSONCompatible` — число по модулю не меньше 2^53 где угодно внутри → `rejected reason=invalid_op` (C-02 v1.6: иначе создание записало бы то, чего не может записать `set`); `duplicate_entity`, если `id` уже есть (с тем же `proposal_id` — дедуп; с другим — `rejected reason=duplicate_entity`; новая причина, §14); проверка `attributes` по таблице типа (`entity.CheckAttributes`: обязательные атрибуты и виды `data-model.md` §3, для `player` — `hp, hp_max, atk, def, dmg, flee, status, position, scope, inventory, actor_kind`; `name` — в `entity.name`; нет обязательного атрибута или вид чужой → `rejected reason=invalid_op` сразу после `JSONCompatible`, до дедупликации и `duplicate_entity`; C-02 v1.8b) *(изм. T-472; прежде список без `inventory` и без места проверки в порядке)*, владение (кто может создавать тип), инварианты; `Version = 1`, `CreatedAt = UpdatedAt = timestamp предложения`; факт `entity.created {entity, version:1, attributes, proposal_id}` (`proposal_id` в факте обязателен с C-02 v1.6).
...
sequenceDiagram
    ...
    Note over ST: объект.version == mem+1 и last_change.fact_event_id == "" → «висящая запись»: принять объект в память без публикации; недостающий факт публикует повтор предложения (§4.5 п. 2, §18, изм. T-057); объект.version > mem+1 → state_divergence → /health fail
...
- **Разрыв журнала** (`cursor < earliest offset` — retention; или журнал моложе снапшота — шина в памяти после рестарта): восстановить из объектов `entities-{world}` (они всегда ≥ снапшота), `/health degraded {log_gap}` до первого снапшота после восстановления, `analytics.consistency.violated code=log_gap severity=warn` (UC-025 E3) — после строки издателя `core/state` в реестре *(изм. T-059, §19 п. 7; до неё след — `Warn code=log_gap` и `/health`)*. Разрыв распознаётся по первому офсету, который доставило `ReadRange(cursor, End)`: он не равен `cursor`. **Отметка вышедших фактов** *(изм. T-059, §19 п. 1)*. Объект впереди снапшота несёт запись коммита с пустым `fact_event_id` (§4.5 п. 12: повторного PUT нет), и по нему не видно, вышел ли факт. Факты `entity.created/updated`, которые чтение с разрывом всё же доставило (остаток журнала после retention), не применяются — истина здесь объекты, — а только отмечают: если у объекта совпадают `id`, `version` и `last_change.proposal_id` с фактом, в память ставится `fact_event_id` = id этого факта. Объект, чей факт не найден, остаётся с пустым `fact_event_id`: повтор его предложения публикует факт под id первого вывода (§4.5 п. 2), возможно второй раз, и потребители гасят его по id (C-01, «Гарантии»). Остаточный случай принят: такой факт мог уйти только в утраченную часть журнала, а предложение старше retention издатели не повторяют. Recovery факты не публикует и здесь (§18).
...
Триггеры: каждые `MV_SNAPSHOT_EVERY_FACTS=200` применённых фактов (счётчик на мир; Дополнение после G2: префикс `MV_`); `analytics.session.ended` (State подписан на `analytics_events` только как на триггер, группа `core.state.triggers`; в replay не читается — снапшот по счётчику; **I2**); `SIGTERM` (`Stop()` ждёт завершения текущего предложения, пишет снапшот с `reason=shutdown`, ≤ 10 с); admin `POST /v1/admin/state/{world}/snapshot` (харнесс S3; доступ решает `runtime.AdminOnly` по `X-Client-Id` из `MV_CORE_ADMIN_CLIENTS`, `X-Actor-Kind` необязателен и только сверяется с перечнем — ADR-009 п. 9, C-06; изм. T-059, прежде «`X-Actor-Kind: ci`»; маршрут смонтирован на HTTP-сервере процесса `shared/runtime`, `MV_CORE_ADDR`; в compose доступен через прокси gateway `/v1/admin/*` — C-06); `bootstrap` (seq 0 после `world init`, §4.10). Ротация: после успешной записи удаляются снапшоты старше пяти последних (`K=5`), `latest.json` не считается. Событие `snapshot.created {component: state, snapshot{id, seq, taken_at, cursor, laws_version, state_hash, size_bytes, key}}` публикуется после записи `latest.json`. ~~Снапшот **не** блокирует обработку дольше сериализации (≤ 1 МБ, единицы мс): worker делает копию списка сущностей под своим же исполнением (сериализация в горутине, курсор фиксируется в момент копии).~~ **Снапшот пишется целиком на worker'е мира, синхронно** *(изм. T-057; ADR-011, дополнение 2026-09-14, п. 1)*: сущности, окно и курсор берутся между двумя предложениями, затем PUT объекта, PUT `latest.json`, ротация и `snapshot.created` — до следующего предложения мира. По счёту фактов снапшот пишется в `Apply` того предложения, чьи факты добрали счёт, и `snapshot.created` выводится из него (`Derive`, id из причины `WithCauseID("state", seq)`, без `scope`); по `Context.Snapshot` (admin-маршрут T-059, bootstrap T-058) — задачей на worker'е, корнем `NewRoot` с миром и `actor_kind=system`; по `SIGTERM` — после возврата подписки и worker'ов, до `Close` шины. Остановленный мир (`panic`, `persist_failed`, `publish_failed`) снапшот не пишет: его память может отставать от объектов. `seq` — следующий за наибольшим объектом снапшота в хранилище, а не за указателем. *Почему синхронно:* `snapshot.created` стоит в `system_events` среди фактов, и только так его место не зависит от латентности MinIO — replay даёт те же события в том же порядке (NFR-061). Цена — одно предложение из `MV_SNAPSHOT_EVERY_FACTS` ждёт записи снапшота.

**Тело и ответы admin-маршрута снапшота** *(изм. T-475, §20 п. 1–2)*. Тело необязательно — `{"reason": "admin"|"bootstrap"}` (`state.SnapshotRequest`, читается не больше 4 КБ). Без тела, `{}` или без `reason` — `admin`, как в T-059. Другой `reason`, в том числе причины собственных триггеров State (`interval`, `session_ended`, `shutdown`), — `400 invalid_reason`. Не JSON, неизвестное поле, больше одного документа — `400 invalid_body`. При `400` ничего не пишется. **`reason=bootstrap` принимается только у мира без `latest.json`:** на worker'е мира, перед записью и в пределах `SnapshotTimeout`, State читает указатель; указатель есть или не читается (`ErrUndecodable`) — `409 world_initialized`, ничего не пишется, счёт `MV_SNAPSHOT_EVERY_FACTS` и признаки снапшота в `/health` не меняются; иная ошибка чтения указателя — `500 snapshot_failed`. Правило живёт в State, а не в маршруте, и действует для любого вызывающего `Context.Snapshot`, в том числе для State внутри `world init --bus memory` (после `--force` указатель уже удалён) *(код — итерация 2 T-475)*. Остальные ответы — по T-059: `200` с `latest.json` как записан (в том числе когда `snapshot.created` не вышел — тогда `/health` говорит `snapshot_event_failed`), `404 unknown_world`, `409 no_object_store`, `503 not_running` и `world_stopped`, `500 snapshot_failed`. Тело ошибки — `{"error": {"code", "message"}}`, машинный признак — `code`; `403` у `runtime.AdminOnly` пока `{"error": "<текст>"}` (C-01 v1.11), клиент разбирает тело терпимо.
- *Почему неизвестное поле — отказ, а не пропуск:* опечатка (`reson`) записала бы снапшот `admin` там, где просили `bootstrap`, и `world init` узнал бы об этом только по указателю.
- *Почему `bootstrap` — только до указателя:* данные такого снапшота над живым миром верны — это тот же снапшот, что `admin` (текущие сущности, окно и курсор; `seq` — следующий за наибольшим объектом, то есть `> 0`), и указатель переписывается так же, как любым снапшотом. Неверна метка: `bootstrap` значит «снапшот, которым мир начался». Реальный путь к ней — не опечатка, а два `world init --bus kafka` одновременно: оба видят мир без указателя, второй `POST` пишет `seq 1, reason=bootstrap` и завершается `0`, как будто мир создал он. Проверка на worker'е мира последовательна с записью снапшотов этого мира и гонку закрывает; проверка в маршруте или в команде — нет.
- *Почему условие — указатель, а не `seq == 0`:* объект снапшота без указателя (процесс упал между двумя PUT, §4.4) навсегда запер бы `world init` отказом `409`, хотя мир не инициализирован. Условие совпадает с предусловием самой команды (шаг 2 §4.10) и с признаком мира `uninitialized` (§18). Цена — в этом редком случае снапшот `bootstrap` получает `seq > 0`.
...
| `snapshots/state/latest.json` | указатель на снапшот seq 0 (формат §4.4) — эталон для read-model потребителей | `entities_count: 6`, `cursor.system_events: 0`, `state_hash` пересчитывается тестом *(курсор 0 — значение фикстуры-примера; снапшот seq 0 настоящего `world init` несёт офсет за последним предложением bootstrap, изм. T-057)* |
| `snapshots/state/{ts}-000000.json` | **сам объект снапшота seq 0** (формат §4.4) — сверх исходного списка, подтверждено при приёмке T-016 | `entities[]` отсортированы по `(type, id)`, `applied_proposals` — шесть `proposal_id` bootstrap (`bootstrap:{world}:{type}/{id}`) в порядке §4.10 *(изм. T-057; прежде `[]`: окно входит в снапшот по C-14 v1.2 (а), фикстура и её тест несут окно bootstrap)*; `size_bytes` и `state_hash` указателя пересчитываются тестом по этому файлу |
...
// ждёт ответ на каждое (таймаут 10 с на ответ) через Journal.Tail от End, взятого до первой публикации.
// Повтор идемпотентен и ответа на уже применённое не ждёт. Что уже создано, решает источник, который задают opts:
// - по умолчанию журнал: сущность, чей entity.created с тем же proposal_id уже есть в system_events [0, End),
//   не предлагается — State гасит повтор применённого предложения без ответа (§4.5 п. 2, C-02 v1.6);
// - с WithObjects(store) — объекты entities-{world}, журнал для этого решения не читается: объект есть — сущность
//   не предлагается и попадает в Skipped; объекта нет (ErrNotFound, бакета ещё нет) — предлагается; иная ошибка
//   чтения — ErrBootstrap до первой публикации (путь --bus kafka, правило (б) ниже; изм. T-475).
// Ответ duplicate_entity (сущность есть, а окно и last_change этот proposal_id уже не называют) — тоже пропуск;
// иной отказ — ErrBootstrap вместе с *Refusal{Entity, Reason, Details, EventID} (errors.As: details отказа —
// invariant_id, expected_version, actual_version — доходят до оператора); нет ответа — ErrBootstrap. Созданное остаётся.
// Возвращает созданные и пропущенные сущности. opts задают ожидание (WithAnswerTimers — таймеры теста) и источник
// решения «уже создано» (WithObjects); содержание и порядок предложений opts не меняют.
...
func WithObjects(objects Store) BootstrapOption // только чтение: GetEntity
```

*(изм. T-058; прежде «повтор идемпотентен (`duplicate_entity` с тем же `proposal_id` = дедуп → тихий пропуск)» и сигнатура без `opts`. State на повтор применённого создания не отвечает ничем — C-02 v1.6 «State гасит по нему повтор создания», — и ожидание «тихого пропуска» кончилось бы таймаутом. Строка §4.5 «с тем же `proposal_id` — дедуп» описывает State и не меняется.)* Проверка по журналу годится для State на свежем журнале того же процесса (`world init --bus memory`, харнессы e2e); путь `--bus kafka` решает «уже создано» по объектам — опцией `WithObjects`, ниже *(изм. T-475; прежде «opts меняют только ожидание»)*.

`mvctl world init --world dark-forest-world --fixtures testdata/fixtures/ [--bus kafka|memory]`: (1) `objstore.EnsureBucket` для `entities-{world}`, `snapshots-{world}` (versioning/ILM — §4.3); (2) если `latest.json` уже есть и `--force` не задан — отказ «мир инициализирован» (exit 2); (3) `Bootstrap`; (4) снапшот `seq 0, reason=bootstrap` через admin-маршрут State (`POST /v1/admin/state/{world}/snapshot` с `X-Client-Id: mvctl`, `X-Actor-Kind: ci` и телом `{"reason": "bootstrap"}` — §4.9; изм. T-475, прежде назывался только `X-Actor-Kind: ci`) или — при `--bus memory` — in-process `state.Context`; (5) печатает `entities_count`, `state_hash`. Требует запущенного `core` с контекстом `state` (при `--bus kafka`; адрес — `MV_CORE_URL`, для CLI на хосте `http://127.0.0.1:8090`, брокеры — `MV_KAFKA_BROKERS`, изм. T-475); e2e-харнессы вызывают `Bootstrap` напрямую в процессе `--contexts=all --bus=memory`. Права: предлагающий `author` (§4.6) — `Create` любых типов, `cause=init` *(изм. T-470; прежде «`system` proposer в `OwnershipRules`»)*.

**Правила `world init` (изм. T-058; решения system-architect#1 по T-058).**
- **Коды выхода.** `0` — мир создан, снапшот записан. `1` — находка: книга правил, фикстуры, хранилище (в том числе `latest.json`, который не читается, — изм. T-475), `core` и шина пути `--bus kafka`, отказ или молчание State, снапшот не записан. `2` — вызов, который команда в этом состоянии не выполняет, и ничего не записано: неизвестный флаг или значение, «мир инициализирован» без `--force`, недоступное сочетание (`--bus memory` с хранилищем, отличным от `memory`; `--bus kafka` с `--force`, с хранилищем, отличным от `minio`, или без `MV_CORE_URL` — изм. T-475; прежде `--bus kafka` отказывал целиком до своей задачи). *Почему «мир инициализирован» — `2`, а не `1`:* это не находка о мире, а вызов без явного согласия оператора. Идемпотентный скрипт спрашивает `mvctl world status` (`0` — мир есть, `1` — нет) и stderr не разбирает.
- **`--bus memory` — только с `--store memory`** (значение по умолчанию). State внутри команды живёт, пока идёт команда; другое хранилище — отказ `2` до его открытия. *Почему:* `cursor.system_events` снапшота — офсет журнала `membus`, который исчезает вместе с командой, и процесс `core` над Redpanda восстановил бы мир от чужого офсета (§4.8); рядом с работающим `core` State команды — второй писатель мира. Мир в MinIO создаёт только путь `--bus kafka` — через State процесса `core`.
- **Снапшот `shutdown` у State внутри команды не пишется.** Этот State пишет один снапшот — `bootstrap`; триггер `SIGTERM` §4.9 относится к State, обслуживающему мир. Команда закрывает хранилище для записи после снапшота `bootstrap`, а также после неудачи bootstrap или снапшота; отказ закрытого хранилища — единственная ошибка `Stop`, которую команда не показывает. *Почему не признак в `state.Config`:* у короткоживущего State один вызывающий, и правило живёт рядом с ним; закрытие заодно не даёт оборванному bootstrap оставить `latest.json` половины мира. Признак вводится, когда появится второй такой вызывающий.
- **`--force` — мир создаётся заново.** До запуска State команда удаляет объекты State этого мира: всё в `entities-{world}/` (включая `_intents/`) и в `snapshots-{world}/state/`, указатель `state/latest.json` — последним; `swarm/` и `gateway/` не трогает. Снапшот `bootstrap` получает seq 0, курсор — за последним предложением bootstrap. *Почему очистка, а не `seq > 0` поверх старого мира:* восстановление (T-059) при старте State поднимает старый мир вместе с окном `proposal_id`, и предложения bootstrap гасятся без ответа — `--force` кончился бы таймаутом; объект сущности не из фикстур остался бы вне снапшота и при сверке §4.8 дал бы `state_divergence` или вернулся бы в мир, которого фикстуры не описывают. Удалённое в бакете с versioning живёт прежней версией 30 дней (§4.3). *Почему указатель последним:* оборванная очистка оставляет указатель, и следующий `init` без `--force` отказывает, а не создаёт мир поверх остатков. **Указатель, который не читается** *(изм. T-475)*: `ReadLatest` помечает его `ErrUndecodable` (как объект снапшота). Без `--force` — находка `store` (`1`) с именем `latest.json`: мир мог быть, и создать его поверх — выбор оператора. С `--force` очистка идёт как обычно, указатель удаляется, не читаясь. Ошибка транспорта при чтении указателя — находка `store` в обоих случаях.
- **Путь `--bus kafka`** — T-475 *(прежде «отдельная задача после T-059; до неё отказ `2` до любых действий»)*. (а) Снапшот — `POST /v1/admin/state/{world}/snapshot` State процесса `core` с телом `{"reason": "bootstrap"}` (§4.9); ответ не `200` — находка `snapshot` с кодом и текстом ответа, `1`. (б) Что уже создано, решают объекты `entities-{world}` (только чтение через `state.NewObjectStore`, опция `WithObjects` у `Bootstrap`): объект есть — сущность не предлагается, объекта нет — предлагается. Журнал для этого не годится: `system_events` живёт 30 дней (`build/redpanda-init.sh`), и повтор оборванной инициализации после retention получил бы тишину дедупа вместо ответа, а после очистки хранилища старые `entity.created` пометили бы созданным то, чего нет. Сущность, чей факт не вышел, потребители получают снапшотом `bootstrap`, который пишется следом (C-14 v1.4: проекция — снапшот и факты). (в) `--force` — отказ `2`: мир держит в памяти работающий State, и очистка хранилища снаружи его не сбрасывает. Пересоздать мир под стеком — остановить `core`, очистить бакеты мира, поднять `core` (мир `uninitialized`, §18), `world init`. (г) Прогон против поднятого стека — стендовая часть, с разрешения пользователя. (д) **Порядок** *(изм. T-475)*: отказы `2` (`--force`, `--store` не `minio`, пустой `MV_CORE_URL`) — до любых действий; затем фикстуры → бакеты → указатель (есть — `2` с процедурой (в); не читается — находка `store`) → `GET /health` `core` → шина → `Bootstrap` → `POST` снапшота → `GET /health`. Книга правил не читается: мир держит своими правилами State `core`. (е) **`/health` `core` до предложений** *(отступление исполнителя T-475 принято)*: `core` недоступен, контекст `state` не запущен, мир не в `MV_STATE_WORLDS` или мир `fail` — находка `core` (`1`) до открытия шины. Health без секции `state` — не отказ, решают ответы. *Почему:* без проверки неработающий `core` стоил бы `BootstrapTimeout` ожидания и оставил бы в журнале предложения, которые State после рестарта не прочтёт (он читает с `End`). (ж) **После `200`** — предупреждения при коде `0`: у мира в `/health` `snapshot_event_failed`; `reason` указателя не `bootstrap` (`core` без маршрута T-475); `/health` не прочитан. Указатель записан, и второй `init` отказал бы как «мир инициализирован». (з) **Умолчание `--bus`** — `MV_BUS` (по манифесту `kafka`), как у процесса: голый `mvctl world init` идёт в стек окружения *(§20 п. 4)*. Явный `--bus kafka` не требуется: от случайного вызова защищают предусловия — над миром с указателем `2`, `--force` на этом пути `2`, `reason=bootstrap` у мира с указателем `409` (§4.9), — а не имя флага, которое оператор копирует из runbook вместе с командой.
...
| Ошибка PUT (MinIO) — сущность или интент *(изм. T-057)* | повтор ×3 с паузами 100/300/900 мс (`Config.Timers`); сам PUT не видит отмены контекста — `Stop` доводит начатую запись до конца, отмена обрывает только паузы; затем мир `persist_failed`, обработка мира остановлена, факты ответа **не** публикуются, память не тронута; дальше — строка «Предложение остановленного мира». Частично записанный пакет: атомарный — интент остаётся, roll-forward при рестарте (ADR-013 п. 4, §4.8); неатомарный пишется без интента — записанные сущности досылаются повтором предложения, незаписанные наборы остаются без ответа (C-02 v1.8 п. 5; ограничение, §18). Ошибка `DeleteIntent` после всех PUT мир не останавливает: ответ публикуется, оставшийся интент roll-forward пропускает — сущности уже на `to_version` | `warn handled=true` на попытку; `error handled=false` при остановке мира; `DeleteIntent` — `error handled=true` | `/health fail {reason: persist_failed}` |
| Ошибка `Publish` ответа — факта или отказа, до и после PUT *(изм. T-057 по решению Ma-1 T-055; прежде «Ошибка `Publish` факта после PUT: повтор ×3; затем факт помечен неподтверждённым, worker продолжает; `/health degraded {unpublished_facts: n}`»)* | то же событие (тот же id и байты) публикуется снова: пауза 100 мс, удвоение до 5 с, на реальных таймерах, без предела; вышедшие события ответа не повторяются; шине ошибка не отдаётся; следующее предложение мира не решается, пока ответ не вышел. `Stop` обрывает попытки: мир `publish_failed`, память не тронута, предложение не фиксируется и после рестарта приходит снова, снапшот `shutdown` не пишется. Если PUT уже прошёл, объекты опережают память; после рестарта повтор предложения гасится дедупом (§4.5 п. 2), а недосланные факты публикуются с id и `applied_at` первого вывода (`last_change.proposal_event_id`, `applied_at`) без второй записи. *Почему не «×3 и продолжить»:* следующий ответ по той же сущности объявил бы версию, чей предшественник не вышел, а ошибка шине после вышедшей части ответа дала бы два факта одной версии (зонд P1 ревью #1 T-055) | `warn handled=true` на попытку; `error handled=false` при `Stop` | пока идут попытки — `/health degraded {publish_attempts_failed: n}`; после `Stop` — `/health fail {reason: publish_failed}` |
| Снапшот повреждён (схема, мир, ключ, курсор, hash) | предыдущий из K; все битые → **мир** не обслуживается: остаётся без сущностей и остановлен, предложения получают `ErrWorldStopped` (строка «Предложение остановленного мира»), `Stop` снапшот не пишет, `latest.json` не трогается. `Start` процесса не падает, остальные миры работают; `Start` падает, только если не читаются хранилище или журнал *(изм. T-059, §19 п. 5)* | `error handled=false` | `/health fail {reason: snapshot_corrupted, snapshot: corrupted}` |
| Факт журнала с версией ≠ mem+1 или с элементом `changed[]` `a[n]` за концом списка (§4.8); объект сущности больше чем на версию впереди памяти после догона | `state_divergence`, мир не обслуживается | `error handled=false` | `/health fail {reason: state_divergence}`; `analytics.consistency.violated {code: state_divergence, severity: break}` — после строки издателя `core/state` в реестре *(изм. T-059, §19 п. 7)* |
| Разрыв журнала (retention) | восстановление из объектов с отметкой вышедших фактов (§4.8), продолжение | `warn handled=true` (`code=log_gap`) | `/health degraded {log_gap: true}` до первого снапшота; `consistency.violated {code: log_gap, severity: warn}` — после строки издателя (§19 п. 7, изм. T-059) |
...
| Ошибка записи снапшота *(изм. T-057)* | не влияет на обработку; счётчик фактов сбрасывается, повтор при следующем триггере; указатель остаётся на предыдущем целостном снапшоте; `snapshot.created`, не вышедший после записанного снапшота, не повторяется — снапшот записан, потребители читают указатель; до следующего вышедшего `snapshot.created` секция мира несёт `snapshot_event_failed: true` *(изм. T-059, N-4)*; ошибка снапшота `shutdown` входит в ошибку `Stop` и остановку не прерывает | `error handled=true` | `/health degraded {snapshot_stale: age_s}` — возраст последнего успешного снапшота в секундах, `null` без него (T-059; в T-057 значение — текст ошибки); `/health degraded {snapshot_event_failed: true}` — пока событие последнего снапшота не вышло |
...
- `/health` секция `state`: `{status, worlds: {<id>: {entities, cursor, lag: End-cursor, snapshot: {seq, taken_at, age_s}, pending_intents, unpublished_facts, applied_total, rejected_total_by_reason}}}`; `mechanics: {rules_version, rules_path, invariants: 10}`; `mode`. *(изм. T-059, §19 п. 6: действует состав секции `worlds.<id>` по коду — всегда `entities`, `state_hash`, `rules_version`, `cursor`, `pending_intents`, при снапшоте `seq` и `snapshot {seq, taken_at, age_s}`; `fail` — `reason ∈ panic, persist_failed, publish_failed, snapshot_corrupted, state_divergence`, при битом снапшоте ещё `snapshot: corrupted`; `degraded` — `publish_attempts_failed`, `world: uninitialized`, `snapshot_stale`, `snapshot_event_failed`, `log_gap` и `no_snapshot` до первого снапшота после восстановления. `lag`, `unpublished_facts`, `applied_total`, `rejected_total_by_reason` в MVP-1 не выводятся.)*
...
## 18. Решения по T-057 (2026-09-14, system-architect#1)

Основание — вопросы и отступления исполнителя T-057 (карточка `epics/EPIC-002-state-mechanics/tasks/T-057.md`, раздел «Решения system-architect»), код ветки `task/T-057-state-objstore-snapshots`, фикстура seq 0 и её тест (T-016), решение Ma-1 T-055. Контракты не меняются: C-02 и C-14 не называют ни поведения при ошибке `Publish`, ни состава блока метаданных объекта; политика мира `snapshot.created` — уже C-14 v1.3 (T-470).

| # | Что | Решение | Где |
|---|---|---|---|
| 1 | Ошибка `Publish` ответа | Прав код (решение Ma-1 T-055, доказано тестами и мутантами T-055 и T-057): повтор того же события без предела до `Stop`, затем `publish_failed`; после PUT — досылка повтором предложения. Строка «×3 и продолжить» снята | §9 |
| 2 | Имя поля `/health` | `publish_attempts_failed` — число неудачных попыток текущего ответа. `unpublished_facts` не вводится: мир не идёт дальше с недосланным фактом, и такой величины нет. **§10**: в секции `worlds.<id>` вместо `unpublished_facts` действует `publish_attempts_failed` (только пока идут попытки), при `fail` — `reason ∈ panic, persist_failed, publish_failed`; строка §10 текстом не переписана, чтобы не пересечься с правкой T-470 соседней строки, — действует эта строка | §9, §10 |
| 3 | Блок метаданных объекта снапшота | Прав код и фикстура: только `size_bytes` — лишь в указателе | §4.4, ADR-011 доп. п. 2 |
| 4 | Синхронный снапшот | Отступление принято, ADR-011 п. 6 заменён | §4.9, ADR-011 доп. п. 1 |
| 5 | Мир без `latest.json` | T-059 — по правилу ниже | §18 |
| 6 | Переменная | `MV_SNAPSHOT_EVERY_FACTS` (§4.9, §16 п. 1, манифест `shared/env`); `MV_STATE_SNAPSHOT_EVERY` и `MV_STATE_PERSIST_RETRIES` из `design.md` §7 не вводятся | §4.9 |
| 7 | Admin-маршрут снапшота | T-057 даёт `Context.Snapshot`; `POST /v1/admin/state/{world}/snapshot` — `admin.go` T-059 | §4.9 |
| 8 | Интент | Только `atomic=true` и > 1 сущности (ADR-013 п. 4, §4.5 п. 11) | §4.5 |

**Правила для T-059 (восстановление).**
- **Одна публикация факта через рестарт.** Факт, чья сущность записана, а публикация не подтверждена, публикует один путь — повтор предложения (дедуп §4.5 п. 2, `resendUnpublished`). Recovery принимает висящий объект в память и сам факт не публикует. *Почему:* предложение без полного ответа не фиксируется ни курсором снапшота, ни офсетом группы — снапшот пишется между предложениями, у остановленного мира не пишется, а `Stop` и падение не коммитят. Значит, предложение всегда приходит снова, и второй путь публикации дал бы тот же id дважды. Страж `oneStateOverTheWorld` ловит это. Случай `log_gap` T-059 решает с system-architect.
- **Догон атомарного пакета после `publish_failed`** (DoD T-059, приёмка T-055). Недостающие факты досылаются по объектам сущностей (commit record), а не по интенту и не через `state_divergence`: при порядке T-057 интент удаляется до публикации. Если объект после догона той же версии, что память, и его `last_change.proposal_id` равен `proposal_id` факта этой версии из журнала, в память не попадает пустой `fact_event_id` — ставится id факта. Иначе повтор предложения опубликует вышедший факт снова. Тест DoD T-059 (`player-A v2` в журнале, `wolf-alpha v2` только в объектах): после повтора ровно один факт `wolf-alpha v2` и ни одного второго `player-A v2`.
- **Мир без `latest.json` и без объектов сущностей** — `uninitialized`: `/health degraded {world: uninitialized}`. Любое предложение, кроме `entity.create.proposed` с `cause=init` (bootstrap, §4.10), отклоняется `unknown_entity` с сущностью мира **до записи**. Признак снимается, когда в рабочем наборе появилась сущность мира. *Почему до записи:* иначе предложение шлюза до `mvctl world init` при подключённом хранилище останавливает мир `persist_failed` (бакетов ещё нет), и `world init` без рестарта процесса уже не пройдёт.

**Ограничение (принято для MVP-1).** Неатомарный пакет из нескольких сущностей пишется без интента. Если PUT отказал между сущностями или отказ того же ответа не вышел до `Stop`, после рестарта записанные сущности получают факты, а незаписанные наборы и невышедшие отказы остаются без ответа (C-02 v1.8 п. 5). Издатели MVP-1 шлют многосущностные пакеты атомарными (§7.1, §7.2). Пересмотр — при первом издателе неатомарных многосущностных пакетов.

---

## 19. Решения по T-059 (2026-09-14, system-architect#1)

Основание — вопросы и отступления исполнителя T-059 (карточка `epics/EPIC-002-state-mechanics/tasks/T-059.md`, разделы «Выполнение» и «Решения system-architect»), код ветки `task/T-059-state-recovery`, C-01 (`AdminOnly`, `Journal`), C-14 v1.4, ADR-009 п. 9, ADR-011 п. 3, `infrastructure.md` §2.2, §2.3, §6.3, §7.2. Контракты `contracts.md` не меняются: C-14 обещает при разрыве только восстановление из объектов и `/health degraded {log_gap}`, а статус пробы контейнера — предмет `infrastructure.md`, а не C-01.

| # | Что | Решение | Где |
|---|---|---|---|
| 1 | `log_gap` и публикация фактов | Временное решение исполнителя подтверждено с уточнением: факты, которые чтение с разрывом доставило, отмечают `fact_event_id` у объектов с тем же `id`, `version`, `proposal_id`; для остальных повтор предложения публикует факт под id первого вывода, возможно второй раз, — принято | §4.8 |
| 2 | Конверт досылки (Mi-3 T-057) | Как в коде: id, `timestamp`, `causation_id`, payload — от первого вывода; корреляция, `actor_kind`, `locale`, `scope` — от повтора. Конверт в `last_change` не хранится, `shared/entity` не меняется. Правило «досылать только при `p.Event.ID == proposal_event_id`» отвергнуто | §4.5 п. 2 |
| 3 | `batch_size` (Mi-2 T-057) | Число применённых наборов, не записанных | §4.5 п. 10 |
| 4 | N-4 T-057 | `snapshot_event_failed: true` (`degraded`) до следующего вышедшего `snapshot.created`; повтора события нет (§9). Рестарт признак снимает: указатель записан, а новый процесс о невышедшем событии не знает | §9 |
| 5 | Битый снапшот и `Start` | Отступление принято: останавливается мир, а не процесс | §9 |
| 6 | Секция мира `/health` | По коду T-059; `lag` и счётчики не выводятся — `lag` потребовал бы `End` у брокера на каждый опрос, счётчики MVP-1 считает `mvctl report` по событиям (§10) | §10 |
| 7 | `analytics.consistency.violated` от State | Отступление принято до строки издателя `core/state` в `shared/contracts/registry_ops.go` (владелец EPIC-005, `contract-change`); схема `analytics.consistency.violated.v1.json` уже знает `state_divergence` и `log_gap` (`layer: state`, `detected_by: snapshot_diff`). В `analytics.replay.completed` поля `log_gap` нет, схема закрыта и не расширяется | §4.8, §9 |
| 8 | Число запросов записи (N-3 T-057) | До 12 запросов и ≈ 3,7 с пауз | §4.5 п. 11 |
| 9 | Доступ к admin-маршруту | Отступление принято: доступ решает `X-Client-Id` (`runtime.AdminOnly`), `X-Actor-Kind` необязателен. Формулировка DoD «отклоняет запрос без `X-Actor-Kind`» устарела: оператор приходит через прокси шлюза с `human`, и такой запрет закрыл бы маршрут оператору | §4.9 |

**Мир `uninitialized` и проба контейнера.** Мир без `latest.json` и без объектов остаётся `degraded {world: uninitialized}` (§18): оператору нужен сигнал «выполните `mvctl world init`», и дать его может только State. Меняется проба: `multiverse health` отвечает `0` при HTTP 200 и `status ∈ {ok, degraded}`, `1` — при `fail` (HTTP 503), другом коде, недоступном адресе, неразборчивом теле или статусе вне перечня.
- *Почему проба, а не статус мира:* `infrastructure.md` §7.2 уже говорит, что `degraded` — работающий процесс (HTTP 200, не `down`), а §6.3 — что `make up` без LLM не падает. Проба «только `ok`» противоречит обоим: с настоящим контекстом `llm` `core` без LLM не станет `healthy`, а `gateway` с `projection=stale` не пустит `telegram-bot` (`depends_on: service_healthy`). `uninitialized` — первый случай, где это видно, а не единственный.
- *Отвергнуто:* (а) `uninitialized` как `ok` с деталью — сигнал «мир не создан» пропадает из `make health`, а пробу это не чинит; (в) `core` без хранилища до инициализации — ровно то, что закрыла приёмка T-057: пустая память над хранилищем, и `world init --bus kafka` создал бы мир, который State не записал, а переход к хранилищу потребовал бы рестарта.
- Строгость оператора остаётся в `make health`: он печатает настоящий статус каждого процесса, и `degraded` даёт ненулевой код, кроме вызова из `make up`/`deploy`/`rollback` (`DEGRADED_STRICT=0`, по образцу `LLM_STRICT`). Правка `Makefile` и `infrastructure.md` — devops-engineer, текст — карточка T-059.
- Код пробы (`cmd/multiverse/health.go` и строки теста `runHealth`) входит в итерацию 2 T-059 по образцу `ownership.md` §3 п. 4 (б) — в ветке задачи с обязательным просмотром tech-lead#1; файла `health.go` в перечне (б) нет, и исключение называет это решение. Без него слияние T-059 в `develop` ломает `make up` на свежем стеке.

**State без хранилища над журналом, который переживает процесс.** Без хранилища State читает `system_events` с офсета 0 при каждом новом объекте контекста. Над шиной в памяти это верно: журнал умирает вместе с процессом. Над kafka это заново отвечало бы на все предложения журнала при каждом рестарте. Правило: State без хранилища стартует только над шиной в памяти, иначе `Start` отказывает с именами `MV_MINIO_ACCESS_KEY` и `MV_MINIO_SECRET_KEY`.

**Место стража `OneStateOverTheWorld`** — `shared/testkit/state` подходит: подпакет поставщика контракта C-02 (EPIC-002, `ownership.md` строка `shared/testkit/**`), тесты `cmd/multiverse`, `test/e2e` и `internal/state` его видят, production — нет (`no-testkit-in-production`). Условие — страж не импортирует `internal/*`: иначе подпакету понадобилось бы новое исключение depguard. Окончательное подтверждение — tech-lead#1.

---

## 20. Решения по T-475 (2026-09-14, system-architect#1)

Основание — вопросы и отступления исполнителя T-475 (карточка `epics/EPIC-002-state-mechanics/tasks/T-475.md`, разделы «Выполнение (developer)» и «Решения system-architect»), код рабочей копии ветки `task/T-475-world-init-kafka` (`internal/state/{admin,bootstrap,store,snapshot,context}.go`, `cmd/mvctl/internal/world/*`), C-01 v1.11 (`AdminOnly`), `shared/env/vars.go`, `plan/ownership.md` §1 (строки `.env.example` и `shared/env/**`). Контракты `contracts.md` не меняются: тело и коды admin-маршрута State не входят ни в C-06, ни в C-14 (C-14 называет триггер и `snapshot.created`, а не запрос), а вызывающие маршрута — `mvctl world` и харнесс S3 — внутри EPIC-002.

| # | Что | Решение | Где |
|---|---|---|---|
| 1 | Тело admin-маршрута снапшота | Подтверждено как в коде: необязательное `{"reason": "admin"\|"bootstrap"}`; без тела, `{}` или без `reason` — `admin`; иной `reason` — `400 invalid_reason`; не JSON, неизвестное поле, второй документ — `400 invalid_body`; при `400` ничего не пишется; предел тела 4 КБ | §4.9 |
| 2 | `reason=bootstrap` над инициализированным миром | Риск есть: потери данных нет (это снапшот `admin` с `seq > 0`, указатель переписывается текущим состоянием), но метка ложна, и путь к ней реален — два одновременных `world init --bus kafka`, второй завершается `0` с `seq 1, reason=bootstrap`. Решение: `bootstrap` только у мира без `latest.json`, проверка в State на worker'е мира, `409 world_initialized`. Код — итерация 2 T-475 | §4.9 |
| 3 | Комментарий `Bootstrap` «opts меняют только ожидание» | Переписан: `opts` задают ожидание и источник решения «уже создано»; в сигнатуре `WithAnswerTimers`, `WithObjects`, `*Refusal` | §4.10 |
| 4 | Голый `mvctl world init` идёт в стек | Соответствует КД: `--bus` берёт `MV_BUS`, как процесс (T-058 уже брал это умолчание, отказывая). Ужесточения нет: защищают предусловия — указатель (`2`), `--force` (`2`), `409 world_initialized` | §4.10 (з) |
| 5 | Описание `MV_CORE_URL` | Строка бэклога EPIC-001, не эта ветка. Код от описания не зависит; чтение CLI на хосте уже названо в `.env.example` (блок CLI: `MV_CORE_URL=http://127.0.0.1:8090`); правка трогает `shared/env/vars.go`, `.env.example` и эталон `infrastructure.md` §4.2 — зоны EPIC-001 и devops-engineer, с проверкой `mvctl env check` и `make secrets-scan` | карточка T-475 |
| 6 | Отступления исполнителя 3–5 | Приняты: `/health` `core` до предложений (находка `core`); `ReadLatest` помечает неразборчивый указатель `ErrUndecodable`; новые файлы — внутри пакетов из «Файлов» | §4.10 |
