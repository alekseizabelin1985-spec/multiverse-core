...
### 7.7. Ход с механикой и конец сессии по `leave` и смерти *(изм. T-313; реализация — новые задачи Н-1 и Н-2, карточка T-313)*
...
- Ход с Phase 1 завершается, когда выполнены оба условия: нарратив подтверждён всеми адресатами и механика записана (`mechanics_at`). `turn.completed` публикует шаг, выполнивший условие последним: `OnDelivered` последнего адресата или `OnMechanics`, в транзакции вызывающего. Ход без Phase 1 завершается по ack, как в §7.2.
- `narrative_at` — момент последнего ack, а не момент публикации. Механика, записанная после него, даёт `narrative_ms = 0`; метрика NFR-002 шаблоны и так исключает.
- Дедлайн: `Sweep` завершает ход, доставленный всем адресатам, но без механики (отказ State по предложению шлюза, потерянное событие), со статусом нарратива (`ok` или `degraded`), без полей механики и с `Warn`; `turns_failed` не растёт. `timeout` — ходу, нарратив которого не доставлен.
