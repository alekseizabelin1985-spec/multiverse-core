...
> **Изменения 2026-09-14 (T-309, architect#3).** Решения по вопросам исполнителя T-309 — карточка `epics/EPIC-004-gateway-bot/tasks/T-309.md`, раздел «Решения architect#3». §7.6 — порядок восстановления при старте. §11.2 — шлюз не ждёт `analytics.replay.completed` (`MV_GATEWAY_REPLAY_WAIT` не существует); в replay лизинг работает, выключен sweeper; снапшот шлюза пишется только в `live`, концы сессий во время записи сливаются в один снапшот; `laws_version` — только из проекции; правило снятия `projection=stale`. §11.4 — плоские `details` вместо `deps{}`, `bus` по ошибке подписки, `missing` без указателя State не деградирует, `core_admin` появляется с T-356, `snapshot: no_laws_version`, каданс проверки БД в replay, `/health` бота с порогом 7. Контракты не менялись; вопросы к system-architect — в карточке. Итерация 2 (ревью #1, решения оркестратора): §11.2 — в replay после `End` только подписки, без таймеров и sweeper'ов; проверка `component = state` при загрузке снапшота State на старте; отказ `Start` при ошибке догона; починка `stale` в догоне не может отказать `Start`; в `Stop` сжатие `links.db` не зависит от хранилища, у снапшота свой срок; отказ от сверки хеша обоснован заново после синхронного снапшота State (ADR-011, дополнение 2026-09-14). §11.4 — `bus: fail` только по ошибке подписки после старта.
...
> **Изменения 2026-09-14 (T-313, T-315, architect#3).** Решения по вопросам исполнителей T-313 и T-315 — карточка `epics/EPIC-004-gateway-bot/tasks/T-313.md`, раздел «Решения architect#3». §4.2 — ход `narrated`, ждущий механику; `end_reason` активной сессии — объявленный конец. §7.2 — ход завершается после ack и механики. Новый §7.7 — ход с Phase 1 ждёт механику; конец сессии по `leave` и смерти (реализация — новые задачи, в карточке Н-1 и Н-2). §8.1 — порядок между видами доставок не гарантирован. §10.4 — темп опроса бота после раннего пустого ответа. §15 — e2e шлюза в `--mode=live` на `FakeGateway`, место e2e бота. Контракты не менялись; запросы к system-architect (C-08 — порядок доставок и темп клиента; `ownership.md` и `.golangci.yml` — место e2e бота) — в карточке.
...
*(изм. T-313)* Ход действия с Phase 1 может стоять в `narrated` при `delivered_count = recipients_count` и пустом `mechanics_at`: нарратив доставлен, механика ещё не записана (§7.7). `sessions.end_reason` у строки с `state = 'active'` — объявленный конец сессии, который ждёт завершения её открытых ходов (§7.7); `CHECK` и миграции не меняются.
...
sequenceDiagram
    ...
    B->>G: ack d-2
    G->>G: turns.OnDelivered: delivered_count == recipients_count и механика записана (§7.7) → narrative_at → status completed
...
- Восстановление после рестарта: таблица `sessions` — истина (ADR-003 п. 8 допускает и это); при старте активные сессии с `last_action_at` старше 30 мин закрываются `idle`, остальные продолжаются; открытые `turns` с истёкшим дедлайном → `timeout`. *(изм. T-309)* Восстановление идёт в `Start` до догона журнала, подписок и первого HTTP-запроса: `turns.Sweep` (просроченные ходы → `timeout`), затем `session.Manager.Sweep` (сессии, простаивающие не меньше `MV_GATEWAY_SESSION_IDLE`, → `ended idle` с `ended_at = last_action_at + IDLE`). В `--mode=replay` — без публикации `analytics.*`.

### 7.7. Ход с механикой и конец сессии по `leave` и смерти *(изм. T-313, T-478; реализация — T-478, T-479 и задача «нарратив хода по виду», карточка T-478)*

Уточняет §7.6, пункты «Ход» и «Завершение».

**Ход с Phase 1 ждёт механику.** Механика хода (`combat.decided` в `game_events`, факт State в `system_events`) и его нарратив (`narrative_output`) приходят в шлюз разными подписками, и C-01 не упорядочивает топики между собой. С шаблонным нарратором ack нарратива нередко обгоняет механику; без правила ниже `mechanics_at`, `mechanics_ms`, `result_event_id`, `phase1_mode` и `lod` в `turn.completed` не попадали бы.
- Phase 1 есть у `enter`, `leave`, `attack`, `flee`, `rest`; нет — у `look`, `say`, `defend` (`api-contracts.md` §1.4, столбец «Phase 1»). Предикат — таблица по `turns.action_type`.
- *(уточн. T-478, принято architect#3)* **Механика хода с Phase 1 — событие своего типа:** у `attack` и `flee` — первое `combat.decided` корреляции, у `enter`, `leave`, `rest` — первое `entity.updated` корреляции. «Первое» — по офсету своего топика: порядок внутри топика C-01 гарантирует. Прочие события механики этой корреляции в ход не пишутся и его не завершают. У боя механика есть в двух топиках: решение в `game_events` и вызванные им факты State в `system_events`. «Первое из любых» делало бы `result_event_id` зависимым от того, какой топик consumer прочитал раньше. Решение — это результат, который получает игрок (§7.2: факты боя применяются без доставки; S2 `group_view_consistency` в `metrics.md` сравнивает `result_event_id` участников). У перемещения и отдыха результат — факт State по предложению шлюза, он же доставка `kind=mechanics` (§8.1). У действий без Phase 1 записывается первое `combat.decided` или `entity.updated`, как прежде.
  - Неудачное бегство: удар вслед — второе `combat.decided` той же корреляции; механика хода — первое, само бегство.
  - `enter`, открывший встречу: встреча создаётся фактом `entity.created` (ADR-026, ADR-028) и объявляется `encounter.started`. Ни то, ни другое механикой хода не является; механика — `entity.updated` перемещения. `combat.decided` при входе в MVP-1 не бывает: у `enter` Phase 1 — «позиция» (`api-contracts.md` §1.4), бой решает роль `encounter` по действию игрока.
  - Отказ State по предложению шлюза (`entity.update.rejected`) механикой не является — в том числе у `rest` при встрече, которую проекция шлюза ещё не видела. Доставленный ход закрывается на дедлайне по правилу ниже.
  - `group.leave` во встрече исполняется как `flee` (§7.4): его механика — `combat.decided`. Строка таблицы и тест — T-352.
- Ход с Phase 1 завершается, когда выполнены оба условия: нарратив подтверждён всеми адресатами и механика записана (`mechanics_at`). `turn.completed` публикует шаг, выполнивший условие последним: `OnDelivered` последнего адресата или `OnMechanics`, в транзакции вызывающего. Ход без Phase 1 завершается по ack, как в §7.2. *(итерация 2 T-478, подтверждено architect#3)* Сбой публикации `turn.completed` в `OnMechanics` эффект механики не откатывает (довод §5.5). Завершение идёт в точке сохранения транзакции consumer'а — только в транзакции: вне её точка сохранения сама открывает транзакцию на соединении. При сбое откатывается одно завершение (статус хода и счётчики сессии), механика и доставки события остаются, пишется `Warn`. Ход с тем же id публикует первый из шагов, которые снова пробуют завершение: следующий тик `Sweep` — независимо от дедлайна, — следующее событие механики того же типа в корреляции (удар вслед при неудачном бегстве) или ack доставки другого нарратива корреляции. Повторную доставку того же события отсекает `processed_events`, и завершения она не пробует.
- `narrative_at` — момент ack, которым `delivered_count` достиг `recipients_count`, а не момент публикации. У нарратива без адресатов со связкой это момент его применения (`OnNarrative`). Механика, записанная после него, даёт `narrative_ms = 0`; метрика NFR-002 шаблоны и так исключает.
- *(уточн. T-478, принято architect#3)* Доставки считаются до `recipients_count`. Ход, ждущий механику, не учитывает ack второго нарратива корреляции (смерть после последнего удара): `delivered_count` и `narrative_at` остаются такими, какими были бы при завершении по ack. Предел — страховка: какой именно нарратив подтверждён, он не различает. Точный счёт — по `narrative_event_id`, пункт ниже.
- **Нарратив хода при нескольких `narrative.output` в корреляции** *(решение architect#3 по T-478; реализация — новая задача после T-479, номер выдаёт оркестратор; до неё в коде — первый по офсету)*. Нарратив хода — первый по офсету нарратив **вида хода** (C-05 `kind`): в соло `entry` у `enter` и `look` (C-05 v1.2), `turn` у остальных действий; у хода раунда группы — `round` (§7.3, I2).
  - `world_event`, `death` и нарратив другого вида — сопутствующие. Они доставляются как обычно, но полей хода (`narrative.*`, `recipients_count`, `narrative_event_id`, `absence`) не пишут, ход в `narrated` не переводят и без адресатов его не завершают.
  - `OnDelivered` считает только доставки, у которых `data.narrative_event_id` равен нарративу хода.
  - Нарратива своего вида к дедлайну нет — `timeout`; фолбэка на сопутствующий нет.
  - C-10 и схемы не меняются: C-10 не говорит, какой из нарративов корреляции описывает ход, а `absence` по `metrics.md` §4.2 копируется из нарратива персонального GM — это `entry`, а не `world_event`.
  - *Почему не «первый по офсету».* `entry` и `world_event`, `turn` и `death` публикуются из разных подписок. Порядок между ними C-05 v1.4 п. 8 задаёт только для смерти у настоящего GM, для `FakeNarrator` инверсия допустима. С роем `world_event` GM региона может обогнать `entry` персонального GM, и ход входа потерял бы `absence` и `agent_level=task`. *Почему не `based_on`:* у `attack` основание — последнее решение обмена, а не `result_event_id`, и правило зависело бы от механики. *Почему не агрегат:* меняет смысл полей C-10 (`contract-change`).
- Дедлайн: `Sweep` завершает ход, доставленный всем адресатам, но без механики (отказ State по предложению шлюза, потерянное событие), со статусом нарратива (`ok` или `degraded`), без полей механики и с `Warn`; `turns_failed` не растёт. `timeout` — ходу, нарратив которого не доставлен. Число, которое возвращает `Sweep`, включает все закрытые им ходы, в том числе оставленные сбоем публикации (пункт выше).
- *Почему не второе событие и не «известное поведение».* Второе событие по ходу меняет C-10 («одно на ход») и заставляет отчёт EPIC-005 склеивать события; `gateway.db` отчёт не читает (§4.2). Потеря полей не случайна: теряются ходы, где нарратив обогнал механику, и `mechanics_latency_p95_ms` (`metrics.md` §4.2) считался бы по смещённой выборке — мгновенный шаблон деградации есть и у настоящего роя. Цена правила — публикация позже ack на задержку чтения механики.

**Конец сессии по `leave` и смерти** (UC-027 п. 3, US-038, C-10 `end_reason`).
- Факт-граница соло: `entity.updated` игрока с `cause=move` по корреляции хода с `action_type = leave` (строка `turns`, как в `consumer.OnRejected`); `entity.updated` игрока со `status → dead`. Отказ State (`entity.update.rejected`) границей не является. Группа (I2): смерть всех участников — T-351; `group.disbanded` → `leave` — §7.4.
- Consumer в транзакции факта объявляет конец: активной сессии scope ставится `end_reason` (`UPDATE … WHERE state = 'active' AND end_reason IS NULL`).
- Сессия закрывается, когда у неё не осталось открытых ходов: сразу в транзакции факта, иначе в `turns.finish` последнего открытого хода; страховка — `session.Sweep` и восстановление в `Start`. `ended_at` — момент закрытия. Так счётчики `session.ended` равны статусам её `turn.completed`, а `session.ended` идёт после `turn.completed` хода-границы.
- Закрытие выполняется в транзакции вызывающего и без мьютекса `session.Manager`: у `Touch` порядок «мьютекс → соединение», и мьютекс под транзакцией дал бы взаимоблокировку на единственном соединении `gateway.db`. Идемпотентность — `UPDATE … WHERE id = ? AND state = 'active'` по числу строк; `Touch` так же проверяет свой `UPDATE last_action_at`. Откат закрытия при сбое публикации возвращает объявленный `end_reason`, а не `NULL`. *(architect#3, ревью #2 T-478; реализация — T-479)* Закрытие сессии в `turns.finish` стоит внутри той же точки сохранения, что и завершение хода: отказ публикации `turn.completed` или `session.ended` откатывает к ней оба закрытия, объявленный `end_reason` и эффект факта остаются, и `Sweep` публикует сначала ход (тот же id; повтор по id допустим), затем `session.ended`, — так ход `leave` с отказом шины сохраняет порядок `turn.completed` → `session.ended`.
- Новое действие scope при объявленном конце: `Touch` закрывает сессию с объявленной причиной и открывает новую.
- После конца сессии отказ действия мёртвого персонажа `turn.completed` не публикует: отказ не открывает сессию (`Tracker.Rejected`, T-306).
- `/forget` закрывает сессию сразу хуком (§7.5), без объявления.
...
**Порядок между видами доставок** *(изм. T-313, T-315)*. Порядок очереди игрока — порядок постановки, то есть порядок, в котором шлюз обработал события. Механика (`game_events`, `system_events`), нарратив (`narrative_output`) и `world_event` (`world_events`) приходят разными подписками, и C-01 не упорядочивает их между собой. Поэтому нарратив хода может встать в очередь раньше механики того же хода: с шаблонным нарратором так бывает часто, с LLM нарратив приходит на секунды позже. Шлюз не переупорядочивает (C-05 v1.4 п. 8), клиент тоже. Уточнение гарантии C-08 — запрос к system-architect (карточка T-313, «Решения architect#3», вопрос 4). Удержание нарратива в `Lease` до механики той же корреляции — бэклог EPIC-004, если инверсия помешает игроку на T-391 или T-392.
...
**Темп опроса** *(изм. T-315; значения — по коду T-315)*. Пустой ответ, пришедший раньше `deliver.EarlyEmpty` (половина `wait`), бот считает остановкой шлюза (C-01 v1.8, `runtime.ShuttingDown`) и делает паузу, как после неудачного опроса: 1, 2, 4 … 30 с. Пустой ответ, дождавшийся `wait_ms`, паузы не даёт и сбрасывает счёт неудач. Ack идёт отдельным клиентом с коротким сроком и одним повтором, чтобы ack при `503` не держал цикл дольше лизинга 30 с. Обязанность клиента делать паузу после раннего пустого ответа — запрос к system-architect в C-08 (карточка T-313, вопрос 5); порог и паузы остаются реализацией клиента.
...
- Старт: открыть БД → миграции → `readmodel.LoadFromStateSnapshot` (`snapshots-{world}/state/latest.json` через `objstore`; нет объекта → пустая проекция, `/health.projection=missing`, worlds заполнятся из `entity.created`) → подписки с курсора **снапшота State** для проекции; побочные эффекты (outbox, turns, rounds, analytics) — только для событий с `offset > cursors[topic]` из `gateway.db` (два курсора: «проекция» и «эффекты»); `processed_events` — дедуп внутри окна. *(изм. T-309)* Порядок в коде: БД → миграции → загрузка проекции → восстановление (§7.6) → догон каждого из четырёх топиков до `Journal.End()` от меньшего из двух курсоров (снапшот State — для проекции, `курсор эффектов + 1` из `gateway.db`; топик без обоих сразу отдаётся подписке) → подписки → проверка последнего снапшота шлюза → первый HTTP-запрос. *(изм. T-309, ит. 2.)* Ошибка догона или выход за `CatchUpBudget` (2 мин) — отказ `Start`, а не старт в `degraded`: шлюз не проверяет действия по проекции, которую не довёл до конца журнала. `/health` остаётся `fail {started: false}` (§11.4), процесс перезапускает оркестратор контейнеров. Загрузка снапшота State проверяет указатель и объект так же, как починка `stale` (ниже): `component = state` у обоих, мир объекта, `entity.StateHash` сущностей = `state_hash` указателя, `id` у каждой сущности. Указатель или объект с чужим `component` не загружается: проекция пустая, `projection: missing`, `projection_error: snapshot_unreadable` (`degraded`). Истёк `SnapshotLoadBudget` (30 с) — `snapshot_timeout`. В обоих случаях старт продолжается по журналу. **Шлюз не ждёт `analytics.replay.completed`** ни в одном режиме и ни в одной раскладке процессов. Проекция — это снапшот State плюс факты журнала. Факты, которые State допубликует при своём восстановлении (висящая запись, интенты ADR-013), придут подпиской и лягут по правилу версии. Ожидание не сделало бы проекцию вернее. Недоступность State видна в `/health` процесса `core` и в `turn.completed status=timeout` (§7.6). Поэтому переменной `MV_GATEWAY_REPLAY_WAIT` нет: C-14 v0.4 (б) вводит её только для дизайна с ожиданием.
- `--mode=replay`: таймеры раундов и sweeper (сессии, ходы, `ReleaseExpiredLeases`, `Expire`, чистка) не работают; `round.opened/closed` и `analytics.*` не публикуются (читаются); снапшот шлюза не пишется и `snapshot.created` шлюза не публикуется. *(изм. T-309; прежде «лизинг outbox не работает».)* Лизинг и `Ack` работают во всех режимах: их ведут запросы клиента, а не таймеры, и e2e в `--mode=replay` (§15) получает доставки через харнесс. Ожидание long-poll идёт по настенным таймерам соединения (C-01 v1.8, T-307). Истечение лизинга без ack проверяет запрос `Lease` по часам процесса: в replay, пока события не двигают `EventClock`, повторной выдачи нет; `Publish` действий харнесса разрешён (e2e подаёт действия). Признак «конца журнала» — `Journal.End(ctx, topic)` (C-01 v1.1, G-9 принято с изменением: отдельного `Bus.Lag()` нет): gateway читает через `Journal.ReadRange` от курсоров до `End` по каждому из четырёх читаемых топиков; когда по всем `позиция ≥ End − 1`, догон закончен и запускаются подписки `Subscribe` с consumer-группой `gateway.consumer`. *(изм. T-309, ит. 2; прежде «включение таймеров/sweeper'ов» при переходе.)* Запуск подписок — единственное, что происходит на `End`, в обоих режимах; «переход в live» означает только его, режим процесса не меняется (`/health mode` остаётся `replay`). Таймеры раундов, sweeper и горутина снапшотов запускаются после подписок и только в `--mode=live`, от `deadline_at`/`last_action_at` из `gateway.db`. В `--mode=replay` их нет и после конца журнала: иначе хвост записи дал бы таймауты и `analytics.*`, которых нет в эталоне (NFR-061). Реализуют и `membus`, и kafka-адаптер — одна семантика (contract-тест F-5t).
- Снапшот gateway (`internal/gateway/snapshot`) *(изм. T-309; прежде «по `SIGTERM` и по каждому `analytics.session.ended`», «быстрая сверка проекции»)*. Объект `snapshots-{world}/gateway/{YYYYMMDDTHHMMSSZ}-{seq:06d}.json` = `{schema_version, component, snapshot{…}, world_id, cursors{projection, effects}, projection_hash, active_sessions[], open_rounds[], laws_version}`. Затем указатель `gateway/latest.json` (формат указателя State, `state-and-mechanics.md` §4.4), ротация до 5 последних (ADR-021 п. 3) и `snapshot.created component=gateway {snapshot{id, seq, taken_at, cursor, laws_version, state_hash: projection_hash, size_bytes, key}}` с миром в конверте (C-14 v1.3).
  - **Триггеры — только в `live`.** Первый — `Stop`: после остановки подписок, sweeper'а и писателя, пока шина открыта, в пределах срока `Stop`. *(изм. T-309, ит. 2.)* У записи снапшота в `Stop` свой срок — часть оставшегося срока `Stop`, а не весь он: контекст `Stop` общий для контекстов процесса (`runtime.StopTimeout`). Сжатие отложенного `links.db` (§7.5) и закрытие БД от хранилища не зависят и выполняются при любом исходе записи: ошибка, зависшее хранилище, истёкший срок. Порядок «сжатие, затем снапшот» или «снапшот со своим сроком, затем сжатие» выбирает реализация. Недопустимо одно: чтобы зависшее хранилище оставило байты внешнего ID в `links.db` (ADR-019). Не записанный в `Stop` снапшот — только `Warn`. Второй — конец сессии: `session.Config.OnEnded` после записанного и опубликованного конца. Концы, пришедшие во время записи, дают **один** следующий снапшот, и он отражает их все.
  - **В `--mode=replay` снапшот не пишется, `snapshot.created` не публикуется** — ни по концу сессии, ни в `Stop`. Это служебная публикация шлюза того же класса, что `analytics.*`. Её место в последовательности событий асинхронно (горутина, слияние запросов), что противоречит NFR-061. Восстановление идёт из `gateway.db` и снапшота State, а не из снапшота шлюза.
  - **`laws_version`** — атрибут мира из проекции, другого источника нет. Мир ещё не известен проекции — снапшот пропускается с `Warn`: так начинается новый мир, сессий в нём нет (персонаж создаётся только в известном мире). Мир известен, но без `laws_version` (обязательный атрибут, `data-model.md` §3.1) — `Error` и `/health snapshot: no_laws_version`.
  - **Проверка при старте** — в обоих режимах, только чтение. Указатель → объект → сверка (`component`, мир, ключ из `taken_at` и `seq`, `id`, `projection_hash` = `state_hash` указателя, `size_bytes`). Не сходится — `/health snapshot: corrupted`; не читается — `unreadable`; указателя нет — норма.
  - **Назначение** — аудит; истина для gateway — `gateway.db`. Сверка `projection_hash` с `snapshot.created` State во время работы в I1 не выполняется. *(изм. T-309, ит. 2; прежний довод «сериализация снапшота State асинхронна, ADR-011 п. 6» снят: после T-057 снапшот синхронный, ADR-011, дополнение 2026-09-14, п. 1.)* Теперь `snapshot.created` стоит в `system_events` сразу за фактами своего снапшота, и сверка в обработчике технически возможна. Её не делают по трём причинам. (1) Расхождение нечем исправить. Известные разрывы уже помечены `stale` и чинятся этим же событием (ниже). Неизвестное расхождение сверка лишь покажет, а исправить его может только перезагрузка проекции во время работы, отклонённая ниже. (2) `entity.StateHash` по всем сущностям мира под мьютексом модели на каждом анонсе (раз в `MV_SNAPSHOT_EVERY_FACTS` фактов) задерживает обработчики и чтения HTTP. Требования, которое оправдало бы эту цену, в I1 нет. (3) Опора на синхронность хрупкая: ADR-011 (дополнение, «когда пересмотреть») допускает возврат PUT в горутину, и тогда перед `snapshot.created` окажутся факты следующих предложений. Сверка на одном курсоре — задача офлайн-аудита (`mvctl report --audit`, бэклог). Пересмотреть, если аудит найдёт расхождения без `stale`.
- **`projection=stale` и его снятие** *(изм. T-309)*. `stale` означает, что у проекции есть сущности, заведомо отстающие от State: факт пришёл с версией не `+1` к известной или для неизвестной сущности (T-304). Журнал такую сущность не починит: `entity.updated` несёт только изменения, а недостающих фактов в читаемом журнале нет. Полный источник — снапшот State.
  - **Когда.** Событие `snapshot.created` с `payload.component = state` и миром контекста (`MV_WORLD_ID`), в проекции есть `stale`-сущности, хранилище настроено. Иначе хранилище не читается.
  - **Как.** В подписке — синхронно в обработчике `system_events`, до фиксации курсора. *(изм. T-309, ит. 2.)* В догоне годится любой из двух способов: (а) хранилище читается один раз, по последнему `snapshot.created component=state` своего мира в прочитанном диапазоне `system_events`, после его `ReadRange` и до подписок; (б) чтение на каждом анонсе, как в подписке, но суммарное время чтений в догоне ограничено общим потолком — долей `CatchUpBudget`. После исчерпания потолка починка в догоне пропускается. Обязательны оба условия. Первое: ожидание починки не отказывает `Start`, худший исход — старт с `projection: stale` (`degraded`), а следующий анонс в подписке повторит починку. Второе: курсоры и эффекты событий догона от починки не зависят. Ранние анонсы читать бесполезно: ротация хранит пять последних снапшотов State (ADR-021 п. 3), а версии сущностей в них ниже, чем у последнего. Сама замена ниже одинакова для подписки и догона. Объект читается по `snapshot.key` события, а не по `latest.json`, со сроком `StaleRepairBudget` = 5 с. Сверяются мир, `component` и `entity.StateHash(entities)` с `snapshot.state_hash` события. Затем под мьютексом модели для каждой `stale`-сущности: версия в снапшоте не ниже версии проекции → сущность заменяется копией из снапшота и снимается со `stale` (встреча переиндексируется); версия ниже или сущности в снапшоте нет → остаётся до следующего снапшота. Прочие сущности, курсор проекции и признак загрузки не меняются.
  - **Ошибка** чтения или сверки (объект удалён ротацией, хеш, мир, срок) — `Warn` с кодом `Reason*`. `stale` остаётся, событие не уходит в DLQ, курсор сдвигается.
  - *Почему это верно:* копия версии `v` — точное состояние сущности на `v` при любом порядке прихода событий. Факты `≤ v` отбрасывает существующее правило версии, факт `v+1` применяется. Правило одинаково в обоих режимах: ни таймеров, ни публикаций. State снимает снапшоты каждые `MV_SNAPSHOT_EVERY_FACTS` (200) фактов, по `SIGTERM` и по admin-запросу, поэтому `stale` держится не дольше этого. Рестарт сбрасывает `stale` загрузкой снапшота (`LoadFromStateSnapshot`).
  - *Отклонённые варианты:* полная перезагрузка проекции во время работы откатывает сущности, применённые после курсора снапшота. Перечитывание журнала до `End` возвращает те же неполные факты. Опрос `latest.json` по таймеру в replay невозможен и читает хранилище без повода.
...
`GET /health` процесса — агрегат `runtime.Status` (C-01 v1.3, `runtime.Aggregate`): `{status, details{contexts{gateway{status, details{…}}}}}`. Общий контракт задаёт только `status` и `details map[string]any`; форма `details` контекста — уровень КД. *(изм. T-309; прежде вложенный `deps{}`, `bus` «по последней успешной операции ≤ 30 с и `Bus.Health()`», `stale` «> 60 с без событий при активных сессиях», `degraded` при `projection=missing`.)* `details` шлюза — **плоские ключи**, в той же нотации, что `{state_replay: missing}` у C-14 и `{snapshot: corrupted}` у State:

| Ключ | Значения и как считается | Влияние на `status` |
|---|---|---|
| `links_store`, `gateway_store` | `ok\|fail`. Чтение БД (`sqlite_master`; `session.Manager.Active`, `rounds` в `open\|closing`, `outbox.Store.Pending`) не чаще раза в 10 с, ожидание единственного соединения ≤ 100 мс; соединение занято → последнее известное значение | `fail` → `fail` |
| `bus` | `ok\|fail`: `fail` — только если подписка завершилась ошибкой после старта (`consumer.Err`). *(изм. T-309, ит. 2.)* Ошибка догона или выход за `CatchUpBudget` — не `bus: fail`, а отказ `Start` (§11.2): контекст не запущен, `/health` отдаёт `fail {started: false}`. Метода `Bus.Health()` в C-01 нет | `fail` → `degraded` |
| `projection` | `ok\|missing\|stale` (§11.2) | `stale` → `degraded`. `missing` без указателя State не деградирует: так стартует новый мир, и `make up --wait` не должен падать до `mvctl world init` (решение T-304) |
| `projection_error` | код `Reason*` (`store_misconfigured`, `snapshot_unreadable`, `snapshot_timeout`, `hash_mismatch`, `world_mismatch`) — только если снапшот State есть, но не загрузился | → `degraded` |
| `snapshot` | `corrupted\|unreadable` (проверка при старте), `write_failed`, `no_laws_version`; снимается следующей успешной записью | → `degraded` |
| `links_compaction` | `pending` — стоит отметка отложенного сжатия `links.db` (§7.5; изм. T-456) | → `degraded` |
| `mode` | `live\|replay` | — |
| `sessions_active`, `rounds_open`, `outbox_pending`, `outbox_oldest_age_s` | числа; возраст старейшей `pending` по `created_at`, `0` при пустой очереди | — |
| `core_admin` | `ok\|unavailable` — **появляется с прокси T-356**. До неё ключа нет: заглушка `skipped` не вводится, потому что проверка, которой нет, не должна выглядеть выполненной | `unavailable` → `degraded` |

До `Start`, после отказа `Start` (в том числе по ошибке догона) и после `Stop` — `fail {started: false}`. Каданс проверки БД в `live` идёт по `Deps.Clock`, в `--mode=replay` — по настенным часам (`clock.Real`, только `Now`, без таймеров). Иначе `EventClock`, стоящий между событиями, прятал бы отказ БД после последнего события (NFR-016). Внешних ID, ключей и адресов хранилища в `details` нет; текст ошибок — только в лог.

`/health` бота (`cmd/telegram-bot/health.go`) *(изм. T-309)*:
- `degraded` (HTTP 200) после `deliver.DegradedAfterFailedPolls` = 7 неудачных long-poll подряд. При паузах 1, 2, 4, 8, 16, 30 с седьмая неудача приходит через 61 с после первой: штатный рестарт шлюза не краснит бота, а отказ самого шлюза виден в его `/health` ≤ 30 с. Смена последовательности пауз требует пересчёта константы.
- `degraded` после `401` при отправке. Успешный опрос обнуляет счётчик, отправленное сообщение снимает признак `401`.
- `details`: `deliveries_failed_polls`, `telegram_token_unauthorized`. `telegram-bot health` на `degraded` отвечает 1.

Логи — `slog` JSON: `service=gateway|telegram-bot, context, request_id, correlation_id, event_id, handled` (NFR-033).
...
| e2e (`//go:build e2e`, один процесс: `gatewaytest.FakeGateway` + `FakeState` + `swarm.FakeContext` хука `MV_SWARM_FAKE` на общей `membus`, `--mode=live`, `clock.Manual`, id из последовательности) *(изм. T-313; прежде «`--contexts=all --mode=replay`». В replay шлюз не публикует `analytics.*`, а восстановление после сбоя идёт в `live`; поведение replay проверено на уровне контекста в T-309)* | `solo-30` через HTTP-харнесс + `FakeState` + `FakeEncounter` + `FakeNarrator`; `group-3x30` с явным `rounds/close` (три клиента `ci-harness` = три `external_id` платформы `ci`); `recovery` (10 ходов → рестарт контекста gateway в процессе → сессии/раунды/outbox сохранены, `identical=true`); `death`, `flee-fail`, `forget` (доставки после forget не выдаются); `privacy-scan` (регистрация с известным ID → grep по `gateway.db`, событиям membus, логам). Конец шага сценария — `turn.completed` (T-313); сценарий в `--mode=replay` определяет конец шага по доставкам (карточка T-313, «Решения architect#3», вопрос 3) | `test/e2e/gateway_*_test.go`, `shared/testkit/gateway`, `testdata/` |
| e2e бота (детерминированный) | `FakeUpdateSource` подаёт сценарий обновлений (`/start` → согласие → имя → `/enter` → `/attack`…), `FakeSender` записывает исходящие; gateway — `gatewaytest.FakeGateway` in-process на `membus`; проверяются тексты, клавиатуры, `action_key` стабильность, ack, порядок ответов; доставки шага сравниваются набором, порядок между видами не гарантирован (§8.1) *(изм. T-315)* | `cmd/telegram-bot/e2e/` (`//go:build e2e`; `internal` бота из `test/e2e` не импортируется; правило depguard `cmd-telegram-bot-e2e` — до подтверждения system-architect) |
