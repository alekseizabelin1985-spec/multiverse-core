...
**Сверка T-409 (2026-09-11, architect#1).** «Закрыто» — документ приведён к дереву; «открыто» — оставлено намеренно.

| # | Статус | Что сделано |
|---|---|---|
| 1 | закрыто | C-14: обе переменные ожидания объявляются в манифесте **тем изменением, которое вводит их чтение** (EPIC-003, EPIC-004) — манифест держит только читаемые переменные (урок T-408). До этого их отсутствие — не расхождение |
| 2 | закрыто | исправлено в коде T-408 (манифест — источник истины, флаг перекрывает, `kafka\|memory`); документы: `infrastructure.md` §4.2, `foundation.md` §2, `overview.md` §17.4, `gateway-and-bot.md` §11.3 |
| 3 | открыто | `--recording` оживёт вместе с `internal/replay` (EPIC-002). Открытый вопрос T-408 — переменная для журнала воспроизведения — решается по тому же правилу C-14: заводится с кодом, который её читает |
| 4 | закрыто | `overview.md` §17.4: абзац «Ожидание сигнала ограничено» |
