...
**Сверка T-409 (2026-09-11, architect#1).** «Закрыто» — документ приведён к дереву; «снято» — расхождения не было или оно исчезло; «код» — прав контракт, названа задача; «открыто» — оставлено намеренно.

| # | Статус | Что сделано |
|---|---|---|
| 1 | закрыто | `overview.md` §12: основной рантайм — нативный llama-server, Ollama — второй, облако — тот же `openai_compat` за гейтом; абзац «Сверено с деревом» под диаграммой |
| 2 | закрыто | `overview.md` §12: связь харнесса с `/v1/admin/*` помечена «будущее», названы владельцы маршрутов (EPIC-003, EPIC-002) |
| 3 | снято | в целевом §12 GitHub Actions нет и не было; §2 — картина as-is и остаётся исторической |
