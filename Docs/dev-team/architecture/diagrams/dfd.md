...
| # | Статус | Что сделано |
|---|---|---|
| 1 | закрыто | C-01 v1.3, абзац «Приватность `dead_letters`»: топик разбора под правилами доменных топиков, скан NFR-041 его включает |
| 2 | закрыто | `threat-model.md`: все `LLM_*` получили префикс (`MV_LLM_STORE_PROMPTS`, `MV_LLM_PROVIDER`, `MV_LLM_CLOUD_*`), `BOT_ALLOWED_USER_IDS` → `MV_TELEGRAM_ALLOWED_USER_IDS` (по манифесту и `contracts.md` v0.4) |
| 3 | открыто | не расхождение, а факт эксплуатации: ротация логов бота уже описана в `infrastructure.md` §7.1 |
| 4 | открыто | принятое ограничение статического линтера: локальный файл переопределения не в Git и из-под проверки выходит по построению |
