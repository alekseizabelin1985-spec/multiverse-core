...
Дата: 2026-09-13 · architect#2 (EPIC-003, T-439)
...
sequenceDiagram
    ...
    alt mode=replay или нарратив из записи
        ...
        G->>G: prompt_hash совпал? нет → warn llm_replay_prompt_drift, при непустых массивах ярлыки не разрешаются
    else live
    ...
    alt ярлык в text
        ...
        G->>BUS: llm.output (response_raw с ярлыками)
        G->>BUS: llm.output.rejected: до C-07 v1.4 reason=other + element, после — unknown_entity + ref
...
- Ярлык, которого не было в промпте, стоит отброса одного элемента, а не повтора всего ответа (п. 5). Форма события отброса зависит от решения по C-07 v1.4 (п. 6).
- В replay расхождение `prompt_hash` не останавливает прогон, но ярлыки ответа с непустыми массивами тогда не разрешаются, чтобы ссылка не ушла другому id (п. 8).
