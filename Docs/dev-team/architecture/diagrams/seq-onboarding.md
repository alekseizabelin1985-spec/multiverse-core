...
| # | Статус | Что сделано |
|---|---|---|
| 1 | открыто | не расхождение документа: строку проверит настоящий State (EPIC-002 T-056); до него двойник владения не проверяет — это записано в комментарии `statusRefusal` в `shared/testkit/state/apply.go` и в dev-log T-017 |
| 2 | закрыто | C-02 v1.4, абзац «Связка путь ↔ причина», и ADR-025 п. 4: норма — текст контракта, State проверяет её поверх таблицы |
| 3 | закрыто | `infrastructure.md` §1.3: профиль `bot` описан, но до `cmd/telegram-bot` падает на старте контейнера, а не на проверке конфигурации |
