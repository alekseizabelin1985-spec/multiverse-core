...
**Сверка T-409 (2026-09-11, architect#1).** «Закрыто» — документ приведён к дереву; «снято» — расхождения нет; «код» — прав контракт, названа задача.

| # | Статус | Что сделано |
|---|---|---|
| 1 | снято | `data-model.md` §7.2 и §9.7 в редакции v0.2.1 уже перечисляют шесть значений; одиннадцать значений упоминаются только как удалённые — строка отставала от документа |
| 2 | закрыто | ADR-016: пометки об устаревании п. 2/п. 4 (`rejected_language` → `invalid` + `reason=language`) и п. 3 (`filter_error` без `response_raw`) |
| 3 | код | схема `llm.output` не ограничивает `error.message`; правка схемы — у её владельца EPIC-003 (задачу определяет tech-lead EPIC-003, естественно — вместе с реализацией записи) |
| 4 | закрыто | C-03, абзац «Отдых — действие механики, а не боя»: два перечня названы разными множествами |
