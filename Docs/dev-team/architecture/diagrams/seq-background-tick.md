...
| # | Статус | Что сделано |
|---|---|---|
| 1 | закрыто | `contracts.md` §0 и шапка `swarm-llm-laws.md`: каталог `blueprints/` назван будущим (EPIC-003 T-203); образец старого формата — `shared/agent/examples/` (as-is) |
| 2 | закрыто | `overview.md` §17.3: модель тика — из блупринта, в базовой конфигурации E та же, что для нарратива |
| 3 | открыто | замер конфигураций (F-8) не выполнен — это работа стенда, а не документа; до него потолок вызовов фона чисел не имеет |
