...
| # | Статус | Что сделано |
|---|---|---|
| 1 | закрыто | C-08 «Заглушка» и `contracts.md` §17: `FakeGateway` помечен «будущее, EPIC-004 T-308»; сказано, что `Harness` v0 бота не разблокирует |
| 2 | закрыто | C-05 v1.3 п. 3: встреча не отвечает на непринятое действие, потребитель различает состояние по `encounter.started`/`encounter.ended` и отвергает такое действие сам |
| 3 | закрыто | `gateway-and-bot.md` §5.1, §11.1, §11.3: адрес — `MV_CORE_ADDR`; `MV_GATEWAY_ADDR`/`MV_GATEWAY_LISTEN` не существуют |
| 4 | закрыто (T-411; сверка T-416) | имя по манифесту `MV_GATEWAY_CLIENT_IDS` (+ `MV_GATEWAY_ACTOR_KIND_CLIENTS`) в C-08 v1.3, `gateway-and-bot.md`, `api-contracts.md`. Значения: три списка клиентов compose передаёт ключом без значения, и умолчание у них одно — манифеста; возврат класса держит правило 8 линтера композиции (`infrastructure.md` §3.1.1). Платформа клиента для SEC-12 в MVP-1 — фиксированное соответствие в коде, отдельная переменная появится со вторым ботом (решение оркестратора по T-409; EPIC-004 §5.1) |
