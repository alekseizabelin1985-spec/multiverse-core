...
> **Пометка tech-lead#1 (T-416, 2026-09-11).** Это запись ревью на 2026-09-09, текст не переписывается. Два имени в ней с тех пор сменились. (1) Подкоманда отчёта `mvctl session-report` (§2, §4, §5.1 п. 2 и 6, §5.2 п. 4) теперь называется **`mvctl report`** (C-10, T-409). (2) «Статичная таблица» `contracts.OwnershipRules` (§3.3, §5.3) больше не копия: по ADR-025, подтверждённому в T-416, это **единственная истина** таблицы владения, а `shared/agent/levels.go` своей таблицы не держит.
