...
Версия 0.3.3 · 2026-09-13 · tech-lead#2 (TEAM-2) · статус: в работе (перезапуск трёх команд; подволна A).
...
| **0.3.3** | **2026-09-13** | **Приёмка T-202, tech-lead#2** (рабочее дерево `.worktrees/T-202`, база `d5237b0`). T-202 — `done`. Замечания ревью #1 Mi-1, Mi-2, N-1…N-3 закрыты при приёмке (проверки, файлы корпуса `invalid/`, строки тестов, мутанты), N-4 — комментарий и строка DoD T-204. Бэклог ревью и исполнителя разнесён строками DoD в T-204 и T-222 и бэклогом T-202; запрос depguard — tech-lead#1. Подволны §6.1 и объём задач не менялись. |
...
Инкремент I1a · подволна **B** *(ревизия 4; было 1.2)* · developer#1 · Размер M · Статус **done** *(принята tech-lead#2 2026-09-13, итераций ревью 1; подробности — `tasks/T-202.md`)* · ветка `task/T-202-levels-validator`
...
DoD (специфика):
- [x] По одному невалидному файлу на каждое правило 1–14; тест таблицей «файл → ожидаемые `Field`/`Reason`/`Severity`».
...
- [x] **(T-416, 2026-09-11; ADR-025, C-02 v1.4)** Валидатор проверяет `owned_entity_types` блупринта по `ValidationEnv.OwnedEntityTypes`, который собирает вызывающий из `contracts.OwnershipRules`. Тест: тип вне строки своего уровня → `error` с `Field`/`Reason`. `shared/agent` не импортирует `shared/contracts`; `go list -deps ./shared/agent/...` — в тесте или в правиле depguard.
- [x] Правило 7а: `env.Models == nil` → `info: models not checked`; модель вне списка → `error`; поле `provider` в блупринте → `error` (`KnownFields`).
- [x] `monitor`/`object` — `info: reserved level, spawn disabled`, не `error`.
- [x] **(приёмка T-201, 2026-09-13; ревью #1 T-201 Mi-3, ревью #3 T-201 бэклог п. 2)** Правило 13 использует и `UnknownPlaceholders`, и `SuspiciousPlaceholders`.
...
- [x] **(приёмка T-201, 2026-09-13; ревью #3 T-201 бэклог п. 2; C-02 v1.6, T-448)** `warning` на значения в `ops[].value` и `conditions[].value`, которые State отвергнет только при применении:
  - число за |x| ≤ 2^53−1;
...
**Приёмка (tech-lead#2, 2026-09-13): принято.** Итераций ревью 1, вердикт ревью #1 — «принять» (0/0/2/4). Все строки DoD выполнены, отклонения исполнителя 1–9 приняты. Замечания закрыты при приёмке правками tech-lead#2 в рабочем дереве задачи; подробности — `tasks/T-202.md`, «Приёмка».
- **Mi-1.** `ttl` обязателен у уровня `task`; у резервного `monitor` не требуется: его стоп не определён. `max_instances ≥ 1` у всех уровней. `round` обязателен у роли `encounter`, `round.timeout` — положительная длительность. `background_events` непуст у `global`.
- **Mi-2.** У роли, в белом списке которой нет `entity.*.proposed`, любой элемент `owned_entity_types` → `error` «role … proposes no entity changes». C-02 и ADR-025 это не противоречит: вывод делается из белого списка событий `levels.go`, таблицы владения в пакете по-прежнему нет.
- **N-1.** Индексы `[n]` в поле сортируются как числа.
- **N-2.** Добавлены строки на `NaN` в `temperature` и на чужой тип scope у `global-gm`, `region-gm`, `group-narrator`.
- **N-3.** `@vN` переводится в файл только у `laws_ref`; форма `laws/<мир>@vN` обязательна.
- **N-4.** Исправлен комментарий `projectFileExists`; выравнивание символических ссылок — строка DoD T-204.

Корпус `invalid/` — 20 файлов, добавлены `rule-05-ttl-required.md`, `rule-06-instances-at-least-one.md`, `rule-08-ownership-row.md`, `required-round.md`. Мутанты в копии дерева: контрольный и 25 рабочих убиты тестами. Прогоны зелёные: `go build/vet`, `go test -short` (`shared/agent` 95.6 %), `golangci-lint run ./...` (0 issues), `mvctl contracts check`, `make test`. С кончиком эпика `2ad8d4f` путей не делит; `shared/agent` проходит тесты на `shared/contracts` кончика. Строку статуса в этом разделе правил исполнитель — правка принята, статус переписан.

**Бэклог T-202 (приёмка T-202, 2026-09-13; без номера — номер выдаёт оркестратор при взятии в работу):**
1. **Правило depguard `shared-agent`** — запрос tech-lead#1, путь EPIC-001 (`.golangci.yml`). Deny `multiverse-core.io/shared/contracts` для `**/shared/agent/**`, `!$test`. После него `TestPackageDoesNotImportContractsOrContexts` можно оставить или снять.
2. **`npc_table[].stats_ref` в `rules`** (`api-contracts.md` §3.2) валидатор не проверяет: статов `rules` нет в окружении. Покрыто тестом согласованности I1b в DoD T-232. Если нужна статическая проверка — решение architect#2 о поле `ValidationEnv`.
3. **Правила вне §13.2** (вопрос architect#2): `immediate_broadcast` — только `world.*`/`region.*` (КД §13.1); ограничение `trigger.event_name` ролью.
...
- [ ] **(приёмка T-202, 2026-09-13; ревью #1 T-202 бэклог п. 1)** Уникальность `name` в каталоге (`api-contracts.md` §3.1): два блупринта с одним `name` → `error` с обоими файлами. `EnvFromProject` читает только `<root>/blueprints` без рекурсии. Подкоманда либо передаёт корень проекта, либо ищет родителей в том же `<dir>`; решение закреплено тестом на `blueprint validate <другой каталог>`.
- [ ] **(приёмка T-202; ревью #1 T-202 бэклог п. 2, бэклог исполнителя п. 2)** Один построитель `OwnedEntityTypes` из `contracts.OwnershipRules` (фильтр `Proposer == level`, без `AnyType`), его же берёт рантайм (T-222). Место — вне `shared/agent/...`: тест `go list -deps ./...` из каталога пакета видит и подпакеты. Перевод `ParseError` → `Issue` — функцией API `shared/agent` вместо копии `issuesOfFile` из теста.
- [ ] **(приёмка T-202; ревью #1 T-202 N-4)** Символические ссылки в `EnvFromProject` обрабатываются одинаково у трёх читателей. Сейчас схемы (`WalkDir`) ссылку молча пропускают, блупринты (`ParseFile`) идут по ссылке, `FileExists` (`os.Stat`) идёт по ссылке и за пределы `root`. Тест со ссылкой пропускается там, где ссылку создать нельзя (Windows без прав), в CI на Linux выполняется.
- [ ] **(приёмка T-202; вопрос 1 architect#2)** Серьёзность правила 7а в CLI и в рантайме — по решению architect#2 (поле окружения или стабильный код находки). До решения подкоманда не сравнивает `Reason` как текст.
...
DoD: [ ] фикстуры каталога с одним невалидным блупринтом → активны остальные, список в `Health()`; [ ] `ScopeIndex` покрыт таблицей переходов (вход в регион, группа, встреча, выход); [ ] `MV_SWARM_BLUEPRINTS_DIR` объявлен. [ ] **(приёмка T-202, 2026-09-13; КД §13.2 п. 7а, вопрос 1 architect#2)** Модель вне `Provider.Models()` в рантайме — `warning`: агент активируется, `/health degraded {llm: model_missing}`, вызовы уходят в шаблон. Форма API — по решению architect#2, одна с T-204; тест на оба режима. [ ] **(приёмка T-202; ревью #1 T-202 бэклог п. 1–2)** `LoadDir` собирает `ValidationEnv.OwnedEntityTypes` тем же построителем, что и CLI (T-204), и отвергает второй блупринт с тем же `name`; тест.
