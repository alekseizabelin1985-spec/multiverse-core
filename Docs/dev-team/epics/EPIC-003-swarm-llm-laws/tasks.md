...
Версия 0.2.1 · 2026-09-09 · tech-lead#2 (TEAM-2) · статус: к G3 (Flow A, A4 шаг 3).
...
Ветка эпика: `epic/EPIC-003-swarm` от `integration/mvp-1`. Диапазон номеров команды: **T-200…T-299** (TEAM-1 — T-001…T-199, TEAM-3 — T-300…T-399). Использовано: T-201…T-256 (разработка) + T-260…T-266 (стенд). Свободны: T-257…T-259, T-267…T-299.
...
Файлы: `shared/agent/{levels,validator}.go`, `shared/agent/testdata/blueprints/{valid,invalid}/*.md`, **`shared/contracts/ownership.go` (копия — тем же PR, чужой путь, ревью tech-lead#1 + system-architect)**.
Зависит от: T-201, T-006 (EPIC-001 F-4b-1 — исходная копия `OwnershipRules` со строкой gateway).
Ссылки: US-015, US-037, FR-090…092, FR-061, BR-16, **`contracts.md` v0.4 C-02 v1.2, §16 п. 6**, C-11 v1.1, SEC-21, ADR-015, `consolidation.md` §14.1 (TL2-3, З-2), design §3.1 A2, КД §13.2.
DoD (специфика):
- [ ] По одному невалидному файлу на каждое правило 1–14; тест таблицей «файл → ожидаемые `Field`/`Reason`/`Severity`».
- [ ] Тест равенства `levels.AllowedEventTypes/OwnedEntityTypes` ↔ `contracts.OwnershipRules` (расхождение = падение теста, а не тихая правка чужой таблицы); тест запускается в job `contracts` и **блокирует merge**.
- [ ] **(сведение 3)** строка gateway присутствует в обеих таблицах: `Character.status → abandoned` и `Group.leader_id` (в т. ч. `null`); PR помечен `contract-change` и содержит обе правки.
...
DoD (специфика):
- [ ] `MV_SWARM_FAKE=true` → процесс `core` поднимается с фейковым контекстом `swarm`; `MV_SWARM_FAKE` не задан → контекст `swarm` отсутствует (или настоящий, если `internal/swarm` есть) — тест на обе ветки.
...
- [ ] I1-α: бой с волком и шаблонный нарратив работают в собранном `cmd/multiverse` (не только в тестах).
...
Зависит: T-223. Ссылки: US-037, FR-121…123, КД §5.2–5.3, design §3.2 R2.
DoD: [ ] по одному тест-кейсу на каждую строку таблицы видимости (5 ролей); [ ] «вверх» и «вниз» проверены: `region.event_occurred` доходит до `global-gm` и не доходит до игроков другого региона (US-037); [ ] смена родителя не публикует событий.

#### T-230 · R7 · Роль `encounter` (соло)
I1b · 1.10 · developer#2 · M · todo
Что: `roles/encounter.go` — Phase 1 синхронно: `Resolve` атаки/бегства/`npc_attack`, `dice.rolled` ×k, `combat.decided` ×2, один `entity.update.proposed{atomic, expected_version}`, трофей один раз, `encounter.ended reason=npc_dead|players_out`, `agent.child_resolved`; `rest` во встрече не обрабатывается (отклоняет State).
**Дополнение (сведение 3, C-02 v1.2, C-04 v1.1; З-2; внесено tech-lead#1 от имени tech-lead#2)**: роль трактует `status = abandoned` **как `dead`** — покинутый игрок исключается из целей (`NPCTarget`), из `expected[]`/`acted[]` и из `participation=active`; gateway при `/forget` во встрече ничего дополнительно не публикует, роль сама видит `entity.updated status=abandoned` и завершает встречу `encounter.ended reason=players_out`, если игроков со `status=alive` не осталось. `narrative.output kind=death` по `abandoned` **не** издаётся.
Зависит: T-223, F-10/EPIC-002 (`FixedMechanics` → `Resolve`). Ссылки: US-002, US-003, FR-020…024, FR-061, NFR-060, C-03, **`contracts.md` v0.4 C-02 v1.2, C-04 v1.1**, КД §8.3, design §3.2 R7.
DoD: [ ] seed-детерминизм: два прогона с одним `causeEventID` дают одинаковые броски (NFR-060); [ ] провал `flee` → свободная атака NPC; смерть игрока → `players_out`; [ ] **(сведение 3)** тест: `entity.updated status=abandoned` последнего игрока → `encounter.ended reason=players_out`, без `narrative.output kind=death`; [ ] `expected_version` берётся из `WorldView`, конфликт версии обрабатывается повтором.
...
Зависит: T-223 (интерфейсы `Behaviour`/`Phase2Runner`), T-235, T-213; интеграция с Pipeline (T-228) проверяется e2e T-242. Ссылки: US-002, US-004, US-005, US-036, FR-013/015/127, NFR-002/006, C-05, КД §8.2, design §3.2 R8.
DoD: [ ] по кейсу на каждую строку таблицы триггеров; [ ] ровно один вызов LLM на ход соло (счётчик `FakeProvider`); [ ] `world.*` вне хода не порождает вызова, а попадает в контекст следующего нарратива; [ ] смерть игрока → `kind: death`, дальше агент живёт до TTL.
...
Зависит: T-225, T-234. Ссылки: US-011, FR-031…033/087, NFR-010…012, C-14 v1.1, ADR-011, КД §4.3, design §3.2 R10.
DoD: [ ] round-trip: снапшот → восстановление → `state_hash` совпадает; сортировка map при сериализации (NFR-061); [ ] `latest.json` пишется после успешного PUT объекта; [ ] повреждённый снапшот → отказ старта с сообщением в `/health`, не «тихий пустой рой».

#### T-237 · R10b · `swarm.Context`: подписки, фаза догона, режимы live/replay
I1b · 1.11 · developer#3 · M · todo
...
Зависит: T-236, T-228, **T-017 (EPIC-001 — `FakeState` v0 публикует `replay.completed`)**. Ссылки: US-011, NFR-010…014, C-01 v1.1, **C-14 (уточнение v0.4)**, ADR-003, `consolidation.md` §14.1 (TL2-6), КД §4.3, §5.1, design §3.2 R10, §13 п. 7.
...
Зависит: T-233, T-232, T-230, T-237, T-221. Ссылки: US-002, US-004, US-017, NFR-061, ADR-010, КД §18, design §3.2 R13.
...
Зависит: T-230; EPIC-002 I2 (atomic группы, `Participation`) — до слияния используем `FakeState`. Ссылки: US-006, US-007, FR-025, FR-061, BR-13 v0.4, NFR-005, **`contracts.md` v0.4 C-02 v1.2, C-04 v1.1**, КД §8.3.
...
| **1.10** | T-225 R3 Lifecycle | T-230 R7 encounter | T-236 R10a snapshot | 3 | бой соло на `FixedMechanics` детерминирован; снапшот round-trip |
| **1.11** | T-226 R4a Scheduler | T-233 R8 personal-gm | T-237 R10b runtime/догон | 3 | один вызов LLM на ход; догон без эмиссии |
| **1.12** | T-227 R4b тики/бюджет | T-231 R6a global-gm | T-238 R10c integration | 3 | `tick.fired` с `lod_allowed`; integration на testcontainers зелёный |
| **1.13** | T-228 R5 Pipeline | T-232 R6b region-gm | T-239 R11 admin/health | 3 | встреча по тику региона; `/health` с `agents_by_level`; текст admin → tech-lead#3 |
| **1.14** | T-240 R12 миграция | T-241 S6 второй регион | T-242 R13a e2e solo/death/flee | 3 | `gm_path` в 100 % событий; S6 без Go-диффа; S1 на записях |
| **1.15** | T-245 доки I1b **и T-256 удаление хука** (обе S) | T-243 R13b e2e фон/деградация/recovery | T-244 R13c golden | 3 | **готовность I1** → передача tech-lead#1 на интеграцию (порядок 002 → 004 → 003); хук `MV_SWARM_FAKE` удалён из `cmd/multiverse` **до** тега `mvp-1/i1` |
...
Правило внутри подволны: задачи не пересекаются по файлам (потоки A/B/C и рантайм/роли/контекст разведены по пакетам). Пересечения-исключения: `roles/triggers.go` — только developer#2 (T-231/T-233), `router.go` — только developer#1 (T-224), интерфейсы `Behaviour`/`Phase2Runner`/`TickRunner` фиксируются в T-223 и далее меняются только задачей тимлида.
...
- [ ] S3 снапшот роя восстанавливается, рестарт → `llm_calls=0`, `identical=true` (T-236, T-237, T-238, T-243).
...
| I1b | 25 | 3 / 22 | ≈ 23,5 | 1.7–1.15 (9, с нахлёстом на I1a) |
| I2 | 8 | 3 / 5 | ≈ 6,5 | 2.1–2.4 (4) |
| **Всего разработка** | **56** (55 при конфигурации E) | 9 / 47 | ≈ 51,5 (**50,5** при конфигурации E) | 17 |
...
**Критический путь эпика** (17 подволн):
...
| 9 | Расхождение `levels.go` ↔ `contracts.OwnershipRules` обнаружится после волны 0 | тест равенства в T-202 падает на первой же несостыковке и **блокирует merge**; **(сведение 3, §16 п. 6)** копию `shared/contracts/ownership.go` правит владелец истины (TEAM-2) **тем же PR**, метка `contract-change`, ревью tech-lead#1 + system-architect (см. §10 п. 3) |
...
3. **РЕШЕНО (сведение 3, TL2-3).** `contracts.md` v0.4 §16 п. 6: истина — `levels.go` (TEAM-2), копия `shared/contracts/ownership.go` правится **тем же PR** владельцем истины (метка `contract-change`, ревью tech-lead#1 + system-architect, блокер); тест равенства блокирует merge; State — потребитель. Внесено в T-202. Исходно: **Процедура при расхождении `levels.go` ↔ `contracts.OwnershipRules`.** Тест равенства (T-202) падает, но `shared/contracts` принадлежит EPIC-001/system-architect. Нужен пункт процедуры: кто и в какой волне правит копию (предложение — задача EPIC-001 по запросу TEAM-2, приоритет блокера).
