...
Версия 0.3.5 · 2026-09-13 · tech-lead#2 (TEAM-2) · статус: в работе (перезапуск трёх команд; подволна A).
...
| **0.3.5** | **2026-09-13** | **Приёмка T-207, tech-lead#2** (рабочее дерево `.worktrees/T-207`, база `d5237b0`; совместимость с кончиком `cc5d31e` проверена на копии без слияния). T-207 — `done`, итераций ревью 2. Отклонение «чтение `*.jsonl`/`llm_records` вне пакета» принято по решению T-457 (`shared/recording`, T-458 EPIC-002). Nit N-1 ревью #2 закрыт при приёмке (комментарий теста). Строки DoD в T-212 (удаление `recorded.WithCall`, ключ из `llm.Request` C-15 v1.4, попытка 2 в replay, записи без текста, адаптер `Source` над `shared/recording`) и T-221 (`Writer` проверяет таблицу C-07 до записи). Строка T-210 про `countingProvider` уже в T-212 (0.3.3). Поставка C-07/C-15 — следующим контрольным слиянием EPIC-003 в `develop`. Подволны §6.1 и объём задач не менялись. |
...
- [ ] **(T-439, architect#2, 2026-09-13; итерация 2 — ADR-029, ревью #1 T-439 Mi-1…Mi-3)** Потолок нарратива — значения конфигурации **E** (действуют до исхода T-438; источник — КД §13.4.1, `ops/metrics/baseline.md` §5): `player-gm.llm.phase2.max_tokens: 160`, `group-narrator.llm.phase2.max_tokens: 160` — своё число группы, не ссылка на `player-gm`; `schemas/agent/narrative.json`: `text.maxLength: 185` *(итерация 2; было 210 — нижняя граница плотности c_min)*, `mentions.maxItems: 4`, `background_refs.maxItems: 2`, элементы массивов — строки-ярлыки **по ADR-029 п. 3**: `pattern` `^e[1-9][0-9]?$` и `^b[1-9][0-9]?$`, схема статична и на вызов не сужается. `$comment` схемы — в форме `narrative cap: config=E; c=2.5; c_min=2.0; overhead=66; …` с указателем на `baseline.md` §5, КД §13.4.1 и ADR-029; YAML-комментарий у `max_tokens` указывает туда же. `## phase2` обоих блупринтов просит «1–2 предложения, до ~140 символов» (L, КД §13.4.1): ровно одно вхождение формы `до ~<L> символов`, тест читает его выражением `до ~(\d+) символов` *(приёмка T-439, ревью #2 N-8)*. Числа Qwen3.6-35B-A3B (210 → 285 предварительно; 400 → 665 после T-438) в файлы не вносятся: переход — одной правкой данных в T-260. Тест: `max_tokens` двух блупринтов, `maxLength` и `maxItems` схемы и L в `## phase2` равны строке E таблицы КД §13.4.1; c и c_min тест читает из `$comment`, служебные поля считает как 30 + 6 × сумма `maxItems` и проверяет три инварианта КД §13.4.1: `overhead + ⌈maxLength / c_min⌉ ≤ max_tokens`, `maxLength ≤ (max_tokens − overhead) × 0,9 × c`, `L ≤ 0,8 × maxLength`. Мутант `maxLength: 210` при `c_min=2.0` — тест красный. Что тест не ловит (реальную плотность токенизатора) — КД §13.4.1, проверяет стенд T-260.
- [ ] **(T-439)** `temperature: 0.7` во всех фазах с LLM — то же значение, что умолчание фазы в шлюзе и профиль замера (`bench-matrix.json`, `request.sampling_non_thinking`); КД §13.3 приведён к 0.7 в T-439, строка `api-contracts.md` §3.3 с 0.8 — у владельца документа.
- [ ] **(T-439; итерация 2 — по ADR-029 п. 7)** Фикстуры `testdata/fixtures/events/llm.output.v1.{valid,invalid}.json` (владелец типа — EPIC-003) приведены к КД §13.4: `response_raw` — ответ по `narrative.json` (`mentions`/`background_refs` — массивы строк-ярлыков, а не объекты `{entity:{id,type}}`; в `text` ярлыков нет), `params.max_tokens: 160`, `response_len` пересчитан по `response_raw`. Пара `llm.output.rejected.v1.*` не меняется: фикстура валидна по схеме C-07 v1.3. Для нарратива сценарий `entity` — существующая, но невидимая агенту сущность, и тип берётся из `View`: в MVP-1 сущности не удаляются, а появится удаление — таблица ярлыков понесёт тип. Выдуманная ссылка без типа — ADR-029 п. 6; фикстура с `ref` — после решения по C-07 v1.4. `go test -short ./shared/contracts/...` и `mvctl contracts check` зелёные.
...
Инкремент I1a · подволна **B** *(ревизия 4; было 1.2)* · developer#2 · Размер M · Статус **done** *(принята tech-lead#2 2026-09-13, итераций ревью 2; подробности — `tasks/T-207.md`)* · ветка `task/T-207-fake-recorded-providers`
...
Что сделать: `providers/fake` — таблица `(phase, matcher) → Response`, счётчик вызовов, задержка ответа (для тестов уступки), генерация «грязных» ответов с преамбулой/`<think>` для тестов парсера, `Models()` из таблицы; `providers/recorded` — ключ `(meta.correlation_id, meta.agent.id, phase, attempt)`, чтение `*.jsonl` и журнала `llm_records` *(приёмка T-207: вне пакета — решение system-architect#1 по T-457, формат, чтение и индекс записи — `shared/recording`, T-458 EPIC-002; в пакете — функция `recorded.Source`, адаптер над `shared/recording` — DoD T-212)*, `ErrIncompleteRecord` при промахе.
...
DoD (специфика):
- [x] Промах ключа у `recorded` → `ErrIncompleteRecord` (не тихий шаблон, не живой вызов).
- [x] Счётчик `FakeProvider.Calls()` используется в тесте «replay: `llm_calls=0`».
- [x] ~~Пакет доступен потребителям вне EPIC-003 (EPIC-002 `replay`, EPIC-005 golden) — экспортируемое API описано в `internal/llm/providers/README.md`.~~ **(ревизия 4)** Экспортируемое API описано в `internal/llm/providers/README.md`, там же — кто может импортировать пакеты по `depguard` (абзац выше). Тест `mode=replay` → `recorded` — в T-212.
- [ ] ~~Ветка задачи слита в `integration/mvp-1` сразу после приёмки (ранняя поставка C-07/C-15).~~ **(ревизия 4)** После приёмки и слияния в эпик tech-lead#2 просит у оркестратора контрольное слияние эпика в `develop` (поставка C-07/C-15). Условия — шапка индекса. *(приёмка T-207: поставка — следующим контрольным слиянием EPIC-003 в `develop` после слияния ветки задачи в эпик)*

**Приёмка (tech-lead#2, 2026-09-13): принята.** В дереве `.worktrees/T-207` (база `d5237b0`) зелёные: `go build ./... && go vet ./...`, `go test -short -count=1 ./...` (32 пакета), `golangci-lint run ./...` (0 issues), `mvctl contracts check`, `make test` (`internal/llm` 99.6 %, `fake` и `recorded` 100 %). Совместимость с кончиком эпика (`cc5d31e`, T-210 и T-202) проверена на копии: сборка, `vet`, тесты `internal/llm/...`, линтер — зелёные. Ревью #2 — «принять», 0/0/0/1; Nit N-1 закрыт при приёмке (комментарий `TestAMalformedRecordFailsTheSource`: провайдер отвергает не больше, чем схема; `null` в поле таблицы — отсутствие поля для провайдера, отказ для схемы).
- **Чтение записей.** Отклонение принято: решение system-architect#1 по T-457 — пакет `shared/recording` (T-458, EPIC-002); провайдер принимает источник функцией `Source`.
- **`recorded.WithCall`.** Принято временно: C-15 v1.4 переносит `AgentID`/`Attempt` в `llm.Request` (добавляет T-211, уточняется итерацией 3 T-457). Удаление — DoD T-212.
- **Строки DoD:** T-212 (ключ из `llm.Request`, `ErrNoCallKey`, попытка 2 в replay, записи без текста, адаптер `Source`), T-221 (`Writer` проверяет таблицу C-07 до записи). `countingProvider` → `providers/fake` уже в DoD T-212 (приёмка T-210).
...
- [ ] **(T-439, ADR-029 п. 3–4; вносит architect#2 по решению оркестратора, подтверждает tech-lead#2)** `schemas/agent/narrative.json` компилируется при старте, как остальные схемы: сужения на вызов и кэша скомпилированных схем нет. Для `Kind=narrative` после валидации схемой парсер проверяет `text` на ярлык вызова выражением `(^|[^0-9A-Za-z_])[eb][1-9][0-9]?([^0-9A-Za-z_]|$)` → причина `schema_invalid`; содержимое строки не меняется. Корпус: ошибка — `Альфа-волк [e3] рычит`, `e12 отступает`, `ветер стих, b2`, ярлык в начале и в конце строки; не ошибка — обычный русский текст, `re3`, `e0`, `e100`, `е3` с кириллической «е». `CheckLanguage` ярлыки не исключает: ответ с ярлыком до неё не доходит. Тест на каждый кейс.
...
- [ ] **(приёмка T-207, 2026-09-13; решение T-457, C-15 v1.4)** Ключ записи — из полей `llm.Request`: `recorded.WithCall` удаляется, `recorded.KeyOf(req)` берёт `CorrelationID`, `AgentID`, `Phase`, `Attempt` запроса. Поля в `llm.Request` добавляет T-211 (уточняется итерацией 3 T-457); если к старту T-212 их нет — задача ждёт их, а не возвращает `WithCall`. Пустая часть ключа (`CorrelationID`/`AgentID` пусты, `Attempt < 1`, фаза вне `llm.output`) → `ErrNoCallKey`, не `ErrIncompleteRecord`; `TestACallWithoutItsKeyIsNotAMiss` переписан под поля запроса. Шлюз строит `Request` каждой попытки заново с её номером `Attempt`. Тест на `recorded`: запись попытки 1 `invalid`, попытки 2 `valid` → в `mode=replay` попытка 2 читает запись попытки 2, `Result` — её текст, `FakeProvider.Calls()` = 0.
- [ ] **(приёмка T-207, 2026-09-13)** Записи без текста воспроизводятся по статусу записи: `error` (`recorded.ErrRecordedFailure`) — как ошибка провайдера этой попытки, `quarantined`/`filter_error` (`recorded.ErrResponseWithheld`) — как тот же исход фильтра, без повторного фильтра по пустому тексту и без `ErrIncompleteRecord`. Тест на каждый из трёх статусов.
- [ ] **(приёмка T-207, 2026-09-13; решение T-457, C-01 v1.9)** Источник записей `recorded` — адаптер `recorded.Source` над `shared/recording` (T-458, EPIC-002; в эпик приходит из `develop`): в `mode=replay` — из `runtime.Deps.Recording`, при восстановлении — через `recording.ReadJournal`. Адаптер соблюдает контракт doc `Source`: все события или ошибка, ошибка обработчика возвращается без повторов и `dead_letters`, неполное чтение — ошибка. Своего разбора JSONL и чтения журнала в `internal/llm` нет. Тест: неполный источник → `New` возвращает ошибку, провайдер не создаётся. Если T-458 не в `develop` к старту T-212 — адаптер выносится отдельной задачей (номер у оркестратора), остальное DoD T-212 от неё не зависит.
- [ ] В `mode=replay` провайдер подменяется на `recorded`, новых `llm.output` не издаётся.
- [ ] **(T-439, ADR-029 п. 4 и 8; вносит architect#2 по решению оркестратора, подтверждает tech-lead#2)** На шаге парсера для `Kind=narrative` вызывается проверка ярлыка в `text` (T-209): ответ с ярлыком — `invalid`, `llm.output.rejected reason=schema_invalid` без `element`, повтор с подсказкой в `user`, `system` не меняется. **Replay.** В `mode=replay` и при нарративе из записи (КД §8.4) шлюз сверяет `prompt_hash` пересобранного промпта с `prompt_hash` записи `recorded`. Совпал — запись используется как есть. Не совпал — `warn` `llm_replay_prompt_drift` (`agent`, `phase`, `attempt`): у тика и у нарратива с пустыми массивами — только предупреждение; у нарратива с непустыми `mentions`/`background_refs` массивы в `Result` пусты, `text` — из записи. Тесты на `recorded`: совпадение; расхождение у тика; расхождение у нарратива с `background_refs` — `Result` без ссылок и запись в логе; во всех трёх `FakeProvider.Calls()` = 0 и новых `llm.output` нет.
...
- [ ] **(T-439, ADR-029 п. 6; приёмка T-439, tech-lead#2, 2026-09-13)** Отброс выдуманной ссылки нарратива (отброс стража без id: ярлыка нет в таблице вызова) публикуется одной функцией отображения вердикта в событие. Форма — по решению system-architect#1 в T-457: до решения — `llm.output.rejected reason=other` с `element{index, type: mentions|background_refs}` и лог `warn` с ярлыком; после C-07 v1.4 — `unknown_entity` с `ref`. `llm.output.reasons[]` содержит ту же причину. Тест: ответ `mentions: ["e1","e7"]` при таблице `e1…e3` — `partially_rejected`, ровно одно `llm.output.rejected` с `element.index=1`, событие проходит схему C-07 действующей версии, в `Result` — текст и `e1`.
...
- [ ] **(T-439, ADR-029 п. 5; приёмка T-439, tech-lead#2, 2026-09-13; ревью #2 T-439 N-6)** Для `Kind=narrative` элементы — ярлыки вызова: `Input.Mentions` и `AbsenceEventIDs` — `map[string]string` (ярлык → id; прежде `AbsenceEventIDs map[string]bool`). Правило 3 по элементу: ярлык в таблице и сущность видима — элемент остаётся; в таблице, но не видима — `unknown_entity`, `entity` (id и тип) из `View`; ярлыка нет в таблице — элемент отброшен, текст остаётся, в `Verdict` отброс без id (форма события — T-213). Таблица не передана — **условие по массиву**: непустой `mentions` при `Mentions == nil` или непустой `background_refs` при `AbsenceEventIDs == nil` → весь ответ `invalid`, `other` без `element`, лог `error`. Правило 3 тика не меняется. Тест таблицей на схеме E (≤ 4 / ≤ 2): три ветки элемента; пустые не-`nil` карты при непустых массивах — выдуманные ярлыки, а не ошибка программы; `nil` одной карты при непустом своём массиве — ошибка программы.
...
- [ ] **(T-439, ADR-029 п. 2–3; приёмка T-439, tech-lead#2, 2026-09-13)** `<format>` нарратива — статичный текст правила ярлыков: «на сущности и события ссылайся только ярлыками из промпта; в `text` ярлыков не пиши, называй по имени; сослаться не на что — пустой массив». Подсказка повтора за ярлык в `text` — только в `user`. Тест: `system` двух вызовов нарратива с разными таблицами ярлыков побайтно равен.
...
- [ ] **(приёмка T-207, 2026-09-13; ревью T-207 Ma-1)** `recorded.Writer` до записи проверяет таблицу C-07 «поле ↔ `validation_status`» тем же кодом, что разбор `recorded` (`parseRecord`/`tableOfC07`), а не своей копией: `response_raw` обязателен у `valid`/`partially_rejected`/`invalid` и запрещён у `quarantined`/`filter_error`/`error`, `filter.status` и `error{code}` — по статусу. Нарушение → ошибка с id события, в выход ничего не пишется. Формат строки `*.jsonl` — из `shared/recording` (T-458, решение T-457), свой сериализатор не заводится. Тест «записал → прочитал через `recorded.New`» на каждый статус и контрпример на каждую строку таблицы.

#### T-439 · Потолок длины нарратива под целевую конфигурацию — **передана из EPIC-001 (ревизия 4)**
Инкремент I1a · подволна **A** · **architect#2** (первая задача architect#2 в ветке эпика), подтверждает tech-lead#2 · Размер S · Статус **done** *(принята tech-lead#2 2026-09-13, итераций ревью 2; подробности — `tasks/T-439.md`)* · ветка `task/T-439-narrative-length-cap`
...
DoD:
- [x] КД §13.3–13.4 и строка `:977` не противоречат `baseline.md` §5.
- [x] В T-203 записаны значения `max_tokens` и `maxLength`/`maxItems` для текущей конфигурации (tech-lead#2 подтверждает правкой индекса).
- [x] Формула расчёта и «символов на токен» (допущение 2,5 до калибровки T-438 шаг 5) записаны рядом с числами.
- [x] Стендовая проверка «ответ предельной длины не обрезается» передана в T-260 (не блокирует приёмку).

**Приёмка (tech-lead#2, 2026-09-13): принята** после ревью #2 (0/0/3/6; ревью #1 — 0/1/8/5). DoD сверен: КД §13.3, §13.4 и допущение 3 §19 (прежняя `:977`) сходятся с `baseline.md` §5; числа E в DoD T-203 (160 / 185 / L 140 / 4 / 2) пересчитаны; формула с c и c_min — в КД §13.4.1 и `$comment` схемы; стендовая проверка — строка T-260. Решение о ярлыках — ADR-029 (статус «предложено»; поправки ADR-016/017, C-07 v1.4 `ref` и хеш таблицы, запись для business-analyst — у system-architect#1 в T-457). Mi-8 ревью #1 подтверждён: правка критерия T-260 и чек-листа I1a по N-1 ревью #2 T-440 верна. При приёмке закрыты правкой текста: Mi-9 и Mi-10 (формулировки ADR-029 п. 6 и условий пересмотра), Mi-10, N-6, N-9, N-11 и Mi-11 (а, б) — в КД; N-7, N-8 — в DoD T-233 и T-203; строки DoD из dev-log итерации 2 с учётом N-6, N-9, N-10, Mi-10, Mi-11 — в T-213, T-217, T-218, T-235, T-237, T-248, T-260.
...
DoD: [ ] `background_refs`, попадающие в промпт, доступны стражу как `AbsenceEventIDs` (стыковка с T-217); [ ] `NopMemory` используется в `mode=replay` (детерминизм `prompt_hash`); [ ] секции соответствуют порядку КД §11.4. [ ] **(T-439, ADR-029 п. 2; приёмка T-439, tech-lead#2, 2026-09-13)** Для `Kind=narrative` сборщик выдаёт ярлыки: сущности `<state>` — `e1…eN` по id (байтовое сравнение), события `<absence>` — `b1…bM` по `at`, затем по id; N ≤ 99 (сверх — сущность без ярлыка и `warn`), M ≤ 20; форма в промпте — `Имя [eK]` и `[bK] …`, в `<events>` — имя и ярлык; id в `user` нарратива нет; карты `Mentions` и `AbsenceEventIDs` всегда не `nil` (пустая таблица — пустая карта) и уходят в `Call.Guard`; вызовы тика несут id, как прежде. Тесты: повтор сборки даёт те же ярлыки и `prompt_hash`; перестановка входа не меняет ярлыки; N = 0 и M = 0 дают пустые не-`nil` карты; ни один id из `View` и `AbsenceSummary` не встречается подстрокой в секциях `<state>`, `<events>`, `<absence>` промпта нарратива — не во всём `user`, где `<player_text>` может законно содержать id (ревью #2 T-439, N-10).
...
DoD: [ ] ≤ 2 вызова LLM на раунд группы (счётчик); [ ] один `narrative.output` на раунд, дублей от персональных GM нет; [ ] мёртвый участник получает только `kind: death` от своего персонального GM. [ ] **(T-439, ADR-029 п. 5 и 7, КД §13.4.1 «`mentions` группы»; вносит architect#2 по решению оркестратора, подтверждает tech-lead#2)** `group-narrator` разрешает ярлыки так же, как `personal-gm` (T-233): в `narrative.output.background_refs` — id событий, повторы схлопнуты. `mentions` не обязаны покрывать всех участников: `recipients[]` берутся из состава группы по статусам, а не из `mentions`. Тест: группа из 6 игроков, ответ с 4 `mentions` → `recipients[]` содержит всех 6 участников в статусах `active|idle|out_of_combat`.
...
DoD: [ ] недоступная память не ломает ход (тест на таймаут); [ ] авторский факт приоритетнее `generated` (тест с подложенным противоречием); [ ] инъекция через факт памяти экранируется (входит в `injections-10`, T-251). [ ] **(T-439, ADR-029 п. 8; приёмка T-439, tech-lead#2, 2026-09-13; ревью #2 T-439 Mi-11)** Расхождение промпта в replay с `NopMemory` против записи, снятой с живой памятью (`<facts source="memory">`, `AbsenceSummary`), покрыто сверкой `prompt_hash` (T-212) и принимается: у такой записи ссылки нарратива в replay теряются всегда, а не в редком случае. Тест: запись с непустыми `background_refs`, снятая с памятью, replay с `NopMemory` — прогон не падает, `background_refs` пусты, `warn llm_replay_prompt_drift` в логе; e2e на записях с памятью `background_refs` не сверяют.
...
|---|---|---|---|---|
| **T-260** stand | **Замер на llama-server и подтверждение моделей в пяти блупринтах по `ops/metrics/baseline.md` (сведение 2)**: прогон `scripts/llm-bench.ps1` против `/v1/chat/completions` (`response_format json_schema`, `chat_template_kwargs.enable_thinking=false`, `usage`/`timings` в CSV, VRAM по `nvidia-smi`) для конфигурации **E** (`Qwen3.8-27B-UD-Q3_K_XL`, `num_ctx` 8192/16384, KV f16/q8_0); **Ollama привлекается только если F-8 выбирает C или A** (тогда же стартует T-254 B3b) | architect#2 + человек | после F-8, до конца I1a | правка пяти YAML-полей блупринтов **по `baseline.md`**; NFR-002 (p95) / NFR-090 (русский) / NFR-022 (`valid_first_try` с грамматикой llama.cpp) подтверждены для E **или** зафиксирован переход по `baseline.md` §5 (ADR-005 доп. 3) с записью в `baseline.md` и `dev-log.md` *(T-440; редакционно T-439 — N-1 ревью #2 T-440)* · **(T-439, КД §13.4.1 «Проверки», ADR-029; приёмка T-439, tech-lead#2, 2026-09-13)** в сессии T-438, на числах текущей конфигурации (`max_tokens` = N), три прогона промпта с заданием на длинный нарратив: `finish_reason` не `length`; валиден по схеме с первой попытки, в том числе `pattern` ярлыков под грамматикой; длина `text` ≤ `maxLength`; `completion_tokens` ≤ N; доля ответов у потолка (длина `text` ≥ `maxLength` − 5) ≤ 5 %; конец фразы ≥ 95 %; доля **попыток** с ярлыком в `text` ≤ 2 % и выдуманных ярлыков (вне таблицы промпта) ≤ 5 на 100 вызовов — пороги пересмотра ADR-029, превышение — вопрос о пересмотре решения (ревью #2 T-439, Mi-10); калибровка: c — медиана, c_min — 5-й процентиль символов на токен (`/tokenize`) по ответам длиной ≥ L/2 (ревью #2 T-439, N-9), при c_min ≠ 2,0 — пересчёт `maxLength` и L одним PR; смена конфигурации или текста `## phase2` (в том числе L) — одним PR с перезаписью записей с нарративом T-261/T-262, иначе replay теряет их ссылки (ADR-029 п. 8; ревью #2 T-439, Mi-11) |
...
- [x] **(ревизия 4)** T-439 закрыта: потолок нарратива записан в КД и T-203; T-215 принята после слияния T-445. *(T-439 принята tech-lead#2 2026-09-13; T-215 принята на эпике, `3141755`)*
...
- [ ] Блупринты содержат стартовую модель E; модели соответствуют `ops/metrics/baseline.md` **или** зафиксирован переход по `baseline.md` §5 (ADR-005 доп. 3) (T-203, T-260) *(T-440; редакционно T-439 — N-1 ревью #2 T-440)* — стендовая часть **не блокирует** слияние I1a.
