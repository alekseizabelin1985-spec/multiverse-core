...
| A2 | Реестр уровней и валидатор | `shared/agent` (`levels.go`, `validator.go`) | `AllowedEventTypes/OwnedEntityTypes(level, role)` по `data-model.md` §4 (источник `contracts.OwnershipRules`, C-02 — статичная копия в F-4b синхронизируется тестом равенства); 14 правил КД §13.2 + 7а (модели); `ValidationEnv`, `EnvFromProject(root, types, invariants, models)`; glob по сегментам | по одному invalid-файлу на правило; тест равенства `levels` ↔ `contracts.OwnershipRules` |
...
| BR-16 уровни через данные | белые списки уровня — `levels.go` ⊇ блупринт; `Emitter` и страж; State — последняя линия (`contracts.OwnershipRules`) | A2 тест равенства; R2; C2 |
...
| 10 | Рассинхрон `levels.go` ↔ `contracts.OwnershipRules` (статичная копия в F-4b) | низкая / высокое (State отклоняет предложения роя) | тест равенства в A2; изменение таблицы — только запрос system-architect |
...
2. `contracts.OwnershipRules` в волне 0 — статичная таблица в `shared/contracts` (F-4b) по `data-model.md` §4; `shared/agent/levels.go` — источник для неё с волны 1, синхронизация тестом.
...
6. Профиль `legacy` собирается devops в F-6; EPIC-003 в I1b только подтверждает `gm_path` и deprecated-фильтр; сравнение нарративов legacy/agent — `session-report` EPIC-005 (Should).
