...
Версия 0.3a · 2026-09-11 · architect#2 (TEAM-2), 0.3a — system-architect#1; v0.2 — правки сведения 2 внесены tech-lead#2; **v0.3 — решения architect#2 по §10 п. 14 и T-427 индекса, раздел §14**; **v0.3a — проверка system-architect#1 (T-431): §14.5, уточнения без переписывания §14.1–§14.4** · статус: к нарезке tech-lead#2 (Flow A, A4 шаг 2). ~~Ветка эпика: `epic/EPIC-003-swarm-llm-laws` от `integration/mvp-1` (после `mvp-1/wave-0`).~~ **v0.3b (2026-09-13, tech-lead#2, ревизия 4):** ветка эпика `epic/EPIC-003-swarm-llm-laws` от `develop` (`1c2ee7e`), `integration/mvp-1` не используется; поставки — контрольным слиянием эпика в `develop` (`tasks.md`, шапка).
