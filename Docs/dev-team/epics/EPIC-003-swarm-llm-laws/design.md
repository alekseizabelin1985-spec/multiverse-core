...
Версия 0.3c · 2026-09-13 · architect#2 (TEAM-2), 0.3a — system-architect#1; v0.2 — правки сведения 2 внесены tech-lead#2; **v0.3 — решения architect#2 по §10 п. 14 и T-427 индекса, раздел §14**; **v0.3a — проверка system-architect#1 (T-431): §14.5, уточнения без переписывания §14.1–§14.4**; **v0.3c — T-459 (architect#2): §3.1 A2 — источник белых списков и `EnvFromProject`** · статус: к нарезке tech-lead#2 (Flow A, A4 шаг 2). ~~Ветка эпика: `epic/EPIC-003-swarm-llm-laws` от `integration/mvp-1` (после `mvp-1/wave-0`).~~ **v0.3b (2026-09-13, tech-lead#2, ревизия 4):** ветка эпика `epic/EPIC-003-swarm-llm-laws` от `develop` (`1c2ee7e`), `integration/mvp-1` не используется; поставки — контрольным слиянием эпика в `develop` (`tasks.md`, шапка).
...
| **0.3b** | **2026-09-13** | **tech-lead#2, ревизия 4 — редакционно, по решениям system-architect#1 (журнал 2026-09-13; тексты контрактов — T-444, линтер и схемы — T-445) и gitflow.** Ветка — от `develop`, `integration/mvp-1` и «ранний merge» заменены контрольным слиянием. C2: `guardian.Input` без типов `laws`/`mechanics` (`View`, `InvariantFunc`, `LawsSource`, решение 1a). C5: писатель записей — `internal/llm/providers/recorded.Writer`, фильтр SEC-23 в писателе (1b). Рою разрешены `llm/prompt` и `llm/guardian`, `providers/fake` — только в тестах, имя схемы в `Call` — строкой (1c). Шаблоны деградации — листовой `internal/swarm/template`, `testkit/swarm` видит его отдельной строкой `depguard` (1d). `laws.WorldVersions` — интерфейс потребителя (1f). `config.cloud_enabled {enabled, provider, external_players_ack}` без URL и ключей (2). Хеши `sha256:<64 hex>` (3). Id инвариантов `inv-01…inv-10` + `inv-11`. `MV_LLM_URL` без умолчания. Правки помечены «(ревизия 4)», решения §14 не менялись. |
...
| 0.3c | 2026-09-13 | **architect#2 (T-459):** §3.1 A2 — источник белых списков `AllowedEventTypes` — `api-contracts.md` §2.4, а не `data-model.md` §4; сигнатура `EnvFromProject(root, eventTypes, ownedEntityTypes, invariants, models)` и `Issue.Code` — по коду T-202 и КД §13.2. Остальной текст не менялся. |
...
| B1 | Типы шлюза, конфигурация, реестр провайдеров | `internal/llm` (`types.go`, `config.go`, `providers/registry.go`) | `Provider{Generate, Embed, Health, Models}` (C-15 **v1.1**), `Request/Response/Params`, `Gateway`, `Call/Result`, `ValidationStatus`, ошибки; **(сведение 2, внесено tech-lead#2)** `Params{Temperature, TopP, TopK, MinP, PresencePenalty, MaxTokens, Think}` (`Think=false` по умолчанию для всех фаз MVP-1), `Response.Tokens{Prompt, Completion, Cached}` (`Cached` — из `usage.prompt_tokens_details.cached_tokens`, 0 при отсутствии) и `Response.ReasoningLen` (длина отброшенного `reasoning_content`, сам текст не сохраняется); `MV_LLM_*` через `env.Declare`, добавлены `MV_LLM_URL` (~~по умолчанию `http://127.0.0.1:1234`~~ *ревизия 4: обязательна, без умолчания*) и `MV_LLM_API_KEY` (`MV_OLLAMA_URL` — только при `MV_LLM_PROVIDER=ollama`); значения `MV_LLM_PROVIDER = openai_compat \| ollama \| anthropic \| recorded \| fake`; **гейт облака по host `MV_LLM_URL`** (не loopback / не `host.docker.internal` / не RFC1918 → отказ старта без `MV_LLM_CLOUD_ENABLED=true`, ADR-005 доп. 2 п. 3); таблица цен `config/llm-prices.yaml` по ключу `(endpoint_host, model)`, локальные адреса = 0 | компилируется с `fake`; `env check` видит переменные; внешний `MV_LLM_URL` без `MV_LLM_CLOUD_ENABLED` → отказ старта |
...
| C2 | Страж | `internal/llm/guardian` | `Evaluate(value, Input) Verdict`, правила 3–6 × (`narrative`, `tick`), `visibility.go` (таблица КД §10.3), `reasons.go`, гипотетическое применение `ops` к копии сущности, инварианты по `check`-ключам законов, `inv-11`. **(ревизия 4, решение 1a)** `guardian.Input` не содержит типов `internal/laws` и `internal/mechanics`: мир — `View`, инвариант — `InvariantFunc`, версия законов — `LawsSource`. Реализации подаёт рой (Pipeline, R5) | таблица 9 правил × 2 вида схем (ADR-017); без шины; `go list -deps` без `laws`/`mechanics` |
...
| C5 | `RecordingWriter` + `mvctl record` | ~~`shared/testkit/swarm/recording_writer.go`~~ **`internal/llm/providers/recorded` (`Writer`)** *(ревизия 4, решение 1b: `cmd/**` не импортирует `shared/testkit`; фильтр SEC-23 — в писателе)*, `cmd/mvctl/internal/record` | запись `llm.output` из живого прогона (`Journal.ReadRange` по `llm_records` за окно сценария) в `*.jsonl` по ключу; отказ при `actor_kind≠ci` или игроке вне `player-A/B/C`; `mvctl record diff`; `.gitattributes` уже в F-1 | запись `solo-30.jsonl` снимается на стенде (стендовая задача); unit на фикстуре |
...
| R5 | Pipeline + шаблоны деградации | `internal/swarm/pipeline.go`, `template` (из C4; **ревизия 4, решение 1d:** `internal/swarm/template` — листовой пакет, только stdlib; `shared/testkit/swarm` импортирует его по строке `depguard` `internal/swarm/template$`) | `HandleEvent/HandleTick`, постановка LLM-заданий, `fallback_reason`, «рестарт между Phase 1 и 2» (КД §8.4), per-instance FIFO, сериализация по `correlation_id` | unit: все `fallback_reason` |
...
Минимально необходимое от EPIC-003 (единица C4, первая волна I1a, слить в ~~`integration/mvp-1`~~ `develop` *(ревизия 4: исполнено — T-219, T-220, T-255 в `develop` с `1c2ee7e`)* сразу после приёмки tech-lead#2, до I1-α):
...
Правило: заглушку потребитель не правит — запрос владельцу (~~через tech-lead#1~~ *ревизия 4: через тимлида своего эпика; двойники `shared/testkit/swarm` меняет поставщик — EPIC-003*).
...
| C-07 записи LLM | ~~`testkit/swarm.RecordingWriter`~~ `llm/providers/recorded.Writer` *(ревизия 4)*, `providers/fake` как источник записей; `testdata/recordings/*.jsonl` | EPIC-005 (golden, `llm usage`), EPIC-002 (`replay` — через контекст `llm` в `mode=replay`: `internal/replay` провайдеры не импортирует, `depguard`) | C5 (I1a), записи — стенд |
...
| `laws/dark-forest-world.v1.yaml` | ~~`inv-1…inv-11`~~ `inv-01…inv-10` + `inv-11` *(ревизия 4: id как в `internal/mechanics/invariants.go`)* (`check`-ключи ↔ `mechanics.Invariants().ID`), декларативные `law-1…` (уточняет автор) | A7 | I1a |
...
- **События (совместимые, уже в contracts v0.2)**: `llm.output.parse{}`, `error.code=yielded`, `narrative.output.narrative_event_id`, `tick.fired.tick.lod_allowed` (обязателен), `agent.spawned.content_hash`, `encounter.started.round{}`, `config.cloud_enabled{by, provider, allow_external_players}` *(ревизия 4, решение 2: в схеме — `{enabled, provider, external_players_ack}`)*. Новых типов вне реестра ADR-007 нет.
...
- **Env**: таблица КД §14 (все `MV_*`), объявление через `shared/env.Declare`; **(сведение 2)** добавлены `MV_LLM_URL` (~~по умолчанию `http://127.0.0.1:1234`; из контейнеров `http://host.docker.internal:1234`~~ *ревизия 4: обязательна и без умолчания, хвост `/v1` отбрасывается — T-404, C-15 в `contracts.md` v0.10*), `MV_LLM_API_KEY` (только облако), `MV_SWARM_FAKE` (читает `cmd/multiverse`, не контекст); `MV_LLM_PROVIDER` по умолчанию `openai_compat`; `MV_OLLAMA_URL` — только при `MV_LLM_PROVIDER=ollama`; `.env.example` — ~~правка через tech-lead#1/devops~~ *ревизия 4: в ветке задачи, которая впервые читает переменную; объявление в `shared/env/vars.go` — тем же изменением (C-14 уточнение v0.4)* (NFR-074).
...
5. Шаблоны деградации (`template/ru.go`) живут в `shared/testkit/swarm/template` и переиспользуются ролями — один текст шаблонов для I1-α и для деградации роя. **(ревизия 4, решение 1d)** С T-228 шаблоны переезжают в листовой пакет `internal/swarm/template` (только stdlib), двойник `FakeNarrator` импортирует его по строке `depguard` `internal/swarm/template$`; `shared/testkit/swarm/template` удаляется. Один текст шаблонов сохраняется.
