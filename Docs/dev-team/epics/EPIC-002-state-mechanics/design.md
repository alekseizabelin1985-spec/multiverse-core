...
4. **I1-α как точка приёмки I1** внутри TEAM-1: `solo-30` на `FakeNarrator(WithEncounterStub(Rules))` + `Harness` v0 в одном процессе `--bus=memory --mode=replay`; затем живая игра через бота (EPIC-004 I1) на стенде.
...
| I1-11 | e2e S1 / S3 | `internal/state/e2e_test.go` | `solo-30` (Harness v0 + FakeNarrator `WithEncounterStub(Rules)`), `recovery` (10 ходов → `Stop/Start` in-process → `identical=true, llm_calls=0`), `--chaos=duplicate` | I1-8…I1-10, F-10 |
...
- **`FakeNarrator.WithEncounterStub`** (EPIC-001 §5.1) — потребитель `Resolve`/`ChangesFor` в I1-α; после слияния EPIC-003 I1b заменяется агентом встречи. EPIC-002 не владеет им, но e2e S1 EPIC-002 на нём строится.
- **Сверка `OwnershipRules`**: в I1 State читает статичную таблицу `shared/contracts/ownership.go`; когда EPIC-003 I1a добавит `shared/agent/levels.go`, тест `contracts` сверяет обе таблицы (задача EPIC-003; EPIC-002 ничего не меняет).
...
| C-05 (`combat.decided` и др.) | потребитель косвенно (агент встречи публикует предложения) | `entity.update.proposed cause=combat` из `ChangesFor` | `FakeNarrator.WithEncounterStub` |
...
| e2e I1 | `solo-30` (S1) на `FakeNarrator(WithEncounterStub(Rules))`; `recovery` (S3) in-process; `--chaos=duplicate` | I1 «30 ходов без расхождений», «рестарт identical, llm_calls=0» |
...
| `FakeNarrator.WithEncounterStub` расходится с будущим агентом встречи в порядке `dice.rolled → combat.decided → proposed` | заглушка пишется по C-03 (порядок зафиксирован) и §7.1; e2e S1 EPIC-003 на реальном агенте — критерий I1 интеграции |
