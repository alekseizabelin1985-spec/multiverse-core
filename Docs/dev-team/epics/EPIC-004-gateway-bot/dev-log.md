...
- **e2e бота** — `cmd/telegram-bot/e2e/` (`//go:build e2e`). В тесте бот, собранный как в `serve.go`: `updates.Fake`, записывающие отправители, настоящий шлюз `gatewaytest.FakeGateway`, `FakeState`, `FakeEncounter` и `FakeNarrator` на одном `membus`. Сценарий от `/start` до `/forget confirm` проходит трижды с одинаковой записью. Отдельные тесты: группа молчит и посторонний получает один отказ (SEC-06/07); разметка нарратива доходит текстом (SEC-10); `429` и `503` не теряют команду, и тот же `update_id` даёт тот же `action_key`.
