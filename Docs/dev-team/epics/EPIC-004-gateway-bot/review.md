...
## T-316 · ревью #1 · 2026-09-14 · code-reviewer#3 (TEAM-3)

### Границы ревью
- Ветка `task/T-316-consumer-integration` (`.worktrees/T-316`), база — кончик эпика `599e883`. Изменения не закоммичены: новый `internal/gateway/consumer/redpanda_integration_test.go` (`//go:build integration`), правки карточки `tasks/T-316.md` и `dev-log.md`. Код платформы не менялся.
- Сверено с разделом «T-316» в `tasks.md` (DoD и строка приёмки T-307), карточкой, `dispatcher.go`, `shared/eventbus/kafka.go` (`Subscribe`, `ReadRange`, `End`, `stopped`), `shared/testkit` (`StartRedpanda`, `Wall`, `After`), `outbox/types.go`, `turns/tracker.go`, `client.DefaultBackoff`, `deliver/loop.go` (`flush`), `.golangci.yml` и job `integration` в `.github/workflows/go.yml`.
- Решения оркестратора учтены и замечаниями не считаются: (1) шаг 8 теста принимает два исхода, пока отдельная задача не сделает переподписку `Dispatcher` с backoff; (2) долгий `Dispatcher.Stop` при входе в группу — отдельный пункт бэклога. По решению 1 проверено, насколько узко тест принимает исходы, — это Mi-1.
- Карточку и `dev-log.md` в ветке задачи ведёт исполнитель (правило раздела T-302). Это не замечание.

### Вердикт
**Принять.** Critical (Blocker) 0 · Major 0 · Minor 2 · Nit 3.

Все пункты DoD закрыты проверками на настоящем брокере, прогон ревьюера зелёный, контейнеров не осталось. Замечания касаются точности шага 8 и формулировки одной цифры в отчёте. Mi-1 лучше исправить до задачи переподписки: после неё шаг 8 должен стать строгим, а сейчас он может пройти при частично глухом потребителе.

### Проверено ревьюером (go1.26.8 windows/amd64, cgo выключен — без `-race`)
- `go vet -tags integration ./internal/gateway/...` — 0; `gofmt -l internal/gateway/consumer/` — пусто.
- `golangci-lint run --build-tags integration ./internal/gateway/consumer/...` — `0 issues.`
- `go test -short -count=1 ./internal/gateway/...` — 14 пакетов `ok`.
- Интеграционный прогон — один: `go test -tags integration -count=1 -v -timeout 10m -run TestConsumerOnRedpanda ./internal/gateway/consumer/...`, 01:47–01:48. До прогона `docker ps -a --filter label=org.testcontainers=true` пуст, после прогона (через 15 с) — тоже пуст. Итог `ok` за 46,9 с, все 5 подтестов `PASS`. Лог — в scratchpad ревьюера (`t316-r1-integration.log`).

| Величина (`T316 measure`) | Прогон ревьюера | Карточка, прогон 4 |
|---|---|---|
| Старт Redpanda | 2,45 с | 2,65 с |
| Вход группы и 4 события | 274 мс | 204 мс |
| Хвост после рестарта группы | 222 мс | 103 мс |
| Остановка брокера | 1,92 с | 1,38 с |
| Ack при остановленном брокере, `503` | 1 / 1 / 1 мс | 102 / 1 / 1 мс |
| Запуск брокера | 2,36 с | 1,56 с |
| Повтор ack принят | через 3 мс, отказов 0 | через 3 мс, отказов 0 |
| Попытки `turn.completed` / id | 4 / 1 | 4 / 1 |
| Подписка после сбоя | закончилась: `read system_events: unexpected EOF` | закончилась: `read narrative_output: failed to dial` |
| Остановка перезапущенного потребителя при входе в группу | 29,5 с | 29,7 с |
| `TestConsumerOnRedpanda` / пакет | 46,5 с / 46,9 с | 39,9 с / 40,3 с |

Цифры карточки согласуются с кодом и с прогоном ревьюера. Логов прогонов исполнителя в scratchpad карточка не называет, поэтому сверены только с кодом и с этим прогоном.

### Разбор по пунктам поручения

**1. Изоляция и детерминизм.**
- Топики и группа общие (`consumer.Topics`, `consumer.Group` — константы пакета), поэтому уникальных имён нет. Это обосновано в комментарии файла и в карточке. Подтесты разводятся иначе: `from` = `End` всех топиков, свои id событий, счёт по префиксу (`effectsOf`, `LIKE prefix-%`). Разобрал, что остаётся от прошлых подтестов: подписка группы стартует с `FirstOffset` или с зафиксированного offset, чужие события попадают в `effects_log`, но проверки их не считают. Утверждения от этого не зависят.
- Порядок подтестов важен только для `CatchUp` (`End` = 0 у `world_events`) и для `Outage` (последний, останавливает брокер). Оба зафиксированы порядком `t.Run` и комментарием.
- Контейнер — свой: `testcontainers.CleanupContainer` сразу после старта, `Stop`/`Start` вызываются на `b.rp.Container()`. Порт хоста закреплён в `StartRedpanda`, поэтому после `Start` адрес тот же. Порядок очистки LIFO верный: остановка потребителей, закрытие БД и шины, затем контейнер.
- Все ожидания — опрос условия с потолком (`within`, циклы ack и шага 8 через `testkit.After`). `time.Sleep` нет. `time.Now` нет: время — `testkit.Wall()`/`clock.Real{}`, таймеры — `testkit.After`/`clock.RealTimers{}`. `forbidigo` в тестах не ослаблен (исключение `_test.go` снимает только `depguard`), линтер чист.
- Потолки: вход в группу и переподключение — 90 с, `stop` — 1 мин, `findDeadLetter` — 30 с. В зелёном прогоне ≈ 47 с, DoD ≤ 3 мин соблюдён.

**2. Содержательность проверок.**
- **Дубль.** `OrderAndARepeatOfTheBus` публикует одно событие дважды: эффекты `[1, 2, 3]` в порядке `rowid`, 3 отметки `processed`, hp 4 и версия 3, курсор проекции `base+4`. Мутант «без проверки `processed_events`» дал бы `[1, 2, 2, 3]`, так что проверка живая.
- **Курсоры группы.** `groupRestart` проверяет, что второй потребитель не получил событие до зафиксированного offset (`player-1` нет в новой проекции). Если потерять offset группы (`FirstOffset`), проверка покраснеет. Третий потребитель читает журнал с `base`: 4 персонажа в проекции, эффектов по-прежнему 4.
- **DLQ.** Письмо читается с брокера простым reader'ом: ошибка эффекта и id в `error`, `consumer = gateway.consumer`, `attempts = 4`, в `gateway.db` ни отметки, ни эффекта, следующее событие взято.
- **`Journal.End`.** Одинаковый догон на `membus` и Redpanda сравнивается одной структурой, эталон `membus` проверен отдельно. `ReadRange` за концом ограничен `End` внутри адаптера и не виснет.
- **Outbox при остановленном брокере.** Три ack → `503 bus_unavailable`, время ≤ `DefaultPublishTimeout` + 1 с. `pending = 2` после отказа ловит частичный откат: при частичном ack было бы 1. Ход `narrated`, `delivered_count = 1`. Повторная выдача после `DefaultLease` с `attempts = 2`. После возврата брокера — `acked:2`, затем `acked:0`, на топике аналитики ровно один `turn.completed` с id всех попыток, все доставки `delivered`, ход `completed`.
- Оговорка по «соединение `gateway.db` свободно». Проба идёт после ответа ack, а остановленный брокер отказывает сразу (1 мс). Ветка «публикация висит до срока» здесь не проходит. DoD разрешает остановку контейнера, исполнитель это сам записал в «Не замерено». Замечанием не считаю.

**3. Шаг 8 и решение оркестратора 1: какие исходы принимает тест.**
- Принимаются два исхода: (а) событие в `system_events` взято старым потребителем; (б) `d.Err() != nil` с любой ошибкой любой из четырёх подписок, затем новый потребитель берёт событие догоном из журнала.
- Верно: «тихая смерть» не проходит. `stopped` считает `io.EOF` остановкой, `Subscribe` вернёт `nil`, и `Dispatcher` не запишет `Err`. Тогда цикл через 90 с падает в `Fatalf` с диагностикой, и маскировки нет.
- Недостаточно узко (Mi-1):
  - ветвление после цикла идёт по `taken()`, а не по `d.Err()`. Если подписка `system_events` пережила сбой, а подписка `narrative_output` кончилась (как в прогоне 4 исполнителя), тест пишет «the subscription outlived the broker» и проходит. Потребитель при этом глух к нарративу;
  - в ветке (б) принимается любая ошибка, включая ошибку commit или записи в `dead_letters`, а не только ошибку чтения при сбое транспорта;
  - `again.Err()` проверяется сразу после `Start`, пока подписки только входят в группу, поэтому проверка всегда проходит;
  - что подписка перезапущенного потребителя доставляет новое событие, не проверено: событие приходит догоном из журнала.

**4. Тег и CI.** `//go:build integration` в первой строке. Job `integration` запускает `go test -tags integration -race -count=1 -timeout 20m ./...` и включает пакет. Под `-race` гонок в тесте по чтению не видно, кроме N-1: там фактической гонки нет, публикации синхронны в горутине теста.

**5. Цифры оценки ревью #1 T-307.** Длительность ack замерена. «Период ≈ 31 с» и «flush ≈ 1,4–1,8 с» — расчёт по `outbox.DefaultLease` (30 с), `WakeEvery` (1 с), `client.DefaultBackoff` (200/400/800 мс) и `Loop.flush` (id хранятся до успешного ack). Константы сверены, расчёт верен. Тест проверяет повторную выдачу сдвигом ручных часов, период не измеряет. В `dev-log.md` расчёт стоит под «Замеры» — Mi-2.

### Замечания

| # | Серьёзность | Файл:строка | Замечание | Предложение | Статус |
|---|---|---|---|---|---|
| Mi-1 | Minor | `internal/gateway/consumer/redpanda_integration_test.go:571-603` | Шаг 8 принимает больше, чем два исхода из решения оркестратора 1. (1) Ветвление по `taken()` (581): при `d.Err() != nil` и уже взятом событии тест идёт путём «подписка пережила сбой». Потребитель, у которого кончилась подписка другого топика, проходит без перезапуска и без записи в лог. Это достижимо сейчас: в прогоне 4 кончилась `narrative_output`, а событие публикуется в `system_events`. (2) В ветке (б) принимается любая ошибка `Err()`, не только ошибка чтения при сбое. (3) `again.Err()` (595-597) проверяется сразу после `Start`, поэтому проверка тавтологична. (4) Путь (а) останавливает `d` только в очистке `f.start` со сроком 10 с. После задачи переподписки подписка, которая снова входит в группу, может не уложиться в 10 с (находка 2 исполнителя), и тест покраснеет не по делу — вопреки фразе «после исправления проходит первым путём без правки». | Ветвиться по `d.Err()`. Путь (а) — только при `d.Err() == nil`: подождать, пока событие будет взято, и явно вызвать `stop(t, d)` с замером, как в пути (б). В пути (б) сузить ошибку: сообщение содержит `eventbus: read ` (ошибка `readerError` из `FetchMessage`), а не commit, доставку или DLQ; при другой ошибке — `t.Fatalf`. `again.Err()` проверять после события-сигнала, опубликованного после `Start` и взятого подпиской, как sentinel в `groupRestart`. Либо убрать проверку и честно записать, что подписку перезапущенного потребителя проверяет задача переподписки. | открыто |
| Mi-2 | Minor | `Docs/dev-team/epics/EPIC-004-gateway-bot/dev-log.md:403`; `tasks/T-316.md:68` | DoD требует замерить цифры оценки ревью #1 T-307: длительность ack и период повторной выдачи. Ack замерен. Период «≈ 31 с», «flush ≈ 1,4–1,8 с» и «около 20 копий за 10 минут» — расчёт по константам, тест период не измеряет (ручные часы). В `dev-log.md` расчёт записан под «Замеры». В карточке он в разделе «Пересчёт оценки», но не назван расчётом. | В карточке и `dev-log.md` пометить эти цифры как расчёт по `DefaultLease`, `WakeEvery`, `DefaultBackoff` и `Loop.flush`, со ссылкой на константы. Добавить пункт в «Отклонения»: период не замерен, повторная выдача проверена ручными часами. Принимать ли это вместо замера, решает tech-lead. | открыто |
| N-1 | Nit | `redpanda_integration_test.go:545` | `len(tracked.ids)` читается без `tracked.mu`, хотя остальной доступ под мьютексом. Сейчас гонки нет: все публикации синхронны в горутине теста. | Добавить метод `attempts.count()` под мьютексом или вернуть длину из `distinct`. | открыто |
| N-2 | Nit | `redpanda_integration_test.go:563, 585-586` | «a subscription ended %s after an event was published» меряет время от публикации до выхода из цикла, а не момент конца подписки. В прогоне ревьюера — 43 мс, хотя `unexpected EOF` пришёл ещё при остановке брокера. | Писать «was found ended … after»; или проверять и логировать `d.Err()` до публикации события. | открыто |
| N-3 | Nit | `tasks/T-316.md:10`; `tasks.md:414` | Имя ветки в карточке и индексе — `task/T-316-integration-consumer`, фактическое — `task/T-316-consumer-integration`. Исполнитель это отметил. | Поправить при приёмке (tech-lead), как в T-317 (N-7). | открыто |

### Вопросы
- **tech-lead#3:** принимается ли расчёт периода повторной выдачи вместо замера (Mi-2)? Замер требует живого цикла бота и 30 с настенного лизинга. Зависший брокер тоже остаётся незамеренным, способ предложен в бэклоге исполнителя (п. 3).

### Предложения в бэклог (вне границ T-307/T-316)
1. Задача переподписки `Dispatcher` (решение оркестратора 1): в её DoD — сделать шаг 8 T-316 строгим (только «подписка жива», `d.Err() == nil`, событие в каждом из четырёх топиков взято), с учётом Mi-1.
2. Долгий `Dispatcher.Stop` (решение 2): после исправления вернуть в `stop()` теста срок `runtime.StopTimeout` вместо 1 мин.
3. `Subscribe` kafka-адаптера возвращает `nil` на `io.EOF` (`stopped`), и такую смерть подписки `Dispatcher.Err()` и `/health` не видят. Сейчас сбой транспорта приходит как `unexpected EOF` или `failed to dial`, но голый `io.EOF` от kafka-go при обрыве соединения стоит проверить в задаче переподписки. Владелец `shared/eventbus` — system-architect.

### Риски и допущения
- `-race` локально не запускался (cgo выключен). Первый прогон под `-race` будет в CI.
- В CI job `integration` пакеты идут параллельно. `Container().Start` снова занимает порт хоста, выбранный `freePort()` до старта. Пока брокер остановлен (1–2 с), другой пакет теоретически может взять этот порт, и `Start` упадёт. Вероятность низкая, это свойство `testkit`, не T-316.
- Исход шага 8 недетерминирован: какая подписка кончится и успеет ли `system_events` взять событие. Зелёный итог от этого не зависит, но из-за Mi-1 лог может назвать не тот путь.
- Ревьюер сделал один интеграционный прогон пакета. Ни до, ни после него контейнеров testcontainers не было. Код, Docker владельца, `.env` и стенд `:8888` не трогал. В рабочей папке добавлены только этот раздел и раздел «Ревью» карточки.

## T-308 · ревью #1 · 2026-09-14 · code-reviewer#3 (TEAM-3)

### Границы ревью
- Ветка `task/T-308-fake-gateway-harness` (`.worktrees/T-308`), база `599e883`, изменения не закоммичены.
- Код:
  - новые файлы `internal/gateway/gatewaytest/{gatewaytest.go, gatewaytest_test.go}`, `shared/testkit/gateway/{http.go, http_test.go, README.md}`;
  - изменённые `shared/testkit/gateway/harness.go` (только комментарий пакета), `internal/gateway/README.md`, `Docs/ops/runbook.md` §6.
- Сверено с разделом `### T-308` в `tasks.md`, C-08 v1.4 и C-04 «Заглушка» (`contracts.md`), ADR-001 доп. 2026-09-13 п. 5 (1e), design §7, КД шлюза §15, `.golangci.yml` (`shared`, `shared-testkit-gateway`, `no-testkit-in-production`, `internal-gateway`, исключение для `_test.go`).
- Прочитан код шлюза, от которого зависят обвязка и сквозной тест: `shared/runtime/http.go` (`Stop`, `ShuttingDown`), `internal/gateway/context.go` (`New`, `Start`, `Stop`, `Health`, `objectStore`), `client/longpoll.go`, `api/router.go`, `actions/service.go` (`publish`), `turns/tracker.go` (`OnNarrative`, `OnDelivered`, `finish`), `shared/testkit/swarm/fake_narrator.go` (`entry`, `publish`), `shared/eventbus/membus` (`Publish`).

### Прогоны
- `go build ./... && go vet ./...` — ок.
- `go test -short -count=3 ./internal/gateway/gatewaytest/... ./shared/testkit/gateway/...` — ок.
- `go test -short -count=15 -cpu=1,2,8` для двух сквозных тестов `gatewaytest` — 45 прогонов, все зелёные.
- `golangci-lint run ./...` — 0 issues.
- `-race` не запускался: на Windows нет cgo.
- Зонды в копиях scratchpad `t308r-*`, без `-overlay`. Копии удалены по точным путям, каталогов `gatewaytest-*` в `%TEMP%` не осталось.

| # | Зонд | Результат |
|---|---|---|
| 0 | Контрольный: код без изменений, добавлен тест ревьюера «висящий long-poll отвечает на `Close` пустым списком, `/health` после `Close` не отвечает» | зелёный, весь пакет тоже |
| 1 | `FakeGateway.Close` без `g.srv.Stop` | весь набор пакета зелёный; тест ревьюера красный: long-poll ответил `500 internal`, сервер после `Close` отвечает `503` (Mi-2) |
| 2 | `actions.Service.publish`: запись хода `Turns.Accepted` на 30 мс позже публикации | `TestAwaitDeliveryTakesATurnThroughTheLongPollAndCompletesIt` красный: `analytics.turn.completed` не приходит (Mi-1) |

### Вердикт
**Принять.** Critical (Blocker) 0 · Major 0 · Minor 4 · Nit 4.

Minor не ломают поставку. Закрыть их в этой задаче или вынести в бэклог, решает tech-lead#3. Mi-1 указывает на дефект шлюза вне T-308. Его стоит завести до e2e T-313.

### Что проверено и в порядке
1. **Границы импорта.**
   - `gatewaytest.go` — production-файл в `internal/gateway/`. Он импортирует `internal/gateway`, `internal/gateway/client` и `shared/*` без `shared/testkit`, что допускают `internal-gateway` и `no-testkit-in-production`.
   - `http.go` импортирует из шлюза ровно `client` и `api`: правило `shared-testkit-gateway` с `$`.
   - Импорты `shared/testkit/*` есть только в `_test.go`; исключение снимает лишь запрет `no-testkit-in-production`.
   - `.golangci.yml` не менялся, `//nolint` не добавлялся, линтер чист. Мутанты разработчика 13–16 проверяют границы с другой стороны.
2. **API `Harness` v0.**
   - `harness.go` в T-308 отличается от `harness.go` в `.worktrees/EPIC-002` и `.worktrees/EPIC-003` только абзацем комментария.
   - Потребители v0 во всех трёх деревьях побайтно совпадают: `cmd/multiverse/fake_contexts_test.go`, `shared/testkit/swarm/fake_narrator_stand_test.go`, `test/e2e/stubs_v0_test.go`. В T-308 они собираются (`go vet ./...`).
   - Новые экспортируемые имена `ClientID`, `Platform`, `DefaultHTTPTimeout`, `ErrNoDelivery`, `ErrRoundsNotMounted`, `HTTPHarness`, `NewCIClient`, `NewHTTPHarness`, `Entered` в пакетах EPIC-002/003 не заняты. Сборка в копии не понадобилась.
3. **`FakeGateway`.**
   - Окружение процесса не читается. Все переменные шлюза берутся через `c.src` (`StringFrom`/`IntFrom`/`ListFrom`/`BoolFrom`), а `runtime.AdminOnly`, который читает `env.CoreAdminClients.List()`, шлюз не использует.
   - `MV_GATEWAY_DATA_DIR` всегда равен `Dir`, в `/data` и в корень диска ничего не пишется. `MV_MINIO_*` в `MapSource` нет, поэтому хранилища нет и снапшот отсутствует.
   - Часы и таймеры — `clock.Manual`, идентификаторы — `SequenceIDs("gw")`.
   - Двойник закрывает шину и удаляет каталог, только если создал их сам (`ownBus`/`ownDir`). Неудачный `Start` освобождает созданное.
   - `Close` идемпотентен (`sync.Once`), порядок «сервер → контекст → шина → каталог» совпадает с `serve`. Висящий long-poll отвечает пустым списком (зонд 0), но тестом это не закреплено (Mi-2).
   - Старт и остановка занимают около 0,02 с, запас до DoD «≤ 1 с» большой.
4. **`HTTPHarness`.**
   - Доставки других корреляций и видов не теряются. Всё, что выдал опрос, подтверждается и кладётся во «входящие». `AwaitDeliveryOf` берёт оттуда по совпадению и не опрашивает шлюз, если нужное уже там (`TestAwaitDeliveryAcknowledgesAndKeepsTheRest`).
   - `ErrNoDelivery` оборачивается через `%w`, в сообщении перечислено пришедшее.
   - `wait_ms` не превышает остатка срока. При остатке меньше 1 мс уходит `wait_ms=0`, а не умолчание шлюза.
   - Опрос идёт под `ctx`, а не под дедлайном, поэтому доставка не остаётся в лизинге у оборванного запроса.
   - `CloseRound` переводит в `501` только `404` без кода. `404` с кодом и `409 no_open_round` проходят как есть, тест на `404` с кодом есть. Остаток — N-1.
5. **Тесты.**
   - `time.Sleep` нет. Паузы опроса идут через `clock.RealTimers`, тест обвязки на таймаут ограничен контекстом 5 с.
   - Проверка NFR-092 во время компиляции корректна.
   - Сквозной тест проверяет «ровно один» `analytics.turn.completed` корректно. Завершение хода публикуется синхронно в транзакции ack (`OnDelivered → finish`). Sweeper на `clock.Manual` без `Advance` не сработает, так что позднему дублю взяться неоткуда.

### Замечания

| # | Серьёзность | Файл:строка | Замечание | Предложение | Статус |
|---|---|---|---|---|---|
| Mi-1 | Minor | `internal/gateway/gatewaytest/gatewaytest_test.go:274-322`; карточка T-308, «Риски и допущения»; вне задачи — `internal/gateway/actions/service.go:202-222`, `turns/tracker.go:282-287, 301-313` | Разработчик неверно назвал причину гонки. `FakeNarrator.entry` не зависит от факта персонажа: имя и HP он берёт из события, для неизвестного персонажа ставит `template.Unknown`. Настоящая гонка — в шлюзе. `actions.Service.publish` публикует действие до `Turns.Accepted`, а `membus` доставляет асинхронно. Если нарратор и consumer шлюза обработают `narrative.output` раньше `INSERT` хода, `OnNarrative` обновит 0 строк. Ход останется `accepted`, `OnDelivered` (`status = narrated`) после ack его не завершит, и `analytics.turn.completed` не будет до таймаута sweeper. Зонд 2 (задержка записи 30 мс) это воспроизводит. Без задержки окно узкое: 45 прогонов зелёные. Под `-race` и нагрузкой CI возможен флак. Со стороны теста гонку не убрать: ожидание условия не упорядочит `INSERT` в обработчике действия и обработку нарратива consumer'ом. | Исправить в карточке описание риска. Завести дефект шлюза (владелец `actions`/`turns`, T-305/T-306) до e2e T-313 `solo-30`: там тот же `FakeNarrator` на `membus`. Варианты: записывать ход до публикации (строка `accepted` до `Publish`, при сбое шины — откат или пометка) или не терять шаг в `OnNarrative`/`OnMechanics`, если хода ещё нет (ошибка → повтор обработчика шиной). | открыто (бэклог, дефект шлюза) |
| Mi-2 | Minor | `internal/gateway/gatewaytest/gatewaytest.go:189-199`; `gatewaytest_test.go:55-97` | Обещание `Close` не закреплено тестами: висящий long-poll отвечает сразу, сервер остановлен, порт свободен. Зонд 1 убирает `g.srv.Stop` из `Close`, и весь набор пакета остаётся зелёным. Сервер при этом слушает порт дальше, long-poll отвечает `500`. `TestAFakeGatewayStartsAndStopsWithinASecond` проверяет каталог и `Health`, но не сервер. | Добавить тест. Запрос `Deliveries(ctx, "", 10, 15s)` в горутине, затем `Close`. Ожидать `err == nil` и пустой список за ≤ 2 с под контекстом теста. Затем `http.Get(g.URL+"/health")` должен вернуть ошибку соединения. Тест ревьюера из зонда 0 зелёный на текущем коде и красный на зонде 1. | открыто |
| Mi-3 | Minor | `Docs/ops/runbook.md:813-814`; `shared/testkit/gateway/README.md:81-82`; раньше то же — `internal/gateway/README.md:204-205` (T-307) | Фраза «в prod `ci-harness` отсутствует в `MV_GATEWAY_CLIENT_IDS` и получает `403`» подана как факт. На деле манифест по умолчанию допускает `ci-harness` (`shared/env/vars.go:100`), а compose передаёт переменную без своего значения (`docker-compose.yml:267`). Только комментарий в `.env.example:141` говорит «в prod без ci-harness». Шага, который убирает харнесс из списка, runbook не описывает. Оператор стенда с ботом может решить, что защита уже стоит. А allow-list — единственная техническая преграда запрещённому сценарию. | runbook §6: записать как действие оператора. Например: «на стенде и в prod, где работает бот, задать в `.env` `MV_GATEWAY_CLIENT_IDS` без `ci-harness` (по умолчанию манифест его допускает) — тогда харнесс получит `403 client_unknown`». В README обвязки — так же. Пункт для devops вместе с согласованием правки §6. | открыто (документы) |
| Mi-4 | Minor | `shared/testkit/gateway/http.go:74-81, 309-313`; README обвязки | Не описано, что на один шлюз допустим ровно один `HTTPHarness`. Все клиенты `ci-harness` делят очередь платформы, как бот с харнессом. Второй экземпляр (например, по одному на игрока в будущем `group-3x30`) получит `409 poll_in_progress` или заберёт и сразу подтвердит чужие доставки, и первый их уже не увидит. Кроме того, `polling.Lock()` берётся до расчёта `deadline`. Параллельные вызовы `AwaitDelivery` одного харнесса ждут друг друга, и второй может вернуться намного позже своего `timeout`. | В godoc `HTTPHarness` и в README записать: «один харнесс на шлюз, он играет всеми фикстурами; вызовы `AwaitDelivery` сериализуются». Срок считать до `polling.Lock()` или записать, что он отсчитывается от захвата. Для I2 (три `external_id` одним `ci-harness`) — пометка в T-313/T-35x. | открыто |
| N-1 | Nit | `shared/testkit/gateway/http.go:47-48, 365-372` | `404` без кода бывает не только у немонтированного маршрута I1, но и при неверном базовом URL или чужом сервере. Тогда харнесс ответит «closeRound is not mounted by the gateway yet (T-354)», и причина потеряется. К тому же `ErrRoundsNotMounted` — экспортируемый изменяемый `*client.APIError`: вызывающий может его испортить. | В T-354 явным пунктом снять перевод `404 → 501`. До этого возвращать копию значения: например, функцию или `errors.Join(ErrRoundsNotMounted, apiErr)` с сохранением `errors.Is`. | открыто |
| N-2 | Nit | `shared/testkit/gateway/http.go:340-347` | При `unknown` в ответе ack возвращается ошибка. Доставки того же опроса, которые шлюз подтвердил, при этом не попадают во «входящие» и теряются для следующих вызовов. При ошибке `Ack` доставки остаются в лизинге до его истечения. Для теста это конец сценария, но godoc об этом молчит. | Класть известные доставки во «входящие» до возврата ошибки или записать в godoc, что после ошибки ожидания харнесс не пригоден. | открыто |
| N-3 | Nit | `internal/gateway/gatewaytest/gatewaytest_test.go:397-406` | `waitFor` ограничен числом итераций (500 × 10 мс), а не сроком. Реальная длительность зависит от гранулярности таймеров: на Windows в зонде 2 тест упал примерно через 26 с вместо заявленных 5. | Ждать по дедлайну (`wall.Now()` + срок) или под `context.WithTimeout`. | открыто |
| N-4 | Nit | `internal/gateway/gatewaytest/gatewaytest_test.go:130-145` | `TestAFailedStartLeavesNothing` проверяет только отказ до открытия БД: `MV_GATEWAY_INPUT_FILTER` читается раньше файлов. Закрытие собственной шины двойника при неудачном старте не проверено, мутант 9 разработчика ловит только каталог. | По желанию: добавить отказ после открытия БД или проверку, что шина двойника закрыта. | открыто |

### Вопросы
- **tech-lead#3 / оркестратор:** Mi-1 — завести дефект шлюза «ход не завершается, если `narrative.output` обработан до записи хода» с приоритетом до T-313. Кому он принадлежит — T-305 (`actions`) или T-306 (`turns`)?
- **devops-engineer:** Mi-3 — формулировка запрета в runbook §6 как действия оператора.
- **system-architect** (прежний вопрос T-307): платформа `ci-harness`. Вариант (б) снимает запрет, но Mi-4 для I2 не закрывает: несколько экземпляров харнесса на одном клиенте всё равно делят очередь.

### Предложения в бэклог (вне границ T-308)
1. **Дефект шлюза (Mi-1):** действие публикуется до `Turns.Accepted`, а `OnNarrative`/`OnMechanics` молча теряют шаг хода, которого ещё нет.
2. **T-354:** снять перевод `404 → 501` в `HTTPHarness.CloseRound` (N-1).
3. **T-313 / I2:** один `HTTPHarness` на шлюз или отдельные `client_id` для нескольких харнессов (Mi-4).
4. **`internal/gateway/README.md:204-205`:** та же формулировка о prod, что в Mi-3.

### Риски и допущения
- `-race` не прогонялся. Вероятность Mi-1 под `-race` на Linux оценена по коду, без прогона.
- Совместимость с EPIC-002/003 проверена сравнением файлов и имён, сборки в их деревьях не было.
- В рабочей папке задачи ревьюер добавил только этот раздел и строку в карточке. Зонды выполнялись в копиях в scratchpad, копии удалены по точным путям.

## T-308 · ревью #2 · 2026-09-14 · code-reviewer#3 (TEAM-3)

### Границы ревью
- Итерация 2 после ревью #1 (принять, 0/0/4/4). Ветка `task/T-308-fake-gateway-harness` (`.worktrees/T-308`), база `599e883`, изменения не закоммичены.
- Проверены только исправления и регрессия от них:
  - Mi-1 — код шлюза T-305/T-306 вне объёма T-308, по решению оркестратора: `internal/gateway/actions/{service.go, turns.go, service_test.go, helpers_test.go}`, `internal/gateway/characters/helpers_test.go`, `internal/gateway/turns/tracker.go`, новый `internal/gateway/turns/order_test.go`;
  - Mi-2…Mi-4, N-1…N-4: `internal/gateway/gatewaytest/{gatewaytest.go, gatewaytest_test.go, export_test.go}`, `shared/testkit/gateway/{http.go, http_test.go, README.md}`, `internal/gateway/README.md`, `Docs/ops/runbook.md` §6.
- Сверено с КД шлюза §4.2, §5.5, §7.2, §7.6, `metrics.md` (NFR-003 `ack_latency`), `client/client.go` (повтор только на `503` и ошибку транспорта), `store/open.go` (`SetMaxOpenConns(1)`), `shared/eventbus/kafka.go` (`Publish`), `shared/env/vars.go`, `.env.example`.
- Интерфейс `actions.Turns` сверен с `epic/EPIC-004-gateway-bot` (`git grep`) и с рабочими деревьями `.worktrees/T-309`, `.worktrees/EPIC-004`, `.worktrees/EPIC-002` (только чтение).
...
- Сквозные `-count=15`: `TestRegisterAndEnterBringsAFixtureCharacterAliveIntoTheRegion`, `TestAwaitDeliveryTakesATurnThroughTheLongPollAndCompletesIt`, `TestCloseAnswersAWaitingLongPollAndStopsTheServer` (`gatewaytest`); `TestTheMechanicsAndTheNarrativeRightAfterThePublicationCompleteTheTurn`, `TestAnActionTheBusDidNotAcknowledgeTakesItsTurnBack`, `TestATurnThatHeardFromItsActionIsNotTakenBack` (`turns`) — все зелёные.
- `TestAwaitDeliveryCountsItsTimeoutFromTheCall`: 30 прогонов без нагрузки и 20 прогонов с `-cpu 1` параллельно с тестами `internal/gateway`, `consumer`, `outbox`. Все зелёные, 0,70–0,71 с при пороге 0,95 с.
- `golangci-lint run ./...` — 0 issues.
- Мутанты: копия `scratchpad\t308r2-copy`, без `-overlay`, контрольный первым. Копия и резервный файл удалены по точным путям.

| # | Мутант | Результат |
|---|---|---|
| 0 | контрольный, без изменений | зелёный (`actions`, `turns`, `gatewaytest`, `shared/testkit/gateway`) |
| 1 | `service.go:242`: `Turns.Acked(ctx, …)` вместо `Turns.Acked(out, …)` — отметка подтверждения на контексте с бюджетом | **зелёный** (`actions`, `turns`, `gatewaytest`) — Mi-1 |
| 2 | `service.go:223`: `Turns.Withdrawn(ctx, …)` вместо `context.WithoutCancel(ctx)` — откат хода на контексте с бюджетом | **зелёный** (`actions`, `turns`, `gatewaytest`) — Mi-1 |

### Вердикт
**Принять.** Critical (Blocker) 0 · Major 0 · Minor 1 · Nit 3.

Mi-1 из ревью #1 исправлен по существу. Порядок «ход → `Bus.Publish` → откат при сбое» соответствует КД §5.5. Откат атомарен, счётчик сессии не уходит в минус, повтор ключа после `503` идемпотентен. Новый Minor касается только тестов. Закрыть его в итерации 3 или вынести в бэклог, решает tech-lead#3.

### Разбор по пунктам поручения

**1. Корректность отката.**
- **Гонки `Withdrawn` с `OnMechanics`/`OnNarrative` нет.**
  - У `gateway.db` одно соединение (`store/open.go:116`, `SetMaxOpenConns(1)`), SQLite сериализует пишущие транзакции.
  - `Withdrawn` проверяет условие и удаляет строку одним `DELETE … WHERE status = 'accepted' AND mechanics_at IS NULL` в своей транзакции (`tracker.go:192-204`).
  - `OnMechanics` в той же инструкции переводит `accepted → mechanics_applied`, `OnNarrative` — в `narrated`. Поэтому ход, уже узнавший о механике или нарративе, не удаляется при любом порядке транзакций. `mechanics_at IS NULL` избыточно при `status = accepted`, но не мешает.
  - `Sweep` не участвует: срок хода — 60 с от `received_at`, окно отката ограничено бюджетом решения (5 с).
- **`turns_count` не уходит в минус.**
  - `−1` применяется только если `DELETE` затронул строку (`RowsAffected > 0`).
  - Строку вставил `Accepted` той же сессии, и её `+1` зафиксирован в той же транзакции, что и `INSERT`.
  - Повторный `Accepted` для того же события (`ON CONFLICT DO NOTHING`) счётчик не трогает, поэтому двойного `+1` тоже нет.
- **Расхождение с событиями сессии** возможно в одном узком случае. Сессия должна закончиться (`session.ended` с `turns_count`, с T-309 ещё и снапшот через `OnEnded`) ровно между `Accepted` и неудачным `Publish` действия — окно не больше бюджета 5 с. Простой сессии это исключает: `Begin` только что сделал `Touch`. Остаются явный выход, `/forget` или смерть персонажа в эти секунды. Вероятность пренебрежимая, запись в «Рисках».
- **Идемпотентность повтора после `503`.**
  - Партия удерживается, `b.sent == b.action`. Повтор снова вызывает `Accepted`: после успешного отката строка вставляется заново с тем же `seq`, после неудачного отката — `DO NOTHING`. Затем публикуется то же событие с тем же id.
  - Закреплено `TestAnActionTheBusDidNotAcknowledgeTakesItsTurnBack` (`seq = 1`, `turns_count = 1`) и `TestAFailedPublicationKeepsNothing` в обоих вариантах.
  - Сбой предложения после подтверждённого действия оставляет ход, повтор публикует только предложение. `Accepted` при этом не вызывается: `b.sent > b.action`.

**2. Изменение интерфейса `actions.Turns` и риск слияния — низкий.**
- Реализации и двойники `Turns` в `epic/EPIC-004-gateway-bot`, `.worktrees/EPIC-004`, `.worktrees/T-309` и `.worktrees/EPIC-002`: `turns.Tracker`, `actions/helpers_test.go` (`memoryTurns`, `turns`), `characters/helpers_test.go` (`noTurns`). Все три обновлены в T-308. Других реализаций нет: у `consumer.Deliveries` поле `Turns` — `*turns.Tracker`, у `handlers.Deliveries` — свой интерфейс `TurnDeliveries`.
- T-309 (итерация 2, не закоммичено) эти три файла не меняет. Код T-309 (`context.go`, `consumer/dispatcher.go`, `session/manager.go`, `snapshot/`) интерфейс `actions.Turns` не реализует.
- Ожидаемые текстовые конфликты — документы:
  - `internal/gateway/README.md`: обе ветки правят таблицу статуса в строках 28–33, остальные правки не пересекаются. Конфликт разрешается вручную за минуты.
  - `dev-log.md`: хвостовые записи, их сводит драйвер `appendtail`.
- Остаточный риск: если итерация 2 T-309 добавит новый двойник `actions.Turns` в тестах, после слияния он перестанет компилироваться. Тогда достаточно добавить `Withdrawn`/`Acked`. Оркестратору стоит предупредить developer T-309.

**3. `500` вместо лога.**
- При нормальной работе пути к `500` нет. `Accepted` падает только при ошибке `gateway.db`.
- Под деградацией путь есть. `Accepted` вызывается на контексте с бюджетом (`service.go:215`), а соединение `gateway.db` одно. Транзакция consumer'а может держать его до `PublishTimeout = 5 с`, пока `analytics.turn.completed` уходит на медленный брокер. Бюджет может кончиться и раньше, в `Begin` (публикация `session.started`) или во входном фильтре. В этих случаях `BeginTx` вернёт `context deadline exceeded`, и клиент получит `500 internal`.
- Раньше та же ситуация кончалась ошибкой `Publish` и ответом `503`. Клиент (`client.go:331-340`) повторяет только `503`: бот покажет игроку ошибку, партия пролежит в памяти до `KeyTTL`, а зарезервированный `seq` останется пропуском.
- Сбой `Begin` на том же контексте уже давал `500`, так что это продолжение существующей классификации, а не новый класс ошибок. Отсюда N-2.

**4. Остаток гонки: действие дошло до брокера без подтверждения, нарратив обработан после `Withdrawn`.**
- **Реальность сценария.** `Kafka.Publish` вызывает `kafka-go Writer.WriteMessages` под контекстом решения. Если бюджет кончается, пока брокер не подтвердил запись (пауза Redpanda, смена лидера, обрыв после отправки), `Publish` возвращает ошибку, а сообщение может быть записано. Сценарий реален при деградации брокера, при исправной шине его нет.
- **Последствия уже, чем в описании разработчика.**
  - Доставки игроку от строки `turns` не зависят (outbox): механику и нарратив игрок получит.
  - Бот повторяет `503` через сотни миллисекунд, и `Accepted` вставляет строку заново. Механика, пришедшая до повтора, для хода потеряна: `mechanics_at`, `phase1_mode` и `result_event_id` пусты. Нарратив LLM обычно приходит через секунды, уже после повтора, и ход завершается.
  - Ход уйдёт в `timeout` (`turns_failed + 1`), только если до повтора пришёл и нарратив, например быстрый шаблон. Если клиент не повторил, хода нет вовсе.
  - Итог — искажение аналитики C-10 при сбоях шины, без потери данных игрока.
- **Защита «ошибка → повтор шиной» в `OnMechanics`/`OnNarrative` не рекомендуется.** Consumer at-least-once с ключом-миром остановит обработку всех событий мира за ходом, который может не появиться никогда (клиент не повторил). После `backoff` событие уйдёт в DLQ. Это хуже искажения метрики.
- **Лучше мягкий откат.** `Withdrawn` не удаляет строку, а переводит её в отдельный статус (`received` по КД или `withdrawn`), `OnMechanics`/`OnNarrative` принимают и его. Повторный `Accepted` переводит строку в `accepted` через upsert, sweeper молча удаляет неподтверждённые строки после срока, без аналитики и без `turns_failed`. Это закрывает и пункт 5. Решение за architect#3 и tech-lead#3, предложение в бэклог.

**5. КД §5.5/§4.2: `received` или сразу `accepted`. Оценка риска — низкий для MVP-1.**
- Отличие от КД видно в двух местах:
  - в окне между `Accepted` и подтверждением брокера строка неотличима от подтверждённого хода;
  - при падении процесса в этом окне (миллисекунды) остаётся `accepted` без опубликованного действия. Партия держится в памяти и после рестарта потеряна: повтор ключа строит действие заново с новым id. Старая строка через 60 с даст фантомный `timeout` и `turns_failed + 1`, а снапшот T-309 может её захватить.
- `received` с «мягким откатом» (п. 4) закрывает оба случая, но требует правки всех шагов трекера (`OnMechanics`, `OnNarrative`, `Sweep`, `expired`) и схемы статусов в КД §7.6. Текущий код корректен для рабочих сценариев. Если КД фиксирует `accepted` сразу, в КД §5.5 нужно записать и то, что ход остаётся при сбое публикации после подтверждённого действия (вопрос 4 карточки).

**6. Закрытие замечаний ревью #1.**

| # | Итог | Основание |
|---|---|---|
| Mi-1 | закрыто | Порядок записи хода исправлен, риск в карточке переписан. Мутанты 1–8 разработчика и тесты `order_test.go` проверены. Остаток — Mi-1 этого ревью (тесты) и пункты 4–5 (архитектура). |
| Mi-2 | закрыто | `TestCloseAnswersAWaitingLongPollAndStopsTheServer` делает то же, что зонд 0 ревью #1: long-poll внутри сервера (признак `409 poll_in_progress`), пустой ответ ≤ 2 с, `/health` после `Close` не отвечает. Мутант 9 разработчика красный. |
| Mi-3 | закрыто | runbook §6 и оба README описывают действие оператора. Умолчания сверены: `vars.go:100` (`MV_GATEWAY_CLIENT_IDS`), `:102` (`MV_GATEWAY_ACTOR_KIND_CLIENTS`), `:148` (`MV_CORE_ADMIN_CLIENTS`), `.env.example:139-141`, compose без значений. Согласование с devops остаётся открытым вопросом 6. |
| Mi-4 | закрыто | «Один харнесс на шлюз» — в godoc и README. `deadline` считается до `polling.Lock()` (`http.go:343-344`). Запас теста — N-1. |
| N-1 | закрыто частично — **принимаю** | Сентинел `errors.New` неизменяем. `As` отдаёт каждому свою копию `501`, текст сохраняет ответ сервера. Голый `404` немонтированного маршрута действительно не отличить от `404` неверного URL: mux шлюза отвечает на неизвестный путь стандартно. Обоснование верное, снятие перевода записано за T-354. |
| N-2 | закрыто | Подтверждённые доставки кладутся во «входящие» до возврата ошибки, тест дополнен. |
| N-3 | закрыто | `waitFor` ждёт по сроку 5 с. |
| N-4 | закрыто частично — **принимаю** | `WatchBuses` через `export_test.go` доказывает, что неудачный `Start` закрывает свою шину (`ErrClosed`). Отказ после открытия БД снаружи не вызвать без изменения API двойника — не стоит того. Подмена переменной `newBus` безопасна: в пакете нет `t.Parallel`. |

**7. `TestAwaitDeliveryCountsItsTimeoutFromTheCall`.**
- **Флак на CI маловероятен.** Верный код проходит за ~0,70 с: первый вызов держит опрос семь раз по 100 мс, второй возвращается сразу после захвата блокировки. Порог — 0,95 с. Для падения горутина первого вызова должна стартовать с задержкой больше ~240 мс. Под `-race` на общем раннере это редко, а локально под нагрузкой разброс 10 мс.
- **Слабее запас распознавания мутанта:** ~1,0 с против 0,95 с, всего ~50 мс. Быстрый раннер и недобор таймера `After(100ms)` мутанта не спасут, но значение на грани (N-1).
- **На `clock.Manual`** перевести можно, но это дороже, чем кажется. `h.wall` не экспортирован, а тест во внешнем пакете `gateway_test`. Нужен `export_test.go`, который подменяет `wall`, и синхронизация: тест должен знать, что второй вызов уже посчитал срок, до первого `Advance`. Без крючка в production-коде это не гарантировать.
- **Проще сделать проверку относительной.** Отметить момент возврата первого вызова в его горутине и требовать, чтобы второй вернулся не позже чем через `pollWait` (100 мс) после него. Верный код не делает ни одного опроса (разница — единицы мс), мутант делает три (разница ≥ 300 мс). Запас с обеих сторон — сотни мс, от старта горутин проверка не зависит.

### Замечания

| # | Серьёзность | Файл:строка | Замечание | Предложение | Статус |
|---|---|---|---|---|---|
| Mi-1 | Minor | `internal/gateway/actions/service.go:223, 241-242`; `actions/pending_test.go:340-372`; `actions/helpers_test.go:283-326`; `turns/tracker_test.go:357-383` | Итерация добавила контексты без отмены для `Withdrawn` и `Acked`, но тесты их не закрепляют: мутанты 1 и 2 зелёные на `actions`, `turns` и `gatewaytest`. Последствия на настоящем трекере такие. С `Acked(ctx)` при бюджете, истёкшем после последней публикации, `UPDATE` падает на отменённом контексте, и `acked_at` остаётся моментом записи до публикации: NFR-003 `ack_latency` занижен. С `Withdrawn(ctx)` действие, чья публикация упёрлась в бюджет, не забирает ход. Если клиент не повторит, ход через 60 с уйдёт в фантомный `timeout`. Кроме того, две прежние проверки потеряли смысл. `pending_test.go:370` (`acceptedOn[0]`, «ход записан на отменённом контексте») теперь проверяет `Accepted`, который идёт до публикации, и не может упасть. Имя и комментарий `TestTheTurnIsRecordedWhenTheBudgetRunsOutAfterTheLastPublication` («между последней публикацией и `Turns.Accepted`») описывают старый порядок. | (а) В двойнике `turns` записывать `ctx.Err()` у `Acked` и `Withdrawn` (`ackedOn`, `withdrawnOn`). Перенести проверку `pending_test.go:370` на `ackedOn[0]` и добавить аналог для `withdrawnOn`, где `player.*` висит до бюджета, как `TestABudgetThatRunsOutHoldsTheBatch`, но на событии действия. (б) В `turns/tracker_test.go` проверять `row.AckedAt.Equal(first.AckedAt)` и переименовать тест или исправить комментарий. Добавить тест «публикация действия упёрлась в бюджет → `503`, строки хода нет, `turns_count = 0`». Мутанты 1 и 2 этого ревью должны покраснеть. | открыто |
| N-1 | Nit | `shared/testkit/gateway/http_test.go:297-325` | Порог 0,95 с даёт ~240 мс запаса верному коду, но только ~50 мс для распознавания мутанта «срок после блокировки» (~1,0 с). Проверка зависит от абсолютного времени от старта горутин. | Сравнивать моменты возврата: второй вызов возвращается не позже чем через `pollWait` после первого. Порог 150–200 мс, мутант даёт ≥ 300 мс (п. 7 разбора). | открыто |
| N-2 | Nit | `internal/gateway/actions/service.go:215-217` (и уже существующий `:171-177`) | Сбой `Turns.Accepted` из-за истёкшего бюджета (занятое соединение `gateway.db`, медленный `Touch` или фильтр) отвечает `500 internal`. Клиент `500` не повторяет, хотя действие не решено и партия удержана под ключом. Раньше та же ситуация давала `503` через `Publish`. При нормальной работе пути нет. | Классифицировать ошибку по причине: `context.Cause(ctx) != nil` или `errors.Is(err, context.DeadlineExceeded)` → `503 bus_unavailable`, как при ожидании блокировки игрока (`:130-135`). Настоящий сбой БД оставить `500`. Сделать одинаково для `Begin` и `Accepted`, можно в бэклог T-305. | открыто |
| N-3 | Nit | `internal/gateway/turns/tracker.go:206-214`; godoc `Accepted` `:160-162` | `Acked` не трогает завершённый ход. Если нарратив доставлен и подтверждён раньше, чем опубликовано предложение (`enter`/`leave`/`rest`, шаблонный нарратив), `analytics.turn.completed` уже ушёл с `acked_at` — моментом записи до публикации. `ack_latency_p95_ms = acked_at − received_at` (`metrics.md:125`) для таких ходов занижен на задержку брокера. Поведение описано в godoc, окно узкое. | Записать в КД §7.6 / `metrics.md`, что `acked_at` завершённого до ответа хода — момент записи хода. Либо писать в `Accepted` `acked_at = NULL` и считать его в `Payload` по `received_at`, если `Acked` не успел. Решает architect#3 вместе с вопросом 4. | открыто |

### Вопросы
- **architect#3** (вопросы 4–5 карточки):
  - принять «сразу `accepted` + удаление при откате» или перейти на мягкий откат со статусом `received`/`withdrawn`, который закрывает остаток гонки и фантомный `timeout` при падении процесса (пп. 4–5 разбора);
  - защиту «ошибка → повтор шиной» в `OnMechanics`/`OnNarrative` ревьюер не рекомендует.
- **tech-lead#3:** Mi-1 закрыть в итерации 3 или вынести в бэклог T-305/T-306; N-2 — в бэклог T-305.
- **Оркестратор:** предупредить developer T-309, что у `actions.Turns` появились `Withdrawn` и `Acked`. Новый тестовый двойник в T-309 после слияния не скомпилируется.

### Предложения в бэклог
1. **Мягкий откат хода** (T-306, после решения architect#3): статус `received`/`withdrawn` вместо `DELETE`; `OnMechanics`/`OnNarrative` его принимают; sweeper удаляет неподтверждённые строки без аналитики.
2. **Классификация ошибок стора по причине** в `actions.Service` (T-305, N-2): истёкший бюджет → `503`, сбой БД → `500`.
3. **`acked_at` хода, завершённого до ответа `202`** (N-3): зафиксировать в КД/`metrics.md` или изменить расчёт.
4. Из ревью #1 остаются: T-354 — снять перевод `404 → 501`; T-313/I2 — несколько `ci-harness` на один шлюз.

### Риски и допущения
- `-race` не прогонялся (нет cgo). Гонок по данным в изменённом коде по чтению нет: поля двойника `turns` под мьютексом, `newBus` меняется без `t.Parallel`.
- Реальность записи сообщения после ошибки `kafka-go WriteMessages` оценена по семантике синхронного `Writer`, без прогона против Redpanda (интеграционные прогоны не запускались).
- Расхождение `turns_count` с `session.ended` и снапшотом T-309 (п. 1) оценено по коду, вероятность пренебрежимая.
...
### Границы ревью
- Ветка `task/T-313-e2e-solo` (`.worktrees/T-313`), база `585de13`, изменения не закоммичены.
- Код:
  - новые файлы `test/e2e/gateway_{world,solo30,death,fleefail}_test.go`;
  - изменённые `shared/testkit/gateway/{http.go, http_test.go, README.md}` (метод `HTTPHarness.Drain`).
- Сверено с разделом `### T-313` в `tasks.md`, design EPIC-004 §7, КД шлюза §5.5 и §7.6, C-08 и C-10 в том объёме, в каком их касаются проверки теста.
- Прочитан код, от которого зависят проверки: `membus.Chaos`, `consumer/dispatcher.go` (дедуп `processed_events`), `turns/tracker.go` (`OnMechanics`, `OnNarrative`, `Sweep`, `finish`), `turns/store.go` (id `turn.completed` из причины), `outbox/store.go` (`created_at`), `.github/workflows/go.yml` (jobs `e2e` и `race`), `Makefile` (`test-race`).
- Решения оркестратора не оцениваются как замечания: фикстура — вариант (а), механика после завершения хода и конец соло-сессии только по простою — к architect#3.

### Прогоны
- `go build ./... && go vet ./... && go vet -tags e2e ./test/e2e/...` — ок.
- `go test -tags e2e -count=2 -run TestGateway ./test/e2e/...` — ок (GOMAXPROCS = 32).
- `go test -short -count=1 ./shared/testkit/gateway/...` — ок.
- `golangci-lint run ./...` — 0 issues.
- **Слитое дерево** (кончик эпика `6b67fec` с T-309 + T-313 + незакоммиченная T-315, копия `t313r-merged`): `go build ./... && go vet ./...`, `go vet -tags e2e ./test/e2e/... ./cmd/telegram-bot/e2e/...` — ок; `TestGateway*` `-count=1` — ок; `golangci-lint` по `test/e2e`, `shared/testkit/gateway`, `cmd/telegram-bot` — 0 issues. Файлы T-313 с эпиком не пересекаются.
- **Повторы при малом GOMAXPROCS** (без искусственной нагрузки; на машине могли идти прогоны других агентов):
  - `TestGatewayFleeFail -count=6`: `-cpu=1` — 2 из 6 красные, `-cpu=2` — 4 из 6, `-cpu=4` — 1 из 6, `-cpu=16` — 0 из 6;
  - `-count=5 -cpu=1,2,4` — 5 из 15 красные;
  - под фоновой нагрузкой (`go test -short -p 16` в копии) `-count=2 -cpu=1,2` всех трёх тестов — 7 из 8 сравнений журналов красные.
- `-race` не запускался: на Windows нет cgo.
- Зонды — в копии `t313r-merged`, без `-overlay`; контрольный прогон копии первым (зелёный). Копия и логи удалены по точным путям.

| # | Зонд | Результат |
|---|---|---|
| 0 | Контрольный: слитое дерево без изменений | зелёный |
| 1 | `consumer.Handle`: пауза 60 мс на `combat.decided` | прогоны 1 и 2 зелёные; прогон `--chaos=duplicate` красный: доставка `mechanics` ушла в следующий шаг |
| 1б | То же, пауза 150 мс | `death` и `flee-fail` красные («0 mechanics», «enter of the dead character delivered 1 texts»); `solo-30` 1 и 2 зелёные, дубли красные. Отказ эвристики виден как падение, не как ложная зелень (Mi-2) |
| 2 | `Tracker.OnMechanics` ничего не пишет | все три теста зелёные (Mi-1) |
| 3 | `Tracker.finish` публикует второй `turn.completed` с тем же id и `status=timeout` | все три теста зелёные, включая `--chaos=duplicate` (Ma-2). С проверкой, предложенной в Ma-2 (временно в копии), — красный: «a copy with another body»; на неизменённом коде та же проверка зелёная |
| 4 | Инструментирование сравнения: оба канонических журнала упавшего `flee-fail -cpu=2` в файлы | причина расхождения — Ma-1 |

### Вердикт
**Вернуть.** Critical (Blocker) 0 · Major 2 · Minor 2 · Nit 4.

Ma-1 — пункт DoD «детерминированные» не держится при GOMAXPROCS ≤ 4, а job `race` гоняет `test/e2e` с `-race`: после слияния в эпик CI будет мигать у всех команд. Ma-2 — у проверки NFR-013 слепое пятно ровно там, где в коде живёт идемпотентность (id из причины). Оба исправляются в тесте, без правок шлюза.

### Что проверено и в порядке
1. **Структура и границы.** Помощники с префиксом `gateway`, `TestMain` в `test/e2e` один (`main_test.go`, Mi-3 T-444). Импорты `shared/testkit/*` — только в `_test.go`. `.golangci.yml` не менялся, `//nolint` нет. Глобальные `eventbus.SetClock`/`SetIDSource` сбрасывает очистка `testkit.Deterministic`, она выполняется последней — после остановки шлюза, роя и State.
2. **Лимит темпа.** `gatewayPace = 2 s` — ровно один токен при 30 действиях в минуту; часы `clock.Manual` двигаются перед каждым действием, переменные лимита не поднимаются. Строка DoD из приёмки T-308 выполнена, мутант 2 разработчика это подтверждает.
3. **Таймер `202`.** Меряется настенным временем (`testkit.Wall`) вокруг `harness.Act`, а не `clock.Manual`. p95 по 29 действиям: локально 1,5–2 мс, под нагрузкой до 4,5 мс, запас до 100 мс большой. p95 вместо «каждого» — отклонение 5, обосновано.
4. **`death`.** Убивающий удар и факт `status → dead` в одной корреляции, `encounter.ended players_out`, нарративы `turn` и `death`, ровно одна доставка `system` с `/start` (`gatewayAssertDeliveries`), `hp = 0`. Затем `look`, `enter`, `attack` → `409 character_dead`, по одному `turn.completed status=rejected`, ни одной доставки. Соответствует §5.5 п. 2.
5. **`flee-fail`.** `flee success=false exchange.last=false`, затем `free_attack` волка по игроку с `exchange.last=true`; 2 `mechanics` + 1 `narrative` с `based_on` обоих решений; персонаж остался во встрече; `turn.completed degraded`. Остаток — N-1.
6. **`--chaos=duplicate`.** Флага процесса нет — на общей шине включён `membus.Chaos{Duplicate}`, это та же реализация, что стоит за флагом. Проверено, что хаос действительно включён (`player_events` вдвое), `FakeState.AppliedProposals` без повторов, доставки без повторов по id и по событию, журнал равен обычному прогону. Слепое пятно — Ma-2.
7. **`Drain`.** Семантика рядом с `AwaitDelivery` согласована: общий `polling`, общий курсор, ack каждой доставки, «входящие» отдаются первыми, `wait_ms` тихого опроса равен `quiet`. Unit покрывает входящие, порядок, ack, повторный `Drain`, неизвестный ack. Абзац README честно называет пустой опрос признаком, а не гарантией. Остаток — N-2.
8. **Отклонения процесса (бинарник не запускается, `live` вместо `replay`).** Смысл DoD для I1 сохраняется: сценарии, аналитика хода и сессии, дубли и лимит проверяются на настоящем шлюзе и тех же заглушках, которые монтирует хук. Теряется три вещи, их стоит записать в `tasks.md`, а не держать только в карточке:
   - связка флагов `cmd/multiverse` (`--contexts=all --mode --id-source --chaos`) этими тестами не проверяется — её частично закрывает `empty_world_test.go`;
   - поведение `replay` (без аналитики и sweeper) e2e не покрыто — остаётся T-314 или I2;
   - «одинаковый журнал» означает журнал с точностью до переименования id, порядка внутри шага и исключённых полей, а не побайтный журнал NFR-061.

### Замечания

| # | Серьёзность | Файл:строка | Замечание | Предложение | Статус |
|---|---|---|---|---|---|
| Ma-1 | Major | `test/e2e/gateway_world_test.go:639-681, 693-699`; причина — `internal/gateway/turns/tracker.go:316-321` (`OnNarrative`); карточка T-313, «Прогоны» и «Риски» | **Сравнение двух прогонов мигает при GOMAXPROCS ≤ 4.** `flee-fail`: 2/6 при `-cpu=1`, 4/6 при `-cpu=2`, 1/6 при `-cpu=4`; под нагрузкой красные все три теста. Заявленные в карточке «`-count=10` и `-cpu=1,4` зелёные» не воспроизводятся. Причина (зонд 4): у шага `enter`, открывающего встречу, два нарратива одной корреляции — `entry` и `world_event`. `FakeNarrator` публикует их из разных горутин, а `OnNarrative` пишет в ход `narrative_event_id` того нарратива, который шлюз обработал первым. В одном прогоне `turn.completed.delivery.narrative_event_id` указывает на `entry`, в другом — на `world_event`. Переименование id превращает это в «перестановку» доставок и нарративов, поэтому сообщение теста указывает не туда. Это та же гонка «что первым дошло до хода», что и отклонение 4, но `gatewayRaceFields` её не покрывает. Тот же класс есть у хода смерти (нарративы `turn` и `death`). Обстоятельство, которое делает замечание блокирующим: job `race` (`make test-race`, `Makefile:228-230`) гоняет `./test/e2e/...` с `-race` на `ubuntu-latest`, а job `e2e` — на том же раннере с 2–4 vCPU. Фраза карточки «CI гоняет e2e без `-race`» неверна. | 1) Нормализовать в `canonical` ссылку хода на нарратив для корреляций с несколькими `narrative.output`: заменять id на `kind` нарратива или исключать `delivery.narrative_event_id`, а вместе с ним поля хода, зависящие от выбранного нарратива (`generated_by`, `fallback_reason`, `agent_*`, `absence`), — только для таких ходов. 2) Добавить в вопрос 2 к architect#3: какой нарратив считать нарративом хода, если их несколько (C-10, КД §7.6). Если решение детерминирует выбор в шлюзе, нормализация уходит. 3) Для повторного ревью приложить `go test -tags e2e -count=10 -cpu=1,2,4 -run TestGateway ./test/e2e/`, по возможности также под фоновой нагрузкой; поправить «Прогоны» и «Риски» карточки. | открыто |
| Ma-2 | Major | `test/e2e/gateway_world_test.go:393-403` (`refresh`); `test/e2e/gateway_solo30_test.go:136-159` (`gatewayAssertCarriedTwice`); проверки «ровно один `turn.completed`» — `gateway_solo30_test.go:204-207`, `gateway_death_test.go:144`, `gateway_fleefail_test.go:130` | **Журнал склеивает записи по `topic/id` без сравнения содержимого, поэтому повторная публикация с тем же id невидима.** Шлюз, State и `FakeEncounter` берут id из причины (`WithCauseID`): так устроены `analytics.turn.completed` (`turns/store.go:140`), предложения шлюза, факты State и `encounter.ended`. Второе применение в этих местах публикует событие с тем же id. Зонд 3 — второй `turn.completed` с тем же id и противоречащим `status=timeout` — оставляет все три теста зелёными, в том числе прогон с дублями. Проверка «ровно один `turn.completed` на ход» и пункт DoD «дубли не порождают вторых применений» (NFR-013) именно этот класс дефектов не ловят. `gatewayAssertCarriedTwice` проверяет кратность только `player_events`. | В `refresh` хранить число копий и тело по ключу. При совпадении ключа и расхождении тела — `t.Errorf`. В `finish` проверять по всем топикам, кроме `dead_letters`, что у каждого ключа ровно 1 запись (обычный прогон) или 2 (`duplicate`). В копии ревьюера такая проверка зелёная на текущем коде и красная на зонде 3. | открыто |
| Mi-1 | Minor | `test/e2e/gateway_world_test.go:693-723` | Исключённые из сравнения поля механики (`result_event_id`, `mechanics_at`, `mechanics_ms`, `phase1_mode`, `lod`) нигде больше не проверяются. Зонд 2 — шлюз вообще перестаёт записывать механику хода — все сценарии зелёные. На стенде I1-α метрика `mechanics_ms` может молча исчезнуть целиком, а e2e — первое место, где цепочка идёт по-настоящему. | Если поле есть, проверять согласованность: `result_event_id` указывает на `combat.decided` или факт State той же корреляции; `phase1_mode` и `lod` равны значениям этого решения; `mechanics_at ≤ acked_at`. Плюс хотя бы у одного хода прогона механика записана — слабая, но детерминированная проверка против полной потери. | открыто |
| Mi-2 | Minor | `test/e2e/gateway_world_test.go:296-327` (`wait`), `:301` | «Шаг осел» не знает о прогрессе consumer'а шлюза. Доставка `mechanics` из `combat.decided`, обработанного после ack нарратива, держится только на тихом опросе в 40 мс. Зонды 1 и 1б: при задержке одного обработчика доставка переезжает в следующий шаг. Падение получается, но с ложным диагнозом («enter of the dead character delivered 1 texts», «0 mechanics»). Ложной зелени зонды не дали: потеря в последнем шаге ловится подсчётом в `finish`. Кроме того, `Drain` вызывается под `w.ctx` без дедлайна: если шлюз не перестаёт отдавать доставки, шаг висит до `-timeout 10m` вместо `gatewaySettleBudget` (20 с). | 1) Добавить в условие «осел» равенство позиций consumer'а шлюза числу записей топиков. После слияния с T-309 это даёт `/health` (`cursors`) или то, на чём он построен в `consumer.Dispatcher`. 2) Вызывать `Drain` под контекстом с дедлайном, равным остатку бюджета шага. | открыто |
| N-1 | Nit | `test/e2e/gateway_fleefail_test.go:108-114, 122-125` | `hp` сверяется только при `hit=true`: при промахе неизменность `hp` не проверяется. Длина `based_on` не проверяется (нарратив с тремя ссылками прошёл бы). `string(rune('0'+i))` там, где рядом используется `strconv.Itoa`. | Проверять `hp` в обеих ветках, длину `based_on == 2`, индекс через `strconv.Itoa`. | открыто |
| N-2 | Nit | `shared/testkit/gateway/http.go:361-372`; `shared/testkit/gateway/README.md:59-65` | «Входящие после вызова пусты» верно только при успехе: при ошибке ack (`unknown`) подтверждённые доставки остаются во «входящих», а `Drain` возвращает `nil`. Не описано поведение при `quiet ≤ 0` (опрос `wait_ms=0`, возврат при первом пустом ответе). | Уточнить doc-комментарий и README. | открыто |
| N-3 | Nit | `test/e2e/gateway_world_test.go:630-638, 664-667` | `created_at` доставок обнуляется перед сравнением, но в перечне исключений комментария этого нет, а комментарий утверждает, что «timestamps» сравниваются. `created_at` берётся из `clock.Manual` (`consumer/deliveries.go:150`) и детерминирован; его расхождение — как раз признак поздней обработки (Mi-2). | Либо сравнивать `created_at`, либо назвать исключение и причину в комментарии. | открыто |
| N-4 | Nit | `test/e2e/gateway_world_test.go:727-739` | При расхождении печатается одна строка после переименования id. Как показал Ma-1, она указывает на следствие переименования, а не на причину. | Печатать заголовок шага и оба шага целиком или класть оба канонических журнала в `t.TempDir()` с путём в сообщении. | открыто |

### Вопросы
- **architect#3** (дополнение к вопросу 2 карточки): какой `narrative.output` записывается в ход, если на одну корреляцию их несколько — вход, открывший встречу (`entry` + `world_event`), и ход смерти (`turn` + `death`)? Сейчас выбирается первый обработанный, и `turn.completed` недетерминирован даже без механики (Ma-1).
- **tech-lead#3:** записать в `tasks.md` значение «одинакового журнала» для T-313 (с точностью до переименования id, порядка внутри шага и перечисленных полей) и то, что `replay` и связка флагов бинарника этим e2e не покрыты.

### Предложения в бэклог (вне границ T-313)
1. **Вынести мир сценария** (`gatewayWorld`, журнал, «шаг осел», канонический журнал) для T-314 и T-357. T-315 строит похожий мир в `cmd/telegram-bot/e2e` и импортировать `test/e2e` не может — стоит решить до T-357, где жить общему коду (например, `internal/gateway/gatewaytest`).
2. **`membus.Chaos`:** дубль пишется сразу за оригиналом. Дубли «через N записей» и повторная доставка после рестарта consumer'а не моделируются — для NFR-013 это слабее kafka-адаптера (T-316 закрывает частично).
3. **Для оркестратора, не T-313:** незакоммиченная T-315 при слиянии с кончиком эпика даёт текстовый конфликт в `cmd/telegram-bot/internal/deliver/loop.go` (соседние добавления `DegradedAfterFailedPolls` из эпика и `EarlyEmpty`/`NewAckClient` из T-315). Разрешается сохранением обоих блоков, после этого сборка, vet и lint чистые.

### Риски и допущения
- `-race` не прогонялся. Поведение на раннере CI оценено по повторам с малым GOMAXPROCS и под нагрузкой на Windows; под `-race` на 2–4 vCPU ожидаю не лучше.
- В повторах с `-cpu` на машине могли параллельно идти прогоны других агентов. Зависимость частоты падений от GOMAXPROCS (0/6 при 16) и найденная причина (зонд 4) говорят, что дело не только в нагрузке.
- Слитое дерево с T-315 собрано из её незакоммиченного состояния на момент ревью.
- В рабочей папке задачи ревьюер добавил только этот раздел в `review.md`. Карточку T-313 не менял (в неё параллельно пишет architect#3). Docker, `.env` и стенд `:8888` не трогал.

## T-313 · ревью #2 · 2026-09-14 · code-reviewer#3 (TEAM-3)

### Границы ревью
- Итерация 2 после ревью #1 (code-reviewer#2, «вернуть», 0/2/2/4). Ветка `task/T-313-e2e-solo` (`.worktrees/T-313`), база `585de13`, изменения не закоммичены.
- Проверены исправления Ma-1, Ma-2, Mi-1, Mi-2, N-1…N-4 и регрессия от них:
  - `test/e2e/gateway_world_test.go` — `wait`, `consumerBehind`, `gatewayJournal`, `canonical`, `gatewayNormalized`, `gatewayAssertMechanics`, `gatewayAssertSameRun`;
  - `test/e2e/gateway_{solo30,death,fleefail}_test.go`;
  - `shared/testkit/gateway/{http.go, http_test.go, README.md}` — `Drain`.
- Прочитан код, на который опираются новые правила:
  - `turns/tracker.go` — `OnMechanics`, `OnNarrative`, `OnDelivered`, `finish`;
  - `turns/store.go` — `Completed`;
  - `consumer/dispatcher.go` — `commit`, `Cursors`;
  - `consumer/handle_encounter.go`;
  - `store/open.go`;
  - `shared/eventbus/types.go` — `Derive`;
  - `testkit/swarm/fake_narrator.go`.
- Карточка: «Итерация 2 (developer)», «Решения architect#3». Решения architect#3 (Н-1, Н-2) и выбор фикстуры (вариант (а)) замечаниями не считаются.

### Прогоны
Windows, go1.26.8, без `-race` (нет cgo).
- `go build ./... && go vet ./... && go vet -tags e2e ./test/e2e/...` — ок.
- `go test -short -count=1 ./shared/testkit/gateway/...` — ок. Под фоновой нагрузкой `-count=60 -run 'TestDrain|TestAwaitDelivery'` — ок.
- `golangci-lint run ./...` — 0 issues.
- `go test -tags e2e -count=6 -cpu=1,2 -run TestGateway ./test/e2e/...` — ок, 267 с: по 12 прохождений каждого теста при GOMAXPROCS 1 и 2. На ревью #1 при тех же настройках `flee-fail` падал в 2 из 6 и 4 из 6.
- `-count=2 -cpu=1,2 -run TestGateway` под фоновой нагрузкой — оба прогона ок:
  - параллельный цикл `go test -count=20 ./internal/state/...` (7 проходов) — 97 с;
  - параллельный цикл `go test -short -count=1 -p 16 ./internal/state/... ./internal/mechanics/... ./internal/gateway/... ./shared/eventbus/... ./shared/testkit/...` (6 проходов) — 110 с. Это та же нагрузка, при которой на ревью #1 красными были 7 из 8 сравнений.
- **Слитое дерево** (копия `t313r2-merged`): кончик эпика `6b67fec` (T-308, T-309) и файлы кода T-313.
  - Пересечений по файлам кода нет. Кончик менял в `test/e2e` только `empty_world_test.go`, а `shared/testkit/gateway` не трогал.
  - `go build ./... && go vet ./... && go vet -tags e2e ./test/e2e/...` — ок.
  - `-run TestGateway -count=1` — ок: смерть на 46-м ходу, та же последовательность `solo-30`, p95 `202` ≈ 2,5 мс.
  - `golangci-lint` по `test/e2e`, `shared/testkit/gateway` — 0 issues.
  - Добавлен незакоммиченный код T-473 (`internal/gateway/consumer/dispatcher.go`, `context.go` и тесты): сборка, vet и `-run TestGateway -count=1` — ок.
- **Общие файлы.**
  - T-315 (`cmd/telegram-bot/**`, `.golangci.yml`, `internal/gateway/api/names_test.go`) и T-473 (`internal/gateway/consumer/**`, `internal/gateway/{context.go, README.md, bus_health_test.go, snapshot_test.go}`) с кодом T-313 файлов не делят.
  - Общие у всех трёх только `dev-log.md` и `review.md` (драйвер `appendtail`). `gateway-and-bot.md` T-313 делит с кончиком — слияние проверил architect#3.
  - Зависимость не по файлам: e2e читает таблицу `cursors`. T-473 оставляет запись курсора в транзакции эффектов (`INSERT INTO cursors` и `Cursors` на месте).

Зонды — в копии `t313r2-probe` (`585de13` + файлы T-313), без `-overlay`. Контрольный прогон — первым.

| # | Зонд | Результат |
|---|---|---|
| 0 | контрольный, без изменений | зелёный |
| 1 | `Tracker.finish` публикует второй `turn.completed` с тем же id и `status=timeout` (зонд 3 ревью #1) | красный во всех трёх тестах: «a copy with another body» |
| 1б | `finish` публикует тот же `turn.completed` дважды, тело совпадает | красный во всех трёх: «analytics_events/… 2 records, want 1» |
| 2 | `Tracker.OnMechanics` ничего не пишет | красный только `solo-30`: ходы 7 и 16 (`rest`), 30 (`leave`) — «turn.completed without its mechanics»; `death` и `flee-fail` зелёные |
| 3 | `consumer.Handle`: пауза 150 мс на `combat.decided` (зонд 1б ревью #1: там красными были `death` и `flee-fail`) | зелёный, 41 с: шаг ждёт курсор consumer'а |
| 4 | `OnNarrative` пишет в ход `narrative_event_id = correlation_id` | красный во всех трёх: ходы с двумя нарративами — «names the narrative "e2e-86", not one of the narratives of its turn» |
| 5 | то же только для нарратива `kind=turn` | `solo-30` и `flee-fail` **зелёные** (Mi-3) |

### Проверка закрытия замечаний ревью #1

**Ma-1 — закрыто.**
- Диагноз разработчика подтверждается кодом:
  - `FakeNarrator` публикует нарративы из подписок разных топиков;
  - `OnNarrative` (`tracker.go:316-320`) пишет поля первого нарратива корреляции: следующий `UPDATE … WHERE status IN (accepted, mechanics_applied)` не срабатывает;
  - consumer шлюза читает `narrative_output` одной подпиской по офсетам.
- Доставка `world_event` называет то событие открытия, которое проекция увидела первым (`handle_encounter.go:12-35`). Id доставки от выбора не зависит (`TransitionDeliveryID`).
- *Не маскирует ли правило ошибки.* Нормализуется ровно выбор одного из нескольких событий одной корреляции (одной встречи), и только он:
  - принадлежность проверяется `t.Errorf` (зонд 4 — красный);
  - нарративы шага сравниваются набором, как все события шага;
  - остальные поля хода, которые берутся из выбранного нарратива (`generated_by`, `fallback_reason`, `agent_*`, `absence`), по-прежнему сравниваются. С `FakeNarrator` у всех видов они одинаковы (`fake_narrator.go:800-813`), поэтому сравнение устойчиво. С настоящим роем это не так — см. «Риски».
- Правило скрывает дефект другого рода: ход, который ссылается на чужой или неверный нарратив, когда нарратив у хода один. Это Mi-3 — пробел, которого нет в правиле Ma-1, но оно проверяет принадлежность только у ходов с несколькими нарративами.
- Прогоны п. 1 задания воспроизведены и зелёные, в том числе под нагрузкой, при которой ревью #1 получало 7 из 8 красных сравнений.

**Ma-2 — закрыто.**
- `refresh` (`gateway_world_test.go:467-496`) считает копии `topic/id` и сравнивает тело с первым; `assertCopies` в `finish` требует 1 копию или 2 при `duplicate`.
- Зонды 1 и 1б красные во всех трёх тестах: противоречащая копия ловится сразу при чтении, одинаковая — счётом копий.
- В `refresh` пропускается только `dead_letters`, а его пустоту проверяет `assertNothingWasDeadLettered`.

**Mi-1 — закрыто. До Н-1 этого достаточно.**
- `gatewayAssertMechanics` проверяет согласованность полей механики, если они есть. Детерминированная защита от полной потери — ходы `rest`/`leave` `solo-30` (зонд 2).
- Потерю механики только у `combat.decided` при сохранённой у фактов State тест не поймает: у ходов `attack`/`flee` запись механики — гонка, которую и снимает Н-1. В DoD Н-1 уже есть «пять полей убраны из `gatewayRaceFields`» и мутант на сравнение журналов `solo-30`. Дублировать это в T-313 не нужно.
- Верхняя граница в той же проверке сформулирована неверно — N-5.

**Mi-2 — закрыто.**
- *Блокировки.* Второе соединение открыто штатным `store.OpenGateway` (WAL, `busy_timeout` 5 с) и выполняет только `SELECT`. Курсор `rows` закрывается сразу (`defer`, в том числе при `Fatalf`), долгой читающей транзакции нет. В WAL читатель не блокирует писателя, автоматический checkpoint пассивный. `PRAGMA journal_mode=WAL` на уже WAL-базе ничего не пишет. Соединение закрывается раньше шлюза (порядок `t.Cleanup`), так что последним файл закрывает шлюз и на Windows не мешает удалению каталога.
- *Семантика.* Курсор — последний офсет, эффекты которого закоммичены (`dispatcher.go:216-259`). Сравнение `at < last` верно. Порядок «курсоры, затем `Records`» консервативен: новая запись между чтениями даёт «ещё не осел», а не ложную готовность. Повторы под `duplicate` тоже двигают курсор. Событие, ушедшее в `dead_letters`, оставит шаг висеть 20 с с верным диагнозом — это падение, а не зелень.
- `Drain` вызывается под `context.WithDeadline` бюджета шага. `quiet ≤ 0` приводится к нулю (`max(quiet, 0)`), unit проверяет `wait_ms=0`.
- Зонд 3 (150 мс) зелёный: медленный, но верный consumer проходит.
- Дублирование запроса курсоров — N-6.

**N-1…N-4 — закрыты.**
- N-1: `hp` сверяется в обеих ветках, `len(based_on) == 2`, `strconv.Itoa`.
- N-2: godoc и README описывают ошибку и `quiet ≤ 0`, unit покрывает оба случая.
- N-3: `created_at` назван в перечне нормализаций с причиной.
- N-4: печатается первый различающийся шаг из обоих прогонов.

**«Что не покрыто»** — полно и честно: связка флагов бинарника, `replay`, смысл «одинакового журнала» со всеми нормализациями. Предлагаю дописать туда остаток Mi-3, если его не исправят в задаче.

### Вердикт
**Принять.** Critical (Blocker) 0 · Major 0 · Minor 1 · Nit 2.

Mi-3, N-5 и N-6 — однострочные правки теста. Их можно сделать до слияния без повторного ревью или перенести в Н-1: та задача всё равно правит `gatewayNormalized` и `gatewayAssertMechanics`.

### Замечания

| # | Серьёзность | Файл:строка | Замечание | Предложение | Статус |
|---|---|---|---|---|---|
| Ma-1 | Major | ревью #1 | мигание сравнения при GOMAXPROCS ≤ 4 | — | закрыто (итерация 2) |
| Ma-2 | Major | ревью #1 | журнал склеивал копии без сравнения тел | — | закрыто (итерация 2) |
| Mi-1 | Minor | ревью #1 | поля механики не проверялись | — | закрыто (итерация 2) |
| Mi-2 | Minor | ревью #1 | «шаг осел» не знал о consumer'е, `Drain` без дедлайна | — | закрыто (итерация 2) |
| N-1…N-4 | Nit | ревью #1 | — | — | закрыто (итерация 2) |
| Mi-3 | Minor | `test/e2e/gateway_world_test.go:899-908` (`gatewayNormalized`) | **`delivery.narrative_event_id` хода проверяется, только если у корреляции несколько нарративов.** У хода с одним нарративом (`attack`, `flee`, `look`) поле только сравнивается между прогонами, а одинаково неверная ссылка после переименования id одинакова в обоих. Зонд 5 — ход `attack`/`flee` ссылается на своё действие вместо нарратива — `solo-30` и `flee-fail` зелёные. Unit-тестов на это поле в `internal/gateway/turns` нет (`grep narrative_event_id` по `*_test.go` пакета пуст). Между тем по нему отчёт EPIC-005 связывает ход с нарративом (C-10). Правило Ma-1 ввело проверку принадлежности, но асимметрично. | Проверять принадлежность при `len(narratives) > 0`, а заменять на `gatewayNarrativeOfTurn` только при `> 1`. У хода без нарратива (`timeout` у `say`/`rest`/`leave`) проверять, что поля нет. | открыто |
| N-5 | Nit | `test/e2e/gateway_world_test.go:959-962` (`gatewayAssertMechanics`); карточка, «Итерация 2», Mi-1 | Верхняя граница `mechanics_at` — `ev.Timestamp` события `turn.completed`. Это не момент завершения хода: `Completed` строит событие через `Derive` от восстановленного действия с `Timestamp: r.ReceivedAt` (`turns/store.go:131-140`, `shared/eventbus/types.go:187`). Фактически проверяется `mechanics_at == received_at`, а не «`received_at ≤ mechanics_at ≤` момент `turn.completed`», как написано в комментарии и карточке. Сейчас это верно только потому, что часы не двигаются между действием и оседанием шага. Сценарий, где часы сдвинутся раньше факта механики (T-314 `recovery`, помощники которого переиспользуются), получит ложное падение. Кроме того, строки RFC3339Nano сравниваются лексикографически, а у них отброшены хвостовые нули: `…:05Z` > `…:05.5Z`. | Сравнивать `time.Time` (`time.Parse(time.RFC3339Nano, …)`). Верхняя граница — `timings.narrative_at`, если оно есть, иначе `received_at + turns.DefaultTimeout` (плюс шаг sweeper'а). Либо оставить равенство, но назвать его так в комментарии и карточке. | открыто |
| N-6 | Nit | `test/e2e/gateway_world_test.go:359-377` (`consumerBehind`) | Запрос `SELECT topic, "offset" FROM cursors` и цикл сканирования повторяют экспортированный `consumer.Cursors` (`internal/gateway/consumer/dispatcher.go:303-319`; в T-473 он сохранён). Тест завязан на схему таблицы там, где есть API пакета. | `cursors, err := consumer.Cursors(w.ctx, w.gatewayDB)`. | открыто |

### Вопросы
- **architect#3** (вопрос ревью #1, в разделе architect#3 без ответа; разработчик счёл его ненужным для T-313). Какой `narrative.output` считать нарративом хода, если на корреляцию их несколько (вход, открывший встречу, — `entry` + `world_event`; ход смерти — `turn` + `death`)? Для T-313 ответ не нужен: шлюз детерминирован, e2e нормализует выбор. Для C-10 и метрик нужен:
  - `narrative.generated_by`, `agent_level`, `fallback_reason` хода берутся у первого по офсету нарратива;
  - `recipients_count` — тоже у первого;
  - завершает ход ack любой доставки `narrative` этой корреляции (`OnDelivered` считает по `correlation_id`).

  С настоящим роем `entry` и `world_event` могут прийти от разных агентов и с разным `generated_by`. Тогда поля хода зависят от порядка топиков, а сравнение журналов e2e снова начнёт мигать уже на этих полях. Естественное место решения — Н-1, та же функция `completeDelivered`.

### Предложения в бэклог (вне границ T-313)
1. Н-1: вместе с `gatewayRaceFields` пересмотреть N-5 — после Н-1 у хода с Phase 1 появится настоящий момент завершения, и граница станет проверяемой.
2. Unit в `internal/gateway/turns/tracker_test.go`: `narrative_event_id` в `turn.completed` равен id нарратива, а при `payload.narrative_event_id` — ему (ветка `tracker.go:292-295` не покрыта).

### Риски и допущения
- `-race` не запускался (нет cgo). Job `race` гоняет `./test/e2e/...` с `-race` на 2–4 vCPU. Главная причина мигания ревью #1 устранена, и на GOMAXPROCS 1 и 2 под нагрузкой сравнения зелёные, но поведение под `-race` на раннере оценено косвенно.
- В первом цикле фоновой нагрузки (`-p 16`, одновременно с e2e) два прохода из шести завершились с ошибкой в каком-то пакете нагрузки. Лог перезаписывался, пакет не установлен. Повторный цикл той же нагрузки (8 проходов) — зелёный. `Drain` T-313 проверен отдельно: `-count=60` под нагрузкой — ок. Сам e2e в обоих случаях зелёный.
- Слитое дерево с T-473 собрано из её незакоммиченного состояния на момент ревью.
- Нормализация Ma-1 устойчива только для `FakeNarrator`, у которого все виды нарратива несут одинаковые `generated_by` и агента (см. «Вопросы»).
- Копии `t313r2-merged`, `t313r2-probe`, скрипт зонда и логи — в scratchpad, удаляются по точным путям. В рабочей папке задачи ревьюер изменил только `review.md` (этот раздел) и таблицу раздела «Ревью» карточки. Docker, `.env` и стенд `:8888` не трогал.

## T-473 · ревью #1 · 2026-09-14 · code-reviewer#2 (TEAM-3)

### Границы ревью
- Ветка `task/T-473-consumer-resubscribe` (`.worktrees/T-473`), база `6b67fec`. Изменения не закоммичены: 9 изменённых файлов и 2 новых.
- Код:
  - `internal/gateway/consumer/dispatcher.go` — переподписка, `Config.Timers`, `Err`, `Stop`;
  - `internal/gateway/context.go` — таймеры consumer, срок снапшота в `Stop`;
  - тесты: `consumer/resubscribe_test.go` (новый), `bus_health_test.go` (новый), `snapshot_test.go`, `consumer/redpanda_integration_test.go`, `consumer/{dispatcher,deliveries}_test.go`;
  - `internal/gateway/README.md`.
- Сверено с:
  - разделом `### T-473` в `tasks.md`, включая три строки DoD из приёмки T-309;
  - разделом «Выполнение (developer)» карточки;
  - КД `gateway-and-bot.md` §11.2 и §11.4 из этой же папки;
  - C-01 (`Subscribe`, `Close`), `shared/eventbus/kafka.go`, `delivery.go`, kafka-go v0.4.51 (`reader.go`, `consumergroup.go`);
  - NFR-061, логами прогонов исполнителя `t473-run1.log`, `t473-run2.log`, `t473-mutants.log`.
- Вопросы 1–3 исполнителя замечаниями не считаются, ниже только оценка риска. В карточку ревьюер не писал: по поручению оркестратора её параллельно ведёт system-architect#2.

### Вердикт
**Вернуть.** Blocker 0 · Major 1 · Minor 4 · Nit 5.

Переподписка, правило `RestoredAfter`, `Stop` с отказом новых обработчиков и строгий шаг 8 сделаны верно. Потерь и повторных эффектов, гонок данных и утечек горутин без верхней границы я не нашёл. Мой интеграционный прогон зелёный, замеры совпали с замерами исполнителя.

Возврат из-за Ma-1: переписанный тест снапшота в `Stop` сверяет дедлайн записи со своим моментом начала, а не с моментом начала `Stop`. На Windows он проходит только из-за грубого монотонного таймера, а на `ubuntu-latest` в CI будет красным при каждом прогоне. Правка — одна строка теста.

### Проверено ревьюером (go1.26.8 windows/amd64, cgo выключен — без `-race`)
- `go build ./... && go vet ./... && go vet -tags integration ./internal/gateway/...` — 0.
- `go test -short -count=3 ./internal/gateway/...` — 16 пакетов `ok`, флака нет.
- `golangci-lint run --build-tags integration ./internal/gateway/...` — `0 issues.`
- **Интеграция, один прогон** (10:13–10:17): `go test -tags integration -count=1 -v -timeout 20m -run TestConsumerOnRedpanda ./internal/gateway/consumer/...` — `ok`, **205,6 с** (пакет 206,2 с), 7 подтестов `PASS`. До прогона `docker ps -a --filter label=org.testcontainers=true` был пуст. Сразу после прогона в списке ещё был `ryuk`, через 14 с список опустел. Лог — `scratchpad/t473r-integration.log`.

  | Замер | Ревьюер | Исполнитель |
  |---|---|---|
  | Остановка сразу после `Start` | 2,001 с | 2,003 / 2,0 с |
  | Возобновление группы с офсета (свежая `gateway.db`) | 769 мс | 594 / 832 мс |
  | `AnEventThatKeepsFailing…` после брошенного входа | 29,9 с | 26,7 / 26,4 с |
  | Курсоры ниже начала лога: `ReadRange` / `Start` | `[17]`, 18, `nil` / `nil` за 492 мс | то же |
  | Строгий шаг 8, от запуска брокера | 29,7 с | 29,1 / 29,5 с |
  | Ack при зависшем брокере; `gateway.db` через 1 с | 503 за 5 с; занята | то же |
  | Период повторной выдачи | 30,5 с, ack бота 21,4 с | 30,5 с |
  | Шаг 8 после снятия паузы | 5,7 с, `narrative_output` видна кончившейся (`Rebalance In Progress`) | 5,6 / 5,5 с |
  | Остановка во время переподписки | 2 с | 2 с |
  | Первая доставка у потребителя после сбоя | 14,95 с от `Start` (33,0 с от запуска брокера) | 14,3 / 14,9 с |

- **Мутанты.** Копия `scratchpad/t473r-mut`: без `.git`, `Docs`, `services`, `.worktrees`, без `-overlay`, контрольный первым. Каждый фрагмент проверялся на единственность, после прогона файлы возвращались. Перед удалением три затронутых файла сверены `cmp` с рабочей папкой. Копия удалена по точному пути.

  | # | Мутант | Результат |
  |---|---|---|
  | K | контрольный: после конца подписки — выход (`dispatcher.go:314`, `if true`) | красный: (а), (б), (г)/nil |
  | R1 | тест: 20 мс между `began` теста и вызовом `Stop` (так ведёт себя монотонный таймер с разрешением в наносекунды) | **красный**: `snapshot_test.go:222` — «had until 1.0104s … want half of the 2s» → Ma-1 |
  | R2 | `context.go:271`: в replay таймеры consumer — `deps.Timers` (`NullTimers`) | **зелёный** по `./internal/gateway/` → Mi-3 |

- Отдельная проба в `scratchpad/t473r-res` повторила арифметику теста на `time.Now` этой машины. Из 200 повторов дедлайн вышел за половину срока в 1 случае. Значит, даже на Windows тест иногда флакает.

### Разбор по пунктам поручения

**1. Переподписка.**
- **Паузы.** `resubscribePause` даёт 1, 2, 4, 8, 15, 15… с, счётчик сбрасывается после «услышанной» попытки. Ожидание — `Config.Timers.After`, `time.*` нет (forbidigo чист).
- **Отклонение 1 — `clock.RealTimers{}` в replay.** На события оно не влияет:
  - пауза и `settle` ничего не публикуют и не штампуют;
  - `processed_at` пишется по `Deps.Clock`;
  - повторная выдача после переподписки гасится курсором эффектов и `processed_events`, проекция — правилом версии.

  В replay меняется только момент по настенным часам. Побайтовое сравнение NFR-061 это не задевает. С `NullTimers` шлюз в replay оглох бы навсегда после первого сбоя, а grace `Stop` не кончался бы. Решение верное и согласуется с C-01 v1.4 и v1.8. Ветку не закрепляет ни один тест (R2 зелёный) — Mi-3.
- **Гонки и утечки.** Состояние `subscriptions`, `stopping`, `handling` и `idle` меняется только под `d.mu`. `heard` и `end` сверяют номер попытки, поэтому поздний `settle` прошлой попытки не отмечает текущую. Горутина `settle` выходит по `ended`, `ctx` или таймеру. Горутина `running.Wait` в `Stop` и брошенная подписка живут не дольше входа kafka-go в группу (~30 с): утечка ограничена. Одно окно остаётся: `Subscribe` после `Stop` — Mi-2.
- **`ErrClosed`.** Остановка, повторов нет, топик остаётся `down`. Тест (г)/closed, M5 исполнителя красный. На закрытой `membus` (`nil`) шлюз переподписывался бы с потолком паузы. В процессе этого не бывает: шина закрывается после `Stop`.
- **Остальные топики.** Цикл свой у каждого топика, тест (а) считает подписки остальных — по одной.
- **Обработчик после `Stop`.** Возвращает ошибку контекста, а контекст подписки к этому моменту уже отменён: `cancel()` стоит до `stopping = true`. `Delivery.Deliver` при `ctx.Err() != nil` не тратит повторы и не пишет в DLQ (`delivery.go:158-166`), событие остаётся незакоммиченным. Ветка `errors.New("consumer: stopped")` недостижима, вреда нет.

**2. `RestoredAfter` = 35 с.**
- **Ложный `ok` при отказывающем брокере** не возникает. Неудачная попытка kafka-go кончается после трёх `cg.Next` с паузой `JoinGroupBackoff` 5 с, это ~10–15 с, до `settle` не доходит. У топика с трафиком `ok` ставит только доставка.
- **Ложный `ok` возможен при зависшем брокере**, если попытка висит в запросе дольше 35 с. Но это та же слепая зона, что у подписок `Start`: они «слышны» сразу, и зависший брокер `/health` не видит и до задачи. Правило её не расширяет, в README расхождение названо. Остаётся риск, не замечание.
- Тест (в) идёт на `clock.Manual`. Он ждёт взведения таймеров 1 с и 35 с и проверяет `fail → fail → ok`, M6 исполнителя красный.
- КД §11.4 (строка `bus`) нового критерия не знает — N-1.

**3. `Stop`.**
- Порядок верный:
  1. отмена подписок;
  2. `stopping` — новые обработчики не входят;
  3. ожидание работающих обработчиков, по сроку `ctx` — ошибка;
  4. ожидание подписок не дольше `StopTransportGrace` = 2 с, затем `Warn`.

  После возврата `Stop` `Dispatcher` в БД и проекцию не пишет (тест д-2, M8 красный). Замер на Redpanda — 2,0 с вместо 29,7 с.
- **Брошенный reader, риск для следующего потребителя.** Reader сам доходит вход и остаётся членом группы до выхода. Новый потребитель той же группы ждёт ребалансировки: в тесте 26–30 с. Потерь нет, события приходят после входа. Для процесса это окно глухоты после рестарта контекста шлюза в том же процессе на Kafka. `/health` в это время показывает `ok`: подписки `Start` «слышны» сразу. Риск средний по эксплуатации, низкий по данным.
- **Цена, которую карточка не называет, — Mi-1.** Ожидание входа не исчезает, а переходит из `Stop` контекста в `Kafka.Close` процесса (`closeBus`).

**4. Снапшот в `Stop`.**
- `shutdownSnapshotContext` берёт `min(now + SnapshotWriteBudget, began + (deadline − began)/2)` от момента начала `Stop`. Сжатие `links.db` идёт до снапшота, БД закрываются при любом исходе, неудача даёт `Warn`, `Stop` ошибкой её не возвращает. Совпадает с КД §11.2 в редакции architect#3 («часть оставшегося срока», «только `Warn`»).
- Код верен, неверна проверка теста — Ma-1.

**5. Integration-тест.**
- **Шаг 8 строгий.** `hearsEveryTopicAgain` ждёт одновременно `d.Err() == nil` и курсор эффектов каждого из четырёх топиков не ниже офсета события, затем проверяет одну отметку `processed` на событие. Путь с перезапуском удалён, `resubscribeLimit` = 90 с обоснован.
- **`groupRestart`.** Второй потребитель — на свежей `gateway.db` с курсором снапшота на конце: хвост приходит только подпиской группы. Замер переименован. Остаточный флак-риск — N-5.
- **Курсор ниже начала лога.** `DeleteRecords` (API 21) через протокол kafka-go. Начало лога 17 выше курсоров 14 и 13. Догон с начала лога без ошибки. Отклонение 3 обосновано: для курсора эффект тот же, что у удалённых сегментов.
- **`docker pause`.** Только своего контейнера: `b.rp.Container().GetContainerID()`, клиент testcontainers, снятие паузы в `t.Cleanup`. Чужие контейнеры тест не трогает.
- **Длительность.** 205,6 с у ревьюера, 195–197 с у исполнителя. DoD T-316 — не больше 3 мин. Путь уложиться без потери замеров есть — Mi-4.

**6. Пересечения с T-313 и T-315.**
- Общий файл один — `Docs/dev-team/epics/EPIC-004-gateway-bot/dev-log.md`, запись в конец, драйвер `appendtail`.
- Файлы кода не пересекаются. Для T-313 сравнивались коммиты от merge-base с эпиком и незакоммиченное: `test/e2e/*`, `shared/testkit/gateway/*`, КД. Для T-315: `cmd/telegram-bot/**`, `internal/gateway/api/names_test.go`.
- Новое обязательное поле `consumer.Config.Timers` ни T-313, ни T-315 не ломает: `consumer.New` вне пакета вызывает только `internal/gateway/context.go`, а его ни одна из этих задач не меняет.

### Оценка вопросов исполнителя (не замечания)
1. **`io.EOF` → `nil` в `Kafka.Subscribe`.**
   - Для шлюза риск закрыт: `nil` без `Stop` здесь — переподписка.
   - Для других потребителей платформы (State, рой) риск средний: `errors.Is(err, io.EOF)` срабатывает на `io.EOF` в любом месте цепочки, например в ошибке коммита, и подписка умирает тихо.
   - Рекомендацию исполнителя поддерживаю: `nil` только при `k.closing.Err() != nil` или отменённом `ctx`, иначе `eventbus: read <topic>: EOF`.
2. **`RebalanceTimeout` 10 с.**
   - Окно глухоты сократится примерно до 10–15 с.
   - Риск: это параметр всех групп платформы. Участник, чей обработчик во время ребалансировки работает дольше 10 с (нарратив роя, LLM), выпадет из поколения, его коммит отклонят, событие придёт повторно. Для MVP-1 дубликат гасит дедуп, но работа LLM повторится.
   - Ребалансировки редки: только рестарты участников. Риск средний, решать system-architect с учётом потребителей EPIC-003.
3. **Отклонения 1, 2, 4.**
   - 1 принять, с тестом Mi-3.
   - 2 принять, с описанием цены Mi-1.
   - 4 не обязательно принимать: см. Mi-4.

### Замечания

| # | Серьёзность | Файл:строка | Замечание | Предложение | Статус |
|---|---|---|---|---|---|
| Ma-1 | Major | `internal/gateway/snapshot_test.go:206-225` | Проверка `by.After(began.Add(deadline/2))` берёт `began` теста. Код правильно считает половину от начала `Stop`: `by = sB + (D − sB)/2 = began + deadline/2 + e/2`, где `e` — время от `began` теста до первой строки `Stop`. При `e > 0` проверка падает. На Windows монотонное время обновляется тиком (~1 мс), и `e` почти всегда 0 (проба: 199 из 200). На `ubuntu-latest` (`.github/workflows/go.yml`, `make test`) разрешение наносекундное, `e` — десятки микросекунд, тест красный при каждом прогоне. R1 (задержка 20 мс) это воспроизводит: «had until 1.0104s». | Сравнивать с допуском, не зависящим от разрешения часов, например `by.Sub(began) > deadline/2 + 50*time.Millisecond`. Мутант M10 исполнителя (весь остаток, `by = began + 2 с`) при этом останется красным. Перепрогнать M10 и R1: R1 должен стать зелёным. | открыто |
| Mi-1 | Minor | `internal/gateway/consumer/dispatcher.go:476-487`; `cmd/multiverse/serve.go:318-321, 489-493`; `shared/eventbus/kafka.go:343-375`; kafka-go `reader.go:757-771`, `consumergroup.go:685-691` | `Stop` бросает reader, который входит в группу, но ожидание входа никуда не девается. После `StopAll` процесс вызывает `closeBus` → `Kafka.Close` → `reader.Close`. Там стоят `<-r.done` и `cg.Close` → `cg.wg.Wait`, а горутина группы сидит в синхронном `joinGroup` до ответа брокера, до ~30 с. Выход процесса задерживается на то же время, что `Stop` занимал до задачи. Данные не страдают: БД закрыты раньше. Но README и карточка («2,0 с вместо 29,7 с») создают впечатление, что задержка устранена. Вывод сделан по чтению kafka-go v0.4.51, замера нет. | README (раздел про `Stop`) и карточка: назвать, что выход процесса после такого `Stop` ждёт вход в `Close` шины до ~30 с. Добавить этот факт к вопросу 2 system-architect: `RebalanceTimeout` или закрытие reader'а без ожидания `JoinGroup` снимают обе задержки. Замер `bus.Close()` на отдельной шине — по желанию или в бэклог. | открыто |
| Mi-2 | Minor | `internal/gateway/consumer/dispatcher.go:298-303, 323, 338-351` | `wait` выбирает случайно, если таймер паузы и `ctx.Done()` готовы одновременно, а перед `Subscribe` `ctx.Err()` не проверяется. Если `Stop` совпал с концом паузы, возможен вызов `Subscribe` после `Stop`, а DoD прямо запрещает новые вызовы. У kafka-адаптера такой вызов не безвреден: `NewReader` сразу запускает вход в группу (`NewConsumerGroup` → `go cg.run`), `closeReader` ждёт его до ~30 с, и группа получает лишнего участника на время ребалансировки. Вероятность мала, исправление — одна строка. | В начале итерации при `attempt > 0` (или сразу после `wait`) — `if ctx.Err() != nil { return }`. Тест: обёртка `Timers`, чей `After(ResubscribeFirst)` отдаёт уже сработавший таймер только после отмены контекста (вызвать `Stop` из обёртки и дождаться `ctx.Done` через `scriptedBus`), затем `bus.subscriptions() == 1`. | открыто |
| Mi-3 | Minor | `internal/gateway/context.go:266-272` | Отклонение 1 — ветка «в replay таймеры consumer настенные» — тестом не закреплена: R2 (`deps.Timers` в replay) зелёный по `./internal/gateway/`. Регрессия сделает шлюз в replay глухим навсегда после первого сбоя подписки, а `Stop` будет ждать транспорт до срока `ctx`. | Экспорт в `export_test.go` (например, `ConsumerTimers(c)`) и проверка в существующем replay-тесте, что это `clock.RealTimers`. Или тест в replay с `breakingBus`: вторая подписка приходит, хотя `Deps.Timers` — `NullTimers`, ценой паузы 1 с настенного времени. | открыто |
| Mi-4 | Minor | `internal/gateway/consumer/redpanda_integration_test.go:332-333, 892` | `TestConsumerOnRedpanda` идёт 195–206 с при DoD T-316 «не больше 3 мин» (отклонение 4). Две составляющие устранимы без потери замеров. (1) Замер «остановка сразу после `Start`» в `groupRestart` бросает reader во вход, и следующий подтест `AnEventThatKeepsFailing…` платит 26–30 с за ребалансировку вместо ~1 с. (2) В `hungBroker` стоит фиксированное ожидание `ResubscribeCeiling + 1 с` = 16 с, и нет проверки, что `Stop` пришёлся на подписку внутри транспорта: об этом говорит только результат 2 с. | Перенести замер «остановка сразу после `Start`» в самый конец теста (последним действием `hungBroker`) — минус ~25 с. В `hungBroker` ждать по условию: `s.d.Err() != nil`, затем `ResubscribeFirst` + ~2 с на начало попытки. Проверять, что `Stop` занял не меньше `StopTransportGrace`, то есть попал на вход. Ожидаемый итог — меньше 3 мин. | открыто |
| N-1 | Nit | `Docs/dev-team/architecture/components/gateway-and-bot.md:1018` | Строка `bus` КД §11.4: «`fail` — только если подписка завершилась ошибкой после старта». Теперь `fail` держится и после `nil` без `Stop`, и до «услышанной» переподписки (`RestoredAfter`). DoD отдал критерий README, КД отстал. | architect#3 при приёмке — дополнить строку ссылкой на критерий README и `consumer.RestoredAfter`. | открыто |
| N-2 | Nit | `internal/gateway/consumer/resubscribe_test.go:295, 401` | `<-heard` и `<-refused` без срока: при дефекте пакет повиснет до `-timeout` go test вместо понятного падения. | `select` с `testkit.After(15 * time.Second)` и `t.Fatal`. | открыто |
| N-3 | Nit | `scratchpad/t473-mutants.log`; карточка, «Мутанты» | Сохранённый лог — только первый набор. В нём M11 не собрался (`declared and not used: log`), а карточка пишет «M11 красный: тест снапшота» и про несобранный M11 молчит, упоминая только M9. Лога повторного набора нет, заявленное не проверить. | Сохранить лог повторного набора рядом, в карточке указать, как исправлены M9 и M11. | открыто |
| N-4 | Nit | `internal/gateway/consumer/dispatcher.go:471-477` | Если `ctx` уже истёк к вызову `Stop` (срок процесса съели контексты, остановленные раньше), первый `select` может выбрать `ctx.Done()`, хотя `idle` закрыт и обработчиков нет. Тогда `Stop` вернёт «handlers of the subscriptions did not stop», и ошибка собьёт при разборе. | Сначала неблокирующая проверка `idle`, потом `select` со сроком. | открыто |
| N-5 | Nit | `internal/gateway/consumer/redpanda_integration_test.go:286-290` | Первый потребитель останавливается, как только курсор эффектов в `gateway.db` дошёл до события 2. Коммит офсета в Kafka идёт после обработчика и может ещё не уйти. Раньше повтор события 2 гасила общая БД, теперь у второго потребителя БД свежая, и повтор дал бы лишний эффект: проверка `effects == [3, 4]` упадёт. kafka-go досылает уже поставленные в очередь коммиты, так что окно — доли миллисекунды. | Перед `stop(t, first)` дождаться зафиксированного офсета группы (`OffsetFetch` через `kafka.Client`) или опубликовать третье «маркерное» событие и ждать его. | открыто |

### Предложения в бэклог (вне границ T-473)
1. **`/health` после рестарта контекста шлюза на Kafka.** Подписки `Start` «слышны» сразу, но вход в группу после брошенного участника занимает до ~30 с. Можно применить к подпискам `Start` то же правило «первая доставка или `RestoredAfter`», если архитектор сочтёт это нужным.
2. **Предложения исполнителя 1–4 поддерживаю.** Особенно п. 1: ack держит единственное соединение `gateway.db` до 5 с при зависшем брокере, 20 с из 30 с периода выдачи.
3. **`membus.Subscribe` на закрытой шине возвращает `nil`, kafka-адаптер — `ErrClosed`.** Расхождение реализаций C-01, вопрос к system-architect вместе с вопросом 1.

### Риски и допущения
- `-race` не запускался (cgo выключен). Отсутствие гонок разобрано чтением.
- Ma-1 для Linux выведен из арифметики теста и подтверждён мутантом R1 и пробой разрешения часов на этой машине. Прогона на Linux не было.
- Mi-1 и Mi-2 сделаны по чтению kafka-go v0.4.51 без замера: отдельный интеграционный прогон под них не выполнялся.
- Интеграционный прогон ревьюера — один, по разрешению поручения. До прогона список контейнеров testcontainers был пуст, после прогона пуст. Чужие контейнеры, стенд `:8888` и `.env` не трогались.
- Пересечения с T-313 и T-315 оценены по состоянию их рабочих папок на момент ревью.
- В рабочей папке задачи ревьюер добавил только этот раздел `review.md`. Файлы в scratchpad — с префиксом `t473r-`, копия мутантов и проба удалены по точному пути.

## T-473 · ревью #2 · 2026-09-14 · code-reviewer#2 (TEAM-3)

### Границы ревью
- Итерация 2 по ревью #1 (0/1/4/5, «вернуть»). Проверены только исправления и регрессия от них. Рабочая папка `.worktrees/T-473`, изменения не закоммичены. База `6b67fec`.
- Код итерации 2:
  - `internal/gateway/consumer/dispatcher.go` — `wait` (Mi-2), `Stop` (N-4), комментарий `Config.Timers`;
  - `consumer/resubscribe_test.go` — `receive`, тесты Mi-2 и N-4;
  - `bus_health_test.go` — тест Mi-3;
  - `snapshot_test.go` — допуск (Ma-1);
  - `consumer/redpanda_integration_test.go` — `committed`, третий потребитель `groupRestart`, `stopRightAfterStart`, ожидание в `hungBroker`;
  - `internal/gateway/README.md`.
- Сверено с:
  - разделами карточки «Итерация 2 (developer#3)» и «Просмотр system-architect» (C-01 v1.12, T-477);
  - `shared/eventbus/kafka.go` (`Subscribe`, `Close`, `closeReader`, `track`/`untrack`);
  - kafka-go v0.4.51: `reader.go:291-348` (`run`), `:757-778` (`Close`), `:815-837` (`FetchMessage`); `consumergroup.go:584-598` (сроки запросов), `:685-691` (`Close`), `:714-764` (`run`), `:776-803` (`nextGeneration`), `:898-921` (`coordinator`);
  - логами исполнителя `t473i2-run.log`, `t473i2-mutants.log`, `t473i2-snapshot200.log`.
- Интеграционный прогон ревьюер не делал (по поручению): замеры Redpanda взяты из `t473i2-run.log`.

### Вердикт
**Принять.** Blocker 0 · Major 0 · Minor 1 · Nit 3.

Ma-1, Mi-2, Mi-3, Mi-4 и N-2…N-5 закрыты, N-1 передан architect#3. По Mi-1 прав исполнитель: мой вывод в ревью #1 был ошибочным, описание в README принимаю. Новые замечания не блокируют. Mi-5 — риск флака новой проверки integration-теста, не воспроизведён. N-6 — узкий случай, в котором `Close` всё же ждёт: подтверждён зондом. N-7 и N-8 — качество теста и README.

### Проверено ревьюером (go1.26.8 windows/amd64, cgo выключен — без `-race`)
- `go build ./... && go vet ./... && go vet -tags integration ./internal/gateway/...` — 0.
- `go test -short -count=3 ./internal/gateway/...` — 16 пакетов `ok`.
- `go test -short -count=5 -cpu=1,2 -run "Pause|Replay|Stop|Snapshot" ./internal/gateway/...` — 16 пакетов `ok`: `gateway` 44,4 с, `consumer` 13,0 с.
- `golangci-lint run --build-tags integration ./internal/gateway/...` — `0 issues.`; `gofmt -l internal/gateway` — пусто.
- Лог — `scratchpad/t473r2-checks.log`.
- **Зонд Mi-1** (`scratchpad/t473r2-probe.log`, в копии дерева).
  - Условия. Настоящие `eventbus.Kafka` и `consumer.Dispatcher`, `clock.RealTimers{}`. Вместо брокера — TCP-слушатель, который читает и не отвечает: каждый reader kafka-go висит в `FindCoordinator` до срока группы 5 с. Это такое же синхронное ожидание в `cg.run`, как `JoinGroup` на Redpanda.
  - `Stop` вызывается после того, как все четыре reader'а подключились; `Close` шины — сразу после `Stop`.
  - Прогон: `-count=3 -cpu=1,4`, по 6 повторов на сценарий.

  | Сценарий | `Stop` | `Close` | Повторы |
  |---|---|---|---|
  | срок `Stop` 15 с (как у процесса) | 2,0–2,05 с, `nil` | **0 с** | 6 из 6 |
  | срок `Stop` уже истёк | 0 с, `nil` | **4,8 с** | 6 из 6 |
  | срок истёк, 20 мс между `Stop` и `Close` | 0 с, `nil` | 0 с | 6 из 6 |

- **Мутанты.**
  - Копия `scratchpad/t473r2-mut`: без `.git`, `Docs`, `services`, `.worktrees`, без `-overlay`. Контрольный прогон первым.
  - Фрагмент мутанта проверялся на единственность. После каждого мутанта `dispatcher.go` возвращался и сверялся `cmp` с рабочей папкой.
  - Копия, зонд, скрипт и резервный файл удалены по точному пути. Лог — `scratchpad/t473r2-mutants.log`.

  Тесты набора: `TestAPauseThatEndsWithStopSubscribesNoMore`, `TestStopAtAnExpiredDeadlineWithNoHandlerIsNoError`, `TestStopDoesNotWaitForATransportThatDoesNotReturn`, `TestStopEndsAPauseAndSubscribesNoMore`, `TestStopWaitsForARunningHandler`.

  | # | Мутант | Прогон | Результат |
  |---|---|---|---|
  | K | контрольный, без изменений | `-count=100 -cpu=1,4` | зелёный, 213 с: по 200 проходов каждого теста, в том числе двух вероятностных |
  | R3 | `Stop`: из ожидания grace убран `case <-ctx.Done()`, транспорт ждётся без срока | `-cpu=1 -timeout 90s`, затем `-timeout 30s` | **красный только по таймауту пакета**: висит `TestStopAtAnExpiredDeadlineWithNoHandlerIsNoError` → N-7 |
  | R4 | `wait`: после таймера `return true` (M12) при `GOMAXPROCS=1` | `-count=5 -cpu=1` | красный: «round 0/3/5: 2 subscriptions of system_events» |

### Разбор по пунктам поручения

**Mi-1 — ожидание входа в группу и `Kafka.Close`.** Закрыто, вывод ревью #1 ошибочен.
- В ревью #1 я считал, что `Kafka.Close` закрывает брошенный reader и ждёт его вход. По коду это не так:
  - `Subscribe` при отмене `loopCtx` выходит из `FetchMessage` сразу (`reader.go:828-830`);
  - затем отложенный `closeReader` сначала снимает reader с учёта (`untrack`, `kafka.go:472`) и только потом вызывает `r.Close()`;
  - `Close` шины берёт только reader'ы на учёте (`kafka.go:357`).
- Ожидание остаётся в `r.Close()` внутри горутины подписки: `<-r.done` → `cg.Close` → `cg.wg.Wait`, пока `cg.run` сидит в синхронном запросе. Зонд подтверждает: при живом сроке `Stop` занимает 2 с, `Close` — 0 с (6 из 6, в том числе `GOMAXPROCS=1`). Замер исполнителя на Redpanda — `Close` меньше 1 мс.
- Цена в README названа верно: перезапуск контекста в том же процессе и следующий процесс после выхода.
- **Узкий случай, где `Close` ждёт** (N-6). Если `Stop` получает уже истёкший срок, он возвращается сразу: в ожидании grace готов `ctx.Done()`. Горутины подписок к этому моменту ещё не вышли из `FetchMessage` и не сняли reader'ы с учёта. `Close`, вызванный следом, их застаёт и ждёт: на зонде 4,8 с, на Redpanda во время входа — до ~30 с. Пауза 20 мс между вызовами убирает эффект.
  - В процессе окно почти закрыто. После `dispatcher.Stop` шлюз ещё останавливает уборку, сжимает `links.db`, пишет снапшот и закрывает БД, потом останавливаются другие контексты.
  - К тому же случай возможен, только когда срок процесса уже исчерпан.

**Ma-1 — допуск в тесте снапшота.** Закрыто.
- Арифметика: `by − began = D/2 + e/2`, где `e` — время от `began` теста до первой строки `Stop`. Допуск 50 мс покрывает `e` до 100 мс, на Linux это микросекунды, под `-race` — единицы миллисекунд.
- M10 (`by = began + 2 с`) остаётся красным, R1 (20 мс) зелёный — `t473i2-mutants.log`. 200 повторов — `ok` за 212 с (`t473i2-snapshot200.log`).
- Мои `-count=5 -cpu=1,2` зелёные.
- Проверка `at − began ≤ 1,5 с` не зависит от разрешения часов.
- Мутант «половина от момента записи, а не от начала `Stop`» тест не отличит при `e < 100 мс`. Это допустимо: разница — половина `e`.

**Mi-2 — пауза, истёкшая в момент `Stop`.** Закрыто.
- `return ctx.Err() == nil` после таймера (`dispatcher.go:355`). Пауза, совпавшая со `Stop`, к `Subscribe` не ведёт.
- Тест детерминирован для верного кода:
  - `stoppingTimers.After` отдаёт сработавший таймер только после того, как `cancelWatch` увидел `Done` того же `subCtx`;
  - значит, к `select` `ctx.Err() != nil` гарантированно, и ложного красного нет ни при `-race`, ни при `GOMAXPROCS=1`.
- Мутант M12 выживает 20 раундов с вероятностью 2⁻²⁰. Случайный выбор ветки `select` в рантайме не зависит от числа P. R4 при `-cpu=1` красный.
- Горутина `Stop` в тесте не зависает. Grace на ручных часах не истекает, но `returned` закрывается: подписка `world_events` возвращается по отмене, `system_events` — после отданного таймера.

**N-4 — `Stop` при истёкшем сроке без обработчиков.** Закрыто, с N-7.
- Неблокирующая проверка `idle` перед `select` со сроком.
- Тест детерминирован для верного кода:
  - `idle` закрывается под `d.mu` до первого `select`;
  - `returned` не готов, потому что сценарий держит подписку до `release`;
  - `Stop` всегда возвращает `nil`, ложного красного на CI нет.
- M13 ловится с вероятностью 1 − 2⁻²⁰.
- Регрессию «`Stop` ждёт транспорт без срока» (R3) тест ловит только зависанием пакета — N-7.

**Mi-3 — настенные таймеры в replay.** Закрыто.
- `TestReplaySubscribesAgainOnTheWallClock` ждёт вторую подписку через одну паузу настенных часов, ручные таймеры теста не двигаются. R2 красный (`t473i2-mutants.log`).
- Комментарий `Config.Timers` дополнен условием system-architect#2 (C-01 v1.12).
- Под `-race` пауза 1 с укладывается в срок ожидания 15 с с запасом.

**Mi-4 — длительность integration-теста.** Закрыто, с Mi-5.
- 164,7 с вместо 195–206 с. `AnEventThatKeepsFailing…` — 0,76 с: «остановка сразу после `Start`» перенесена в конец, а третий потребитель `groupRestart` останавливается после входа (событие-сигнал `sentinel`).
- Ожидание в `hungBroker` по условию с проверкой `Stop ≥ StopTransportGrace` прошло один раз. Но в прогоне условие фактически не ждёт — Mi-5.

**N-5 — причина флака `groupRestart`.** Закрыто.
- Причина верна: `Subscribe` коммитит офсет после обработчика (`deliver` → `CommitMessages`), и курсор эффектов `gateway.db` может оказаться впереди.
- Ожидание `committed == base+2` через `OffsetFetch` перед `stop(first)` снимает окно: `base` — офсет события 1, зафиксированный офсет — следующий к чтению.
- `t.Fatalf` внутри условия `within` вызывается в горутине теста — допустимо.

**N-2, N-3.** Закрыты.
- N-2: ожидания каналов — `receive` со сроком 15 с.
- N-3: полный лог M0–M13, R1, R2 сохранён, исправления M9/M11 описаны.

**N-1.** Передано architect#3 строкой в «Передать» карточки. Со стороны исполнителя закрыто.

**Регрессия от исправлений.**
- `wait` и `Stop` меняют только выбор при одновременной готовности веток. Порядок `cancel` → `stopping` → обработчики → grace сохранён.
- Состояние по-прежнему меняется только под `d.mu`.
- Новые горутины в тестах завершаются.
- Пересечений с чужими файлами нет.

### Замечания ревью #1 — статус

| # | Серьёзность | Статус |
|---|---|---|
| Ma-1 | Major | закрыто |
| Mi-1 | Minor | закрыто: вывод ревью #1 ошибочен, описание README принято; узкий случай — N-6 |
| Mi-2 | Minor | закрыто |
| Mi-3 | Minor | закрыто |
| Mi-4 | Minor | закрыто; новая проверка — Mi-5 |
| N-1 | Nit | передано architect#3 |
| N-2 | Nit | закрыто |
| N-3 | Nit | закрыто |
| N-4 | Nit | закрыто; тест — N-7 |
| N-5 | Nit | закрыто |
...
| # | Серьёзность | Файл:строка | Замечание | Предложение | Статус |
|---|---|---|---|---|---|
| Mi-5 | Minor | `internal/gateway/consumer/redpanda_integration_test.go:913-935` | Ожидание «по условию» фактически фиксированное: 3 с от момента, когда вернулся `Container().Start`. Подписки кончаются уже при остановке брокера, и к возврату `Start` `s.d.Err()` не `nil`; в прогоне итерации 2 — «a subscription was down 0s after the restart of the broker» (`t473i2-run.log`, строка 72). Комментарий в коде («once a subscription is seen down, the first pause and two seconds … put Stop inside it») этот случай не описывает. Тогда проверка `took ≥ StopTransportGrace` зависит от фазы reader'ов через 3 с после запуска. Если все reader'ы в этот момент в паузе kafka-go `JoinGroupBackoff` 5 с (`consumergroup.go:755-764`, выход по `cg.done`) или в паузе `Dispatcher`, `Stop` возвращается быстро и тест красный. По чтению фаза не гарантирована, прогон один. Моё предложение в Mi-4 ревью #1 было неполным. Воспроизвести без интеграционного прогона нельзя. | Ждать доказательства удержанного входа, а не времени. Вариант: `kafka.Client.DescribeGroups` для `consumer.Group`, пока `GroupState` не станет `PreparingRebalance` (брокер держит `JoinGroup`), в пределах `resubscribeLimit`, затем `Stop`. Если состояние не наблюдалось, писать строку `measure` вместо `t.Errorf`. Можно сделать вместе со строкой EPIC-004 после T-477, когда пересматриваются `resubscribeLimit` и замер шага 8. | открыто, не блокирует |
| N-6 | Nit | `internal/gateway/consumer/dispatcher.go:491-499`; `internal/gateway/README.md:132-143`; `redpanda_integration_test.go:982-984` | README: «`Close` его не ждёт». Это верно, если `Stop` получил живой срок. Если срок к вызову уже истёк, `Stop` возвращается раньше, чем подписки снимут reader'ы с учёта, и `Close` сразу после него ждёт вход. Зонд: 4,8 с на молчащем брокере, 6 из 6; с паузой 20 мс — 0 с. В процессе окно почти закрыто: после `dispatcher.Stop` шлюз ещё останавливается, а срок уже исчерпан. `stopRightAfterStart` только пишет замер `Close`, но не проверяет его. | README: оговорка «кроме `Stop` с уже истёкшим сроком». В `stopRightAfterStart` — `t.Errorf`, если `Close` дольше `StopTransportGrace`. Устранение в адаптере (например, `Close` не ждёт reader'ы, чья подписка уже отменена) — вопрос system-architect в T-477 или бэклоге. | открыто |
| N-7 | Nit | `internal/gateway/consumer/resubscribe_test.go:594` | `d.Stop(expired)` вызывается без сторожа. Мутант R3 (grace без `ctx.Done()`) вешает пакет до `-timeout` go test: по умолчанию 10 мин, вместо понятного падения. Тот же класс, что N-2. | `go func() { stopped <- d.Stop(expired) }()` и `receive(t, stopped, …)`. | открыто |
| N-8 | Nit | `internal/gateway/README.md:152-154` | «процесс, запущенный после сбоя, — через 14,3–14,9 с после своего `Start`» — цифра итерации 1. В итерации 2 то же окно дало 27,9 с после `Start`: потребитель стартует раньше. Величина от `Start` зависит от момента запуска в тесте и вводит в заблуждение. Устойчива величина от запуска брокера — 32,3–32,9 с. | Оставить только время от запуска брокера или привести обе цифры с пояснением. | открыто |

### Предложения в бэклог (вне границ T-473)
1. **T-477 / system-architect.** `Kafka.Close` при reader'е, чья подписка отменена, но ещё не сняла его с учёта, ждёт вход в группу (N-6). Варианты: снимать reader с учёта по отмене `loopCtx` (`context.AfterFunc`) или ограничить ожидание `r.Close()` в `Close`.
2. **Строка EPIC-004 после T-477** (п. 2 system-architect#2): вместе с `RestoredAfter` и `resubscribeLimit` переделать ожидание `hungBroker` (Mi-5).

### Риски и допущения
- `-race` не запускался: cgo выключен. Детерминизм тестов Mi-2 и N-4 разобран чтением и проверен 200 проходами при `-cpu=1,4`.
- Mi-5 выведен из чтения kafka-go и одной строки лога прогона, без интеграционного прогона ревьюера. Вероятность флака не оценена численно.
- Зонд N-6 заменяет `JoinGroup` Redpanda молчащим `FindCoordinator` (срок 5 с). Механизм — синхронный запрос в `cg.run`, который ждёт `r.Close()`, — тот же. Длительность на Redpanda — до `RebalanceTimeout` плюс 5 с, по чтению.
- Ревьюер в рабочей папке добавил только этот раздел `review.md` и строки ревью в карточке. Файлы в scratchpad — с префиксом `t473r2-`; копия дерева, зонд, скрипт мутанта и резервный файл удалены по точному пути. Docker, стенд `:8888` и `.env` не трогались.

## T-480 · ревью #1 · 2026-09-14 · code-reviewer#2 (TEAM-3)

### Границы ревью
- Ветка `task/T-480-fakegateway-stop-grace` (`.worktrees/T-480`), база `df47003`. Изменения не закоммичены.
- Код: `internal/gateway/gatewaytest/gatewaytest_test.go` (длительности фаз в сообщении), `internal/gateway/consumer/stop_membus_test.go` (новый). Код платформы и membus не менялись.
- Сверено с карточкой (DoD под объём «диагностика + защитный тест», решение оркестратора 2026-09-14), `dev-log.md`, `consumer/dispatcher.go` (`Stop`, `StopTransportGrace`), `gatewaytest/gatewaytest.go` (`Close`, `removeDir`), `internal/gateway/context.go` (`Stop`), логами исполнителя `t480-*.log`. Раздела `### T-480` в `tasks.md` нет, его пишет tech-lead.

### Вердикт
**Принять.** Blocker 0 · Major 0 · Minor 1 · Nit 1.

Вывод «grace T-473 не причина» доказан чтением кода. `FakeGateway.Close` вызывает `g.ctx.Stop(context.Background())` без срока, шлюз работает на `clock.Manual`. Если бы `Stop` ждал grace, `Close` висел бы, а не занимал 2,3 с. Защитный тест детерминирован, мутант ловит по сроку и не вешает пакет. Печать фаз корректна, порог 1 с не изменён, гонок нет: все замеры в одной горутине теста.

### Проверено ревьюером (go1.26.8 windows/amd64, cgo выключен — без `-race`)
- `go vet ./internal/gateway/...` — 0.
- `go test -count=20 -cpu=1,4 -run "Stop" ./internal/gateway/consumer/` — `ok`, 49,1 с. Флака нет.
- `go test -count=5 -v -run TestAFakeGatewayStartsAndStopsWithinASecond ./internal/gateway/gatewaytest/` — 5 `PASS`, 0,04–0,11 с.
- `golangci-lint run ./internal/gateway/...` — `0 issues.` Время в новых тестах берётся только через `testkit.Wall()`, `testkit.After`, `clock.RealTimers{}` и `clock.Real{}` (forbidigo).
- **Мутант.** Копия `scratchpad/t480r-mut` без `.git`/`Docs`/`services`, контрольный прогон первым, `-timeout 120s`. Копия и скрипт удалены по точному пути. Лог — `scratchpad/t480r-mutants.log`.

  | # | Мутант | Итог |
  |---|---|---|
  | K | контрольный | зелёный: 7 тестов `Stop`, 0,01–0,27 с |
  | G | «ждать grace всегда»: из второго `select` в `Stop` убрана ветка `case <-returned: return nil` | **красный**. `TestStopOverMembusDoesNotWaitForTheGrace` — за 15,02 с, сообщение «timed out waiting for Stop over membus to return while the clock stands», паники по `-timeout` нет. `TestStopOverMembusOnTheWallClockTakesLessThanTheGrace` — за 2,02 с, «took 2.0008187s». Тесты T-473 зелёные, три из них по 10 с (срок `stopWithin`): они этот мутант не ловят, и новые тесты закрывают этот пробел. |

### Замечания

| # | Серьёзность | Файл:строка | Замечание | Предложение | Статус |
|---|---|---|---|---|---|
| Mi-1 | Minor | `internal/gateway/gatewaytest/gatewaytest.go:53-56, 213-225`; `gatewaytest_test.go:83-93`; карточка, «Итог» п. 2–3 | Причина «нагрузка» правдоподобна, но не доказана. Фазовая печать не отличит от неё механизм, который даёт ровно такие цифры. (1) Замеры зонда (`dispatcher.Stop` 0 с, `Start` 0,38–0,8 с при 16 процессах) в `t480-*.log` не сохранены: `t480-load*.log` содержат только строки `ok` пакетов, и цифры карточки проверить нельзя. (2) Кандидат, не упомянутый в карточке, — `removeDir`: до 50 попыток `os.RemoveAll` с паузой 100 мс по настенному времени. Если на Windows под нагрузкой индексатор или антивирус держит `-wal`/`-shm` (ровно тот случай, ради которого написан повтор), 20–23 попытки дают 2,0–2,3 с. Это совпадает с падениями 2,28–2,5 с. Без нагрузки зонд показал каталог 1–2,5 мс, а под нагрузкой время каталога в карточке не приведено. Новое сообщение отнесёт такое время к `Close` целиком, вместе с `srv.Stop`/`ctx.Stop`, и следующее падение фазу внутри `Close` не назовёт. | Не блокирует. В карточке, в «Рисках», упомянуть `removeDir` как второй чувствительный к нагрузке участок `Close`. По желанию считать попытки удаления, например счётчиком за хуком в `export_test.go`, как у `WatchBuses`, и добавить их в сообщение теста: тогда «Close 2,2 с, удаление каталога 21 попытка» отличимо от «Close 2,2 с в `ctx.Stop`». | открыто |
| N-1 | Nit | `internal/gateway/gatewaytest/gatewaytest_test.go:90-92` | `wall.Now()` вызывается дважды: для `took` и для фазы `Close`. Сумма фаз в сообщении чуть больше `took`. | `ended := wall.Now()` один раз, из него `took` и `ended.Sub(closing)`. | открыто |

### Предложения в бэклог (вне границ T-480)
1. Если порог 1 с продолжит краснеть под нагрузкой, отделить в `TestAFakeGatewayStartsAndStopsWithinASecond` бюджет «поднимается и опускается» от бюджета удаления каталога. Удаление с повтором — свойство Windows-ФС, а не шлюза.

### Риски и допущения
- `-race` не запускался: cgo выключен. Гонок при замере нет по чтению: `began`/`started`/`closing` пишутся и читаются в одной горутине.
- Гипотеза про `removeDir` в Mi-1 выведена чтением, без воспроизведения под внешней нагрузкой: ревьюер нагрузку testcontainers не создавал.
- Ревьюер добавил в рабочую папку только этот раздел `review.md` и раздел «Ревью» в карточке. В scratchpad остался лог `t480r-mutants.log`. Docker, стенд `:8888` и `.env` не трогались.

## T-478 · ревью #1 · 2026-09-14 · code-reviewer#3 (TEAM-3)

### Границы ревью
- Ветка `task/T-478-turn-after-mechanics` (`.worktrees/T-478`), база `23a38c7`, изменения не закоммичены. Кончик эпика — `df47003` (T-473 и develop с EPIC-002).
- Изменённые файлы: `internal/gateway/turns/{tracker.go, tracker_test.go, helpers_test.go}`, `test/e2e/gateway_world_test.go`, КД `gateway-and-bot.md` §7.7, `dev-log.md`, карточка T-478. Файлов T-473 и чужих путей в изменениях нет. Контракты, схемы и миграции не менялись.
- Сверено с: `### T-478` в `tasks.md`; «Решения architect#3» в карточке T-313 (`epic/EPIC-004-gateway-bot`); КД §5.5, §7.2, §7.6, §7.7; C-10 и `metrics.md` (строка `analytics.turn.completed`, примечания о таймингах); `api-contracts.md` §1.4.
- Прочитан код, на который опирается правило:
  - `consumer/handle_{combat,entity,narrative}.go` и `dispatcher.go` (`Handle`, `commit`);
  - `handlers/deliveries.go` (`onDelivered`) и `outbox/store.go` (`Ack`);
  - `actions/validate.go` (`Rules`), `turns/store.go` (`Completed`);
  - `context.go` (`restore`), `restore_test.go`;
  - `internal/state/facts.go` на `df47003` (`updatedFact`, `rejectedFact`), `shared/eventbus/kafka.go` (одна партиция на топик).
- Два уточнения §7.7 и вопрос C-10 замечаниями не считаются, по ним дана оценка риска: они на просмотре у architect#3.

### Прогоны
Windows, go1.26.8, без `-race` (нет cgo).
- `go build ./... && go vet ./... && go vet -tags e2e ./test/e2e/...` — ок.
- `go test -short -count=3 ./internal/gateway/turns/... ./internal/gateway/actions/...` — ок.
- `golangci-lint run ./...` — 0 issues (с `--allow-parallel-runners`).
- `go test -tags e2e -count=4 -cpu=1,2 -run TestGateway ./test/e2e/...` в рабочей папке, четыре прогона:
  - **прогон 1 — красный**, но не на сравнении журналов. Шёл одновременно с распаковкой двух копий в scratchpad и с `go test -tags e2e ./...` соседней папки. `TestGatewayDeath`, второй прогон: `POST /v1/players/…/actions: attempt 1: … EOF` на ходу 12. `TestGatewaySolo30`, второй прогон: `p95 of 202 is 1.1s, want at most 100ms` (max 2,46 с). Оба падения — проверки T-313 (клиент и бюджет `202`), путей T-478 они не касаются (`actions` не менялся);
  - прогон 2, без моих фоновых задач (соседний e2e ещё шёл) — ок, 208 с;
  - прогон 3, параллельно цикл `go test -count=20 ./internal/state/...` (10 проходов, все ок) — ок, 183 с;
  - прогон 4, `-count=2 -cpu=1,2`, параллельно цикл `go test -short -count=1 -p 16 ./internal/state/... ./internal/mechanics/... ./internal/gateway/... ./shared/eventbus/... ./shared/testkit/...` (5 проходов) — ок, 91 с. Один проход нагрузки завершился с ошибкой; лог перезаписан следующим проходом, пакет не установлен. Три повтора того же набора отдельно и `-count=5` пакетов `turns`, `consumer`, `handlers` и корня шлюза — зелёные.
- Сравнения журналов («the journals differ») и проверки механики красными не были ни разу: 28 прохождений каждого теста при GOMAXPROCS 1 и 2, из них два в прогоне 1 оборвались до сравнения.
- **Пробное слияние** (копия `t478r-merged`: `git archive df47003` и файлы кода T-478):
  - файлы кода T-478 и КД между `23a38c7` и `df47003` не менялись (`git diff --stat`), так что пересечений с T-473 (`consumer/*`, `context.go`, README шлюза) нет;
  - `go build ./... && go vet ./... && go vet -tags e2e ./test/e2e/...` — ок;
  - `go test -short -count=1 ./internal/gateway/...` — ок, 16 пакетов;
  - `-tags e2e -count=1 -run TestGateway -v` — ок, 22 с: смерть на 46-м ходу, p95 `202` ≈ 1,5 мс;
  - `golangci-lint` по `internal/gateway`, `test/e2e` — 0 issues;
  - `consumer.Cursors` на кончике есть (`dispatcher.go:667`).

Мутанты и зонд — в копии `t478r-probe` (`23a38c7` + файлы кода T-478, без `-overlay`, совпадение с рабочей папкой проверено `diff -rq`). Контрольный прогон — первым.

| # | Мутант / зонд | Результат |
|---|---|---|
| 0 | контрольный, без изменений | unit `turns` зелёный |
| 1 | `OnDelivered` без предела `delivered_count < recipients_count` | красный `TestATurnWaitingForItsMechanicsCountsItsDeliveriesUpToItsRecipients` |
| 2 | `completeDelivered` перезаписывает `narrative_at` при каждом ack (без `row.NarrativeAt == nil`) | красный тот же тест |
| 3 | `Sweep` засчитывает доставленному ходу без механики `turns_failed + 1` | красный `TestADeliveredTurnWithoutItsMechanicsCompletesAtItsDeadline` |
| 4 | зонд-тест: `attack` доставлен и ждёт механику, публикация `turn.completed` из `OnMechanics` падает | зелёный: `ErrPublish`, после отката строка `narrated` и `mechanics_at IS NULL`, счётчики сессии не тронуты; повтор публикует один `turn.completed` с тем же id |

### Проверка по пунктам задания

**1. Предикат Phase 1 и тип механики.**
- Таблица `phase1` (`tracker.go:98-104`) совпадает со столбцом «Phase 1» §1.4. Без Phase 1 — `look`, `say`, `defend`.
- `OnMechanics` (`tracker.go:309-327`) отсекает ходы чужого типа условием `action_type NOT IN (…)`. У `look`/`say`/`defend` запись прежняя: первое `combat.decided` или `entity.updated`.
- *Засада при `enter`.* Факт перемещения публикует State по предложению шлюза раньше любой реакции роя: `encounter.started` идёт после факта, который он объявляет (C-05 v1.4 п. 4). У топика одна партиция (`kafka.go:41-43`), поэтому первое `entity.updated` корреляции — перемещение игрока. `combat.decided` засады в ход `enter` не пишется и его не завершает. Гонки нет.
- *Отказ State.* Такого типа, как `entity.refused`, нет. Отказ приходит как `entity.update.rejected` и обрабатывается `OnRejected` (`handle_entity.go:95-122`), который ход не трогает. Ход с доставленным нарративом ждёт дедлайн (60 с) и закрывается `ok`/`degraded` с `Warn`. Так решил architect#3, это не замечание; предложение — в бэклоге.
- *`rest` во встрече* отклоняется предусловием (`Rules[rest].Encounter = Outside`): `4xx`, `status=rejected`, ожидания нет. Отказ State (C-02 v1.1) возможен только в гонке проекции, например `/rest` сразу после `enter` с засадой. Это случай «отказ State» выше.
- *`rest` при полном HP* не зависает: State публикует `entity.updated` и с пустым `changed` (`internal/state/facts.go`, `updatedFact`).
- *Штатные сценарии I1* до дедлайна не висят. У всех ходов с Phase 1 в `solo-30`, `death` и `flee-fail` механика записана во всех прогонах: новая проверка `gatewayAssertMechanics` («Phase 1 с доставленным нарративом — с механикой») зелёная.

**2. Транзакции.**
- `OnDelivered` пишет `narrative_at` в транзакции ack (`completeDelivered`, `tracker.go:418-424`). `OnMechanics` вызывает `completeDelivered(…, nil)` в транзакции consumer'а. `finish` больше не трогает `narrative_at`.
- Откат при сбое публикации проверен зондом 4. Unit-теста на это нет — Mi-1.
- *Идемпотентность.*
  - Повтор механики после завершения: `status IN (open)` → 0 строк, `completeDelivered` не вызывается.
  - Повтор ack завершённого хода: `UPDATE` по `status = narrated` → 0 строк, `completeDelivered` выходит на проверке статуса.
  - Повтор механики у ждущего ack хода: `COALESCE` полей не меняет, `completeDelivered` выходит на `delivered < recipients`.
  - Тест 4 проверяет повтор того же `combat.decided` и чужого факта.
- *«Механика раньше ack».* Порядок §5.5 не менялся: `actions` T-478 не трогает, `Accepted` по-прежнему перед `player.*`. Поведение при механике до ack — тест 5.
- *`Sweep`* (`tracker.go:439-473`):
  - `delivered(row)` — `narrated`, `delivered ≥ recipients`, `narrative_at` записан. Такой ход получает `narratedOutcome`: `Degraded: 1` у шаблона, иначе ничего.
  - Прочие ходы получают `timeout` и `Failed: 1`.
  - `turns_count` меняют только `Accepted`/`Withdrawn`, T-478 его не трогает.
  - `Warn` пишется после коммита и только у доставленного хода.
  - Мутант 3 красный.

**3. Предел `recipients_count`.**
- Условие в `UPDATE` (`tracker.go:401-403`) защищает `delivered_count`, а `narrative_at` защищает проверка `row.NarrativeAt == nil`. Мутанты 1 и 2 ловит один и тот же unit-тест, детерминированно.
- В e2e мутант ловится в 1 из 2 прогонов из-за самой гонки: ack второго нарратива и чтение механики из другого топика. Детерминировать это в e2e можно только искусственной паузой consumer'а. Unit-теста достаточно, усиливать e2e не нужно.
- В карточке («уточнение 2») сказано, что без предела «`narrative_at` сдвинулся бы». В коде `narrative_at` не сдвинулся бы: его держит отдельная проверка (мутант 2). Предел защищает только `delivered_count`. На код это не влияет, но формулировку для architect#3 стоит уточнить.

**4. Совместимость с T-308 и T-309.**
- `Acked` обновляет `acked_at` и у ждущего хода (`status IN (…, narrated)`). Ход с Phase 1 теперь чаще публикуется с моментом ответа `202`, а не с моментом записи. Это правильнее и с §5.5 п. 6 совпадает.
- `Withdrawn`: см. N-3. Факт чужого типа больше не переводит ход `accepted → mechanics_applied`, и защита T-308 «ход, у которого есть механика, остаётся» в окне §5.5 чуть уже.
- `restore` (T-309) вызывает `Sweep` до догона журнала. Доставленный ход, ждавший механику, закроется без неё, даже если механика лежит в непрочитанном журнале. До T-478 тот же ход закрылся бы по ack и тоже без механики, так что потеря та же. Это риск, а не регрессия.
- Лог `restore: turns timed out` (`context.go:776`, файл T-473) теперь неточен — N-2, в бэклог после слияния T-473.

**5. e2e.**
- `gatewayRaceFields` и её обход удалены. Поля механики сравниваются между прогонами как опубликованы.
- Mi-3 (`gateway_world_test.go:854-880`): принадлежность проверяется при `len(narratives) > 0`, у хода без нарратива поле запрещено, замена — только при `> 1`.
- N-5 (`:939-949`): сравнение `time.Time`, граница `[received_at, received_at + DefaultTimeout]`, комментарий называет проверку так, как она выполняется.
- N-6 (`:362`): `consumer.Cursors`.
- Флак — см. «Прогоны».

**6. Слияние с `df47003`** — см. «Прогоны». Файлы кода T-478 (`turns/*`, `test/e2e/gateway_world_test.go`) и КД между `23a38c7` и `df47003` не менялись, поэтому текстовое слияние бесконфликтное. Общие с T-473 только `dev-log.md`/`review.md` (`appendtail`).

### Вердикт
**Принять.** Critical (Blocker) 0 · Major 0 · Minor 1 · Nit 6.

Mi-1 и N-1, N-3…N-6 — правки тестов, комментариев и формулировки в карточке на несколько строк. Их можно сделать до слияния без повторного ревью или перенести в T-479: та задача правит те же `turns.finish` и `tracker_test.go`. N-2 — после слияния T-473.

### Замечания

| # | Серьёзность | Файл:строка | Замечание | Предложение | Статус |
|---|---|---|---|---|---|
| Mi-1 | Minor | `internal/gateway/turns/tracker_test.go` (новые тесты); `tracker.go:326` | **Новый путь отказа не покрыт тестом.** `OnMechanics` теперь публикует `turn.completed` в транзакции consumer'а, а до T-478 этот шаг не публиковал ничего. Сбой публикации откатывает эффект `combat.decided`/`entity.updated` целиком, вместе с доставкой механики игроку, и уходит в повторы шины. Зонд 4 показал, что код ведёт себя верно: строка `narrated`, `mechanics_at IS NULL`, счётчики сессии не тронуты, повтор публикует один `turn.completed` с тем же id. Но регрессию здесь (двойной счёт `turns_degraded`, частичная запись механики без публикации) не поймает ни один тест. Отказ публикации сейчас проверяется только у `Sweep` (`outbox_steps_test.go:106`). | Unit в `tracker_test.go`: `attack` доставлен, хук `f.bus.set` отказывает `analytics.turn.completed` → `OnMechanics` = `ErrPublish`; после `Rollback` строка `narrated` без механики, `turn.completed` нет. Хук снят, повтор → ровно одно событие с id неудачной попытки, `turns_degraded` у шаблона = 1. Сценарий — зонд 4 выше. | открыто |
| N-1 | Nit | `internal/gateway/turns/tracker.go:9` (doc пакета) | «A turn past its deadline times out» — теперь неверно для хода с доставленным нарративом без механики. godoc `Sweep` уже точный. | «A turn past its deadline times out, unless its narrative reached every recipient and only its mechanics is missing (Sweep).» | открыто |
| N-2 | Nit | `internal/gateway/context.go:776` (файл T-473) | `log.Info("restore: turns timed out", count)`: `Sweep` теперь возвращает и ходы, завершённые `ok`/`degraded` без механики. | После слияния T-473: «restore: turns past their deadline closed». Передать вместе с правкой README шлюза (бэклог разработчика). | в бэклог |
| N-3 | Nit | `tracker.go:313-319` (`OnMechanics`), `:242-243` (`Withdrawn`) | Факт чужого типа (например, `entity.updated` урона у `attack`, прочитанный раньше `combat.decided`) больше не переводит ход `accepted → mechanics_applied`. `Withdrawn` удаляет строку по `status = accepted AND mechanics_at IS NULL`. В окне §5.5 (kafka, брокер записал `player.*`, подтверждение потерялось) ход, по которому уже пришёл факт его цепочки, теперь может быть забран. Последствие по §5.5 — только аналитика, окно узкое. | Либо в §5.5 «Известное поведение» дописать, что чужой по §7.7 факт ход не удерживает. Либо в `OnMechanics` для чужого типа оставить только `status = CASE WHEN status = accepted THEN mechanics_applied …` без записи полей: `completeDelivered` смотрит на `mechanics_at`, а не на статус. | открыто |
| N-4 | Nit | `test/e2e/gateway_world_test.go:919-924` (`gatewayAssertMechanics`) | Проверка принимает у любого действия и `combat.decided`, и `entity.updated`. Уточнение 1 §7.7 (`attack`/`flee` → решение, `enter`/`leave`/`rest` → факт) e2e ловит только косвенно, сравнением двух прогонов, и только если гонка проявилась. | Сверять `mech.Type` с типом по действию для ходов с Phase 1 (своя таблица теста, как `gatewayWithPhase1`) — детерминированный оракул. | открыто |
| N-5 | Nit | `test/e2e/gateway_world_test.go:780-786` (`canonical`) | Набор нарративов корреляции собирается по `ev.ID`, а ход называет `payload.narrative_event_id`, если он есть (`tracker.go:349-352`; C-05 — общий id у раунда группы). Сейчас `FakeNarrator` ставит его равным `ev.ID` (`fake_narrator.go:814`), поэтому проверка Mi-3 верна. С роем или в групповом e2e (T-314) она даст ложное падение. | Класть в набор `payload.narrative_event_id`, если он есть, иначе `ev.ID`. | открыто |
| N-6 | Nit | карточка T-478, «Уточнения», п. 2 | Обоснование «без предела `narrative_at` сдвинулся бы» не совпадает с кодом: `narrative_at` держит проверка `row.NarrativeAt == nil` (мутант 2), а предел держит только `delivered_count` (мутант 1). | Уточнить формулировку для architect#3 при ответе на ревью. Код не менять. | открыто |

### Оценка риска уточнений §7.7 и вопроса C-10 (не замечания; для architect#3)

**Уточнение 1 — механика хода — событие своего типа.**
- *Что снимает.* `result_event_id` у боя больше не зависит от порядка `game_events` и `system_events`. Тип совпадает с тем, что игрок видит как результат (§7.2), и с `metrics.md` («`mechanics_at` — получение события Phase 1 `combat.decided` / `entity.updated`»).
- *Цена.* Шлюз завязан на тип механики по действию. Если настоящий рой EPIC-003 решит действие другим событием (например, `flee` без `combat.decided`, только `encounter.ended` и факт State), ход с доставленным нарративом будет ждать дедлайн: `turn.completed` на 60 с позже, без полей механики, с `Warn` на каждый ход.
- *Оценка.* Для I1 риск низкий: `FakeEncounter`/`FixedMechanics` и State дают нужные типы, e2e это проверяет. При замене заглушек на рой — средний. Митигировать можно проверкой «каждое действие с Phase 1 даёт событие своего типа» в contract-тесте роя и T-352 (`group.leave` → `combat.decided`).

**Уточнение 2 — доставки считаются до `recipients_count`.**
- *Что снимает.* `delivered_count` хода смерти больше не зависит от того, пришла ли механика раньше ack второго нарратива.
- *Цена.* Счёт идёт по числу, а не по нарративу. Ack доставки второго нарратива корреляции, пришедший раньше ack нарратива хода, засчитывается за нарратив хода.
- *Оценка.* В соло I1 очередь игрока FIFO, а смерть идёт после текста хода (C-05 v1.4 п. 8), так что риск низкий. В группе I2 у нарративов корреляции разные наборы адресатов, и `narrative_at` может оказаться раньше фактической доставки нарратива хода. Это та же проблема, что в вопросе C-10, и вариант (б) закрывает её.

**Вопрос C-10 — какой нарратив хода, если их несколько.**
- (а) «Первый по офсету»: детерминирован для журнала, но поля `narrative.*` и `recipients_count` зависят от порядка публикации роя. С настоящим роем e2e снова начнёт мигать на `generated_by`.
- (б) «По `kind`» — согласен с рекомендацией разработчика. Контракт не меняется. Нужна гарантия C-05, что у хода соло ровно один нарратив `turn`/`entry`, а у раунда — `round`, и фолбэк на первый нарратив.
  - Реализация трогает сигнатуру `Tracker.OnDelivered`: нужен `narrative_event_id` доставки, который `handlers.onDelivered` уже держит в `outbox.Delivery.Data`. Интерфейс внутренний.
  - Вместе с (б) естественно закрыть и уточнение 2: считать доставки только нарратива хода, а не до числа.
- (в) и (г) сложнее и в случае (г) меняют смысл полей C-10.
- Делать после T-479: обе задачи правят `turns`.

**Риск правила §7.7 «`OnMechanics` публикует в транзакции consumer'а»** (исходное решение architect#3, не уточнение):
- Сбой публикации аналитики теперь отказывает эффект `combat.decided`/`entity.updated`. Шина повторяет его (`DefaultBackoff` 100 мс, 500 мс, 2 с), каждая попытка держит единственное соединение `gateway.db` до `PublishTimeout` = 5 с. После повторов событие уходит в `dead_letters`, и игрок теряет текст механики.
- Именно этот довод §5.5 приводит против ошибок `OnMechanics` («игрок терял бы текст ради счётчика»). Раньше так мог отказать только `OnNarrative` без адресатов.
- Окно — частичный отказ шины: аналитика не пишется, а `game_events` читаются. Для I1 риск низкий. Стоит решить явно: принять и записать в §7.7, или завершать ждущий ход из `OnMechanics` без отказа эффекта, оставив публикацию следующему тику `Sweep`.

### Предложения в бэклог (вне границ T-478)
1. Отказ State по предложению шлюза (`entity.update.rejected`, найденный `OnRejected` по ходу) как сигнал завершения доставленного хода с Phase 1. Тогда ход не ждал бы 60 с и не писал бы `Warn` на штатную гонку `rest`/`enter` (решение architect#3).
2. После слияния T-473: README шлюза (строка `turns/`) и лог `restore` (N-2).
3. Contract-тест роя EPIC-003: каждое действие с Phase 1 даёт механику своего типа по §7.7.
4. Вариант (б) вопроса C-10 отдельной задачей после T-479, вместе с пересмотром уточнения 2.
5. Флак-кандидат T-313 (см. «Риски»): `EOF` на `POST` действия без повтора и p95 `202` под дисковой нагрузкой — наблюдать на CI, при повторе завести задачу на e2e-клиент.

### Риски и допущения
- `-race` не запускался (нет cgo).
- Падение прогона 1 (`EOF` на `POST` действия и p95 `202` = 1,1 с) не воспроизвелось в трёх следующих прогонах, в том числе под более тяжёлой нагрузкой. На базе `23a38c7` его не сравнивал. Путей T-478 оно не касается, но само по себе это кандидат во флак T-313: клиент e2e не повторяет `POST` после сетевой ошибки, а бюджет `202` на 100 мс чувствителен к дисковой нагрузке. Пусть tech-lead#3 проследит на CI.
- На машине параллельно шли прогоны других команд (в том числе `go test -tags e2e ./...` соседней папки). «Чистый» прогон e2e поэтому не изолирован.
- `internal/state/facts.go` и `shared/eventbus/kafka.go` прочитаны на `df47003`. Поведение настоящего роя (EPIC-003) для `flee`/`enter` оценено по C-05 и заглушкам.
- Метаданные карточки называют ветку `task/T-478-turn-waits-mechanics`, фактическая — `task/T-478-turn-after-mechanics`. Это для tech-lead#3 при слиянии.
- Копии `t478r-merged` и `t478r-probe`, скрипт мутантов и логи `t478r-*` лежат в scratchpad и удаляются по точным путям. В рабочей папке ревьюер изменил только `review.md` (этот раздел). Карточку, Docker, `.env` и стенд `:8888` не трогал.

## T-478 · ревью #2 · 2026-09-14 · code-reviewer#3 (TEAM-3)

### Границы ревью
- Итерация 2. Ветка `task/T-478-turn-after-mechanics` (`.worktrees/T-478`), база `23a38c7`, изменения не закоммичены. Кончик эпика — `711b941` (T-473, T-480, develop).
- Проверены исправления итерации 2 и регрессия от них:
  - `turns/tracker.go`: `completeByMechanics`, `due`, `Sweep`, doc пакета;
  - новый тест `TestACompletionTheBusRefusesLeavesTheEffectOfTheMechanics`, `helpers_test.go` (`options.log`);
  - `test/e2e/gateway_world_test.go` (N-4, N-5);
  - КД §7.7 (фраза «итерация 2 T-478, на подтверждение architect#3»);
  - карточка (N-6, имя ветки), `dev-log.md`.
- Изменения только в файлах задачи. Контракты, схемы и миграции не менялись.
- Прочитан код, на который опирается решение:
  - `consumer/dispatcher.go` (`commit`) в рабочей папке и на `711b941`;
  - `consumer/handle_{combat,entity,narrative}.go`, `outbox/store.go` (`Ack`), `handlers/deliveries.go` (`Ack`, `onDelivered`);
  - `store/open.go`, `session/manager.go` (`CountTurn`), `context.go` (`sweep`, `restore`);
  - миграция `0001_init.sql` (индексы `turns`).

### Прогоны
Windows, go1.26.8, без `-race` (нет cgo).
- Рабочая папка:
  - `go build ./... && go vet ./... && go vet -tags e2e ./test/e2e/...` — ок;
  - `go test -short -count=3 ./internal/gateway/turns/...` — ок;
  - `go test -tags e2e -count=2 -cpu=1,2 -run TestGateway ./test/e2e/...` — ок, 91 с;
  - `golangci-lint run ./...` — 0 issues (`--allow-parallel-runners`).
- **Пробное слияние** (копия `t478r2-merged`: `git archive 711b941` без `services`/`Docs` и четыре файла кода T-478):
  - файлы кода T-478 и КД между `23a38c7` и `711b941` не менялись. На кончике изменились только `consumer/dispatcher.go` и тесты `consumer` (T-473, T-480). `commit` по-прежнему выполняет эффекты в одной транзакции `*sql.Tx`, других точек сохранения в коде шлюза нет;
  - `go build ./...`, `go vet ./internal/gateway/... ./test/e2e/...`, `go vet -tags e2e ./test/e2e/...` — ок;
  - `go test -short -count=1 ./internal/gateway/...` — ок, 16 пакетов;
  - `go test -tags e2e -count=1 -run TestGateway -v ./test/e2e/...` — ок, 22 с (`Death`, `FleeFail`, `Solo30`);
  - `golangci-lint` по `internal/gateway`, `test/e2e` — 0 issues;
  - текстовое слияние бесконфликтное: общие с кончиком файлы — только `dev-log.md`/`review.md` (`appendtail`).

Мутанты и зонды — в копии `t478r2-probe` (`23a38c7` и файлы кода T-478; `diff -rq` по `internal`, `test`, `shared` с рабочей папкой пуст; без `-overlay`). Контрольный — первым. Штатный набор — `go test -short -count=1 -skip T478R2 ./internal/gateway/turns/`, зонды — отдельно.

| # | Мутант / зонд | Штатный набор | Зонды |
|---|---|---|---|
| 0 | контрольный | зелёный | зелёные |
| 1 | `due` без `delivered_count >= recipients_count` | зелёный | зелёные — **эквивалентный**: `narrative_at` пишет только `completeDelivered` после проверки доставок, так что `narrative_at IS NOT NULL` уже её содержит |
| 2 | `due` без `mechanics_at IS NOT NULL` | красный `TestADeliveredTurnWithoutItsMechanicsCompletesAtItsDeadline` | красный П2 |
| 3 | без `RELEASE turn_completion` | зелёный — ожидаемо: внутри транзакции `COMMIT` снимает точку сохранения | красный П3 (вызов вне транзакции) |
| 4 | `Sweep` пишет `Warn` «without its mechanics» и готовому ходу с механикой | зелёный — N-9 | зелёные |
| 5 | `Sweep` засчитывает готовому ходу `Failed: 1` | красные `…AtItsDeadline`, `…LeavesTheEffectOfTheMechanics/the_sweeper` | красный П1 |

Зонды (все зелёные на коде задачи):
- **П1.** В одной транзакции: запись до `OnMechanics`, `OnMechanics` с отказом шины, запись после, `processed_events`, коммит. Обе записи есть, строка `narrated` с механикой.
  - `Sweep` при всё ещё отказывающей шине возвращает ошибку и 0, счётчики сессии 0.
  - Шина исправна: `Sweep` = 1, повторный `Sweep` после дедлайна = 0, `turns_degraded` = 1, `turns_failed` = 0, `turn.completed` ровно один.
- **П2.** Отказ шины, затем consumer откатывает транзакцию целиком. Эффекта нет, `mechanics_at IS NULL`, `Sweep` до дедлайна = 0. Повторная доставка того же события завершает ход одним `turn.completed`.
- **П3.** `OnMechanics` с `*sql.DB` вместо транзакции и отказом шины. Механика записана автокоммитом, точка сохранения открыла и `RELEASE` закрыл транзакцию соединения, следующий `BeginTx` проходит, `Sweep` = 1. Без `RELEASE` (мутант 3) соединение остаётся в открытой транзакции.
- **П4** (внутри П1): повторные тики `Sweep` при отказе шины счётчики не накручивают.
- **П5.** Готовый ход (после отказа), затем ack второго нарратива корреляции (смерть) при исправной шине. Ход завершается один раз, `Sweep` после дедлайна = 0, `turns_degraded` = 1.
- **План запроса `due`** (`EXPLAIN QUERY PLAN`): `MULTI-INDEX OR` по `ix_turns_open` — `(status=? AND deadline_at<?)` и `(status=?)`. Полного просмотра `turns` на тике нет.

### Проверка по пунктам задания

**1. `SAVEPOINT` в SQLite.**
- Драйвер — `modernc.org/sqlite` v1.58.0 (`store/open.go:112`), `SetMaxOpenConns(1)`. Точка сохранения внутри `BEGIN` транзакции `database/sql` — штатная возможность SQLite, `database/sql` о ней не знает и не мешает.
- Последовательность `SAVEPOINT` → `ROLLBACK TO` → `RELEASE` верна. `ROLLBACK TO` не снимает точку, поэтому `RELEASE` после него нужен, и он есть на всех путях, кроме отказа самого `ROLLBACK TO`. В этом случае ошибка возвращается и consumer откатывает транзакцию (`dispatcher.go`, `commit`, `defer`).
- Последующие операции транзакции не ломаются: П1 — запись после `OnMechanics`, `processed_events`, коммит.
- Откат транзакции после точки сохранения работает: П2.
- Вложенности нет: `SAVEPOINT` в коде шлюза только здесь (рабочая папка и `711b941`). На одно событие `OnMechanics` вызывается один раз: `OnCombat`/`OnUpdated` — единственные эффекты своего типа. Одноимённая вложенность в SQLite тоже корректна: `ROLLBACK TO`/`RELEASE` берут ближайшую.
- Отмена `ctx` при остановке: `publish` оборачивает и отмену в `ErrPublish`, дальше `ROLLBACK TO` с отменённым `ctx` падает или `database/sql` сам откатывает транзакцию. В любом варианте ничего частичного не фиксируется, событие приходит снова. Остаётся только ложный `Warn` — N-8.
- Вне транзакции (П3) результат тот же, но только без конкурентов. При `*sql.DB` между `Exec` соединение возвращается в пул, чужая запись может попасть в открытую точкой сохранения транзакцию и откатиться `ROLLBACK TO`. При отказе `ROLLBACK TO` соединение остаётся в транзакции. Все вызывающие (`handle_combat.go:20`, `handle_entity.go:33`) передают `*sql.Tx`, так что это не дефект, а неявное требование — N-10.

**2. Идемпотентность и счётчики.**
- Id `turn.completed` выводится из действия (`Completed`, `store.go:131`): попытка в `OnMechanics`, `Sweep` и ack второго нарратива публикуют один и тот же id. Проверено тестом разработчика (`ev.ID == tried[0]`) и П5.
- `finish` выполняется в транзакции вместе с `UPDATE status` и `CountTurn`. После успешного завершения ход не `open`: `Sweep` грузит строку в своей транзакции и выходит на `!open(row.Status)`, повтор механики не проходит `WHERE status IN (…)`. Двойного закрытия нет (тест разработчика, П1, П5).
- Счётчики:
  - отказ в `OnMechanics` откатывает `CountTurn` вместе с завершением (`turns_degraded` = 0 после отказа);
  - неудачный тик `Sweep` откатывает свою транзакцию целиком (П4);
  - успешное закрытие добавляет ровно `narratedOutcome`: `Degraded: 1` у шаблона, `Failed` у готового хода не растёт (мутант 5 красный).
- Если брокер записал событие, но подтверждение потерялось, `Sweep` опубликует тот же id повторно. Это то же поведение, что у ack (503 и повтор), и потребители C-10 дедуплицируют по id.
- **Уточнение.** Путь «повтор механики шиной» в реальном consumer не срабатывает: повторная доставка того же события отсекается `processed_events` (`dispatcher.go`, `commit`), и `OnMechanics` не вызывается. Завершить готовый ход может только следующее событие механики того же типа в корреляции (удар вслед у неудачного бегства), иначе — `Sweep`. Подслучай теста вызывает трекер напрямую и проверяет его идемпотентность, а не путь consumer'а. На код не влияет — N-7.

**3. `Sweep` и гонки.**
- У `gateway.db` одно соединение (`SetMaxOpenConns(1)`). Транзакция consumer'а или ack держит его до коммита, поэтому `due` (через `cfg.DB`) не видит промежуточного состояния «всё пришло, завершение ещё не выполнено»: запрос ждёт коммита.
- Готовое состояние (`narrated` + `narrative_at` + доставки + `mechanics_at`) фиксируется только после отката точки сохранения:
  - последний ack при записанной механике завершает ход в своей транзакции, при отказе откатывается целиком (503);
  - нарратив без адресатов делает то же;
  - механика до полного ack выходит из `completeDelivered` на проверке доставок.

  Лишних закрытий в штатной работе нет.
- Между `due` и транзакцией `Sweep` состояние хода может только продвинуться (завершиться), и это отсекает `!open`. Критерии `due` внутри транзакции не перепроверяются, но это не нужно: мутант 1 эквивалентный, «просрочен» и «готов» не откатываются назад.
- В `restore` (T-309) `Sweep` теперь закрывает и готовые ходы, оставленные отказом до рестарта. Это верно.

**4. Nit ревью #1.**

| # | Итог |
|---|---|
| Mi-1 | **закрыто**: тест с отказом шины, оба подслучая. Мутанты разработчика («отказ возвращается», «без `ROLLBACK TO`») красные |
| N-1 | **закрыто**: doc пакета (`tracker.go:9-11`) |
| N-2 | в бэклог (строка DoD T-479, решение architect#3), как предлагалось |
| N-3 | **закрыто обоснованием**: окно §5.5, последствие только в аналитике, правило T-308 формально соблюдено. Выбор между фразой в §5.5 и `CASE` без полей — за architect#3, для T-478 не блокирует |
| N-4 | **закрыто**: `gatewayMechanicsOf`, сверка `mech.Type` и проверка «Phase 1 с доставленным нарративом — с механикой» (`gateway_world_test.go:926-942`). Мутант разработчика красный детерминированно |
| N-5 | **закрыто**: набор нарративов строится по `payload.narrative_event_id`, иначе по `ev.ID` (`:785-791`), как в `OnNarrative` |
| N-6 | **закрыто**: формулировка уточнения 2 в карточке |

**5. Вопрос 3 разработчика — `OnNarrative` без адресатов.**
- *Когда публикует.* `OnNarrative` с `recipients = 0` публикует `turn.completed` в транзакции consumer'а только у хода без Phase 1 или у хода с уже записанной механикой (`completeDelivered`). У хода с Phase 1 без механики публикации нет, её сделает `OnMechanics` через точку сохранения.
- *Что теряется при отказе.* Откатывается эффект нарратива без доставок, текст игрок не теряет. После повторов шины (`DefaultBackoff`) событие уходит в `dead_letters`, ход остаётся `accepted`/`mechanics_applied` и на дедлайне получает `timeout` и `turns_failed + 1` вместо `ok`/`degraded`. Это искажение аналитики, а не данных игрока.
- *Цена.* Каждая попытка держит единственное соединение `gateway.db` до `PublishTimeout` (5 с), пока шина отказывает: при частичном отказе до ~20 с блокировки ack и `202` на одно такое событие.
- *Частота.* Нарратив без связанных адресатов — редкость: все адресаты без связки или после `/forget`. У `FakeNarrator` в I1 не встречается.
- **Оценка: риск низкий, не регрессия T-478** (поведение T-306). **Рекомендация — не расширять T-478, внести строкой DoD T-479.** T-479 правит `turns.finish` и добавляет в неё закрытие сессии в транзакции вызывающего (§7.7, «Конец сессии»), поэтому ей в любом случае решать, что откатывает точка сохранения. Правка — обобщить `completeByMechanics` до «завершения в точке сохранения» и вызвать её из `OnNarrative`, плюс unit по образцу `TestACompletionTheBusRefusesLeavesTheEffectOfTheMechanics`. Если T-479 откладывается — отдельная задача XS в бэклог EPIC-004.

### Вердикт
**Принять.** Critical (Blocker) 0 · Major 0 · Minor 0 · Nit 4.

Все замечания итерации 2 — комментарии, одна фраза КД и две строки теста. Их можно сделать до слияния без повторного ревью или перенести в T-479: она правит те же `tracker.go`/`tracker_test.go` и §7.7.

### Замечания

| # | Серьёзность | Файл:строка | Замечание | Предложение | Статус |
|---|---|---|---|---|---|
| N-7 | Nit | КД `gateway-and-bot.md:823`; `tracker_test.go:682, 685`; карточка, «Итерация 2», «Кто завершает» | «…или повтор механики»: повторная доставка того же события в consumer отсекается `processed_events` (`dispatcher.go`, `commit`), и `OnMechanics` не вызывается. Готовый ход завершает `Sweep` или следующее событие механики того же типа в корреляции (удар вслед). Подслучай теста проверяет идемпотентность трекера, а не путь consumer'а. | КД и комментарий теста: «…следующий тик `Sweep` — независимо от дедлайна — или следующее событие механики этого типа в корреляции; повтор того же события отсекает `processed_events`». Подслучай можно назвать «another mechanics of the turn» и подать второй `decided(action, "m2")`. | открыто |
| N-8 | Nit | `tracker.go:349-351` (`completeByMechanics`) | `Warn` «a turn is left to the sweeper» пишется до коммита транзакции consumer'а. Если следующий шаг эффекта или коммит упадёт (в том числе отмена `ctx` при остановке, которую `publish` тоже оборачивает в `ErrPublish`), событие придёт снова, а лог уже сообщил, что ход оставлен `Sweep`. `Sweep` свой `Warn` пишет после коммита. | Уточнить текст: «…unless the transaction of the consumer is rolled back». Либо не считать отказом публикации `ctx.Err() != nil` родительского контекста и возвращать ошибку. | открыто |
| N-9 | Nit | `tracker_test.go:738-760` (подслучай «the sweeper»); `:741` | Условие `finished.MechanicsAt == nil` у `Warn` «a delivered turn completed without its mechanics» не проверено: мутант 4 (`Warn` и готовому ходу с механикой) зелёный. Сообщение `:741` «want the turn left open» противоречит проверке `n != 1`. | После `Sweep` проверить, что лог не содержит «without its mechanics». Сообщение: «want the turn closed». | открыто |
| N-10 | Nit | `tracker.go:337-340` (godoc `completeByMechanics`), `:317` (`OnMechanics`, `q DB`) | Точка сохранения корректна только в транзакции. С `*sql.DB` (тип `DB` это допускает) `SAVEPOINT` открывает транзакцию на пуловом соединении: между `Exec` в неё может попасть чужая запись и откатиться `ROLLBACK TO`, а при отказе `ROLLBACK TO` соединение остаётся в транзакции (П3, мутант 3). Сейчас все вызывающие передают `*sql.Tx`. | В godoc: «q must be a transaction: a savepoint outside one begins a transaction on a pooled connection». Сигнатуру не менять: интерфейс `consumer.Turns` общий с тестами. | открыто |

### Предложения в бэклог (вне границ T-478)
1. **Строка DoD T-479:** `OnNarrative` без адресатов завершает ход через точку сохранения, как `OnMechanics`: сбой публикации не откатывает эффект нарратива, ход публикует `Sweep`. Плюс unit с отказом шины (п. 5 выше). Там же — решить, откатывает ли точка сохранения закрытие сессии в `finish`.
2. **Для T-479, риск порядка аналитики.** Ход `leave` — ход с Phase 1, и его механика (`entity.updated cause=move`) — это и факт-граница сессии. Если завершение в `OnMechanics` не опубликовано, ход остаётся открытым до тика `Sweep`, и сессия по правилу §7.7 закроется в `finish` уже из `Sweep`. Порядок `turn.completed` → `session.ended` сохраняется, но только если закрытие сессии в `finish` стоит внутри той же точки сохранения. В unit T-479 стоит проверить сценарий «отказ шины на ходе `leave`».
3. Прежние: N-2 (лог `restore` и README шлюза, строка DoD T-479); вариант (б) C-10 — новая задача architect#3; отказ State как сигнал завершения — бэклог architect#3, п. 3.

### Риски и допущения
- `-race` не запускался (нет cgo). Гонки п. 3 оценены по коду: единственное соединение, транзакции consumer'а и ack.
- При частичном отказе шины `turn.completed` хода с Phase 1 уходит с задержкой до одного тика `Sweep` (`store.SweepInterval`, минута), и `Warn` пишется на каждое событие механики с отказом. Соединение держится до `PublishTimeout` один раз на событие, а не на каждый повтор шины.
- Фраза КД §7.7 «итерация 2 T-478» — на подтверждении architect#3. Ревьюер её поддерживает с правкой N-7.
- Прогон e2e в рабочей папке шёл параллельно с unit и мутантами в копиях scratchpad. Флак T-313 (`EOF` на `POST`, p95 `202`) в этом ревью не проявился.
- Копии `t478r2-merged` и `t478r2-probe`, скрипты `t478r2-mutants.py`/`t478r2-write.py` и лог `t478r2-e2e.log` — в scratchpad, удаляются по точным путям. В рабочей папке ревьюер изменил только `review.md` (этот раздел) и раздел «Ревью» карточки. Docker, `.env` и стенд `:8888` не трогал.

## T-483 · ревью #1 · 2026-09-14 · code-reviewer#3 (TEAM-3)

### Границы ревью
- Задача: дубль `analytics.turn.completed` на брокере в `TestConsumerOnRedpanda/OutboxWhileTheBrokerIsAway` на Linux (CI, прогон 34827849541, job `integration`). DoD — поручение оркестратора в карточке `tasks/T-483.md`; раздела в `tasks.md` пока нет.
- Папка `.worktrees/T-483`, ветка `task/T-483-outbox-duplicate-turn-completed`, база `dfcad6b` (кончик `epic/EPIC-004-gateway-bot`). Изменения не закоммичены: `git status`/`git diff` рабочей папки.
- Код: `internal/gateway/consumer/redpanda_integration_test.go` (изм., 11 строк), `internal/gateway/consumer/turn_completed_copies_test.go` (нов.). Production-код не менялся. Документы: карточка T-483, запись в `dev-log.md`. Правка артефактов в ветке задачи — порядок проекта (драйвер `appendtail`), не замечание.
- Логи разработчика в scratchpad: `t483-ci-integration.log`, `t483-int-run1.log`, `t483-int-run2-mutant.log`.

### Прогоны (go1.26, windows/amd64, cgo выключен — без `-race`)
| Команда | Результат |
|---|---|
| `go build ./... && go vet ./... && go vet -tags integration ./internal/gateway/...` | exit 0 |
| `go test -short -count=3 ./internal/gateway/consumer/...` | `ok` |
| `go test -short -run TestOneTurnCompleted -v ./internal/gateway/consumer/` | 7 подслучаев `PASS` |
| `golangci-lint run --build-tags integration ./internal/gateway/...` | 0 issues |
| Мутанты помощника через `-overlay` (ключ относительный, мутант в scratchpad) | контроль — `ok`; M3 «сравнивается только первая копия» (`published[:1]`) — красный: «2 copies, ids [t-1] = "", want holds=false»; M4 «пустой брокер принимается» — красный: «0 copies … want holds=false». Файлы `t483r-*` удалены по точным путям |
| Интеграционный прогон | не выполнялся (по поручению) |

### Проверка по пунктам задания

**1. Механизм дубля — подтверждён.** Прочитан kafka-go v0.4.51 из модульного кэша.
- `writer.go:600-608`, godoc `WriteMessages`: при отменённом контексте «there are no guarantees made on whether messages were written to kafka, they might also still be written after this method has already returned … (which could then cause duplicates)». Библиотека сама документирует это поведение.
- `writer.go:671-677`: синхронный `WriteMessages` ждёт `batch.done` или `ctx.Done()`. По `ctx.Done()` возвращает `ctx.Err()`, пачку из очереди не снимает.
- Пачка вообще попадает в очередь при остановленном брокере: `w.partitions` берёт метаданные из кэша транспорта (`transport.go:339-373`), а `connPool.update` при ошибке обновления сохраняет прежнее известное состояние (`transport.go:524-530`, «prioritize a previously known state»). Иначе `WriteMessages` вернул бы ошибку метаданных до постановки пачки.
- `writer.go:1101-1185`, `partitionWriter.writeBatch`: до `maxAttempts()` попыток, по умолчанию 10 (`writer.go:786-795`; в `shared/eventbus/kafka.go:418-429` `MaxAttempts` не задан). Пауза `backoff(attempt)` = attempt² · 100 мс, не больше 1 с (`dialer.go:437`, `writeBackoffMax`): 0,1 + 0,4 + 0,9 + 6 × 1 = 7,4 с сна на пачку. Повтор продолжается, пока `isTemporary(err) || isTransientNetworkError(err)`.
- `error.go:613-618`, `isTransientNetworkError`: `io.ErrUnexpectedEOF`, `syscall.ECONNREFUSED`, `ECONNRESET`, `EPIPE`. Ошибка набора в транспорте не оборачивается (`transport.go:1140-1174` возвращает `*net.OpError` как есть), поэтому `errors.Is` на Linux срабатывает. Тот же прогон CI показывает именно эту ошибку на порту брокера: «dial tcp 127.0.0.1:36309: connect: connection refused» (строка T473 measure). На Windows код ошибки — `WSAECONNREFUSED` (10061), он не равен синтетической `syscall.ECONNREFUSED`, `Errno.Temporary()` для него `false`, и пачка завершается с первой попытки. Ack 503 за 1–2 мс в `t483-int-run1.log` и `t483-int-run2-mutant.log` это подтверждает.
- **Сверка с логом CI — сходится количественно, а не только качественно.** Пачки одной партиции пишет одна горутина `writeBatches` (`writer.go:1011-1024`), строго по очереди. `turn.completed` одного хода — один ключ, одна партиция. Отсчёт от ack 1:
  - пачка ack 1 (0 с) повторяется до ~7,4 с; ack 1 — `503 in 5.001s`;
  - пачка ack 2 (в очереди с ~5 с) начинает на ~7,4 с и исчерпывает попытки к ~14,8 с; ack 2 — `503 in 5.001s`;
  - пачка ack 3 (в очереди с ~10 с) начинает на ~14,8 с и может повторяться до ~22 с; ack 3 — `503 in 5.002s`;
  - брокер запущен через ~0,4 с после ack 3 (`started again in 429ms`). Следующая попытка пачки ack 3 проходит. Пачка повтора ack встаёт в ту же очередь **за** ней, отсюда `the last ack took 787ms` (остаток паузы до 1 с плюс запись).
  - Итог: копий ровно 2 — пачки ack 3 и повтора. Пачки ack 1 и 2 исчерпали попытки до запуска брокера. Это совпадает с «turn.completed on the broker 2» и «4 publications … under 1 id(s)».
- Альтернатива «шлюз публикует дважды» исключена: обёртка `attempts` насчитала 4 попытки под одним id (3 × 503 + 1 × 200, 0 отказов в `ackUntilTaken`). У `look` нет Phase 1, `Sweep` в тесте не вызывается. Id строится `eventbus.WithCauseID()` из причины (`turns/store.go:139-140`), payload `Timestamp` — `ReceivedAt` действия.

**2. Контракт — копия под тем же id допустима.**
- C-01, «Гарантии» (`contracts.md:398`): «At-least-once; … дедуп по `Event.ID` — обязанность потребителя (`eventbus.Dedup`)». Тест, требующий ровно одну запись в топике, требовал больше, чем обещает шина.
- КД §5.5 (`gateway-and-bot.md`, абзац после п. 6): повтор после `503` — «с тем же `event.id`; потребители отбрасывают дубль по `event.id` (NFR-013)». Сказано о партии `player.*`, но довод тот же: неподтверждённое не значит незаписанное.
- КД §7.7, «Конец сессии»: `Sweep` публикует ход «(тот же id; повтор по id допустим)» — прямо о `turn.completed`.
- C-05 v1.6 п. 1 (`contracts.md:155, 673, 766`): «повтор публикации того же события с тем же id — дубль at-least-once». Это контракт роя, а не шлюза, и в тексте п. 1 стоит «с тем же id **и теми же байтами**». Для `turn.completed` байты не совпадают (п. 4). Поэтому для T-483 опорные — C-01 и КД §5.5/§7.7, а C-05 — аналогия. Вывод разработчика от этого не меняется.
- C-10: «на каждый принятый/отклонённый ход» — одно событие на ход, дубль под тем же id — то же событие.

**3. Ослабление теста — нет, проверка id усилена.**
- Ноль копий не пропускается: `len(published) == 0` → сообщение (`turn_completed_copies_test.go:23-25`). Подслучай «nothing on the broker», мутант M4 разработчика и повтор ревьюером — красные.
- Разные id ловятся с обеих сторон: у попыток — `len(ids) != 1` (`:20-22`, в `made()` id уникализируются `slices.Compact`, 0 попыток тоже красный); у копий на брокере — каждая, а не первая (`:26-30`). Прежняя проверка `published[0].ID != ids[0]` смотрела только первую копию. M3 это ловит, повтор ревьюером — красный.
- Мутанты M1–M5 разработчика принимаю: сообщения в карточке соответствуют подслучаям таблицы, M3 и M4 повторены с тем же результатом. Интеграционный мутант «без `WithCauseID()`» принимаю по логу `t483-int-run2-mutant.log`: «attempted under ids [ev-1 ev-2 ev-3 ev-4], want one id for every attempt». Контроль — прогон 1 на тех же исходниках, `-count=2`, 1 копия на брокере.
- Помощник без build-тега: файл `_test.go` пакета `consumer_test`, как и интеграционный. `go vet -tags integration` и lint с тегом проходят, дублирования символа нет. Правило помощника проверяется в `go test -short`, мутанты не требуют брокера.
- Что потеряно — верхняя граница копий (N-2): двойную публикацию шлюзом на штатном пути этот тест больше не поймает. Это закрыто unit-тестами на membus, см. N-2.

**4. Вопрос 1 разработчика — оценка риска (не замечание).**
- *Что различается.* `Completed(r)` строит payload заново при каждой попытке (`turns/store.go:131-141`, `Payload`). У копий под одним id совпадают `id`, `timestamp`, статус, счётчики, `received_at`, `acked_at` (ack действия, `202`). Различаются `narrative_at`, `narrative_ms` и `total_ms`: у неудавшейся попытки — момент отменённого ack, у повтора — момент принятого.
- *Какая копия остаётся после дедупликации.* Детерминированно — копия **неудавшейся** попытки, а не «скорее всего». Пачка повтора стоит в той же очереди писателя партиции за поздней пачкой и не может лечь раньше неё (п. 1). `eventbus.Dedup` оставляет первую по офсету. Значит, в отчёт EPIC-005 попадут тайминги, которых нет в `gateway.db`, так как ack откатился. `narrative_ms` и `total_ms` будут **меньше** фактических на время до повтора.
- *Величина и частота.* Только ходы, завершавшиеся во время отказа шины. На Linux поздняя копия ложится, только если брокер вернулся в окно повторов пачки (~7,4 с плюс ожидание в очереди). Сдвиг — время от отменённого ack до принятого повтора: секунды, в худшем случае до срока аренды `outbox.DefaultLease` (30 с) и паузы бота. Без kafka-go тот же эффект даёт «запись дошла, ответ потерян», поэтому это свойство повторной сборки payload, а не только T-483.
- *Влияние на метрики.* NFR-002 (`metrics.md` §4.2) шаблоны не считает. С LLM-нарратором ход во время отказа шины получит заниженную задержку, то есть p95 в инцидент выглядит лучше реального. Выборка — единицы ходов на отказ.
- **Оценка: риск низкий, для T-483 не блокирует.** Варианты для architect#3:
  - (а) зафиксировать в C-10/`metrics.md`: при дубле по id тайминги копий могут различаться на время повтора, отчёт берёт первую;
  - (б) ограничить окно поздних копий `Writer.MaxAttempts`/паузами в kafka-адаптере (EPIC-001, `contract-change` не нужен, поле внутреннее) — сузит, но не устранит случай потерянного ответа;
  - (в) хранить тайминги первой попытки вне откатываемой транзакции — дорого, противоречит откату ack по T-307.

  Рекомендация ревьюера: (а) сейчас, (б) — в бэклог EPIC-001.

**5. Совместимость с T-482 — пересечений нет.**
- `.worktrees/T-482` (ветка `task/T-482-ci-green-on-linux`, база в `epic/EPIC-001-foundation`) меняет: `cmd/multiverse/dispatch_test.go`, `cmd/telegram-bot/internal/deliver/pace_test.go`, `internal/gateway/gatewaytest/gatewaytest_test.go`, `internal/gateway/snapshot_test.go`, `internal/gateway/store/open_test.go`, `scripts/backup-prune-test.sh`, `shared/testkit/gateway/stand_test.go`, `shared/testkit/swarm/fake_encounter{,_test}.go`, `testdata/script-parity/stand.go`, документы EPIC-001.
- T-483 меняет только пакет `internal/gateway/consumer` (два тестовых файла) и документы EPIC-004. Общих файлов и символов нет; пакет `consumer` T-482 не затрагивает.
- Для зелёного job `integration` нужны обе задачи: в том же прогоне CI падали `TestAFakeGatewayLeavesWhatTheTestPassed` и `TestOpenLinksRefusesAFileWithoutIncrementalVacuum` — зона T-482.

### Вердикт
**Принять.** Critical (Blocker) 0 · Major 0 · Minor 0 · Nit 2.

### Замечания

| # | Серьёзность | Файл:строка | Замечание | Предложение | Статус |
|---|---|---|---|---|---|
| N-1 | Nit | карточка T-483, «Причина» п. 2 и «Открытые вопросы» п. 1; `dev-log.md`, запись T-483 | «Пауза 100 мс–1 с» и «первой по офсету, скорее всего, ляжет копия неудавшейся попытки» неточны. Пауза — attempt² · 100 мс, не больше 1 с, суммарно ~7,4 с на пачку. Пачки партиции пишутся одной горутиной по очереди, поэтому поздняя копия всегда ложится раньше копии повтора. Это же объясняет, почему копия ровно одна: пачки ack 1 и 2 исчерпали попытки до запуска брокера. Для решения architect#3 по вопросу 1 разница важна: после дедупликации остаются тайминги неудавшейся попытки всегда, а не с вероятностью. | Уточнить формулировку вопроса 1 в карточке («первой ложится копия неудавшейся попытки: очередь писателя партиции одна») и сослаться на п. 1 и 4 этого ревью. Код не меняется. | открыто |
| N-2 | Nit | `redpanda_integration_test.go:837-840`; `turn_completed_copies_test.go:19-32` | Верхней границы числа копий больше нет: двойная публикация шлюзом под тем же id на штатном пути в интеграционном тесте не краснеет. Закрыто unit-тестами на membus: `turns/tracker_test.go:97, 222`, `turns/outbox_steps_test.go:52` (`len(completed) != 1`). Жёстче `len(published) <= asked` сделать нельзя: потерянный ответ при записи даёт копию без новой попытки шлюза. | Не требуется. По желанию — одна фраза в godoc `oneTurnCompleted`: «a single publication on the happy path is checked by the unit tests of turns». | на усмотрение |

### Предложения в бэклог (вне границ T-483)
1. **architect#3, C-10/`metrics.md` §4.2:** копии `turn.completed` под одним id могут различаться таймингами (`narrative_at`, `narrative_ms`, `total_ms`) на время повтора ack. После дедупликации остаётся первая по офсету — копия неудавшейся попытки. Решение — вариант (а) п. 4.
2. **EPIC-001, kafka-адаптер (`shared/eventbus/kafka.go`):** явно записать в C-01, что ошибка `Publish` не означает «не записано». Оценить `Writer.MaxAttempts` и `WriteBackoffMax`: сейчас поздняя пачка живёт до ~7,4 с после отказа вызывающего плюс ожидание в очереди партиции, а следующие публикации той же партиции ждут её (задержка 787 мс у повтора ack в CI).
3. **T-473/КД шлюза, риск эксплуатации:** на Linux при отказе соединения ack держит единственное соединение `gateway.db` весь `DefaultPublishTimeout` (5,0 с), а не 1 мс, как в замере T-473 на Windows. Прод работает в Linux-контейнере. При отказе шины каждый ack последнего адресата занимает `gateway.db` на 5 с, и параллельные запросы упираются в `busy_timeout` 5 с. Замер T-473 «1 / 1 / 1 мс» и вывод о свободной `gateway.db` при остановленном брокере нужно пересмотреть на Linux.
4. Прогон `TestConsumerOnRedpanda` на Linux до слияния EPIC-004 в develop (предложение разработчика поддерживаю).

### Риски и допущения
- Дубль локально не воспроизведён: на Windows он невозможен по п. 1. Механизм подтверждён чтением кода kafka-go v0.4.51 и количественной сверкой с логом CI. Окончательное подтверждение — зелёный job `integration` на Linux после слияния T-483 и T-482. Строка `T316 measure … N on the broker` покажет число копий.
- Вывод про `WSAECONNREFUSED` принят по проверочной программе разработчика и замерам ack 1–2 мс. Ревьюер отдельной программы не запускал.
- `-race` не запускался (нет cgo). Помощник — чистая функция без общих данных.
- В рабочей папке ревьюер изменил только `review.md` (этот раздел) и раздел «Ревью» карточки. Мутанты — через `-overlay` из scratchpad с префиксом `t483r-`, удалены по точным путям. Docker, `.env`, стенд `:8888`, `.worktrees/T-482` (только чтение) и CI не трогал.
