...
| e2e (`e2e`, один процесс на `membus`, `--mode=live` и `clock.Manual`: `gatewaytest.FakeGateway`, `FakeState`, `swarm` — хук `MV_SWARM_FAKE=true`: `FakeEncounter` + `FakeNarrator`, T-255; *изм. T-313, прежде `--contexts=all --mode=replay`, КД §15*) | `solo-30`, `death`, `flee-fail`, `forget`, `recovery`, `privacy-scan` (I1); `group-3x30` с `rounds/close`, `forget` в группе (I2); `--chaos=duplicate` для NFR-013 | T-g, T-h, I2 |
