...
### T-306: `characters`, `GET /v1/worlds`, `GET /v1/players/{id}`, `session` + `turns` + аналитика C-10 · I1 · подволна 1.6 · developer#1 · M · **Статус: done**
...
**Приёмка (tech-lead#3, 2026-09-13): принято, статус `done`** — карточка [`tasks/T-306.md`](tasks/T-306.md), раздел «Приёмка (tech-lead)». Ревью: 1 итерация, итог 0/0/6/6, «принять». При приёмке закрыты с тестами: Mi-1 — уборка резервов `seq` читает активные сессии под блокировкой трекера; Mi-3 — sweeper персонажей снимает связку под блокировкой связки, перечитывает проекцию после снятия и возвращает связку, если факт успел прийти; Mi-4 — `api.ValidCharacterName` отвергает пробелы по краям, потому что `api-contracts.md` §1.3 обрезку не требует; N-1, N-2, N-6 — границы таймаута хода, простоя и дедлайна; N-5 — старт отказывает при `MV_GATEWAY_CHARACTER_WAIT` ≥ `MV_GATEWAY_CHARACTER_DEADLINE`. Мутанты ревьюера M1–M6 и мутанты правок приёмки красные. Mi-5 проверен по решению оркестратора 1: в событиях C-10 нет имён, `scope`, копии `world` и внешних ID; проверка по ключам закреплена в `TestSolo30Fixture`. Приняты отступления: обязательные идентификаторы C-10 (решение 1), сигнатуры КД §6, счётчик поколений окна сжатия, запасной путь фикстуры `internal/gateway/testdata/analytics/solo-30.jsonl`. Mi-2, Mi-6 и N-3 — вопросы system-architect в карточке. Строки DoD внесены в T-307, T-309, T-355, T-356; N-4 — §8 З-11. Нужна отметка tech-lead#1: `shared/env/vars.go` и `.env.example` (четыре `MV_GATEWAY_*`), место фикстуры в `testdata/analytics/`.

### T-307: `outbox` (store/lease/ack/render/sweeper), long-poll `deliveries`, `consumer` для `combat.decided`/`narrative.output` · I1 · подволна 1.7 · developer#1 · M · **Статус: done**
...
- [ ] **(приёмка T-306, 2026-09-13; ревью #1 T-306, бэклог п. 1)** Шаги хода T-306 подключены: `turns.OnMechanics` — эффект `combat.decided` и `entity.updated` по `correlation_id` хода, `turns.OnNarrative` — эффект `narrative.output`, оба в транзакции consumer. `turns.OnDelivered` вызывается в транзакции ack доставки `kind=narrative` **ровно один раз на доставку**: повторный ack той же доставки (повтор запроса бота, повтор после сбоя) не увеличивает `delivered_count` и не завершает ход раньше последнего адресата. Unit: двойной ack одной доставки при двух адресатах → ход остаётся `narrated`, `turn.completed` нет; ack второго адресата → один `turn.completed`.
- [ ] **(приёмка T-306; ревью #1 T-306, бэклог п. 1)** Ход, чей `narrative.output` пришёл с `recipients=0` (доставлять некому), завершается при `OnNarrative`: `ok`, или `degraded` при `generated_by=template`; `narrative_at` — момент применения нарратива. По дедлайну такой ход в `timeout` не уходит. Если КД §7.6 или `metrics.md` §2.1 к началу задачи определяют итог иначе — по ним, с записью в карточке. Unit: нарратив без адресатов → один `turn.completed` со статусом `ok`, `turns_failed` сессии не растёт.
- [ ] **(приёмка T-306; ревью #1 T-306, Mi-2)** Публикация `analytics.turn.completed` не держит единственное соединение `gateway.db` без срока на время подтверждения брокера. Способ — по решению system-architect (вопрос 1 приёмки T-306): публикация после коммита с отметкой «не опубликовано» и повтором по отметке (id уже детерминирован), либо `Publish` внутри транзакции consumer, ack и sweeper'а со сроком (`context.WithTimeout` порядка `api.RequestTimeout`), с откатом и повтором. Срок получает и чтение `pending_characters` в `characters.Status` (сейчас `context.Background()`). Unit: брокер, не отвечающий на `Publish`, освобождает соединение `gateway.db` не позже срока, а следующий тик публикует тот же `turn.completed` с тем же id.
- [ ] **(приёмка T-306, 2026-09-13; сверка слияния T-306 с кончиком эпика)** `internal/gateway/README.md` (создан T-317) приведён к слитым T-306 и T-307: в «Статус реализации» пакеты `characters`, `session`, `turns` и `outbox` — в таблице реализованного; в «API» — `listWorlds`, `createCharacter`, `getPlayer` и операции доставок; в «Переменные окружения» — `MV_GATEWAY_SESSION_IDLE`, `MV_GATEWAY_TURN_TIMEOUT`, `MV_GATEWAY_CHARACTER_WAIT`, `MV_GATEWAY_CHARACTER_DEADLINE` (ожидание короче дедлайна, иначе отказ старта) и переменные T-307; sweeper персонажей, сессий и ходов — в описании режимов. Если README обновлён отдельной правкой при слиянии T-306 — пункт закрывается ссылкой на неё.
- [ ] **(отметка владельца T-306, tech-lead#1, 2026-09-13)** Фикстура перенесена: `internal/gateway/testdata/analytics/solo-30.jsonl` → `testdata/analytics/solo-30.jsonl` (перенос с сохранением истории, без правки содержимого). `solo30Path` в `internal/gateway/turns/fixture_test.go` указывает на `testdata/analytics/` в корне модуля. Комментарий к нему и текст флага `-update` исправлены, пустой каталог `internal/gateway/testdata/` удалён. `go test -short -count=1 -run TestSolo30Fixture ./internal/gateway/turns/` без `-update` — `ok`. Мутант «изменена одна строка фикстуры» — красный. После `-update` файл пишется по новому пути, и `git status` по нему пуст. `go run ./cmd/mvctl privacy scan testdata/` — без находок. В «Выполнении» — уведомление EPIC-005 (T-130): путь, число строк, рецепт перегенерации. Условия согласия — в карточке T-306, раздел «Отметка владельца (tech-lead#1)».

**Приёмка (tech-lead#3, 2026-09-14): принято, статус `done`** — карточка [`tasks/T-307.md`](tasks/T-307.md), раздел «Приёмка (tech-lead)». Ревью: 2 итерации, ревью #1 — 0/1/2/3 «вернуть», ревью #2 — 0/0/0/2 «принять». Пункты DoD, строки приёмок T-304 (Mi-2), T-305 (отказ State), T-306 (шаги хода, `recipients=0`, публикация со сроком, README, перенос фикстуры) и дополнения оркестратора (поздний ack, темп клиента, платформа `ci-harness`) закрыты тестами; мутанты исполнителя и ревьюера красные, кроме двух признанных эквивалентными. Приняты отступления: id доставки `d-<UUID v5>` вместо `d-<ULID>`; условие «одна в лизинге» строже SQL КД §8.2; ожидание long-poll по настенным часам, лизинг во всех режимах (КД §11.2 — вопрос); `ci-harness → telegram` условно до решения system-architect; публикация `turn.completed` со сроком, с ценой повторной выдачи при зависшем брокере; `recipients_count` — уникальные адресаты со связкой. При приёмке в README шлюза записан запрет запускать харнесс против шлюза с ботом, а в карточке исправлена фраза об остатке Ma-1 (N-4). Слияние с кончиком эпика `b627f3c` (T-312) проверено в копии: код не пересекается, `go test -short ./internal/gateway/... ./cmd/telegram-bot/...` зелёный, форма `Deliveries`/`Ack` совместима с `deliver`. Строки DoD внесены в T-308, T-309, T-315, T-316, T-353, T-355; бэклог — §8 З-12…З-15; вопросы system-architect и architect#3 — в карточке. Отметку владельца по `shared/env/vars.go` и `.env.example` (`MV_GATEWAY_DELIVERY_LEASE`, `MV_GATEWAY_DELIVERY_TTL`) даёт tech-lead#1 в карточке.
...
- [ ] **(приёмка T-307, 2026-09-14; Mi-1 ревью T-307, решение оркестратора 1)** Платформа клиента `ci-harness` — окончательно по решению system-architect (вопрос 1 карточки T-307). Сейчас `handlers.ClientPlatforms` условно даёт `ci-harness` доставки `telegram`, а описание этой задачи называет фикстуры `player-A/B/C` платформы `ci`, связок которой HTTP не создаёт. Варианты: (а) оставить `ci-harness → telegram`; (б) связки платформы `ci` через HTTP и `ci-harness → ci`; (в) харнесс ходит как `telegram-bot`. Выбранный вариант отражён в таблице `ClientPlatforms`, фикстурах обвязки, описании `pollDeliveries` в OpenAPI и README шлюза; `TestTheHarnessTakesTheDeliveriesOfTelegram` приведён к решению.
- [ ] **(приёмка T-307; ревью #2 T-307, риск общей очереди)** Пока действует вариант (а), `telegram-bot` и `ci-harness` делят одну очередь `telegram`: харнесс, запущенный против шлюза с ботом, берёт в лизинг и подтверждает сообщения живых игроков и видит их внешние ID, а бот пытается отправить сообщения тестовых игроков. В README обвязки и в `Docs/ops/runbook.md` §6 (правка согласуется с devops) записан запрет запускать харнесс против шлюза, у которого работает бот (профиль `bot`), до решения system-architect; в README шлюза запрет внесён при приёмке T-307. При варианте (б) пункт закрывается ссылкой на решение.
- [ ] **(приёмка T-307; ревью #2 T-307, бэклог п. 3)** Сквозная проверка long-poll харнесса: `AwaitDelivery` получает механику и нарратив хода через настоящий `GET …/deliveries` клиента `ci-harness` на `FakeGateway`, подтверждает их `POST …/ack`, и ход завершается одним `analytics.turn.completed`. Сейчас платформу харнесса проверяет только unit `handlers`.
...
- [ ] **(приёмка T-312, 2026-09-13; бэклог исполнителя и ревью #1 T-312)** `/health` бота (`cmd/telegram-bot/health.go`): `degraded` после серии неудачных long-poll доставок подряд (порог — константа, не меньше 5 опросов, то есть больше минуты пауз 1…30 с) и после `ErrUnauthorized` при отправке; `ok` — после успешного опроса. Сейчас `status` — `ok`, пока процесс жив. Unit на `clock.Timers`: N неудачных опросов → `degraded`, успех → `ok`; `401` отправки → `degraded`. Подкоманда `telegram-bot health` на `degraded` уже отвечает кодом 1. Часть бота выполняет разработчик бота (developer#2) по назначению tech-lead#3, отдельно от `/health` gateway.
...
- [ ] **(приёмка T-306, 2026-09-13; бэклог исполнителя T-306)** Восстановление при старте (КД §7.6) идёт до первого HTTP-запроса и до подписок consumer. `session.Manager.Sweep` закрывает сессии, простаивающие не меньше `MV_GATEWAY_SESSION_IDLE`, с `ended_at = last_action_at + IDLE`. `turns.Tracker.Sweep` закрывает просроченные ходы `timeout`. В `--mode=replay` восстановление не публикует `analytics.*` (издатель `nil`, как в T-306). `/health.sessions_active` — число строк `session.Manager.Active`, без внешних ID и без `scope`. Unit: рестарт с простаивающей сессией и просроченным ходом → `ended idle` и `timeout` до ответа на первый запрос; `sessions_active` равно числу активных сессий.
- [ ] **(приёмка T-307, 2026-09-14; бэклог исполнителя и ревьюера T-307)** `/health` gateway отдаёт `outbox_pending` (число доставок `pending`) и `outbox_oldest_age_s` (возраст старейшей `pending` по `created_at`, `0` при пустой очереди) — КД §11.4. Чтение не занимает соединение `gateway.db` дольше короткого дедлайна и кэшируется, как проверка БД выше. Unit: пустая очередь → `0`/`0`; две доставки, старейшая создана 90 с назад → `2`/`90`.
- [ ] **(приёмка T-307; вопрос 4 карточки T-307)** Описание задачи и КД §11.2 выключают лизинг в `--mode=replay`, а T-307 оставила лизинг и long-poll рабочими во всех режимах: e2e T-313 идёт в replay через доставки харнесса. Поведение лизинга в replay — по решению architect#3; до решения лизинг не выключается, и unit `replay` этой задачи его не проверяет.
...
### T-311: Бот — `flow` (онбординг, `/forget`) и `render` (тексты, клавиатуры, ошибки) · I1 · подволна 1.5 · developer#2 · M · **Статус: done**
...
- [ ] **(приёмка T-318; I-2)** Ответ игроку на `503 forget_incomplete` не содержит «удален» (golden `render` кода `forget_incomplete` и проверка `strings.Contains(..., "удален") == false`): иначе игрок прочтёт «связка удалена», пока строка ещё не затёрта. Остальное — пункт приёмки T-303 ниже. *(Приёмка T-311: заменено «после исчерпания повторов на» — клиент `forget_incomplete` не повторяет, C-08 v1.5.)*
...
- [ ] **(приёмка T-303, 2026-09-13; решение system-architect#1 по `503 forget_incomplete`; приведено к C-08 v1.5 при приёмке T-311)** `/forget confirm` при `503 forget_incomplete`: клиент **не** повторяет запрос и сразу возвращает `APIError` с `RetryAfter` (C-08 v1.5, T-456); бот **не** говорит «связка удалена», а просит повторить `/forget confirm` через `Retry-After`. `200 {deleted:false}` — конец того же забвения, не «нечего удалять», в двух случаях: `ForgetResult.Repeated = true` (клиент повторил запрос после сетевой ошибки или другого `503`) или предыдущий `/forget confirm` этого чата получил `forget_incomplete`. Golden-тест `render` для кода `forget_incomplete`; unit `flow` на обе ветки. `Retry-After` читает клиент в целых секундах. *(Приёмка T-311: заменено «клиент повторяет сам (`DefaultBackoff`); после исчерпания повторов бот…» и «ответ после 503 в том же вызове» — расходилось с C-08 v1.5: повтор `forget_incomplete` держит единственное соединение `links.db` до 5 с на попытку.)*
...
**Приёмка (tech-lead#3, 2026-09-13): принято, статус `done`** — карточка [`tasks/T-311.md`](tasks/T-311.md), раздел «Приёмка (tech-lead)». Ревью: 1 итерация, итог 0/0/7/6, «принять». При приёмке закрыты с тестами Mi-1, Mi-2, Mi-3 (б), Mi-4…Mi-7 и N-1…N-6; мутанты ревьюера M1–M5 и мутанты правок приёмки красные (Mi-7 — только под `-race` в CI). Решения: Mi-3 (а) — повтор уведомления по `notice_due` в `route` отложен в T-320 (решение оркестратора, строка DoD T-320); N-1 — нажатие `ConsentButton` игроком с персонажем в кэше показывает `NoticeText` без клавиатуры и шаг не меняет; Mi-4 — ответ без `error.code` показывается как `render.Unavailable`. Приняты отступления от КД §10.3 (Р-2 A, Р-3 A, `/help` без `links/consent`, свободный текст → список команд по US-008) и копия правила имени до `api.ValidCharacterName`. Вопросы system-architect (КД §10.3, §10.5, правило имени) — в карточке. Строки DoD внесены в T-312 и T-320, предложение по `202 creating` — §8 З-10.

### T-312: Бот — `deliver` (long-poll → send → ack) и `main` wiring · I1 · подволна 1.6 · developer#2 · M · **Статус: done**
...
- [ ] **(приёмка T-317, 2026-09-13; ревью #1 T-317, «Вопросы tech-writer», п. 2)** После сборки `main.go` обновлены документы, которые создала или переписала T-317. `cmd/telegram-bot/README.md`: раздел «Статус реализации» — бинарник собирается, в таблице «Ещё не реализовано» нет `deliver` и `main.go`; состав `/health` бота (`bot_denied_total`); коды выхода — из `main` (сейчас текст говорит «`main.go`, который его вызывает, появится в T-312»). `Docs/ops/runbook.md` §6: снята врезка «Статус на 2026-09-13 … выполнима только после T-312» и оговорка «до T-312» в пункте чек-листа `make health`; шаги 4–5 ротации (перезапуск и проверка лога) пройдены сухим прогоном на собранном образе без реального токена в выводе либо явно оставлены за T-390 со ссылкой. Пункт приёмки T-311 про README бота и runbook §6 (таймауты клиентов шлюза, `sender.ReplyPolicy`, состояние диалога в памяти) выполняется той же правкой: README уже есть, в T-317 не передаётся.
...
- [ ] **(приёмка T-311, 2026-09-13; ревью #1 T-311, риск 6)** Клиент шлюза для `flow` — отдельный `client.Client` с коротким `HTTP.Timeout` (≤ 10 с) и своей политикой `Backoff`, не общий с `deliver`: long-poll `deliveries` (`wait=25s`) требует таймаута больше 25 с, а `flow` работает в единственном обработчике обновлений, и зависший шлюз при `client.DefaultHTTPTimeout` 35 с × 4 попытки держит команды всех игроков. Unit `main`: мок шлюза не отвечает на `resolve` → следующая команда другого игрока доходит до мока не позже выбранного таймаута × число попыток (на `clock.Timers`, без реального ожидания там, где это возможно).
- [ ] **(приёмка T-311; ревью #1 T-311, Mi-6)** Логгер `flow` (`flow.Options.Log`) строится на `privacy.NewHandler` с `Redactor` процесса, как у остальных пакетов бота. `flow` сам пропускает текст ошибок через `privacy.Redact`, но ключи с идентичностью отбрасывает только обработчик. Unit `main`: логгер, переданный во `flow`, — `*privacy.Handler`.
- [ ] **(приёмка T-311; решение оркестратора, бэклог п. 1 исполнителя T-311)** `flow.ValidName` заменён на `api.ValidCharacterName` (сделана в T-306, `internal/gateway/api/names.go`): бот и `POST /v1/characters` проверяют имя одной функцией, копия регулярки во `flow` и `TestValidName` удаляются (случаи переносятся в тест `api`, если их там нет). Если T-306 ещё не слита в ветку эпика к началу T-312, пункт переходит в первую задачу бота после её слияния (запись в карточке).
- [ ] **(приёмка T-311)** После сборки `main.go` обновлены README бота (`cmd/telegram-bot/README.md`; если его ещё нет — пункт передаётся в T-317) и `Docs/ops/runbook.md` §6 (правка согласуется с devops): таймауты клиентов шлюза `flow` и `deliver`, `sender.ReplyPolicy` (ответ под флуд-баном дольше 3 с теряется, игрок повторяет команду), состояние диалога и отметка `forget_incomplete` только в памяти (после рестарта повтор `/forget confirm` ответит «нечего удалять», хотя данные стёрты).

**Приёмка (tech-lead#3, 2026-09-13): принято, статус `done`** — карточка [`tasks/T-312.md`](tasks/T-312.md), раздел «Приёмка (tech-lead)». Ревью: 1 итерация, итог 0/0/4/4, «принять». При приёмке закрыты с тестами: Mi-1 — успешный опрос сбрасывает паузу; Mi-2 — отправка, прерванная остановкой, не подтверждается; Mi-3 — неудачный ack уходит до следующего long-poll; Mi-4 (решение оркестратора) — у ответов `flow` свой клиент Telegram 10 с с `sender.ReplyPolicy` (зависший Telegram держит команды остальных ≤ ~21 с), цикл доставки остаётся на 30 с; N-1 — логгер `updates` проверяется тестом сборки; N-2 — повторно отправленная доставка подтверждается один раз; N-3 — цикл доставки стартует после успешного `getMe` (`updates.TelegramOptions.OnReady`), при `401` на `getMe` шлюз не вызывается. Выжившие мутанты ревьюера (5) и 7 мутантов на правки приёмки — красные. README бота и runbook §6 — строки о клиенте Telegram `flow` и старте цикла. Не сделано в T-312 и перенесено: замена `flow.ValidName` на `api.ValidCharacterName` — строка DoD T-315 (в ветке задачи от aa83086 функции нет, замена меняет поведение и тест `flow`); `/health` бота `degraded` — строка DoD T-309. Бэклог: N-4 (ack, отвергнутый шлюзом кодом `4xx`, повторяется без конца — сейчас недостижимо); `.gitignore` для `telegram-bot`/`telegram-bot.exe` в корне; строка `cmd/telegram-bot` в `CLAUDE.md` вносит оркестратор при контрольном слиянии. `docker-compose.bot.yml` (devops, EPIC-001): два устаревших комментария «The binary lands in EPIC-004…» приведены к факту, мягкий режим — отметка для tech-lead#1 в карточке. Слияние — оркестратор.
...
- [ ] **(приёмка T-307, 2026-09-14; дополнение оркестратора п. 2, вопрос 5 карточки T-307)** Цикл доставки бота (`cmd/telegram-bot/internal/deliver`, T-312) держит темп после пустого ответа, пришедшего заметно раньше `wait_ms`: шлюз так отвечает только при остановке процесса. Следующий long-poll идёт после паузы порядка секунды с нарастанием, как после сетевой ошибки; пустой ответ по истечении `wait_ms` паузы не даёт. Порог «заметно раньше» — константа; если system-architect закрепит темп клиента в C-08 — по C-08. Unit на `clock.Timers`: шлюз в остановке отвечает пустым сразу → не больше одного запроса за паузу.
- [ ] **(приёмка T-307; ревью #1 T-307, оценка по решению 2)** Повтор ack при `503 bus_unavailable` не держит цикл доставки дольше срока лизинга (`MV_GATEWAY_DELIVERY_LEASE`, 30 с). Иначе тот же пакет выдаётся и отправляется игроку снова: по оценке ревью — около 14 копий за 10 минут сбоя брокера. Способ — срок на `flush` или один `flush` между опросами. Unit: мок шлюза отвечает `503` на ack через 5 с → следующий long-poll начинается раньше истечения лизинга.
- [ ] Сценарий стабилен: 3 прогона подряд дают одинаковую запись `FakeSender`.
- [ ] **(приёмка T-312, 2026-09-13; строка DoD T-312 из приёмки T-311, решение оркестратора)** `flow.ValidName` заменён на `api.ValidCharacterName` (T-306, `internal/gateway/api/names.go`): бот и `POST /v1/characters` проверяют имя одной функцией; копия регулярки в `flow/onboarding.go` и `TestValidName` удаляются, случаи, которых нет в `internal/gateway/api/names_test.go`, переносятся туда. Поведение бота становится строже в двух местах — два пробела подряд и имя в NFD отвергаются; в тесте `flow` такое имя получает `render.NameInvalid`. Бот по-прежнему обрезает пробелы по краям текста до проверки. В T-312 не сделано: ветка задачи отведена от aa83086, где функции нет.

### T-316: Integration-тесты `consumer` на testcontainers Redpanda · I1 · подволна 1.10 · developer#2 · S · **Статус: done**

**Ветка:** `task/T-316-consumer-integration` (`.worktrees/T-316`). *(Приёмка T-316: заменено «`task/T-316-integration-consumer`» — по фактической ветке, N-3 ревью #1.)*
...
- [ ] **(приёмка T-307, 2026-09-14; ревью #1 T-307, бэклог п. 5)** Контракт-тест outbox на Redpanda с недоступным брокером (контейнер остановлен или приостановлен). Ack последнего адресата нарратива → `503 bus_unavailable` не позже срока публикации (5 с), весь ack откатан, соединение `gateway.db` свободно. После возврата брокера повтор ack публикует один `analytics.turn.completed` с тем же id, доставки не потеряны. Цифры оценки ревью #1 T-307 (длительность ack, период повторной выдачи) замерены и записаны в карточку. Тест — в пакете `internal/gateway/consumer`, либо список пакетов прогона расширяется разрешением оркестратора.

**Приёмка (tech-lead#3, 2026-09-14): принято, статус `done`** — карточка [`tasks/T-316.md`](tasks/T-316.md), раздел «Приёмка (tech-lead)». Ревью: 1 итерация, итог 0/0/2/3, «принять». Mi-1, N-1 и N-2 закрыты итерацией 2 и проверены при приёмке по коду. N-3 исправлен в карточке исполнителем, в индексе — этой приёмкой. На Redpanda проверены дедуп, курсоры группы, `dead_letters`, `Journal.End` и outbox при остановленном брокере. Ack отвечает `503 bus_unavailable` за 1–102 мс, пакет откатывается, `gateway.db` свободна. После возврата брокера публикуется один `turn.completed` под id всех попыток. **Mi-2:** период повторной выдачи (≈ 31 с), `flush` бота и число копий — расчёт по константам, а не замер. Расчёт принят с пометкой, замер при зависшем брокере (`docker pause` своего контейнера) перенесён в DoD T-473. Шаг 8 теста пока принимает два исхода: подписка пережила сбой, или она кончилась с ошибкой чтения и потребитель перезапущен. Строгим его делает T-473 вместе с переподпиской, сроком `Stop` и паузой до первой доставки. Интеграционный прогон при приёмке не делался (решение оркестратора). Есть два зелёных прогона: исполнителя на дереве итерации 2 (39,8 с) и ревьюера на дереве итерации 1 (46,9 с). Проверки приёмки: `go vet -tags integration ./internal/gateway/...` — 0, `golangci-lint run --build-tags integration ./internal/gateway/...` — 0 issues, `go test -short -count=1 ./internal/gateway/...` — 14 пакетов `ok`.

### T-317: Документация блока — README gateway и бота, runbook «ротация токена», сверка `.env.example` · I1 · подволна 1.7 · developer#2 (в паре с tech-writer) · S · **Статус: done**

**Ветка:** `task/T-317-gateway-bot-docs` (`.worktrees/T-317`). *(Приёмка T-317: заменено «`task/T-317-docs-gateway-bot`» — по фактической ветке, N-7 ревью #1.)*
...
**Приёмка (tech-lead#3, 2026-09-13): принято, статус `done`** — карточка [`tasks/T-317.md`](tasks/T-317.md), раздел «Приёмка (tech-lead)». Ревью: 2 итерации, итог 0/0/2/2, «принять». При приёмке закрыты Mi-12 (готовые команды проверки и снятия webhook для Git Bash и PowerShell: токен из `Read-Host -AsSecureString`, `curl.exe -K -` через stdin), Mi-13 (runbook §6: проверка и снятие webhook — шаг 3, до перезапуска; чек-лист в том же порядке), N-9 (шаблон `grep -cE '[0-9]{5,}:[A-Za-z0-9_-]{30,}'` ловит и форму `bot<цифры>:<секрет>`, и голый токен; проверен на синтетических строках), N-10 (пути `cmd/telegram-bot/internal/…` в runbook полностью, опечатка в README шлюза) и N-7 (ветка выше). README бота и врезка runbook приведены к кончику ветки эпика d1d6331: `flow` и `render` готовы (T-311), `deliver` и `main.go` — T-312. Mi-11, часть 2: отдельное согласование runbook с devops не требуется — решение оркестратора: §6 проверили два ревью с поиском утечек, живую сверку делает T-390 или T-391 (строка DoD T-390). Строка DoD T-312 (README бота и runbook §6 после `main.go`) добавлена; строка DoD T-311 из ревью #1 не внесена — T-311 принята, таблицу статуса закрыла эта приёмка. Строки для `CLAUDE.md` — в карточке, подраздел «Передать»: вносит оркестратор при ближайшем контрольном слиянии в `develop`. Проверки: `go run ./cmd/mvctl env check` — 74 переменные, код 0; относительные ссылки в изменённых документах — битых 0. Слияние в ветку эпика даёт конфликт только в этом файле (шапка версии, строка §9) — порядок разрешения в карточке.
...
- [ ] **(приёмка T-311, 2026-09-13; ревью #1 T-311, Mi-3 (а); решение оркестратора — вместе с T-320, Should)** `flow.route` учитывает `notice_due` у согласившегося игрока без живого персонажа (`character_status` `none` или `dead`): перед запросом имени («прежний погиб») отправляется `NoticeText` без клавиатуры согласия (FR-009 «после 30 дней неактивности», US-008, UC-001 A1). Unit: `resolve {consented, dead, notice_due: true}` и `{consented, none, notice_due: true}` → первое сообщение `NoticeText`, второе — подсказка имени; `links/consent` не вызывается.
...
### T-473: Переподписка consumer шлюза после сбоя брокера · I1 (вне подволн) · приоритет I1-α · developer TEAM-3 (по назначению оркестратора) · M

**Ветка:** `task/T-473-consumer-resubscribe` (`.worktrees/T-473`). Брать после слияния T-309 в ветку эпика: T-309 правит `internal/gateway/consumer/dispatcher.go` (догон `Start`), `internal/gateway/context.go` и `/health` шлюза. Параллельно можно вести задачи, которые не правят эти файлы: T-308, задачи бота. `internal/gateway/README.md` может конфликтовать с T-308, разрешается при слиянии.
**Зависит от:** T-316; по файлам — после T-309. **Внешнее:** решение system-architect по `Kafka.Subscribe` (пункт DoD ниже). Правка `shared/eventbus/kafka.go`, если понадобится, — только по его решению (`ownership.md` §1, EPIC-001).
**Ревью:** code-reviewer#3. **Приёмка:** tech-lead#3.
**Ссылки:** карточка [`tasks/T-316.md`](tasks/T-316.md): «Находки» 1–2, новое наблюдение итерации 2, «Предложения в бэклог» п. 1–3, «Приёмка (tech-lead)» (решение Mi-2); `review.md`, «T-316 · ревью #1» (Mi-1, бэклог п. 1–3); C-01 v1.7 (`Subscribe`, `Close`), `contracts.md` v0.15; `shared/runtime.StopTimeout`; ADR-010; component §2.1, §11.2; NFR-013; DoD-common §0.

**Описание.** При сбое брокера подписка `consumer.Dispatcher` на топик кончается. `Kafka.Subscribe` возвращает `eventbus: read <топик>: unexpected EOF` или `failed to dial`, `Dispatcher.subscribe` записывает ошибку в `Err()` и больше не подписывается. До перезапуска процесса шлюз не слышит топик: `/health` — `degraded`, `bus=fail`, новые события до игроков не доходят (T-316: прогоны 3 и 4, итерация 2, прогон ревьюера). Данные не теряются: перезапущенный потребитель продолжает от курсоров. Нужно, чтобы `Dispatcher` подписывался снова сам, с нарастающей паузой на `shared/clock.Timers`, а `/health` возвращался к `ok`, когда подписка восстановлена.

На том же пути есть ещё два дефекта:
- `Dispatcher.Stop` во время входа в группу длится до 29,7 с, а `runtime.StopTimeout` — 15 с;
- потребитель, запущенный после сбоя, получает первую доставку только через ~30 с (29,8 с).

Причина у обоих одна: reader kafka-go внутри `Kafka.Subscribe` ждёт `JoinGroup`, пока брокер держит участников группы, оставшихся со времени до сбоя.

Отдельно: `Kafka.Subscribe` возвращает `nil` на голый `io.EOF` (`stopped` в `shared/eventbus/kafka.go`). Такую «тихую смерть» подписки не видят ни `Err()`, ни `/health`. Публичный API `shared/eventbus` задача не меняет.

**DoD** (+ DoD-common):
- [ ] После ошибки подписки на топик `Dispatcher` подписывается на него снова, остальные подписки не прерываются. Пауза между попытками нарастает до потолка. Значения — константы пакета, обоснование — в комментарии. Пауза ждётся только через `clock.Timers`: `Deps.Timers` передаётся в `consumer.Config` из `context.go`, вызовов `time.*` нет. Каждая переподписка пишет строку лога с топиком и ошибкой, без внешних ID.
- [ ] Если `Subscribe` вернул `nil`, а контекст подписки `Dispatcher` не отменён, это тоже конец подписки, и `Dispatcher` подписывается снова. Отказ закрытой шины (`eventbus.ErrClosed`) и отмена контекста в `Stop` — остановка, повторов нет.
- [ ] `Stop` сразу прерывает паузу перед переподпиской. После `Stop` новых вызовов `Subscribe` нет.
- [ ] `/health` шлюза: пока не восстановлена подписка хотя бы одного топика — `degraded`, `bus=fail`; после восстановления — снова `ok`, если нет других причин деградации. Критерий «подписка восстановлена» выбирает исполнитель и записывает в карточку и в README шлюза (раздел `/health`). `ok` не ставится раньше, чем подписка может брать события; если окно входа в группу это не позволяет — расхождение явно оговорено там же.
- [ ] Unit-тесты переподписки — на `membus` или на фейке `Bus` с инъекцией ошибки `Subscribe`, на `clock.Manual`/`ManualTimers`, без настенного времени и `time.Sleep`:
  - (а) одна подписка падает с ошибкой → повтор после паузы, событие после повтора взято, эффекты применены один раз;
  - (б) пауза нарастает и упирается в потолок;
  - (в) `/health` во время сбоя даёт `bus=fail`, после восстановления — `ok`;
  - (г) `Subscribe` вернул `nil` без `Stop` → переподписка; `ErrClosed` → переподписки нет;
  - (д) `Stop` во время паузы и во время подписки.

  Мутанты «без переподписки», «пауза не через `Timers`» и «`nil` без `Stop` считается остановкой» красные; итоги — в карточке.
- [ ] **Решение system-architect: …** — `Kafka.Subscribe` возвращает `nil` на `io.EOF`. Голый `io.EOF` от транспорта — это остановка по C-01 или ошибка чтения? Вопрос передаёт tech-lead#3, tech-lead#1 уведомлён. Решение вписывается в этот пункт ссылкой. Если оно требует правки `shared/eventbus`, её вносит владелец EPIC-001 или исполнитель по его отметке. Правило «`nil` без `Stop` — переподписка» (второй пункт) остаётся в `Dispatcher` при любом решении.
- [ ] `Dispatcher.Stop` укладывается в `runtime.StopTimeout` (15 с), в том числе во время входа в группу (T-316: 29,7 с). Замер на Redpanda — остановка сразу после `Start` и во время переподписки после сбоя брокера. Хелпер `stop()` integration-теста T-316 получает срок `runtime.StopTimeout` вместо 1 мин. Другой срок или другие параметры reader'а в `shared/eventbus/kafka.go` (`SessionTimeout`, `RebalanceTimeout`, закрытие reader'а без ожидания `JoinGroup`) — только по решению system-architect: запрос через tech-lead#3, tech-lead#1 уведомлён. Публичный API `shared/eventbus` не меняется.
- [ ] Пауза до первой доставки замерена на Redpanda в двух случаях: у потребителя, запущенного после сбоя брокера (T-316: ~30 с), и у переподписавшегося `Dispatcher`. Цель tech-lead#3 — не больше 15 с (`runtime.StopTimeout`). Если без правки `shared/eventbus` цель недостижима, а system-architect правку не принял: замер и решение записаны в карточку, а README шлюза называет окно глухоты после сбоя брокера.
- [ ] Шаг 8 `OutboxWhileTheBrokerIsAway` (`internal/gateway/consumer/redpanda_integration_test.go`) — строгий. После остановки и запуска брокера тот же `Dispatcher`, без перезапуска, имеет `d.Err() == nil` и берёт событие в каждом из четырёх топиков (`publishOnEveryTopic`). Путь (б) с перезапуском потребителя удалён. Срок ожидания обоснован замером из предыдущего пункта.
- [ ] Замер при зависшем брокере — `docker pause` **своего** контейнера testcontainers (через API testcontainers или его клиент Docker; разрешение оркестратора 2026-09-14), **одним прогоном**. В карточку записываются:
  - длительность ack последнего адресата (ожидаемо до срока публикации 5 с);
  - свободна ли `gateway.db` во время ack;
  - период повторной выдачи по настенным часам, со сверкой с расчётом приёмки T-316 (≈ 31 с при остановленном брокере).

  После `unpause` — переподписка и строгий шаг 8. До и после прогона `docker ps -a --filter label=org.testcontainers=true` пуст. Контейнеры владельца, стенд `:8888` и `make test-integration` не трогаются.
- [ ] Интеграционные прогоны — только `go test -tags integration ./internal/gateway/consumer/...`, по одному (DoD-common). Весь `TestConsumerOnRedpanda` зелёный на дереве после слияния T-309 (T-309 меняет догон `Dispatcher.Start`), время прогона — в карточке.
- [ ] В карточке есть отметка, что tech-lead#1 уведомлён о вопросе к `shared/eventbus`. Если правка `shared/eventbus/kafka.go` понадобилась — есть отметка владельца.
...
- [ ] **(приёмка T-307, 2026-09-14; ревью #1 T-307, N-1; относится к T-352/T-353)** Тексты правил в групповом scope. `combat.decided` уходит всем живым участникам, а `render.Mechanics` даёт всем текст от второго лица («Попадание! Урон 3.», «Вы вырвались из боя.»). В группе остальные участники получают текст от третьего лица с именем действующего («Лена: попадание! Урон 3.»), действовавший — прежний. Golden на групповые варианты — в `internal/gateway/outbox/testdata/golden/`; имена из проекции, внешних ID в тексте нет.
...
- [ ] **(приёмка T-306, 2026-09-13; бэклог исполнителя T-306)** Шаг (4) каскада — `session.Manager.End(ctx, scope, session.EndForget, now)` из T-306. Метод идемпотентен: scope без активной сессии не меняется и ничего не публикует. Хук регистрируется в `ForgetHooks` в порядке C-04 v1.1. Персонаж в `creating` (C-02 v1.2): после `/forget` sweeper персонажей T-306 связку не возвращает — `ByPlayer` связку не находит, и удаляется только строка `pending_characters`. Формат `session.id` — по решению system-architect (вопрос 3 приёмки T-306): `End(forget)` и новое действие того же scope в одну секунду не дают `500`. Unit: `/forget` соло-игрока с активной сессией → один `analytics.session.ended {end_reason: forget}`; повторный `/forget` второго события не публикует.
- [ ] **(приёмка T-307, 2026-09-14; ревью #2 T-307, N-4)** Итог хода игрока, сделавшего `/forget` посреди хода, не зависит от гонки. Сейчас нарратив, поставленный до `/forget` и не подтверждённый, `DropForPlayer` переводит в `dropped`, и ход уходит в `timeout` (`turns_failed + 1`); тот же нарратив после удаления связки завершил бы ход сразу. Нарратив, поставленный между `DropForPlayer` и удалением связки, отбрасывает `Serve` (`Drop`, нет маршрута) с тем же итогом. Шаг хода для отброшенной доставки `kind=narrative` делается в одном месте — в `DropForPlayer` и `Drop` (`OnDelivered` в той же транзакции или отдельный `OnDropped`). Если architect#3 решит, что такой ход законно `timeout`, пункт закрывается записью решения в карточке. Unit: нарратив в очереди → `/forget` до ack → ход не `timeout`, `turns_failed` не растёт; то же для `Drop` без маршрута.
...
- [ ] **(приёмка T-306, 2026-09-13; бэклог исполнителя T-306)** `GET /v1/admin/sessions` строится на `session.Manager.Active` (T-306). Поля — из строки `sessions`: `id`, `kind`, `actor_kind`, `started_at`, `last_action_at`, `participants[]` (`player_id`), `turns_count`, `turns_degraded`, `turns_failed`. Внешних ID, `link_id` и имён нет. Unit: две активные сессии и одна закрытая → в ответе две; ответ не содержит внешнего ID связки участника.
...
- [ ] **(приёмка T-317, 2026-09-13; Mi-11 ревью #1 T-317, решение оркестратора)** Живая сверка `Docs/ops/runbook.md` §6 «Ротация токена» на стенде с собранным ботом (после T-312), без отзыва токена стенда и без токена в выводе: путь меню @BotFather из шага 1 сверен с интерфейсом; проверка webhook шага 3 выполнена командой из runbook (Git Bash или PowerShell); перезапуск шага 4, проверка лога шага 5 (ожидается 0) и `make health PROFILES=<набор>,bot` пройдены. Расхождения с текстом §6 записаны в отчёт и оформлены правкой runbook; если T-390 закрывается раньше T-312, пункт переходит в T-391.
...
| **З-10** | architect#3 | **Бэклог (приёмка T-311, 2026-09-13).** КД §7.1 и UC-002 E4: на `202 creating` бот опрашивает `GET /v1/players/{id}` раз в секунду до 10 раз прямо в единственном обработчике обновлений — до 10 с задержки команд всех игроков. Предложение ревью и исполнителя T-311: сразу отвечать «Персонаж создаётся» и доводить итог доставкой `kind=system` (outbox, T-307) или `/status`, без опроса в обработчике | не блокирует: сейчас `202` возникает только при задержке `entity.created`. После решения architect#3 — строка DoD в задаче бота (T-312 или следующей) и в T-307, если нужна доставка о создании |
| **З-11** | architect#3 | **Бэклог (приёмка T-306, 2026-09-13; ревью #1 T-306, N-4).** `session.Manager` держит один мьютекс `m.mu` на все scope и не отпускает его на время `Publish` старта или конца сессии: подтверждение брокера для одной сессии задерживает `Begin` действий всех игроков. На масштабе MVP-1 незаметно. Номер З-11: З-10 занят приёмкой T-311 в ветке эпика | не блокирует. При росте нагрузки — блокировка на scope (как блокировка игрока в `actions`) и строка DoD в задаче нагрузки; вместе с решением system-architect по аналитике вне транзакции (вопрос 1 приёмки T-306) |
| **З-12** | tech-lead#3 | **Бэклог (приёмка T-307, 2026-09-14; ревью #1 T-307, N-3).** Лизинг, начатый в `outbox.Service.Serve` до конца ожидания, может ждать соединение `gateway.db` (его держит ack или consumer до срока публикации 5 с) дольше дедлайна записи ответа `wait + 5 с`. Тогда доставки остаются в лизинге и выдаются снова через 30 с. Дешёвая часть — не начинать лизинг после истёкшего срока — сделана в T-307 | не блокирует, потерь нет. Предложение: контекст `Lease` со сроком «дедлайн записи − запас», запас передаёт обработчик в `Poll`; строка DoD — в ближайшей задаче шлюза по решению tech-lead#3 |
| **З-13** | system-architect + architect#3 | **Бэклог (приёмка T-307; ревью #1 T-307, бэклог п. 3; дополняет З-11).** Публикация `analytics.session.*` в sweeper'е сессий идёт без срока и под `session.Manager.mu`: зависший брокер задерживает `Begin` действий всех игроков, а с T-307 — и `outbox.Sweep` того же тика. Публикация `turn.completed` со сроком сделана в T-307 | не блокирует. Срок публикации (порядка `api.RequestTimeout`, при ошибке — повтор следующим тиком) или вынос из-под мьютекса — вместе с решением system-architect о форме публикации аналитики (вопрос карточки T-307) |
| **З-14** | architect#3 + владелец C-10 (EPIC-005) | **Бэклог (приёмка T-307; ревью #2 T-307, N-5).** После T-307 `delivery.recipients_count` в `analytics.turn.completed` — число уникальных игроков `recipients[]` со связкой на момент нарратива (адресат без связки считается доставленным, решение оркестратора 3); `delivered_count ≤ recipients_count`. Смысл записан только в коде и карточке, и потребитель EPIC-005 может читать поле как `len(recipients[])` | кода не касается: фраза в КД §7.6 (architect#3) и `description` поля в схеме C-10 при следующей правке (EPIC-005) |
| **З-15** | tech-lead#1 (корневой `.gitattributes`, EPIC-001) | **Бэклог (приёмка T-307; бэклог исполнителя T-307, п. 2).** `TestSolo30Fixture -update` пишет `testdata/analytics/solo-30.jsonl` с LF. На checkout с `core.autocrlf=true` `git status` показывает файл изменённым при неизменном содержимом (`git diff --quiet` — 0), пока рабочая копия не вернётся к CRLF | не блокирует. Правило `testdata/analytics/*.jsonl text eol=lf` в корневом `.gitattributes` (каталог — EPIC-005) или запись фикстуры с концами строк рабочей копии |
...
| 0.1.5 | 2026-09-13 | tech-lead#3 | приёмка T-311: статус и строка приёмки в §T-311; строки DoD T-311 (приёмка T-303 и I-2) приведены к C-08 v1.5 — клиент `forget_incomplete` не повторяет. Строки DoD T-312 (короткий таймаут клиента шлюза `flow`, логгер на `privacy.NewHandler`, `api.ValidCharacterName`, README бота и runbook §6) и T-320 (Mi-3 (а), `notice_due` в `route`). §8 З-10 — бэклог `202 creating`. Объём задач не менялся, новые задачи не заводились |
| 0.1.6 | 2026-09-13 | tech-lead#3 | приёмка T-317 (идёт после 0.1.5 — приёмки T-311 в ветке эпика): статус, строка приёмки и имя ветки (N-7) в §T-317; строка DoD T-312 (README бота и runbook §6 после `main.go`); строка DoD T-390 (живая сверка runbook §6, Mi-11). Строка DoD T-311 из ревью #1 T-317 не внесена — T-311 принята. Объём задач не менялся, новые задачи не заводились |
| 0.1.7 | 2026-09-13 | tech-lead#3 | приёмка T-306 (идёт после 0.1.6 — приёмки T-317 в ветке эпика): статус и строка приёмки в §T-306; строки DoD T-307 (`OnDelivered` один раз на доставку, итог при `recipients=0`, публикация аналитики вне долгой транзакции или со сроком, README шлюза после слияния T-306), T-309 (восстановление сессий и ходов при старте, `sessions_active`), T-355 (`session.End(forget)`), T-356 (`GET /v1/admin/sessions`). §8 З-11 — бэклог N-4. Объём задач не менялся, новые задачи не заводились |
| 0.1.8 | 2026-09-13 | tech-lead#3 | приёмка T-312 (идёт после 0.1.7 — приёмки T-306 в ветке эпика): статус и строка приёмки в §T-312; строки DoD T-315 (`api.ValidCharacterName` вместо `flow.ValidName`) и T-309 (`/health` бота `degraded`). Объём задач не менялся, новые задачи не заводились |
| 0.1.9 | 2026-09-14 | tech-lead#3 | приёмка T-307 (идёт после 0.1.8 — приёмки T-312 в ветке эпика): статус и строка приёмки в §T-307; строки DoD T-308 (платформа `ci-harness` у system-architect, запрет общей очереди с ботом, сквозной long-poll харнесса), T-309 (`outbox_pending`, `outbox_oldest_age_s`; лизинг в replay по решению architect#3), T-315 (пауза бота после раннего пустого ответа, повтор ack при `503` не дольше лизинга), T-316 (outbox при недоступном брокере), T-353 (N-1, групповые тексты), T-355 (N-4, шаг хода отброшенного нарратива). §8 З-12…З-15 — бэклог. Объём задач не менялся, новые задачи не заводились |
| 0.1.10 | 2026-09-14 | tech-lead#3 | приёмка T-316 (идёт после 0.1.9 — приёмки T-307 в ветке эпика): статус, имя ветки (N-3 ревью #1) и строка приёмки в §T-316; решение по Mi-2 — расчёт периода повторной выдачи принят с пометкой, замер при зависшем брокере перенесён в T-473. Новая задача T-473 (номер выдал оркестратор) — переподписка consumer шлюза после сбоя брокера, раздел в §1 после T-467. Объём задач T-301…T-393 не менялся |
