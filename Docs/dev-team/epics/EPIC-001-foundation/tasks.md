...
### T-406: Удалить `readySubscriber` из заглушки состояния, закрепить гарантию первого офсета кейсом контракта · Размер: S · Статус: todo · Волна 1, бридж-блок
...
### T-416: Ревизия контрактов волны 1: C-01, C-05, окружение, ADR-025 · Размер: M · Статус: in-progress · Волна 1
...
### T-420: Проход по устаревшим именам в документах вне индексов задач · Размер: S · Статус: in-progress · Волна 1
...
### T-425: Ревизия контрактов 2: подтверждения исполнения волны 1 · Размер: S · Статус: in-progress · Волна 1
...
### T-429: compose-lint: значения из docker compose config --no-interpolate вместо разбора строк YAML · Размер: M · Статус: todo · Волна 1 (бэклог)
...
### T-430: Паника в Stop контекста и цепочка ошибки паники обработчика · Размер: S · Статус: todo · Волна 1 (бэклог)
...
- **Код (после решения)**:
  - п. 1 — по образцу `runtime.start` (T-415): `recover` вокруг `Stop`; тест ленты: паника в `Stop` второго контекста → первый остановлен, шина закрыта последней; мутант на `recover` краснеет;
  - п. 2 — только если архитектор скажет «да»: тест с паникой значением `io.EOF` при сбое записи, сторож `panic_stop_test.go` остаётся зелёным.
- **DoD**: решение архитектора записано с доводами (C-01, ADR-023 или журнал). Если нужен код: тесты и мутанты выше, `go test -short ./...`, e2e и линтер зелёные, dev-log заполнен.
- **Исполнитель**: system-architect (решение), затем developer. Ветка `task/T-430-panic-in-stop`. Можно вести параллельно с остальными задачами волны: трогает только `shared/runtime/lifecycle.go` и `shared/eventbus/delivery.go`. Низкий приоритет, бэклог волны 1.
