...
<!-- dev-log T-402 -->
> **T-402 · devops-engineer#1 (Opus) · 2026-09-11 · стендовый замер LLM, конфигурация E, прогон 1 из 3.** Объём по решению оркестратора: только E, на параметрах сервера владельца; сервер не запускался, не перезапускался и не перенастраивался — только HTTP-запросы. Стенд: роутер Unsloth Studio (`python3.13.exe`, `:8888`) → `llama-server.exe` (`:62450`), билд b10840-58670d128 (пин — b10441), `Qwen3.8-27B-UD-Q3_K_XL.gguf` под id `Qwen3.8`, 4 слота × 221 696, тип KV не известен; VRAM на всю карту 20,9 ГБ с рабочим столом. Отклонение: модель E сопоставлена с id `Qwen3.8` копией матрицы (`--matrix`, временный каталог), `bench-matrix.json` не менялся. Результат (`ops/metrics/bench-20260911-2327.csv`, `bench-E-20260911-2327.json`; 30/30 без ошибок, 2 мин 34 с): phase2 p50 2996 / p95 7602 → fail; phase2-group3 p50 5021 / p95 9753 → fail*; tick p50 3213 / p95 9803; tps 44–48; `valid_json` и `lang_pass` 1,0. Диагностика (10 запросов вне методики): роутер добавляет ~5,3–6,3 с примерно к каждому третьему запросу, напрямую в `llama-server` накладные 0,12–0,31 с, слоты свободны — p95 определяется роутером, а не моделью. U-2 не решается: один прогон, ячейка вне матрицы, путь через роутер. C/A/E+ и четыре ячейки матрицы — не замерено с причинами в `baseline.md`. Предложения в бэклог — в карточке.
>
> **T-402 · продолжение · devops-engineer#1 (Opus) · 2026-09-11 · дополнительный замер Qwen3.6-35B-A3B на чистом llama.cpp (вне матрицы).** Владелец снял роутер Unsloth Studio. На `:8888` теперь чистый `llama-server` b10878-4850c7727 с `Qwen3.6-35B-A3B-UD-Q3_K_XL.gguf` (4 × 200 192, KV не известен), путь `MV_LLM_URL` идёт прямо в него. Прогоны 2–3 E — по сигналу владельца. Конфигурация `Q36-A3B` — копия E в копии матрицы (`--matrix`), модель = id `/v1/models`; сервер не трогался. Результат (`ops/metrics/bench-20260911-2353.csv`, `bench-Q36-A3B-20260911-2353.json`; 30/30, 1 мин 49 с): phase2 p50 2941 / p95 3710 → pass; phase2-group3 3665 / 4244 → pass*; tick 2098 / 3182; tps 75–115; `valid_json` и `lang_pass` 1,0; VRAM всей карты 19,6 ГБ. `first_call_ms` недействителен: прогрев скрипта получил 500 из-за неэкранированного `\` в id-пути; корректный тёплый вызов — 539 мс. Сравнение с E справочное (другая модель, другой путь), качество нарратива не оценивалось; кандидатом U-2 модель станет только через матрицу (architect#1). `baseline.md` §1, §2.1, §2.5, §4, §5; карточка — п. 5 бэклога и раздел продолжения.
>
> **T-402 · итерация 2 · devops-engineer#1 (Opus) · 2026-09-12 · по ревью #1 (ВЕРНУТЬ: C-1, M-1, Mi-1…4, N-1, N-2) и прогоны 2–3 E.** Прогон 2 E — исполнитель (00:03), прогон 3 — оркестратор (00:08, после перезапуска сервера владельцем), оба на чистом `llama-server` b10878, Qwen3.8-27B-UD-Q3_K_XL, 4 × 200 192: phase2 p95 2585 / 2916 → pass / pass; group3 4382 / 4772 → pass*; tick 4968 / 6195; tps 42–48; `valid_json` и `lang_pass` 1,0; «прочее» 0,44–0,82 с во всех 60 запросах. Прогон 1 (через роутер) не засчитан по решению оркестратора; 2 pass из 2 зачётных, U-2 ждёт прогона 4. C-1: `meta.matrix` во всех четырёх JSON заменён нейтральным описанием (по одной строке на файл), грэп по CSV/JSON замера на пути профиля и UUID сессии пуст. M-1: вывод о роутере — гипотеза с доводами за и против, сырые числа диагностик — `ops/metrics/diag-T402-20260911.csv`. Mi-1…4, N-1, N-2 закрыты в `baseline.md` (§1 по периодам стенда, §2.1, §2.5, §3–§6), карточке и dev-log (CRLF). Дефект прогрева и путь матрицы — T-434.
>
> **T-402 · итерация 3 · devops-engineer#1 (Opus) · 2026-09-12 · прогон 4 E и Nit ревью #2 (принять).** Прогон 4 E — оркестратор (00:20:02–00:21:43, после перезапуска сервера владельцем в 00:19:00; чистый `llama-server` b10878, Qwen3.8-27B-UD-Q3_K_XL, 4 × 200 192; ручной прогрев 536 мс, `first_call_ms` недействителен): phase2 p50 2590 / p95 3533 → pass; group3 3980 / 4720 → pass*; tick 2773 / 4488; tps 40–43; `valid_json` и `lang_pass` 1,0; «прочее» 486–854 мс. Итог замера E: три зачётных прогона на чистом llama.cpp (2, 3, 4) — все pass по NFR-002, с оговорками (tps ниже 70, запас исчезает при длинном нарративе, ячейка вне матрицы, KV неизвестен, VRAM всей карты). U-2 не принят — T-435 (architect#1). Nit N-1…N-3 закрыты в `baseline.md`. Стенд не трогался.
...
## architect#1 · T-435 · U-2: базовая конфигурация LLM — E; Qwen3.6-35B-A3B — запасной № 1 · 2026-09-12

Ветка `task/T-435-llm-baseline-decision` (от `9e5b9de`), TEAM-1, Opus. Подробности — карточка `tasks/T-435.md`.

- **Решение:** базовая конфигурация — E. Ячейка платформы — 8192 × 1 слот, KV f16: это уже `.env.example`, правок нет. Зачётные прогоны — 2, 3, 4. Незачёт прогона 1 подтверждён: это замер другой конфигурации (роутер вместо `server_mode: single`).
- **Отклонение от методики:** ячейка 4 × 200 192 названа отклонением. Для вердикта принята, для настройки платформы — нет. Остаток риска — один контрольный прогон в ячейке платформы (вопрос владельцу). Условие пересмотра — `fail` в нём.
- **Qwen3.6-35B-A3B:** запасной № 1, `decision_order` E → Q36-A3B → C → A. Действует после подтверждения владельцем. Три прогона — после T-434. Хвост «прочего» > 2 с должен исчезнуть или быть объяснён по `prompt_ms`.
- **Потолок нарратива:** нужен. `max_tokens` `phase2` соло на E — 160 (бюджет ≈ 163 токена), длина — через `maxLength` схемы и промпт. Сейчас в КД EPIC-003 `max_tokens: 700`, `maxLength: 1500`. Строка для EPIC-003 (КД §13.3–13.4, T-203) передана оркестратору, их индекс не правился.
- **Id модели (бэклог T-402 п. 1):** алиаса в матрице нет, id — stem файла через `--alias` в `llm-server.{sh,ps1}`. Задача devops выносится.
- **Файлы:** ADR-005 («Уточнение исполнения», пп. 1–6), `ops/metrics/baseline.md` (шапка, §2.5, §3 п. 3, §5), `ops/metrics/bench-matrix.json` v0.2, `tasks/T-435.md`. Новых ADR нет, счётчики не трогались.
- Стенд не запрашивался, `.env` не открывался. Числа бюджета посчитаны по `requests[]` отчётов T-402. Не коммитил.

## architect#1 · T-435 · итерация 2 по ревью #1 (вернуть: 0/3/8/2) и ответам владельца · 2026-09-12

Все три Major пришли из ответов владельца, к работе исполнителя Major нет. Код не менялся, стенд не запрашивался.

- **О-2 (рамка).** Правило выбора U-2 стало «прошла пороги **и** даёт нужную длину нарратива». Записано как **дополнение 3 к ADR-005** — это решение пользователя, а не уточнение исполнения; ADR-029 не нужен (провайдер, рантайм, интерфейс и `server_mode` не меняются). E — базовая на переход, Qwen3.6-35B-A3B — целевая конфигурация нарратива. В матрице `decision.chosen: "E"` и `decision.target: "Q36-A3B"` с условием. Ветка «целевая не прошла» → вопрос владельцу о длине или пороге 5 с. В доп. 2 п. 5 — ссылка вперёд. OQ-A-18 закрыт в `open-questions.md`.
- **О-1 (потолок).** Пересчитан под целевую Qwen3.6 по её собственному «прочему»: ≈ 220 токенов с нынешним хвостом, ≈ 410 без него. Потолок E на переход — 160. Строка для EPIC-003 переписана и вынесена задачей **T-439**: формула `maxLength`, предварительные значения, что делает T-203 при раннем старте, плюс Mi-3 (стартовая модель C в КД `:735` и `:977`).
- **О-3 (KV).** `q4_0` названо вторым отклонением от §6.4 п. 3 — в ADR п. 2, таблице §5 (строка 4) и `results` (`kv_cache: "q4_0"` с источником у прогонов 2–4 E и у Q36). Контрольный прогон стал доказательством ячейки платформы, а не «остатком риска»; условие пересмотра расширено на «p95 заметно выше» (> 4400 мс).
- **О-4.** Оговорки «после подтверждения владельцем» сняты везде: порядок `E → Qwen3.6 → C → A` подтверждён 2026-09-12.
- **О-5.** Сессия сведена в одну таблицу в **T-438**: прогон → ячейка → команда (`.ps1` и `.sh`) → что решает, плюс таблица исходов и калибровка «символов на токен» через `/tokenize`. Зависимости — T-434 **и** T-437 (`--alias`). В матрице `pending_owner` → `planned_session`.
- **Mi-1** — «как в замере» убрано: у группы в промптах потолок 512, а 320 не проходит под справочный порог группы 5000 мс. **Mi-2** — записана формула `maxLength` с каркасом JSON и массивами. **Mi-3** — в T-439. **Mi-4** — обе формы команды, `.ps1` основная. **Mi-5** — номера T-437, T-438, T-439 выдал оркестратор, вписаны в §5 и карточку. **Mi-6** — диапазон Q36 `phase2` 107–128 ток/с, бюджет по её «прочему». **N-1** — флажок пункта T-140 честный. **N-2** — нижняя граница VRAM Q36 → 17 443 МБ.
- **Файлы:** ADR-005 (дополнение 3 + переписанное «Уточнение исполнения»), `ops/metrics/baseline.md` (шапка, §1, §2.5, §3, §5), `ops/metrics/bench-matrix.json` v0.3, `tasks.md` (T-437, T-438, T-439), `open-questions.md` (OQ-A-18), карточка T-435. Не коммитил.

## architect#1 · T-435 · итерация 3 по ревью #2 (вердикт «принять», 0/0/6/3) · 2026-09-12

Закрываю Minor и Nit до приёмки. Решение U-2 не меняется. Код не менялся, стенд не запрашивался.

- **Mi-1.** Машинный след внутри документа сведён: в таблицах прогонов E (§2.1) и Qwen3.6 (§2.5) колонка KV — `q4_0` со сноской ³ «со слов владельца, 2026-09-12»; у прогона 1 — «не установлено» (другой старт сервера). Поправлены и две текстовые строки: §2.5 («4 слота × 200 192, KV `q4_0`») и §3 (шапка вердикта).
- **Mi-2.** `infrastructure.md` §6.4 приведён к дополнению 3: цель замера (`:865`) — новое правило выбора и порядок с Qwen3.6, пример JSON (`:899`) — `decision_order` из четырёх элементов, правило 3 (`:985`) — порядок, правило «пороги и длина» и факт двух отклонений, строка риска (`:1205`) — новый порядок и что будет, если E не пройдёт. Файл добавлен в перечень последствий доп. 3 п. 6 с пометкой, что правит его architect#1 (документ за ним по T-399).
- **Mi-3.** Формула `maxLength` стала воспроизводимой: названы каркас ≈ 30 токенов и служебные поля ≈ 36 (`mentions` `maxItems` 4, `background_refs` 2, элемент ≈ 6 токенов), вместе ≈ 66. Проверка в тексте: 210 / 345 / 775 символов получаются из формулы. То же число — в T-439.
- **Mi-4.** В T-438 записана оговорка: промпты замера просят короткий нарратив (68–111 токенов соло, 132–212 в группе при целевых 220 и 410), поэтому NFR-002 при новом потолке держит **расчёт, а не замер**. Закрытие — либо один промпт под целевую длину в прогонах 2–4 (копия `prompts.jsonl` вне репозитория), либо прямая оговорка в `baseline.md` со ссылкой на `narrative_latency_p95_ms` в S1/S2.
- **Mi-5.** Введён один термин: **«E прошла»** = `pass` по NFR-002 **и** p95 ≤ 4400 мс **и** медиана ≤ 3000 мс. Отдельная строка исходов — «прошла по порогу, но не по сигналу»: это сигнал к пересмотру, а не отказ. Записано в ADR (УИ п. 2), `baseline.md` §5, таблицу исходов T-438 и `decision._passed_definition`/`revisit_if` матрицы.
- **Mi-6.** У T-439 появилась ветка `task/T-439-narrative-length-cap` и запись о владении: правки в документах EPIC-003 делает architect#2, подтверждает tech-lead#2; дублирование строки в индексе EPIC-003 — за оркестратором.
- **N-1** — строку ревью #1 в карточке не трогал: это артефакт ревьюера. **N-2** — статус T-435 в индексе приведён к карточке. **N-3** — сказано прямо, что для E взята оценка по соло как более строгая (по данным группы у неё 169 и 380).
- **Файлы:** `infrastructure.md` §6.4 (4 места), ADR-005 (доп. 3 п. 6, УИ пп. 2 и 4), `ops/metrics/baseline.md` (§2.1, §2.5, §3, §5), `ops/metrics/bench-matrix.json`, `tasks.md` (T-435, T-438, T-439), карточка T-435. Не коммитил.

## devops-engineer#1 · T-434 · llm-bench: прогрев, путь матрицы, `first_call_ms`, `prompt_ms`, паритет `.ps1` · 2026-09-12

- Прогрев в `.sh` собирает python-помощник (`warmup`), тело уходит файлом: id-путь Windows и кавычка в id больше не ломают JSON. Это дефект T-402, из-за которого B2 недействителен в прогонах 2–4 E и у Q36; в `.ps1` его не было, то есть близнецы разошлись.
- При не-200 на прогреве `first_call_ms` не пишется вовсе: пусто в CSV, `null` в JSON (`meta` и `cells`), в stderr — строка с кодом ответа. Одинаково в обеих реализациях (в `.ps1` строка переведена из `Write-Host` в stderr).
- `meta.matrix` и `meta.prompts`: внутри репозитория — относительный путь, вне — только имя файла; рядом всегда новые `matrix_sha256` и `prompts_sha256`. Абсолютного пути в отчёте нет ни при каком `--matrix`/`-Matrix` (C-1 ревью #1: в T-402 туда попал путь профиля пользователя).
- `requests[].prompt_ms` из `timings.prompt_ms` (`null`, если сервер поля не прислал). Заодно в `requests[]` `.ps1` добавлено поле `index`, которого там не было, — DoD требует одинаковый набор полей.
- `llamacpp_build`: `MV_LLM_BIN --version` → `GET /props` `build_info` → пин; источник виден в значении (`bNNNN`, `bNNNN(props)`, `bNNNN(pin)`). Сверку стенда с пином не дублирую — она в `make llm-health` (T-404 п. 2). Билд определяется внутри цикла конфигураций: `/props` принадлежит серверу конкретной конфигурации.
- Колонки CSV не менялись; всё новое — только в JSON.
- **Находка сверх DoD:** `.ps1` писал числа по культуре машины. На `ru-RU` (культура этой машины) `tps = 66.7` уходит в CSV как `66,7`, строка становится 25 колонок вместо 24, и `verdict`, `repeat`, `started_at` читаются на колонку левее. `make bench` предпочитает `.ps1`, когда `pwsh` в `PATH`, а на машине владельца он есть. Исправлено `Format-BenchNumber` (фиксированная точка, инвариантная культура) плюс инвариантная культура на весь скрипт; попутно закрыто расхождение `1` против `1.0`. Битых CSV в репозитории нет: все сделаны `.sh`.
...
<!-- dev-log T-399 -->
## architect#1 · T-399 · `infrastructure.md` приведён к состоянию после T-397 · 2026-09-13

Ветка `task/T-399-infrastructure-after-t397` (от `adadca5`), TEAM-1, Opus. Подробности — карточка `tasks/T-399.md`, раздел «Выполнение».

- **Причина** — Mi-2 ревью T-397: документ описывал прежнюю топологию, а эталон `.env.example` в §4.2 был в запрещённом формате.
- **Сверено с деревом**: три compose-файла, `Makefile` (`COMPOSE`, `ACTIVE_PROFILES`, `legacy-src`), шапка и код `scripts/compose-lint.sh`, `testdata/compose-lint/README.md`, `.env.example`, `build/versions.env`, `go.yml`, врезка README.
- **Сделано.**
  - Шапка: строка «Редакционная правка T-399», история версий не тронута.
  - §0 и §1.3: столбец «Файл», почему файлов три, как их подключает `make`, голый `docker compose --profile bot` не поддерживается.
  - §2.2: `$(COMPOSE)`, `legacy-src`, `--fixtures`.
  - §2.4: `versions.env` без инлайн-комментариев.
  - §3.1: шаги задания `compose-lint`.
  - §3.1.1: восемь правил по строке, ссылки на скрипт и README фикстур вместо пересказа.
  - §4.1 п. 5 (новый): формат `.env.example`.
  - §4.2: эталон в текущем формате, состав ключей совпадает с файлом.
  - Ссылки на правило про `llama-server`: п. 5 → п. 6.
- **§6 не трогал.** Хунки T-437 и T-440 против `adadca5` — только `:805–999`, пересечений нет.
- **Проверка (только чтение):**
  - инлайн-комментариев после пустого значения нет ни в блоке §4.2, ни в `.env.example`;
  - `diff` ключей блока и файла пуст;
  - `[required]` при чтении правилом линтера — одинаковые семь в блоке и в файле.

  Первый прогон этой проверки поймал ложную пометку у `MV_TELEGRAM_BOT_TOKEN`: пояснение «без [required]» стояло в блоке комментария. Формулировка заменена.
- **Вне задачи, в бэклог** (в карточке): §2.2 `make build` (три бинарника, в `cmd/` два), `make contracts` (нет `blueprint validate`), `make secrets-scan` (индекс, не рабочая копия), проверка LLM в `make up` (T-404).
- `make`/`compose-lint` не запускались: код не менялся. Не коммитил.
- **Итерация 2 по ревью #1 (вердикт «принять», Minor 2, Nit 3; закрыто в T-399 по решению оркестратора, 2026-09-13).** Только `infrastructure.md`, §6 побайтно равен базе.
  - Mi-1: в §3.1.1 п. 3 вернулось «ключ без значения — только у обязательного секрета», в п. 8 — «`${MV_X}`/`$MV_X` — при непустом умолчании манифеста». Сверено с `compose-lint.sh:936-967`, `:1290-1310`.
  - Mi-2: §9 (вводный абзац, 9.1 п. 1, 9.2, 9.6, 9.8 — бот) приведён к `Docs/ops/runbook.md` — путь через `make up PROFILES=…,bot` либо `-f docker-compose.bot.yml`. Голого `docker compose up -d telegram-bot` в файле нет.
  - N-1: §2.4 — запрет из T-004. N-2: §2.2 — полный перечень целей с `$(COMPOSE)` и правило одинакового `PROFILES=` у `up`/`down`. N-3: §0 — `prod` с ботом после EPIC-004.
  - `docs/ops/runbook.md` → `Docs/ops/runbook.md` в §9, §9.8, §10 (F-9).
  - Бэклог в разделе T-399 индекса: остальная сверка §9 с runbook; `build/versions.env:10` (`cat` против `grep -E`, devops-engineer).
  - Карточка — подраздел «Итерация 2». Не коммитил.
<!-- dev-log T-442 -->
## architect#1 · T-442 · тексты об `--alias`, пине и `ops/models.txt` приведены к слиянию T-437 · 2026-09-13

Ветка `task/T-442-t437-followup-texts` (от эпика с мержем T-437 `c3bfcd5`), TEAM-1, Opus. Подробности — карточка `tasks/T-442.md`.

- **ADR-005 УИ п. 6.** Факт T-402 («скрипты передают `-m` без `--alias`») переведён в прошедшее время. Добавлен абзац «Исполнено (T-437, 2026-09-13; редакционно, T-442)»: что отдаёт `/v1/models`, что проверено только на подставном сервере, что подтвердит прогон 1 T-438. Решение пункта не менялось. Ссылки `llm-bench.sh:743`/`.ps1:384` устарели, их заменил текст сообщения о пропуске фазы.
- **`baseline.md` §5.** Три пункта «вынесено в T-437» стали `[x]` «выполнено в T-437». Названы отклонения: пин и список сделаны до T-438, `build_info` сверяется в начале сессии. К записи «Кто и когда» T-435 дописана строка состояния, сама запись не переписана.
- **`bench-matrix.json`.** Текст `_model_ids` — по факту T-437, `updated` → `2026-09-13`. Ключи и значения не менялись. Скрипты `_model_ids` не читают: они берут `configs`, `request`, `thresholds`, `n_per_cell`, `runs_required`.
- **Грэп** по `Docs/`, `ops/`, `README.md`, `CLAUDE.md`, `scripts/`, `build/`: в своих документах T-437 как будущая не осталась. Чужое отстающее место одно — индекс, раздел T-438, `:845` («Без T-437 сервер поднимается вручную…»), владелец tech-lead#1; в бэклог карточки.
- **Проверки:**
  - `py -3 -m json.tool` по матрице — ok, `ConvertFrom-Json` в `pwsh` — ok;
  - `gitleaks dir --config=.gitleaks.toml` по ADR-005, `baseline.md`, `bench-matrix.json`, `tasks.md`, карточке — `no leaks found`;
  - в `.gitleaksignore` записей на эти файлы нет;
  - Go и скрипты не менялись, тесты не запускались.
- Карточка — новый файл в LF (Write), остальные правленые — CRLF без смешения.
- Стенд `:8888` не трогался, `.env` не открывался. Не коммитил, `git add` не делал.
<!-- dev-log T-443 -->
## developer#1 · T-443 · Kafka: письмо, отвергнутое внутри `Close`, — штатная остановка · 2026-09-13

Ветка `task/T-443-kafka-close-dead-letter-race`, TEAM-1, Opus. Подробности — карточка `tasks/T-443.md`, раздел «Выполнение».
- **Что сделано** (`shared/eventbus/kafka.go`): `stopped` = `ctx.Err() != nil || errors.Is(err, io.EOF) || busClosed(err)`. `busClosed` — та же проверка, по которой лог ставит `bus_closed`, поэтому `Warn` с `bus_closed=true` и `nil` из цикла идут вместе по построению. Комментарии `Subscribe`, `stopped`, `busClosed` и README обновлены. Go-API не менялся.
- **Решения по ходу.**
  - Выбран вариант «ошибка закрытой шины — остановка», а не проверка `k.closing.Err()` и не перестановка в `Close`. Отмена `loopCtx` идёт через `context.AfterFunc`, то есть в своей горутине. Переставить `stopReaders()` под блокировку до `closed=true` окно не закрывает: цикл может увидеть отказ раньше, чем горутина отменит `loopCtx`. Проверка `k.closing` потребовала бы перестановки и отдельной ветки в `Subscribe`, а `Tail`/`ReadRange` (у них нет `loopCtx`) остались бы с той же ошибкой. `stopped` уже признавал две другие приметы закрытия — `io.EOF` читателя и `io.ErrClosedPipe` писателя; `ErrClosed` — третья, и одна правка чинит все три цикла.
  - Безопасность: `ErrClosed` попадает в цепочку ошибки `Deliver` только от приёмника `dead_letters` этой же шины. Причина письма идёт текстом (C-01 v1.6), так что обработчик, сам упавший с `ErrClosed`, остановкой не считается — сторож `panic_stop_test.go` дополнен двумя такими случаями.
  - membus такого окна не имеет: `closed=true` и `close(done)` ставятся под одной `st.mu`, отказ `topic()` читает `closed` под той же блокировкой, а цикл после ошибки спрашивает `stopping` (`done`). Правка не нужна.
- **Тесты**: `TestADeadLetterRefusedInsideCloseIsAnOrderlyStop` (окно без брокера: `loopCtx` как в `Subscribe`, только флаг закрытия, настоящий `kafkaDeadLetters` → `ErrClosed`, `loopCtx` жив, `stopped` = true, один `Warn` `bus_closed=true`) — красный до правки, зелёный после; `TestStoppedTellsAShutdownFromAFailure` + «closed bus», «closed bus, wrapped by the dead letter»; `TestAFailedDeadLetterIsNeverTakenForTheEndOfTheReader` + «a panic with ErrClosed», «an error wrapping ErrClosed, retries spent».
...
<!-- dev-log T-457 -->
## system-architect#1 · T-457 · Очередь решений по вопросам ревью и приёмок (ADR-029, T-054, T-060) · 2026-09-13

Ветка `task/T-457-architect-decisions-queue`, Opus. Подробности и строки DoD — карточка `tasks/T-457.md`.
- **Что сделано.**
  - `contracts.md` v0.13 со сводкой. C-01 v1.9: запись сессии в `shared/recording` и `Deps.Recording`, время в обработчике replay, время корней через `POST /v1/admin/replay/clock`, `Derive` копирует `Replay`, порядок `All()`. C-02 v1.5b: переход терминальной сущности — `dead_entity`. C-05 v1.8: п. 9, `law_violation` в п. 1е. C-07 v1.5: `rejected.ref`, `labels_hash`, ответ нарратива с ярлыками.
  - ADR-029 подтверждён: пометки в ADR-016 и ADR-017 (включая доп. 1 п. 4), строка «см. ADR-029» в ADR-005 УИ п. 4.
  - КД State §4.5 п. 5, §4.6, §5.7; `data-model.md` §3.2, §3.7; `diagrams/erd.md`.
- **Решения по ходу.**
  - Номер новой версии C-07 — v1.5: v1.4 уже занят денежным лимитом (T-449), а ADR-029 называет «v1.4».
  - Временная форма ADR-029 п. 6 (`other`) отменена: решение по `ref` принято до старта T-213.
  - В1: вариант А, но сторона участников inv-01 сужена до «персонаж затронут тем же пакетом». Иначе асинхронный `/forget` отвергал бы пакет встречи, а в группе — весь раунд. Код участников — T-056 (гонка достижима в I1), NPC — T-066.
  - T-060: запись — вход режима процесса, поэтому в `Deps`, а формат — в `shared/recording`; провайдер в `Deps` и второй читатель JSONL отвергнуты.
- **Проверки.**
  - CRLF и отсутствие NUL — в скрипте правки до записи и после; `sed -i` не использовался; каждая замена — ровно одно вхождение.
  - `swarm-llm-laws.md`, `schemas/**`, код, C-08 не менялись (`git status`).
...
<!-- dev-log T-457 итерация 2 -->
## system-architect#1 · T-457 · итерация 2 по ревью #1 и вопросам T-207 · 2026-09-13

Ветка `task/T-457-architect-decisions-queue`, Opus. Решения и переписанные строки DoD — карточка `tasks/T-457.md`, раздел «Итерация 2».
- **Что сделано.**
  - Ma-1: `shared/recording` переносит задача EPIC-002 «запись сессии» — старт сразу, в `serve.go` после T-055, поставка контрольным слиянием EPIC-002. Файлы EPIC-001 — с просмотром tech-lead#1. Один владелец чтения записи и `Deps.Recording` в `serve.go`. T-207 от пакета не зависит, привязка — T-212. Текст — C-01 v1.9 «Порядок поставки».
  - Вопросы T-207. Ключ записи — поля `llm.Request.AgentID`/`Attempt` (C-15 v1.4), `WithCall` не принят. Источники `recorded` — события `*recording.Recording` из файла или из `ReadJournal`. У `ReadJournal` правила из уроков T-207: разбор после чтения, частичное чтение — ошибка. `LLMOutputKeyOf` не усекает дробный `attempt`.
  - Mi-1: `EventClock.Advance`, тег `process`, вызов у процесса с `gateway`. Mi-2: убывающие корни входов — дефект записи, `mvctl record` их отвергает. Mi-5: `maxLength: 128` у id схем тика (T-203), `ref` не усекается.
  - Mi-3, Mi-4: полный перечень EPIC-003 для architect#2; строки `TestInvDeadDoesNotAct` разнесены — две строки у T-056, у T-066 существующие строки не правятся.
  - N-1…N-6: номер v1.5b, `Open`/`Read` и `TypeLLMOutput`, «`npcs[]` не пуст», порядок `definitions`, `ownership.md` v0.7, `labels_hash` при `nil`-таблице.
- **Решения по ходу.**
  - Порядок `(а)`, а не копия в EPIC-001: код формата только в ветке EPIC-002, копия дала бы временный второй читатель и конфликт переноса.
  - Перекрёстный тест двух форм ключа отвергнут в пользу одной функции: у `jsonpath.GetInt` найдено усечение `1.5` до `1`, которого провайдер T-207 не допускает.
  - Сортировка входов по `at` отвергнута: исход раунда группы зависел бы от сортировки.
- **Проверки.**
  - Правка — скриптами Python в scratch `t457i2-*`: точные замены с проверкой числа вхождений, CRLF и отсутствие NUL до записи и после; `sed -i` не использовался.
  - `swarm-llm-laws.md`, C-08, `schemas/**`, код не менялись (`git status`, `git diff`).
  - `gitleaks dir . --redact -c .gitleaks.toml` по копии 12 изменённых и новых файлов с `.gitleaks.toml` и `.gitleaksignore` ветки (scratch `t457i2-*`) — `no leaks found`. Копия удалена по точному пути.
- Не коммитил.

<!-- dev-log T-457 итерация 3 -->
## system-architect#1 · T-457 · итерация 3 по ревью #2 · 2026-09-13

Ветка `task/T-457-architect-decisions-queue`, Opus. Ревью #2 — «принять» (0/0/3/3), правки до коммита. Подробности — карточка `tasks/T-457.md`, раздел «Итерация 3».
- **Что сделано.**
  - «Новая задача EPIC-002» везде заменена номером T-458.
  - R2-Mi-1: исключение T-458 (создание `shared/recording/**`, `Deps.Recording`, тест `Derive`) названо в `contracts.md` §16 п. 8 и `ownership.md` §3 п. 4 (а) — ревью system-architect, просмотр tech-lead#1, поставка контрольным слиянием EPIC-002; ревью system-architect — в DoD T-458.
  - R2-Mi-2: корень входа сужен до `player.*` в ответ на HTTP-вход; корни таймера шлюза (`round_timeout`) не проверяются — C-01, строки T-061 и T-221.
  - R2-Mi-3: поля `Request.AgentID`/`Attempt` добавляет T-211, T-212 их заполняет и зависит от T-211 — C-15 v1.4, строки DoD.
  - R2-N-1…N-3: перечень architect#2 дополнен (КД роя `:837`, `:1060`, `:1117`, `:1123`, ADR-029 `:72`, `:129`, пункты `tasks.md` по содержанию, проверка `git grep`); «Заглушка» C-07 — `ReadJournal`; окно догона — КД роя §4.3.
- **Проверки.**
  - Правка — скриптом Python в scratch `t457i3-*`: точные замены с проверкой числа вхождений, CRLF и отсутствие NUL до записи и после.
